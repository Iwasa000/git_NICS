...
// コンストラクタ
CimpUc102Gensan::CimpUc102Gensan(void)
	: CimpUc000Common()
{
	m_pUcData				= NULL;
// midori 180404 add -->
...
//**********************************************************************
//virtual
int CimpUc102Gensan::Init(CDatabase* pDB)
{
	//--------------------------------------------------
	// 全帳票共通のコード
	//--------------------------------------------------
	...
	m_nItemCount = CSV_102_ITEM_CNT;

	// 内訳書テーブルとの接続を開始
	m_pUcCmn = (CdbUc000Common*)(new CdbUc102Gensen(pDB));
	...
	//--------------------------------------------------
	// １頁内の合計行を含む全体の行数
	m_RowMax		= ID_ROWKOBETU_102;
	// １頁内のデータ行数（合計行は含まず）
	m_RowMaxData	= ID_ROWKOBETU_102 - 1;
	// 帳表固定の合計行数（1:合計あり，0:なし）
	m_RowKei		= ID_ROWKEI_102;

	// 出力設定が通常の場合
	if( m_pUcInfFormType->m_FormType == 0 ){
		m_RowMax		= ID_ROWNORMAL_102;
		m_RowMaxData	= ID_ROWNORMAL_102 - 1;
	}
...
//**********************************************************************
//virtual
int CimpUc102Gensan::SetCsvOneLineData(CStringArray* pstrCsvDataAry)
{
// No.200903 add -->
	int		len=0;
// No.200903 add <--
...
// midori 180404 add <--
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++
	// 一時的にテーブルアクセス領域を拡張
	m_pUcData = (CdbUc102Gensen*)m_pUcCmn;
...
// midori 150899 add <--
	...
	// 01 年月日
// midori 150899 cor -->
	//if (ConvDateStrToDbVal(IMP_DATE_CONV_TYPE_B, pstrCsvDataAry->GetAt(1), &nTemp) != TRUE) {
	//	strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_102_ITEM_01);
	//	return IMP_REC_NG;
	//}
// ---------------------
...
// midori 150895 cor <--
	...
	// 02 所得の種類
	if (CheckImportStr(pstrCsvDataAry->GetAt(2), MAX_KETA_102_SYURUI*2) != TRUE) {
		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_102_ITEM_02);
// midori 150899 cor -->
		//return IMP_REC_NG;
// ---------------------
		m_uImpCsvInfo.nLength = MAX_KETA_102_SYURUI;
...
// midori 150899 cor <--
	...
	// 03 期末現在高
// No.200903 del -->
////	if (CheckImportNum(pstrCsvDataAry->GetAt(3), ID_VAL_102_DATA) != TRUE) {
////	if (CheckImportNum(pstrCsvDataAry->GetAt(3), ID_VAL_102_DATA, TRUE) != TRUE) {		// [2009/07/09 Minus Value Support]
//	if (CheckImportNum2(pstrCsvDataAry->GetAt(3), ID_VAL_102_DATA, TRUE) != TRUE) {		// [2009/07/09 Minus Value Support]
//		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_102_ITEM_03);
//// midori 150899 cor -->
//		//return IMP_REC_NG;
//// ---------------------
//		m_uImpCsvInfo.nLength = ID_VAL_102_DATA;
...
//
//	// 02 所得の種類
//	if (CheckEntryKamoku(pstrCsvDataAry->GetAt(2), m_nFormSeq, &nKnSeq, &nKnOrder) != TRUE) {
//		return IMP_REC_NG;
//	}
//
//
...
//		m_pUcData->m_KnName		= pstrCsvDataAry->GetAt(2);
//// midori 152137 add -->
//		m_pUcData->m_KnKana		= fc.Convert(pstrCsvDataAry->GetAt(2));
//// midori 152137 add <--
//
//		// 03 期末現在高
//		m_pUcData->m_Val		= pstrCsvDataAry->GetAt(3);
...
//**********************************************************************
//virtual
int CimpUc102Gensan::ImpCsvOneLineData(CStringArray* pstrCsvDataAry)
{
	//--------------------------------------------------
	// 前処理（全帳票共通のコード）
	//--------------------------------------------------
...
// midori UC_0054 add <--
	...
	// 02 所得の種類
	GetSpritString(pstrCsvDataAry->GetAt(2),&strTemp,MAX_KETA_102_SYURUI*2);
...
//**********************************************************************
//virtual
int CimpUc102Gensan::GetCsvOneLineData(CStringArray* pstrCsvDataAry)
{
	//--------------------------------------------------
	// 前処理（全帳票共通のコード）
	//--------------------------------------------------
	...
	if (strCsvId.Compare(CSV_ID_DATA) == 0) {
		...
		// 02 所得の種類
		pstrCsvDataAry->SetAt(2, m_pUcData->m_KnName);

		// 03 期末現在高
		pstrCsvDataAry->SetAt(3, m_pUcData->m_Val);
	}
