// dbUc152Kenrikin.cpp : インプリメンテーション ファイル
//
...
IMPLEMENT_DYNAMIC(CdbUc152Kenrikin, CdbUc000Common)

CdbUc152Kenrikin::CdbUc152Kenrikin(CDatabase* pdb)
	: CdbUc000Common(pdb)
{
	//{{AFX_FIELD_INIT(CdbUc152Kenrikin)
	m_Seq = 0;
	m_NumPage = 0;
	m_NumRow = 0;
	m_FgFunc = 0;
...
// インボイス登録番号追加対応_23/10/23 add <--
	...
	m_lo_TableName = STR_TBL_NAME_152;			// テーブル名を記述
}


CString CdbUc152Kenrikin::GetDefaultSQL()
{
	return _T("[dbo].[uc_152_kenrikin]");
}

void CdbUc152Kenrikin::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CdbUc152Kenrikin)
...
#ifdef _DEBUG
void CdbUc152Kenrikin::AssertValid() const
{
	CRecordset::AssertValid();
}

void CdbUc152Kenrikin::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc152Kenrikin::CreateNewRecord()
{
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc152Kenrikin::CreateNewRecord2(int pNumPage,int pNumRow)
// 157153 del <--
// 157153 add -->
// ------------------------------------------------------------------------------------------------
//	CreateNewRecord2()
...
//
// ------------------------------------------------------------------------------------------------
int CdbUc152Kenrikin::CreateNewRecord2(int pNumPage,int pNumRow,int pNullSw)
// 157153 add <--
{
	CRecordset	rs(m_lo_pdb);
	CString		SqlStr;
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc152Kenrikin::UpdateYokukiZeroClear()
//{
//	CString	sql = GetSQLUpdateYokukiZeroClear("Val");			// 支払金額
//	sql += GetSQLUpdateYokukiZeroClear("SpDate");				// 支払年月日
//	//CString		strCommand;
//	//CString		strTemp;
//
//	//// クリア対象項目（2項目）
//	//strCommand.Format("UPDATE %s SET ", m_lo_TableName);
//	//strCommand	+=	_T("SpDate = 0, ");						// 支払年月日
...
***********************************************************************/
// virtual
int CdbUc152Kenrikin::UpdateDataTableWork()
{
