// dbUc041Karibarai.cpp : インプリメンテーション ファイル
//
...
IMPLEMENT_DYNAMIC(CdbUc041Karibarai, CdbUc000Common)

CdbUc041Karibarai::CdbUc041Karibarai(CDatabase* pdb)
	: CdbUc000Common(pdb)
{
	//{{AFX_FIELD_INIT(CdbUc041Karibarai)
	m_Seq = 0;
	m_NumPage = 0;
	m_NumRow = 0;
	m_FgFunc = 0;
...
// midori 152137 add <--
	...
	m_lo_TableName = STR_TBL_NAME_041;			// テーブル名を記述
}


CString CdbUc041Karibarai::GetDefaultSQL()
{
	return _T("[dbo].[uc_041_karibarai]");
}

void CdbUc041Karibarai::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CdbUc041Karibarai)
...
/////////////////////////////////////////////////////////////////////////////
// CdbUc041Karibarai 診断

#ifdef _DEBUG
void CdbUc041Karibarai::AssertValid() const
{
	CRecordset::AssertValid();
}

void CdbUc041Karibarai::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc041Karibarai::CreateNewRecord()
{
...
//			retVal = val.m_iVal;
	...
	m_AdAdd2 = _T("");		// ("") 相手先：(取引)所在地(2行目)
	m_Kankei = _T("");		// ("") 相手先：法人・代表者との関係
	m_Val = _T("");			// ("") 期末現在高
	m_Naiyou = _T("");		// ("") 取引の内容
	m_ShowKeiZero = 0;		// (0)  ０円計表示フラグ
// midori 152137 add -->
	m_KnKana = _T("");		// ("") 科目(カナ)
	m_AdKana = _T("");		// ("") 相手先(名称)(カナ)
// midori 152137 add <--
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc041Karibarai::CreateNewRecord2(int pNumPage,int pNumRow)
// 157153 del <--
// 157153 add -->
// ------------------------------------------------------------------------------------------------
//	CreateNewRecord2()
...
//
// ------------------------------------------------------------------------------------------------
int CdbUc041Karibarai::CreateNewRecord2(int pNumPage,int pNumRow,int pNullSw)
// 157153 add <--
{
	CRecordset	rs(m_lo_pdb);
	CString		SqlStr;
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc041Karibarai::UpdateYokukiZeroClear()
//{
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc041Karibarai::UpdateYokukiZeroClear( int pSw )
{
// midori 152388 add -->
	CString	cs=_T("");
// midori 152388 add <--
...
// midori 152388 add -->
	if(pSw != 0)	{
		...
		cs += _T(" AND AdAdd1 IS NULL AND AdAdd2 IS NULL AND Kankei IS NULL AND Val IS NULL");
		cs += _T(" AND Naiyou IS NULL");
...
/**********************************************************************
	...
	戻値
			int						正の数：行数
									...
									0     ：行なし
***********************************************************************/
int CdbUc041Karibarai::GetKnNameFirstRow(int inPage, CString* pstrKnName, CString strTable)
{
...
***********************************************************************/
long CdbUc041Karibarai::GetColumnSortAddressBank(BYTE ItemSeq)
{
...
***********************************************************************/
// virtual
int CdbUc041Karibarai::UpdateDataTableWork()
{
...
***********************************************************************/
// virtual
int CdbUc041Karibarai::GetSortIkkatuOrder()
{
	return m_KnOrder;
}