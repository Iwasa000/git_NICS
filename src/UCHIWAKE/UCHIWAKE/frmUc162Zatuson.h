...
#define	ID_ROWKEI_162			0		// 帳表固定の合計行数（1:合計あり，0:なし）
#define	ID_COLMAX_162			11		// １行内のコントロール個数
#define	ID_COLMAXDATA_162		6		// １行内の入力コントロール数
#define	ID_CTLSTARTINDEX_162	5		// １行目の入力項目の先頭インデックス番号
#define ID_T1STARTNORMAL_162	110		// 通常時：合計行①の先頭インデックス番号
#define ID_T2STARTNORMAL_162	121		// 通常時：合計行②の先頭インデックス番号
#define ID_T1STARTKOBETU_162	220		// 個別時：合計行①の先頭インデックス番号
#define ID_T2STARTKOBETU_162	231		// 個別時：合計行②の先頭インデックス番号

// 「雑損失」テーブル名称
//	正式版では"CdbUc162Zatuson"のメンバー変数で参照可能としたい
#define	TBL_NAME_162		_T("uc_162_Zatuson")

//////////////////////////////////////////////////////////////////
//	「雑損失」の内訳書列番号（帳表入力画面で使用）
typedef enum {
	ID_COL_162_KNORDER = 1,				// 1	科目
	ID_COL_162_NAIYOU,					// 2	取引の内容
	ID_COL_162_ADNAME1,					// 3	相手先1
	ID_COL_162_ADADD1,					// 4	所在地1
	ID_COL_162_ADADD2,					// 5	所在地2
	ID_COL_162_VAL,						// 6	金額
} EnumIdFormCol162;

//////////////////////////////////////////////////////////////////
//	「雑損失」の内訳書_集計項目
#define	ID_CALC_162_MAX		1			// 集計フィールド件数
#define	ID_CALC_162_FIELD1	_T("Val")	// 集計フィールド名称1

//////////////////////////////////////////////////////////////////
//	「雑益」の内訳書_計ダイアグラムの入力インデックス
#define ID_COL_162_KEI1_MEISYO	0				// 計ダイアログ1行目の計名称（計or合計、頁計、累計）
// midori 152503 del -->
//#define ID_COL_162_KEI1_VAL		4				// 計ダイアログ1行目の金額
//#define ID_COL_162_KEI2_MEISYO	5				// 計ダイアログ2行目の計名称（計or合計、頁計、累計）
//#define ID_COL_162_KEI2_VAL		9				// 計ダイアログ2行目の金額
// midori 152503 del <--
// midori 152503 add -->
#define ID_COL_162_KEI1_VAL		5				// 計ダイアログ1行目の金額
#define ID_COL_162_KEI2_MEISYO	6				// 計ダイアログ2行目の計名称（計or合計、頁計、累計）
#define ID_COL_162_KEI2_VAL		11				// 計ダイアログ2行目の金額
// midori 152503 add <--

//////////////////////////////////////////////////////////////////
//	1レコード分の構造体
typedef struct tagRecuc_162_Zatuson_162
{
	long	m_Seq;			// シーケンス番号
	int	m_NumPage;			// 頁番号
	BYTE	m_NumRow;		// 行番号
...
} REC_UC_162_ZATUSON, *P_REC_UC_162_ZATUSON;

class CfrmUc162Zatuson: public CfrmUc000Common
{
	DECLARE_DYNCREATE( CfrmUc162Zatuson )

public:
	CfrmUc162Zatuson();
	virtual	~CfrmUc162Zatuson();
	void	virSetInitialValue();
// midori 152770 del -->
//	int		UpdateTbl162ChangeFormType();
// midori 152770 del <--
// midori 152770 add -->
	int		UpdateTbl162ChangeFormType(UCHIWAKE_INFO pInfoSv);
// midori 152770 add <--
...
private:
	...
	int		RecBufClearData( P_REC_UC_162_ZATUSON inRecData );
	int		RecBufSaveDataSub( int inPage, int inRow, P_REC_UC_162_ZATUSON inSaveData );
	int		RecBufSetData( CdbUc162Zatuson* inTbl );
	int		RecBufClearAllData();

	// 1ページ分のデータ格納用配列
	// 配列の添え字と、行番号を合わせるため、最大行+1とした
	REC_UC_162_ZATUSON	m_ReadData[ID_ROWKOBETU_162 + 1];
	REC_UC_162_ZATUSON	m_CopyData;		// 行コピー用バッファ
...
// 修正No.157626 del -->
//	CICSDiag	m_DiagMessage;					// 「雑益は、⑯－1で入力して下さい」文言
// 修正No.157626 del <--
// 修正No.157626 add -->
...
// midori 161108 add <--
	void	UpdateControlRowData( CdbUc162Zatuson* inTbl );
	void	UpdateControlRowTotal( CdbUc162Zatuson* inTbl );
