...
#include "frmUc162Zatuson.h"		// 通常/個別切り替えにて"⑯-2"データ更新が必要
...
IMPLEMENT_DYNCREATE( CfrmUc161ZatuekiI, CfrmUc000Common )


BEGIN_MESSAGE_MAP( CfrmUc161ZatuekiI, CfrmUc000Common )
	ON_WM_SIZE()
	...
	ON_COMMAND( ID_PRINTSETUP_MENU, &CfrmUc161ZatuekiI::OnMenuPrintSetup )
	ON_COMMAND( ID_PRINTALL_MENU, &CfrmUc161ZatuekiI::OnMenuPrintAll )
	ON_COMMAND( ID_OUTPUTFORM_MENU, &CfrmUc161ZatuekiI::OnMenuFormTypeSetup )
	ON_COMMAND( ID_SPECIALROW_MENU, &CfrmUc161ZatuekiI::OnMenuSpecialRow )
	ON_COMMAND( ID_SORT_MENU, &CfrmUc161ZatuekiI::OnMenuSort )
	ON_COMMAND(ID_IKKATUADD_MENU, &CfrmUc161ZatuekiI::OnIkkatuAddMenu)
	ON_COMMAND( ID_DISPDIALOG_MENU, &CfrmUc161ZatuekiI::OnMenuIkkatuKingaku )
	ON_COMMAND( ID_ZEROMONEY_MENU, &CfrmUc161ZatuekiI::OnMenuZeroMoney )
	ON_COMMAND( ID_ZEROMONEYMOVE_MENU, &CfrmUc161ZatuekiI::OnMenuZeroMoneyMove )
	ON_COMMAND( ID_AMOUTDIALOG_MENU, &CfrmUc161ZatuekiI::OnMenuOutKeiSetup )
	ON_COMMAND( ID_CLEAR_MENU, &CfrmUc161ZatuekiI::OnMenuClear )
	ON_COMMAND( ID_RENDO_MENU, &CfrmUc161ZatuekiI::OnMenuZaimuRendo )
	ON_COMMAND( ID_TITLEMENU, &CfrmUc161ZatuekiI::OnMenuTitleRestore )
	ON_COMMAND( ID_USER_SETTING_MENU, &CfrmUc161ZatuekiI::OnMenuOptionSetting )
	ON_COMMAND( ID_COPY_SETTING_INFO_MENU, &CfrmUc161ZatuekiI::OnMenuCopySettingInfo )
	ON_COMMAND( ID_DELETE_DATA_MENU, &CfrmUc161ZatuekiI::OnMenuDeleteDataEachYoushiki )
	ON_BN_CLICKED( IDC_PAGEBACK_BUTTON1, &CfrmUc161ZatuekiI::OnPageBack )
	ON_BN_CLICKED( IDC_PAGENEXT_BUTTON1, &CfrmUc161ZatuekiI::OnPageNext )
	ON_BN_CLICKED( IDC_PAGENEW_BUTTON1, &CfrmUc161ZatuekiI::OnPageNew )
	ON_BN_CLICKED( IDC_PAGEINSERT_BUTTON1, &CfrmUc161ZatuekiI::OnPageInsert )
	ON_BN_CLICKED( IDC_ROWCOPY_BUTTON1, &CfrmUc161ZatuekiI::OnRowCopy )
	ON_BN_CLICKED( IDC_ROWPASTE_BUTTON1, &CfrmUc161ZatuekiI::OnRowPaste )
	ON_BN_CLICKED( IDC_ROWINSERT_BUTTON1, &CfrmUc161ZatuekiI::OnRowInsert )
	ON_BN_CLICKED( IDC_YOUSHIKINEXT_BUTTON1, &CfrmUc161ZatuekiI::OnMoveYoushikiNext )
	ON_BN_CLICKED( IDC_YOUSHIKIBACK_BUTTON1, &CfrmUc161ZatuekiI::OnMoveYoushikiBack )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F6, &CfrmUc161ZatuekiI::OnUpdateButtonF6 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F7, &CfrmUc161ZatuekiI::OnUpdateButtonF7 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F8, &CfrmUc161ZatuekiI::OnUpdateButtonF8 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F9, &CfrmUc161ZatuekiI::OnUpdateButtonF9 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F11, &CfrmUc161ZatuekiI::OnUpdateButtonF11 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_F12, &CfrmUc161ZatuekiI::OnUpdateButtonF12 )
	ON_UPDATE_COMMAND_UI( ID_BUTTON_HOME, &CfrmUc161ZatuekiI::OnUpdateButtonHome )
	ON_UPDATE_COMMAND_UI( ID_IKKATUADD_MENU, &CfrmUc161ZatuekiI::OnUpdateIkkatuaddMenu)
	ON_UPDATE_COMMAND_UI( ID_DISPDIALOG_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuIkkatuKingaku )
	ON_UPDATE_COMMAND_UI( ID_SORT_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuSort )
	ON_UPDATE_COMMAND_UI( ID_CLEAR_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuClear )
	ON_UPDATE_COMMAND_UI( ID_ZEROMONEY_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuZeroMoney )
	ON_UPDATE_COMMAND_UI( ID_ZEROMONEYMOVE_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuZeroMoneyMove)
	ON_UPDATE_COMMAND_UI( ID_RENDO_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuZaimuRendo )
	ON_UPDATE_COMMAND_UI(ID_SPECIALROW_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuSpecialRow)
	ON_UPDATE_COMMAND_UI( ID_TITLEMENU, &CfrmUc161ZatuekiI::OnUpdateMenuTitleRestore )
	ON_UPDATE_COMMAND_UI( ID_DELETE_DATA_MENU, &CfrmUc161ZatuekiI::OnUpdateMenuDeleteDataEachYoushiki )
	ON_MESSAGE(WM_USER_REDRAWVIEW, OnUserReDrawView)
END_MESSAGE_MAP()


CfrmUc161ZatuekiI::CfrmUc161ZatuekiI()
	: CfrmUc000Common( CfrmUc161ZatuekiI::IDD )
{
...
CfrmUc161ZatuekiI::~CfrmUc161ZatuekiI()
{
	...
	TRACE(_T("***** CfrmUc161ZatuekiI::~CfrmUc161ZatuekiI\n"));
}

void CfrmUc161ZatuekiI::DoDataExchange( CDataExchange* pDX )
{
	CfrmUc000Common::DoDataExchange( pDX );
	DDX_Control( pDX, IDC_YOUSHIKIDIAG1, m_DiagNormal );
	DDX_Control( pDX, IDC_YOUSHIKIDIAG6, m_DiagNormal2 );
	DDX_Control( pDX, IDC_YOUSHIKIDIAG5, m_DiagNormal3 );
...
void CfrmUc161ZatuekiI::OnInitialUpdate()
{
...
#ifdef _DEBUG
void CfrmUc161ZatuekiI::AssertValid() const
{
	CfrmUc000Common::AssertValid();
}

#ifndef _WIN32_WCE
void CfrmUc161ZatuekiI::Dump( CDumpContext& dc ) const
{
...
BEGIN_EVENTSINK_MAP( CfrmUc161ZatuekiI, CfrmUc000Common )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG1, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG1, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG1, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG1, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG1, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG2, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG2, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG2, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG2, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG2, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG3, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG3, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG3, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG3, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG3, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG4, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG4, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG4, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG4, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG4, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG5, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG5, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG5, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG5, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG5, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG6, 3, CfrmUc161ZatuekiI::TerminationYoushikidiag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG6, 2, CfrmUc161ZatuekiI::EditOFFYoushikidiag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG6, 1, CfrmUc161ZatuekiI::EditONYoushikidiag1, VTS_I2 )
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG6, 12, CfrmUc161ZatuekiI::VScrollYoushikidiag1, VTS_I2)
	ON_EVENT(CfrmUc161ZatuekiI, IDC_YOUSHIKIDIAG6, 8, CfrmUc161ZatuekiI::ComboSelYoushikidiag1, VTS_I2 VTS_UNKNOWN)

	ON_EVENT( CfrmUc161ZatuekiI, IDC_TITLEDIAG1, 3, CfrmUc161ZatuekiI::TerminationTitlediag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_TITLEDIAG1, 2, CfrmUc161ZatuekiI::EditOFFTitlediag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_TITLEDIAG1, 1, CfrmUc161ZatuekiI::EditONTitlediag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_TITLEDIAG1, 6, CfrmUc161ZatuekiI::FocusTitlediag1, VTS_NONE )

	ON_EVENT( CfrmUc161ZatuekiI, IDC_PAGEDIAG1, 3, CfrmUc161ZatuekiI::TerminationPagediag1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_PAGEDIAG1, 2, CfrmUc161ZatuekiI::EditOFFPagediag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_PAGEDIAG1, 1, CfrmUc161ZatuekiI::EditONPagediag1, VTS_I2 )
	ON_EVENT( CfrmUc161ZatuekiI, IDC_PAGEDIAG1, 6, CfrmUc161ZatuekiI::FocusPagediag1, VTS_NONE )
END_EVENTSINK_MAP()

//********************************************************************************
//	初期処理
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::virStartJob()
{
	// 初期値設定
...
//	共通データの初期値設定
//********************************************************************************
void	CfrmUc161ZatuekiI::virSetInitialUchiwakeInfo()
{
	// 「雑益」の固有データ
	m_uInfo.intOutKeiDialog = AM_TYPE_ALL;				// 計設定ダイアログのボタン指定
	m_uInfo.intRowMax = ID_ROWKOBETU_161;				// １頁内の合計行を含む全体の行数
	m_uInfo.intRowMaxData = ID_ROWKOBETU_161 - 1;		// １頁内のデータ行数（合計行は含まず）
	m_uInfo.intColMax = ID_COLMAX_161I;					// １行内のコントロール個数
	m_uInfo.intColMaxData = ID_COLMAXDATA_161I;			// １行内の列数
	m_uInfo.intCtlStartIndex = ID_CTLSTARTINDEX_161I;	// １行目の入力項目の先頭インデックス番号
	m_uInfo.intT1StartIndex = ID_T1STARTNORMAL_161I;	// 合計行①の先頭インデックス番号
	m_uInfo.intT2StartIndex = ID_T2STARTNORMAL_161I;	// 合計行②の入力項目の先頭インデックス番号
	m_uInfo.intCursolDefPos = ID_COL_161I_NAIYOU;		// カーソル移動時の初期位置（タイトルから↓，合計から↑で使用）
	m_uInfo.intCursolKeiPos = ID_COL_161I_KNORDER;		// 合計行のカーソル移動位置（カーソル移動で使用）

	m_uInfo.intFormSeq = ID_FORMNO_161;					// 様式シーケンス番号
	m_uInfo.intFormNo = 16;								// 様式番号（uc_inf_sub）
	m_uInfo.intFormNo2 = 1;								// 様式番号枝番（uc_inf_sub）

	m_uInfo.intRowNormal = ID_ROWNORMAL_161;			// 通常時：１頁内の合計行を含む全体の行数
	m_uInfo.intRowKobetu = ID_ROWKOBETU_161;			// 個別時：１頁内の合計行を含む全体の行数
	m_uInfo.intT1StartNormal = ID_T1STARTNORMAL_161I;	// 通常時：合計行①の先頭インデックス
	m_uInfo.intT2StartNormal = ID_T2STARTNORMAL_161I;	// 通常時：合計行②の先頭インデックス
	m_uInfo.intT1StartKobetu = ID_T1STARTKOBETU_161I;	// 個別時：合計行①の先頭インデックス
	m_uInfo.intT2StartKobetu = ID_T2STARTKOBETU_161I;	// 個別時：合計行②の先頭インデックス
	m_uInfo.intRowKei = ID_ROWKEI_161;					// 帳表固定の合計行数（1:合計あり，0:なし）
	m_uInfo.intKei1StartIndex = ID_COL_161I_KEI1_MEISYO;	// 計ダイアグラム1行目の先頭インデックス
	m_uInfo.intKei2StartIndex = ID_COL_161I_KEI2_MEISYO;	// 計ダイアグラム2行目の先頭インデックス

	// 集計処理の設定
	m_uInfo.uCalcInfo.intMaxCount = ID_CALC_161_MAX;		// 集計フィールド件数
	m_uInfo.uCalcInfo.strField[0] = ID_CALC_161_FIELD1;		// 集計フィールド名称1
...
// 【戻値】    なし
//**************************************************
void	CfrmUc161ZatuekiI::virSetInitialDiag()
{
	int     nIndex;
	...
	// コンボボックスの属性取得
	nIndex = CmnGetControlIndex( 1, ID_COL_161I_KNORDER );
...
//	帳表固有変数の初期値設定
//********************************************************************************
void	CfrmUc161ZatuekiI::virSetInitialValue()
{
	// 1ページ分のローカルバッファをクリア
	RecBufClearAllData();
...
//	帳表コントロールの初期化（データ領域）
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::InitialControlDataArea()
{
	int		intMax;
	int		i;
	int		len=0;
	...
	for ( i = m_uInfo.intCtlStartIndex; i < intMax; i += m_uInfo.intColMax ) {
		// ComboBox：クリア
		CmnDiagSetString(m_pDiag, i + ID_COL_161I_KNORDER - 1, "", 0);	// 科目
		CmnDiagSetString(m_pDiag, i + ID_COL_161I_INVONOT - 1, "", 0);	// 登録番号(T選択)

		// 漢字入力欄の初期化
		CmnDiagSetAttrInpmax( m_pDiag, i + ID_COL_161I_NAIYOU  - 1, MAX_KETA_161_NAIYOU );		// 取引の内容
		CmnDiagSetAttrInpmax( m_pDiag, i + ID_COL_161I_ADNAME1 - 1, MAX_KETA_161_ADNAME1*2 );	// 相手先1
		CmnDiagSetAttrInpmax( m_pDiag, i + ID_COL_161I_ADADD1  - 1, MAX_KETA_161_ADADD1 );		// 所在地1
		CmnDiagSetAttrInpmax( m_pDiag, i + ID_COL_161I_ADADD2  - 1, MAX_KETA_161_ADADD2 );		// 所在地2

		// 金額欄：桁数初期化
		len = ID_VAL_161_K_DATA;
		CmnDiagSetAttrDigit( m_pDiag, i + ID_COL_161I_VAL - 1, len );	// 金額
	...
	// 半角/全角設定
	CmnDiagSetAttrColZenHan( m_pDiag , ID_COL_161I_NAIYOU );		// 取引の内容
	CmnDiagSetAttrColZenHan( m_pDiag , ID_COL_161I_ADNAME1 );		// 相手先1
	CmnDiagSetAttrColZenHan( m_pDiag , ID_COL_161I_ADADD1 );		// 所在地1
	CmnDiagSetAttrColZenHan( m_pDiag , ID_COL_161I_ADADD2 );		// 所在地2
...
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::InitialControlDataAreaHide( CICSDiag* pDiag )
{
	int		intMax;
	int		i;
	int		len=0;
	...
	for ( i = m_uInfo.intCtlStartIndex; i < intMax; i += m_uInfo.intColMax ) {
		// ComboBox：クリア
		CmnDiagSetString( pDiag, i + ID_COL_161I_KNORDER - 1, "", 0 );	// 科目

		// 漢字入力欄の初期化
		CmnDiagSetAttrInpmax( pDiag, i + ID_COL_161I_NAIYOU  - 1, MAX_KETA_161_NAIYOU );	// 取引の内容
		CmnDiagSetAttrInpmax( pDiag, i + ID_COL_161I_ADNAME1 - 1, MAX_KETA_161_ADNAME1*2 );	// 相手先1
		CmnDiagSetAttrInpmax( pDiag, i + ID_COL_161I_ADADD1  - 1, MAX_KETA_161_ADADD1 );	// 所在地1
		CmnDiagSetAttrInpmax( pDiag, i + ID_COL_161I_ADADD2  - 1, MAX_KETA_161_ADADD2 );	// 所在地2

		// 金額欄：桁数初期化
		len = ID_VAL_161_K_DATA;
		CmnDiagSetAttrDigit( pDiag, i + ID_COL_161I_VAL - 1, len );	// 金額
	}
...
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::InitialControlComboBox()
{
...
//	int				nFontSize = 100;
	...
	for(i = 1; i <= intMax; i++) {
		...
		// 科目
		intIndex = CmnGetControlIndex(i, ID_COL_161I_KNORDER);
		// T選択
		intIndex2 = CmnGetControlIndex(i, ID_COL_161I_INVONOT);
...
//********************************************************************************
void	CfrmUc161ZatuekiI::InitialControlTotalArea()
{
	int		nKeiType;			//	計の種別
	...
	if(flg2[0] == 1)	{
		// 一括自動行の科目を入力可能にする
		m_pDiag->ModifyItem( CmnGetControlIndex( (nDataRowPos[0]+1), ID_COL_161I_KNORDER ), (DIAG_MDFY_EDIT | DIAG_MDFY_OPAQUE) );
	}
	if(flg2[1] == 1)	{
		// 一括自動行の科目を入力可能にする
		m_pDiag->ModifyItem( CmnGetControlIndex( (nDataRowPos[1]+1), ID_COL_161I_KNORDER ), (DIAG_MDFY_EDIT | DIAG_MDFY_OPAQUE) );
	}
	...
	m_pDiagKei->ModifyItem(ID_COL_161I_KEI1_VAL, intKeiRow);
	m_pDiagKei->ModifyItem(ID_COL_161I_KEI2_VAL, intKeiRow);
}
...
//		→表示していない方のDiag(通常 or 個別)の設定に使用する
//********************************************************************************
void	CfrmUc161ZatuekiI::InitialControlTotalAreaHide( CICSDiag* pDiag )
{
	int intIndex = 0;
	...
	// 合計行の金額欄は、金額欄 + 1桁
	intIndex = m_intT1StartIndexHide + m_uInfo.intCtlStartIndex + ID_COL_161I_VAL - 1;
	CmnDiagSetAttrDigit( pDiag, intIndex, ID_VAL_161_TOTAL );

	intIndex = m_intT2StartIndexHide + m_uInfo.intCtlStartIndex + ID_COL_161I_VAL - 1;
	CmnDiagSetAttrDigit( pDiag, intIndex, ID_VAL_161_TOTAL );
}
...
//	RET	なし
//********************************************************************************
void	CfrmUc161ZatuekiI::InitialControlSpecialRow( int intRow, int intFgFunc )
{
	int				nIndex;		//	インデックス
	...
	switch ( intFgFunc ) {
		...
		case ID_FGFUNC_IKKATUAUTO:		// 一括集計金額行（自動）
			// 桁数を設定
			CmnDiagSetAttrDigit( m_pDiag, CmnGetControlIndex( intRow , ID_COL_161I_VAL ), ID_VAL_161_TOTAL );
			break;
	}
	...
	switch (intFgFunc) {
		...
		case ID_FGFUNC_IKKATUAUTO:		// 一括集計金額行（自動）
			...
			if(intFgFunc != ID_FGFUNC_IKKATUAUTO) {
				...
				m_pDiag->ModifyItem(CmnGetControlIndex(intRow, ID_COL_161I_KNORDER), DIAG_MDFY_NODROPDOWN);
				// 「登録番号(T選択)」はComboBoxのため、DropDown不可とする
				m_pDiag->ModifyItem(CmnGetControlIndex(intRow, ID_COL_161I_INVONOT), DIAG_MDFY_NODROPDOWN);

				// ComboBoxの選択項目クリア
				m_clsFunc.DiagInit(&diadata);
				diadata.text_combo = _T("\r\n");
				m_pDiag->SetData(CmnGetControlIndex(intRow, ID_COL_161I_KNORDER), (LPUNKNOWN)&diadata);
				m_pDiag->SetData(CmnGetControlIndex(intRow, ID_COL_161I_INVONOT), (LPUNKNOWN)&diadata);
			}
...
// 改良No.21-0086,21-0529 cor <--
				...
				if (intFgFunc == ID_FGFUNC_IKKATUAUTO ){
					//	科目
					m_pDiag->ModifyItem( CmnGetControlIndex( intRow, ID_COL_161I_KNORDER ), DIAG_MDFY_INPUT );
				}
			}
			break;
...
//	帳表タイトル情報の表示
//********************************************************************************
void	CfrmUc161ZatuekiI::virUpdateControlTitle()
{
...
//	帳表ページ情報の表示
//********************************************************************************
void	CfrmUc161ZatuekiI::UpdateControlPage()
{
	BOOL	bBack = TRUE;
	BOOL	bNext = TRUE;
...
//	帳表データの表示（１ページ分）
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::virUpdateControlTblData()
{
	// ダイアログ切替
	if( m_pDiag != NULL ){
		virSetInitialDiag();
	...
	CdbUc161Zatueki	dbRec( m_pDB );

	m_pDiag->DeleteInput();		//	入力削除 
	...
	// 頁計、累計行の桁数をセットする
	CmnDiagSetAttrDigit( m_pDiagKei, ID_COL_161I_KEI1_VAL, ID_VAL_161_TOTAL );
	CmnDiagSetAttrDigit( m_pDiagKei, ID_COL_161I_KEI2_VAL, ID_VAL_161_TOTAL );
	...
	while ( !dbRec.IsEOF() ) {		// ファイル終端まで
		...
		// 科目
		CmnInitialControlFgStringCell( m_pDiag, dbRec.m_NumRow, ID_COL_161I_KNORDER, dbRec.m_FgFunc, UC_KAMOKU_LEN, dbRec.m_KnName);
		// 取引の内容
		CmnInitialControlFgStringCell( m_pDiag, dbRec.m_NumRow, ID_COL_161I_NAIYOU, dbRec.m_FgFunc, UC_161_NAIYOU_LEN, dbRec.m_Naiyou);
...
//	保管件数の表示
//********************************************************************************
BOOL CfrmUc161ZatuekiI::virUpdateControlTblData2()
{
	int					hcnt=0;
	CString				cs=_T("");
...
//		IN		CdbUc161Zatueki*	「雑益」
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::UpdateControlRowData( CdbUc161Zatueki* prmDbRec ) {
	DIAGRAM_DATA		diadata;		// ICSDiag構造体
	int					intCell;		// 保存フィールド
	CString				cs1, cs2, cst;
	...
	// 読込み対象セル取得
	intCell = CmnGetControlIndex( prmDbRec->m_NumRow, ID_COL_161I_KNORDER );
...
//********************************************************************************
//	帳表データの１行表示（累計，頁計行）
//		IN		CdbUc161Zatueki*	「雑益」テーブル情報
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::UpdateControlRowTotal( CdbUc161Zatueki* prmDbRec )
{
	int		intTotalType;
	...
	// 頁計/累計の名称表示
	if ( intRowName > 0 ) {
		CmnDiagSetString( m_pDiagKei, CmnGetKeiDiagIndex( intRowName, ID_COL_161I_KEI1_MEISYO ), prmDbRec->m_KeiStr, 1 );
	}
	// 頁計/累計の金額表示
	if ( intRowVal > 0 ) {
		CmnDiagSetKingaku( m_pDiagKei, CmnGetKeiDiagIndex( intRowVal, ID_COL_161I_KEI1_VAL ), prmDbRec->m_Val );
	}
}
...
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virFncCellCopyProc( CdbUc000Common* rsData )
{
	CdbUc161Zatueki*	rs;
	int					intCol;	// 現在の列
	int					ind1,ind2;
	...
	switch( intCol ){
		// 科目
		case ID_COL_161I_KNORDER:
			CmnDiagSetCombo2( m_pDiag, m_uInfo.intCurCtlIndex, rs->m_KnOrder );
			break;
		// 取引の内容
		case ID_COL_161I_NAIYOU:
			CmnDiagSetString( m_pDiag, m_uInfo.intCurCtlIndex, rs->m_Naiyou, 0 );
			break;
		// 登録番号（T選択）
		case ID_COL_161I_INVONOT:
		// 登録番号
		case ID_COL_161I_INVONO:
			if(intCol == ID_COL_161I_INVONOT) {
...
//********************************************************************************
int	CfrmUc161ZatuekiI::virFncReferenceGetType(int intCol, int* intGrSeq, long* lngItemSeq, int* intAdd1, int* intAdd2, int* intAdd1Col, int* intAdd2Col, REFERENCE_INFO* pudReference)
{
	int		intRefType;
	...
	switch ( intCol ) {
		...
		case ID_COL_161I_ADADD2:
			...
			*intAdd1 = MAX_KETA_161_ADADD1 * 2;			// Byte指定
			*intAdd2 = MAX_KETA_161_ADADD2 * 2;			// Byte指定
			*intAdd1Col = ID_COL_161I_ADADD1;
			*intAdd2Col = ID_COL_161I_ADADD2;
			break;
...
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virFncReferenceSetData( CdlgReference* dlgRef, int intRefType, int intCol )
{
	...
	switch(intRefType) {
		...
		case ID_DLGTYPE_ADDRESS:
			// 選択内容を表示
			//	登録番号（T選択）
			CmnDiagSetCombo3(m_pDiag, CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_INVONOT), dlgRef->m_udAddress.m_InvNo);
			// 登録番号
			InvoNoSplit(dlgRef->m_udAddress.m_InvNo, &str1, &str2);
			CmnDiagSetString(m_pDiag, CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_INVONO), str2, 0);
			// 名称
			CmnDiagSetString(m_pDiag, CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_ADNAME1),
								m_clsFunc.StrDocking(dlgRef->m_udAddress.m_AdName1, dlgRef->m_udAddress.m_AdName2), 0);
			// 所在地（上段）
			CmnDiagSetString(m_pDiag, CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_ADADD1), dlgRef->m_udAddress.m_AdAdd1, 0);
			// 所在地（下段）
			CmnDiagSetString(m_pDiag, CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_ADADD2), dlgRef->m_udAddress.m_AdAdd2, 0);
			break;
...
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virFncReferenceSetICSData(CdlgAddressSearch* dlgAddd)
{
	int			intIndex;

	// 売却(買入)先の所在地1(住所)
	intIndex = CmnGetControlIndex( m_uInfo.intCurRow, ID_COL_161I_ADADD1 );
	CmnDiagSetString( m_pDiag, intIndex, dlgAddd->m_AdAdd1, 0 );

	// 売却(買入)先の所在地2(住所)
	intIndex = CmnGetControlIndex( m_uInfo.intCurRow, ID_COL_161I_ADADD2 );
...
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virFncReferenceUpdateTbl( int intRefType, int nRefMode/*=0*/ )
{
...
void CfrmUc161ZatuekiI::virActivateFrameK(UINT nState, CFrameWnd* pDeactivateFrame)
{
	int		index = 0;
	...
	if(pWnd != NULL)	{
		if(m_DataKakutei == FALSE)	{
			if(m_uInfo.intCurCol == ID_COL_161I_KNORDER)	{
				virKamokuAddCancel();
			}
		}
	}
...
void CfrmUc161ZatuekiI::virKamokuAddCancel()
{
	...
	// 科目列なら
	if(m_uInfo.intCurCol == ID_COL_161I_KNORDER && m_F9OnSw != TRUE)	{
		DIAGRAM_DATA	diadata;
		m_pDiag->GetData(m_uInfo.intCurCtlIndex,(LPUNKNOWN)&diadata);
		// 【新規作成】が選択されていれば
		if(diadata.data_combo > m_CmbCnt)	{
			// 前回の値に戻す
			CmnDiagSetCombo(m_pDiag,CmnGetControlIndex(m_uInfo.intCurRow,ID_COL_161I_KNORDER),m_uInfo.OldCombo);
		}
	}
}
...
//	その他イベント処理
//********************************************************************************
BOOL	CfrmUc161ZatuekiI::PreTranslateMessage( MSG* pMsg )
{
...
// 修正No.168362 add <--
						...
						if(intCol == ID_COL_161I_KNORDER || intCol == ID_COL_161I_INVONOT)	{
							rv=1;
						}
// 修正No.168362 add -->
					} 
// 修正No.168362 add <--
			...
			case VK_RIGHT:
				if(nG_MScroll == 1)	{
					...
					intCol = CmnGetControlCol();
					if(intCol == ID_COL_161I_KNORDER && (pMsg->wParam == VK_LEFT || pMsg->wParam == VK_RIGHT))	{
...
// 頁コントロールのフォーカス設定
void	CfrmUc161ZatuekiI::FocusPagediag1()
{
}

// 頁コントロールのゲットフォーカス
void	CfrmUc161ZatuekiI::EditONPagediag1( short index )
{
// 修正No.168443 add -->
	// ---- 登録番号一覧関連 ----->
	if(pZmSel && pZmSel->IsDisplay() != FALSE) {
...
// 頁コントロールのロストフォーカス
void	CfrmUc161ZatuekiI::EditOFFPagediag1( short index )
{
...
// 頁コントロールのキー操作
void	CfrmUc161ZatuekiI::TerminationPagediag1( short index, short nChar, short length, LPUNKNOWN data )
{
	int nMaxDataCol = 0;
	// カレント頁の合計行の種別を取得
	...
	switch ( nChar ) {
		...
		case VK_TAB:
			...
			if ( CmnCheckShiftKey() == FALSE ) {
				...
				CmnDiagSetFocus(m_pDiag, nMaxDataCol, ID_COL_161I_VAL);
			}
			break;
...
// タイトルコントロールのフォーカス設定
void	CfrmUc161ZatuekiI::FocusTitlediag1()
{
}

// タイトルコントロールのゲットフォーカス
void	CfrmUc161ZatuekiI::EditONTitlediag1( short index )
{
// 修正No.168443 add -->
	// ---- 登録番号一覧関連 ----->
	if(pZmSel && pZmSel->IsDisplay() != FALSE) {
...
// タイトルコントロールのロストフォーカス
void	CfrmUc161ZatuekiI::EditOFFTitlediag1 (short index )
{
	m_Titlefocus = 0;
	virUpdateTitle();
}
...
//	タイトル更新
void CfrmUc161ZatuekiI::virUpdateTitle()
{
...
// タイトルコントロールのキーイベント
void	CfrmUc161ZatuekiI::TerminationTitlediag1( short index, short nChar, short length, LPUNKNOWN data )
{
	// フォーカス移動
	switch ( nChar ) {
	case VK_RETURN:
...
//********************************************************************************
//	「雑益」データコントロール処理（EditON，EditOFF，Terminationイベント）
//********************************************************************************

// 「雑益」データコントロールのゲットフォーカス
void	CfrmUc161ZatuekiI::EditONYoushikidiag1( short index )
{
	DIAGRAM_DATA	diadata;
	...
	switch(m_uInfo.intCurCol) {
		...
		case ID_COL_161I_INVONOT:	// 登録番号(T選択)
			// コンボボックスのオフセット値取得
			m_pDiag->GetData(m_uInfo.intCurCtlIndex, (LPUNKNOWN)&diadata);
			m_uInfo.OldCombo = diadata.data_combo;
			break;
	...
	if ( ( m_uInfo.OldRow > 0 ) ) {
		if ( m_uInfo.OldPage == m_uInfo.intCurPage && m_uInfo.OldPage > 0 ) {
			...
			BOOL adadd1_to_adadd2 = IsSaveMoveReference(ID_COL_161I_ADADD1, ID_COL_161I_ADADD2);
			// [Home]キー押下の住所検索ダイアログの戻り（フォーカスをセットしている）は保存しない（改良依頼No15-1613）
			BOOL adadd1_to_adadd1 = IsSaveMoveReference(ID_COL_161I_ADADD1, ID_COL_161I_ADADD1);	// 所在地1(住所)→所在地1(住所)
			BOOL adadd2_to_adadd2 = IsSaveMoveReference(ID_COL_161I_ADADD2, ID_COL_161I_ADADD2);	// 所在地2(住所)→所在地2(住所)　所在地2から住所が長すぎる場合、ここがFALSEになる
			// T選択⇔登録番号は保存しない
			BOOL tsel_to_invno = IsSaveMoveReference(ID_COL_161I_INVONOT, ID_COL_161I_INVONO);
	...
	if(m_uInfo.intCurCol == ID_COL_161I_INVONO && m_EndSyorityu == 0)	{
// 修正No.168377 del -->
...
// 修正No.168377 add -->
		// 登録番号のT選択の値を取得
		int pos = CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_INVONOT);
...
// データコントロールのロストフォーカス
void	CfrmUc161ZatuekiI::EditOFFYoushikidiag1( short index )
{
	if( m_EndView != FALSE )	return;

// 修正No.168443 add -->
	// ---- 登録番号一覧関連 ----->
	if(m_uInfo.intCurCol != ID_COL_161I_INVONO) {
		if(pZmSel && pZmSel->IsDisplay() != FALSE) {
			pZmSel->DialogOFF();
		}
	}
...
// 修正No.168443 del -->
	...
	//		//// カーソル移動処理
	//		//CmnDiagSetFocus( m_pDiag, m_uInfo.intCurRow, ID_COL_161I_INVONO );
	//	}
	//}

	//// ---- 登録番号一覧関連 ----->
	//if(pZmSel && m_uInfo.intCurCol != ID_COL_161I_INVONO) {
...
//	入力確定
void CfrmUc161ZatuekiI::virInputDecisionEditOFF()
{
	// フォーカスフラグOFF
...
// 預貯金データコントロールのキーイベント
void	CfrmUc161ZatuekiI::TerminationYoushikidiag1( short index, short nChar, short length, LPUNKNOWN data )
{
	//--------------------------------------------------------------------------------
	//	キー操作仕様
...
void CfrmUc161ZatuekiI::VScrollYoushikidiag1(short mode)
{
// 修正No.168430 del -->
...
//********************************************************************************
int	CfrmUc161ZatuekiI::GetFocusPosition( int nChar )
{
	int		intRow;				// カレント行番号
	int		intCol;				// カレント列番号
	...
	else if ( intCol == ID_COL_161I_ADADD1 ) {
		// カレント列が「所在地1」の場合
	...
	else if ( intCol == ID_COL_161I_ADADD2 ) {
		// カレント列が「所在地2」の場合
	...
	else if ( intCol == ID_COL_161I_VAL ) {
		// カレント列が「金額」の場合
		intVector[0] = m_uInfo.intCurCtlIndex - m_uInfo.intColMax;		// ↑
	...
	case VK_RETURN:	// Enterキーでカーソルを下に移動するモード対応
		if(m_ReturnMoveDown) {
			if(intCol == ID_COL_161I_INVONOT)	intNext = intVector[1];			//	カレント列が「登録番号(T表示)」欄の場合「→」キーと同じ
	...
	case VK_TAB:
		if ( CmnCheckShiftKey() == FALSE ) {
			...
			// それ以外は「←」キーと同じ
			if ( ( intRow <= 1 ) && ( intCol == ID_COL_161I_KNORDER ) ) {
				return -1;
			}
...
//********************************************************************************
int	CfrmUc161ZatuekiI::GetTotalRowIndex( int nCharOrg, int intIndex, int intNext )
{
	...
	if ( nNowRow <= nBorder[0] ){		
		...
		if (( intNext > CmnGetControlIndex( m_uInfo.intRowMax, ID_COL_161I_VAL ) ) && ( nCharOrg == VK_TAB )){
			return( -2 );
		}
		//	データ行を超えた？かつ計行あり？
	...
	//	現在、頁計に居る
		...
		case VK_UP:			// ↑：23行目のデータ行の支店名項目へ移動
			nRet = CmnGetControlIndex( nBorder[0], ID_COL_161I_NAIYOU );
			break;
		case VK_LEFT:		// ←/Shift+Tab：23行目のデータ行の摘要項目へ移動
			nRet = CmnGetControlIndex( nBorder[0], ID_COL_161I_VAL );
			break;
		default:			// →/↓/Return/Tab：25行目の合計行②へ移動
			...
			//	累計なし
				...
				nRet = CmnGetControlIndex( ( nBorder[2] + 2 ), ID_COL_161I_KNORDER );
			}
			break;
		}
	}
	//	現在、累計に居る場合
		...
		case VK_LEFT:		// ←/Shift+Tab：24行目の合計行① or 24行目のデータ行の摘要項目へ移動
			nRet = CmnGetControlIndex( ( nBorder[1] + 2 ), ID_COL_161I_KNORDER );
			break;
		default:			// →/↓：移動できない
			// 例外処理：最終項目で"Tab"は別コントロールへ移動
			// それ以外は「→」キーと同じ
...
//		RET		なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virTblEditIkkatuLine( int nType, CdbUc000Common* rsData,
									CString strName, CALCKEI_INFO uCalcKei, SORTKAMOKU_INFO uKamoku )
{
	CdbUc161Zatueki*	rs = (CdbUc161Zatueki*)rsData;
...
//********************************************************************************
int	CfrmUc161ZatuekiI::virTblSortSubSortProc( CdlgSort* pCdlgSort, CdbUc000Common* rsData )
{
	CdbUc161Zatueki*	rs;
	CString				strFilter;
	CString				strSort;
...
/**********************************************************************
	RecBufSetData(CdbUc161Zatueki* prmDbRec)
		ローカルのレコードバッファに、1行分のデータを格納する

	引数
		CdbUc161Zatueki*	prmDbRec	レコードセット(データがあること）
	戻値
		int		成功/失敗
				FUNCTION_OK
***********************************************************************/
int	CfrmUc161ZatuekiI::RecBufSetData( CdbUc161Zatueki* prmDbRec )
{
...
/**********************************************************************
	...
	戻値
		int		成功/失敗
				FUNCTION_OK
***********************************************************************/
int	CfrmUc161ZatuekiI::RecBufClearAllData()
{
...
/**********************************************************************
	...
	引数
		P_REC_UC_161_ZATUEKI inRecData レコードへのポインタ
	戻値
		int		成功/失敗
				FUNCTION_OK
***********************************************************************/
int	CfrmUc161ZatuekiI::RecBufClearData( P_REC_UC_161_ZATUEKI inRecData )
{
...
/**********************************************************************
	...
	戻値
		int				成功(更新あり)/成功(更新なし)/失敗
						...
						FUNCTION_NG		: 失敗
***********************************************************************/
int	CfrmUc161ZatuekiI::virRecBufSaveData( int inPage, int inRow )
{
	REC_UC_161_ZATUEKI	tmpSaveData;	// 画面データ1レコード格納用
	...
	// 科目
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_KNORDER );
	...
	// 取引の内容
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_NAIYOU );
	tmpSaveData.m_Naiyou = m_clsFunc.DiagGetString( m_pDiag, intIndex );

	// 登録番号（T選択）
	intIndex = CmnGetControlIndex(inRow, ID_COL_161I_INVONOT);
	m_pDiag->GetData(intIndex, (LPUNKNOWN)&diadata);
	tsel = diadata.data_combo;

	// 登録番号（法人番号）
	intIndex = CmnGetControlIndex(inRow, ID_COL_161I_INVONO);
...
// 修正No.168534,168539 add <--
	...
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_ADNAME1 );
	...
	if ( tmpSaveData.m_AdName2.IsEmpty() ){
		if(m_uInfo.intCurCol == ID_COL_161I_ADNAME1 && m_uInfo.intCurRow != m_uInfo.OldRow){	// 各カラムごとにvirRecBufSaveData()がコールされるようになり、かつタイミングがEditOffからEditOnに移ったことによりCmnDiagSetString()でカレットが末尾から先頭に移ってしまう為、制御
			CmnDiagSetString( m_pDiag, intIndex, tmpSaveData.m_AdName1, 0 );
		}
	}

	// 所在地1
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_ADADD1 );
	tmpSaveData.m_AdAdd1 = m_clsFunc.DiagGetString( m_pDiag, intIndex );

	// 所在地2
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_ADADD2 );
	tmpSaveData.m_AdAdd2 = m_clsFunc.DiagGetString( m_pDiag, intIndex );

	// 金額
	intIndex = CmnGetControlIndex( inRow, ID_COL_161I_VAL );
...
***********************************************************************/
int	CfrmUc161ZatuekiI::virRecBufSaveDataForPasteInsert( int inPage, int inRow )
{
...
***********************************************************************/
int	CfrmUc161ZatuekiI::virRecBufSaveDataForPaste( int inPage, int inRow )
{
	///////////////////////////////////////////////////////////////
	// 登録処理呼び出し
	return RecBufSaveDataSub( inPage, inRow, &m_CopyData );
...
/**********************************************************************
	...
	戻値
			int					成功(更新あり)/成功(更新なし)/失敗
								...
								FUNCTION_NG		: 失敗
***********************************************************************/
int	CfrmUc161ZatuekiI::RecBufSaveDataSub( 
							int inPage,
							int inRow,
							P_REC_UC_161_ZATUEKI inSaveData )
{
	REC_UC_161_ZATUEKI		udtTemp;
	CString					cst;
	...
	if ( ( udtTemp.m_FgFunc == ID_FGFUNC_DATA ) || ( udtTemp.m_FgFunc == ID_FGFUNC_IKKATUMANUAL ) ) {	// データ行or一括集計金額行（手動）
		...
		// 取引先名称リストへの登録チェック
// 修正No.168534,168539 add -->
		if((udtTemp.m_FgFunc == ID_FGFUNC_DATA) && (m_InvnoErrFlg == 0 && ufgInvNo == TRUE))	{	// 変更ありの場合
			CdbUcLstAddress		rsAdd(m_pDB);

			// 自動登録しない→自動登録する
			if(m_RegAutoRef)	udtTemp.m_AdSeq = rsAdd.CheckAdSeqEntry_Inv(udtTemp.m_AdSeq, udtTemp.m_AdName1, udtTemp.m_AdName2, udtTemp.m_AdAdd1, udtTemp.m_AdAdd2, udtTemp.m_InvNo, ID_ADDRESSGR_KAI, FALSE, TRUE, 0);
			else				udtTemp.m_AdSeq = rsAdd.GetSeqFromAdName_Inv(ID_ADDRESSGR_KAI, udtTemp.m_AdName1, udtTemp.m_AdName2, udtTemp.m_AdAdd1, udtTemp.m_AdAdd2, udtTemp.m_InvNo);
...
////			((ufgAdName1 == TRUE) || (ufgAdName2 == TRUE) || (ufgAdAdd1 == TRUE) || (ufgAdAdd2 == TRUE) || (ufgInvNo == TRUE))) {	// 変更ありの場合							// 修正No.168443 del
//			((ufgAdName1 == TRUE) || (ufgAdName2 == TRUE) || (ufgAdAdd1 == TRUE) || (ufgAdAdd2 == TRUE) || (m_InvnoErrFlg == 0 && ufgInvNo == TRUE)))	{	// 変更ありの場合	// 修正No.168443 del
// 修正No.168534,168539 del <--
// 修正No.168534,168539 add -->
		if ((udtTemp.m_FgFunc == ID_FGFUNC_DATA) && 
			((ufgAdName1 == TRUE) || (ufgAdName2 == TRUE) || (ufgAdAdd1 == TRUE) || (ufgAdAdd2 == TRUE)))	{	// 変更ありの場合	// 修正No.168443 del
// 修正No.168534,168539 add <--
				CdbUcLstAddress		rsAdd( m_pDB );

				// 自動登録しない→自動登録する（2016/01/28）
				if(m_RegAutoRef)	udtTemp.m_AdSeq = rsAdd.CheckEntry_Inv(udtTemp.m_AdName1, udtTemp.m_AdName2, udtTemp.m_AdAdd1, udtTemp.m_AdAdd2, udtTemp.m_InvNo, ID_ADDRESSGR_KAI, TRUE);
				else				udtTemp.m_AdSeq = rsAdd.GetSeqFromAdName_Inv(ID_ADDRESSGR_KAI, udtTemp.m_AdName1, udtTemp.m_AdName2, udtTemp.m_AdAdd1, udtTemp.m_AdAdd2, udtTemp.m_InvNo);
		...
		if ( ufgUpData == TRUE ) {		// テーブルへ更新(1レコード更新)
			...
			if ( !rs.IsEOF() ) {		// レコードが取得できた(取得できないことはないはず)
				...
				// 相手先
				if(ufgAdName1 == TRUE || ufgAdName2 == TRUE || ufgInvNo == TRUE)	{	// 変更ありの場合
					// 末尾スペース削除後に空行削除処理
...
//********************************************************************************
//	「雑益」テーブルのオブジェクトを生成し、ポインタを返す
//********************************************************************************
CdbUc000Common*	CfrmUc161ZatuekiI::virGetDataTableObject()
{
	// 「雑益」テーブルのオブジェクトを生成し、ポインタを返す
	return (CdbUc000Common*)( new CdbUc161Zatueki( m_pDB ) );
}
...
//	RET	なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virRowButtonCopyProc()
{
...
//	RET	なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virTblGetKamokuInfo( CdbUc000Common* rsData, SORTKAMOKU_INFO* uKamoku )
{
	CdbUc161Zatueki*	rs = (CdbUc161Zatueki*)rsData;
...
//	RET	なし
//********************************************************************************
void	CfrmUc161ZatuekiI::virGetCurrentInfo( CURRENT_INFO* uCurInfo )
{
...
//********************************************************************************
int	CfrmUc161ZatuekiI::virUpdateRelationTblChengeFormType()
{
	// ⑯-1で通常/個別を変更した場合、⑯-2の頁構成の更新が必要となる
	//CfrmUc162Zatuson	pfrm162;
	CfrmUc162ZatusonI	pfrm162;
	int					intRet = pfrm162.UpdateTbl162ChangeFormType(m_uInfoSv);
...
//		RET		BOOL		TRUE:正常終了，FALSE:エラー
//********************************************************************************
BOOL CfrmUc161ZatuekiI::virUpdateRelationTblChengeFormTypeSave()
{
	...
	ret = pfrm162.CmnSetInitialUchiwakeInfo(ID_FORMNO_162);

	m_uInfoSv = pfrm162.m_uInfo;
...
//		RET		なし
//********************************************************************************
int	CfrmUc161ZatuekiI::UpdateTbl161ChangeFormType(UCHIWAKE_INFO pInfoSv)
{
	// データ無しならデータ更新しない
	...
	// 初期値設定
	CmnSetInitialUchiwakeInfo(ID_FORMNO_161);	// 共通データ
	virSetInitialValue();						// 帳表固有データ
...
//	【戻値】	なし
//**************************************************
void CfrmUc161ZatuekiI::SetInputMode( int nRow, long nMode )
{
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_KNORDER ), nMode );	//	科目
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_NAIYOU ), nMode );	//	取引の内容
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_INVONOT ), nMode );	//	登録番号（T選択）
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_INVONO ), nMode );	//	登録番号
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_ADNAME1 ), nMode );	//	相手先（上段）
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_ADADD1 ), nMode );	//	所在地（上段）
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_ADADD2 ), nMode );	//	所在地（下段）
	m_pDiag->ModifyItem( CmnGetControlIndex( nRow, ID_COL_161I_VAL ), nMode );		//	金額
}
...
//	【戻値】	なし
//**************************************************
void CfrmUc161ZatuekiI::SetDiagBackColor()
{
...
//		RET		なし					
//********************************************************************************
void CfrmUc161ZatuekiI::virRendoCreateData(CdbUc000Common* rsData, RENTEMP_INFO* uRenInfo)
{
	CString			strTemp;
	...
	switch( uRenInfo->intOpRenType ){
		...
		// 仕訳データの場合
		case ID_RENDATA_DA:
			intSeq = rsLstAdd.CheckEntryRen_Inv(_T(""), _T(""), _T(""), _T(""), _T(""), uRenInfo->strInvNo, ID_ADDRESSGR_URI, TRUE);
			if(intSeq > 0) {
				rs->m_AdSeq		= rsLstAdd.m_AdSeq;
				rs->m_AdOrder	= rsLstAdd.m_OrderNum;
				rs->m_InvNo		= rsLstAdd.m_InvNo;
			...
			m_clsFunc.StrDivisionEx( uRenInfo->strAdName , &strArry , ( MAX_KETA_161_NAIYOU / 2 ) , 2 );
...
//		RET		なし					
//********************************************************************************
void CfrmUc161ZatuekiI::virRendoUpdateData(CdbUc000Common* rsData, RENTEMP_INFO* uRenInfo)
{
	CdbUc161Zatueki*	rs = (CdbUc161Zatueki*)rsData;
...
//***************************************************************************
BOOL CfrmUc161ZatuekiI::virEndProc(void)
{
	TRACE(_T("***** CfrmUc161ZatuekiI::virEndProc\n"));
...
void CfrmUc161ZatuekiI::ComboNew(short index)
{
	short			sv_sel=0;
...
int CfrmUc161ZatuekiI::CursorControl(short nChar, int sw)
{
	int				rv=0;
	int				intCol=0;
	int				pos=0;
	...
	if((nChar == VK_TAB && (key & 0x80) == 0x80) || (nChar == VK_LEFT) || (nChar == VK_RIGHT)) {
		...
		if(intCol == ID_COL_161I_INVONOT)	{	// 登録番号(T選択)
			m_pDiag->ComboDropDown(m_uInfo.intCurCtlIndex, FALSE);
			CmnDiagSetCombo(m_pDiag, m_uInfo.intCurCtlIndex, m_uInfo.OldCombo);
		}
	}
...
void CfrmUc161ZatuekiI::ComboSelYoushikidiag1(short index, LPUNKNOWN data)
{
...
// ------------------------------------------------------------------------------------------------------------------------
void CfrmUc161ZatuekiI::TitleColorChg(void)
{
	int					nCharOver[2]={0};	// 電子申告の文字数を超えたかどうかのサイン		1:電子申告の文字数を超えた
											// 様式⑯－１	[0]:科目(10文字)	[1]:取引の内容(30文字)要(50文字)
	CRecordset			rs(m_pDB);
	CString				strSQL=_T("");
	CString				strData=_T("");

	// 科目
	strSQL.Format(_T("select max(oversgn) from (select CASE when len(%s) > %d then 1 else 0 end as oversgn from uc_161_zatueki where FgShow=%d) tbl1"),
	...
	// 取引の内容
	strSQL.Format(_T("select max(oversgn) from (select CASE when len(%s) > %d then 1 else 0 end as oversgn from uc_161_zatueki where FgShow=%d) tbl1"),
		MakeReplaceSent(_T("Naiyou")),UC_161_NAIYOU_LEN,ID_FGSHOW_OFF);
	rs.Open( CRecordset::forwardOnly, strSQL);
	rs.GetFieldValue((short)0, strData);
	rs.Close();
	nCharOver[1] = 0;
...
LRESULT CfrmUc161ZatuekiI::OnUserReDrawView(WPARAM wParam, LPARAM lParam)
{
	virUpdateControlTblData();
	CmnDiagSetFocus(m_pDiag, 1, 1);
...
//		RET		なし
// --------------------------------------------------------------------------------------------------------
void CfrmUc161ZatuekiI::virDiagGetHjnData(CDatabase* pDB, HJN_REC_NEW* phjnRec)
{
	int				nPage,nRow;
	int				intIndex;	// 対象行の各インデックス
	int				tsel=0;
	...
	// 登録番号（法人番号）
	cst.Empty();
	intIndex = CmnGetControlIndex(nRow, ID_COL_161I_INVONO);
	...
	// 名称1(氏名)
	intIndex = CmnGetControlIndex(nRow, ID_COL_161I_ADNAME1);
	m_clsFunc.StrDivision(m_clsFunc.DiagGetString(m_pDiag, intIndex), &strArray, 2, FALSE, TRUE);
	cst.Empty();
	...
	// 所在地1(住所)
	intIndex = CmnGetControlIndex(nRow, ID_COL_161I_ADADD1);
	cst = m_clsFunc.DiagGetString(m_pDiag, intIndex);
	cst.TrimRight();
	if(cst.IsEmpty() == FALSE)	{
		memset(buf, 0, sizeof(buf));
	...
	// 所在地2(住所)
	intIndex = CmnGetControlIndex(nRow, ID_COL_161I_ADADD2);
	cst = m_clsFunc.DiagGetString(m_pDiag, intIndex);
	cst.TrimRight();
	if(cst.IsEmpty() == FALSE) {
		memset(buf, 0, sizeof(buf));
...
//		RET		なし
// --------------------------------------------------------------------------------------------------------
void	CfrmUc161ZatuekiI::virDiagSetHjnData(int pSw, CString rtInvNo, CString rtName, CString rtAdrs1, CString rtAdrs2, CString rtKana)
{
	int					ret = 0;
	int					intIndex;
...
// 修正No.168361,168369,168371,168378 del <--
		// 登録番号（T選択）
		intIndex = CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_INVONOT);
		CmnDiagSetCombo3(m_pDiag, intIndex, strInvoNo);

		// 登録番号
		InvoNoSplit(strInvoNo, &cs1, &cs2);
		intIndex = CmnGetControlIndex(m_uInfo.intCurRow, ID_COL_161I_INVONO);
...
// --------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------
int CfrmUc161ZatuekiI::virCallChkInvoiceNum(int pSw, int inPage, int inRow)
{
...
//				CString		pInvno	登録番号
// --------------------------------------------------------------------------------------------------------
int CfrmUc161ZatuekiI::ChkInvoiceNum(CDatabase* pDB, int pSw, int inPage, int inRow,
...
// 修正No.168361,168369,168371,168378 del -->
	...
	//		DIAGRAM_DATA	ddata;
	//		ddata.data_combo = 0;
	//		m_pDiag->SetData(CmnGetControlIndex(inRow, ID_COL_161I_INVONOT), (LPUNKNOWN)&ddata);
	//		ddata.data_edit.Empty();
	//		m_pDiag->DataClear(CmnGetControlIndex(inRow, ID_COL_161I_INVONO), TRUE);
	//		// エラーメッセージを表示
...
// 修正No.168443 add <--
			...
			if(pZmSel && pZmSel->IsDisplay() != FALSE && cst.GetLength() < 3) {
				...
				if(tmpzd.errflg != -1) {
					...
					m_pDiag->SetData(CmnGetControlIndex(inRow, ID_COL_161I_INVONO), (LPUNKNOWN)&ddata);
					// 再描画
					m_pDiag->Refresh();
...
// 修正No.168425,168443 add -->
					if(pSw == 3 || m_EndSyorityu == 1)	{
						...
						m_uInfo.intCurCol = ID_COL_161I_INVONO;
