...
// CShinSell31Ex2 ダイアログ

IMPLEMENT_DYNAMIC(CShinSell31Ex2, CSyzBaseDlg)

CShinSell31Ex2::CShinSell31Ex2(CWnd* pParent /*=nullptr*/)
	: CSyzBaseDlg(IDD_DIALOG_SELL31_ex2, pParent)
	, m_pParent ( pParent )
	, m_Idx (0)
	, m_Lin (0)
{
...
CShinSell31Ex2::~CShinSell31Ex2()
{
}

void CShinSell31Ex2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinSell31Ex2, CSyzBaseDlg)
END_MESSAGE_MAP()


BOOL CShinSell31Ex2::OnInitDialog()
{
...
int CShinSell31Ex2::InitDiagAttr()
{
	DIAGRAM_ATTRIBUTE data;
	long dg1[] = { SEL31NAMEex2, -1 };
	...
	while (1)
	{
		if (Sell31Ex2[cnt].Index == 0) {
			break;
		}
		int Flg;
		if ((*m_pSnHeadData)->Sn_Sign4 & 0x01) {	// 確定時は入力不可
			Flg = Sell31Ex2[cnt].AllUnConnect;
		}
		else {
			if ((*m_pSnHeadData)->Sn_Sign4 & 0x80) {	// 非連動
				// 経過措置ありのときは6.3％入力不可
				//if (!((*m_pSnHeadData)->SVmzsw) && (Sell31Ex2[cnt].UnConnect == 2)) {
				//	Flg = 0;
				//}
				//else
				//{
					Flg = Sell31Ex2[cnt].UnConnect;
				//}
			}
			else {
				// 経過措置ありのときは6.3％入力不可
				/*if (!((*m_pSnHeadData)->SVmzsw) && (Sell31Ex2[cnt].Connect == 2)) {
					Flg = 0;
				}
				else*/
					Flg = Sell31Ex2[cnt].Connect;
			}
		}
		ATRIB_MOD(IDC_ICSDIAGCTRL1, Sell31Ex2[cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, Sell31Ex2[cnt].Connect);
		cnt++;
	}
...
//// 追加文言の表示/非表示
////=====================================================
//void CShinSell31Ex2::InitAddText()
//{
//	long flg = 0; 
...
void CShinSell31Ex2::SetItemPosition()
{
...
// -----------------------
	...
	if((*m_pSnHeadData)->Sn_Sign4 & 0x80)	{
		m_Sell31.GetAttribute(SEL3101ex2,(LPUNKNOWN)&da);
		if(da.attr_bcolor == BC_CREAM || da.attr_bcolor == BC_BLUE){
			m_Sell31.SetPosition(SEL3101ex2);
		}
		else	{
			m_Sell31.SetPosition(SEL3101Aex2);
		}
	}
	else {	// 連動
		m_Sell31.GetAttribute(SEL3104ex2,(LPUNKNOWN)&da);
		if(da.attr_bcolor == BC_CREAM || da.attr_bcolor == BC_BLUE){
			m_Sell31.SetPosition(SEL3104ex2);
		}
		else	{
			m_Sell31.SetPosition(SEL3104Aex2);
		}
	}
// 22-0167,22-0361 cor <--
	SetRedrawScroll(0);
...
void CShinSell31Ex2::SetItemPositionEx()
{
	this->SetFocus();
	DIAGRAM_ATTRIBUTE	da;
	// 非連動
...
int CShinSell31Ex2::ChangeColor( unsigned short id, short index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
//---------------------------------------------------------
int CShinSell31Ex2::FocusMoveCheck(int index, int move)
{
	int idx = 0;
	int i = 0 , j = 0;
	BOOL end_flg = FALSE;
	...
	//現在ポジション検索
	if(( move == 0 )||( move == 1 )){
		for(i = 0 ; i < FCS_SEL31CNTex2 ; i++){
			if(index == FCS_Sell31Ex2[i].IDNo){
				break;
			}
		}
		idx = FCS_Sell31Ex2[i].IDNo;//現在ポジションセット
	}else if(( move == 2 )||( move == 3 )){
		for(i = 0 ; i < FCS_SEL31CNTex2 ; i++){
			if(index == FCS_Sell31Ex2_LR[i].IDNo){
				break;
			}
		}
		idx = FCS_Sell31Ex2_LR[i].IDNo;//現在ポジションセット
	}

	//次ポジション検索
	if(move == 0){//下方向
		for(j = i + 1 ; j < FCS_SEL31CNTex2 ; j++){
			m_Sell31.GetAttribute(FCS_Sell31Ex2[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_Sell31Ex2[j].IDNo;
				break;
			}
		}
	}
		...
		for(j = i + 1 ; j < FCS_SEL31CNTex2 ; j++){
			m_Sell31.GetAttribute(FCS_Sell31Ex2_LR[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_Sell31Ex2_LR[j].IDNo;
				break;
			}
		}
	}
...
// 引数　mode		：		0　？
//---------------------------------------------------------
void CShinSell31Ex2::DispList( int mode )
{
	...
	if( !mode ){
		...
		diag_clear( IDC_ICSDIAGCTRL1, SKYEARIDex2, 1, CIcsdiagctrl );
		DiagData.data_disp.Format( _T("（令和%2x年分）"), (*m_pTaxListData)->YEAR );	// 令和固定で大丈夫やんな？？
		diag_setdata( IDC_ICSDIAGCTRL1, SKYEARIDex2, &DiagData, CIcsdiagctrl );

		PostMessage( WM_VSCROLL, SB_TOP, 0 );
	}

	m_Util.DiagOcxIniz( &DiagData );
	diag_clear( IDC_ICSDIAGCTRL1, SKZKBNIDex2, TRUE, CIcsdiagctrl );
	...
	diag_setdata( IDC_ICSDIAGCTRL1, SKZKBNIDex2, &DiagData, CIcsdiagctrl );
...
//----->自動計算機能の削除
	//diag_modify(IDC_ICSDIAGCTRL1, SELChk1ex2, DIAG_MDFY_TRANSPARENT, CIcsdiagctrl);
	//diag_modify(IDC_ICSDIAGCTRL1, SELChk2ex2, DIAG_MDFY_TRANSPARENT, CIcsdiagctrl);
	//diag_modify(IDC_ICSDIAGCTRL1, SELChk3ex2, DIAG_MDFY_TRANSPARENT, CIcsdiagctrl);
//<-----
...
// 22-0167,22-0361 add -->
	...
	int		idx=0;
	for(idx=SELChk1ex2; idx<=SELChk3ex2; idx++)	{
	...
	idx=0;
	if((*m_pSnHeadData)->Sn_GenericSgn & 0x800)		idx=SELChk3ex2;
	...
	if((*m_pSnHeadData)->Sn_Sign4 & 0x01)	{
		...
		DIAGRAM_ATTRIBUTE	da;
		diag_modify(IDC_ICSDIAGCTRL1,SELChk3ex2,DIAG_MDFY_READONLY,CIcsdiagctrl);
		diag_getattr(IDC_ICSDIAGCTRL1,SELChk3ex2,&da,CIcsdiagctrl);
		da.attr_bcolor = BC_GRAY;
		diag_setattr(IDC_ICSDIAGCTRL1,SELChk3ex2,&da,FALSE,CIcsdiagctrl);
	}
	else	{
		...
		diag_setattr(IDC_ICSDIAGCTRL1,SELChk3ex2,&da,FALSE,CIcsdiagctrl);
		diag_modify(IDC_ICSDIAGCTRL1,SELChk3ex2,DIAG_MDFY_EDIT,CIcsdiagctrl);
	}
// 22-0167,22-0361 add <--
...
// ダイアグデータ表示
//-----------------------------------------------
void CShinSell31Ex2::DispSell31DiagData()
{
	short			idx, i;
	KKVAL*			pK = (*m_pTaxListData)->NRec.KUval;
	DIAGRAM_DATA	data;
	...
	for( i = 0 ; i < SKLINCNTex2+1 ; i++ ){
		...
		for(j = 0 ; j < 3 ; j++){
			...
			switch(i){
				...
				case KSELLINE1:
					idx = SEL3101ex2 + j;//インデックスをセット
					break;
				case KSELLINE2:
					idx = SEL3102ex2 + j;//インデックスをセット
					break;
				case KSELLINE3:
					idx = SEL3103ex2 + j;//インデックスをセット
					break;
				case KSELLINE4:
					idx = SEL3104ex2 + j;//インデックスをセット
					break;
				case KSELLINE5:
					idx = SEL3105ex2 + j;//インデックスをセット
					break;
				case KSELLINE6:
					idx = SEL3106ex2 + j;//インデックスをセット
					break;
				case KSELLINE7:
					idx = SEL3107ex2 + j;//インデックスをセット
					break;
				case KSELLINE8:
					idx = SEL3108ex2 + j;//インデックスをセット
					break;
				case KSELLINE9:
					idx = SEL3109ex2 + j;//インデックスをセット
					break;
				case KSELLINE10:
					idx = SEL3110ex2 + j;//インデックスをセット
					break;
	...
	m_Util.DiagOcxIniz( &data );
	diag_clear( IDC_ICSDIAGCTRL1, SEL31NAMEex2, TRUE, CIcsdiagctrl );
	data.data_edit = m_Util.CutHankaku( (*m_pTaxListData)->Head.KHkunm, sizeof((*m_pTaxListData)->Head.KHkunm) );
	diag_setdata( IDC_ICSDIAGCTRL1, SEL31NAMEex2, &data, CIcsdiagctrl );

	// 11	仮受消費税
	st = 0;
	m_Util.DiagOcxIniz(&data);
	diag_clear(IDC_ICSDIAGCTRL1, SEL3111Aex2, TRUE, CIcsdiagctrl);
	...
	if (st) {
		...
		diag_setdata(IDC_ICSDIAGCTRL1, SEL3111Aex2, &data, CIcsdiagctrl);
	}

	// 11	計算部
	st = 0;
	m_Util.DiagOcxIniz( &data );
	diag_clear( IDC_ICSDIAGCTRL1, SEL3111Bex2, TRUE, CIcsdiagctrl );
	if( m_pArith->l_test((pK + KSELLINE10)->KKval3) ){
		st = 1;
	}
	...
	if( st ){
		...
		diag_setdata( IDC_ICSDIAGCTRL1, SEL3111Bex2, &data, CIcsdiagctrl );
	}

	// 12	仮受消費税
	st = 0;
	m_Util.DiagOcxIniz(&data);
	diag_clear(IDC_ICSDIAGCTRL1, SEL3112Aex2, TRUE, CIcsdiagctrl);
	if ((*m_pTaxListData)->Head.KHstax) {
	...
	if (st) {
		...
		diag_setdata(IDC_ICSDIAGCTRL1, SEL3112Aex2, &data, CIcsdiagctrl);
	}

	// 12	計算部
	st = 0;
	m_Util.DiagOcxIniz( &data );
	diag_clear( IDC_ICSDIAGCTRL1, SEL3112Bex2, TRUE, CIcsdiagctrl );
	if( m_pArith->l_test((pK + KSELLINE10)->KKval4) ){
		st = 1;
	}
	...
	if( st ){
		...
		diag_setdata( IDC_ICSDIAGCTRL1, SEL3112Bex2, &data, CIcsdiagctrl );
	}
...
///////////背景色変更
	...
	while( Sell31Ex2[cnt].Index != 0 ){
		
		line = GetLineNum( Sell31Ex2[cnt].Index );
		colm = GetColumNum( Sell31Ex2[cnt].Index );
		sgn = 0x00;
		switch(colm){
			case 0:
				sgn = 0x01;
...
// 22-0167,22-0361 cor <--
			...
			if( Flg ){
				...
				diag_getattr( IDC_ICSDIAGCTRL1, Sell31Ex2[cnt].Index, &attr, CIcsdiagctrl );

				switch( Sell31Ex2[cnt].Index ){	
					case SEL3101ex2:
					case SEL3101Aex2:
					case SEL3101Bex2:
					case SEL3102ex2:
...
// -----------------------
				...
				//	}
				//}
				if(GetColumNum2(Sell31Ex2[cnt].Index) == 2)	{
					if((*m_pSnHeadData)->Sn_GenericSgn & 0x800)	flg2=0;
					else										flg2=1;
				}
				// 金額列であれば入力可／不可を切り替える
				if(flg2 != -1)	{
					ATRIB_MOD(IDC_ICSDIAGCTRL1,Sell31Ex2[cnt].Index,flg2,(*m_pSnHeadData)->Sn_Sign4,Sell31Ex2[cnt].Connect);
				}
				// 列が自動計算でなければ背景色を付ける
				if(flg2 != 0)	{
					diag_setattr( IDC_ICSDIAGCTRL1, Sell31Ex2[cnt].Index, &attr, FALSE, CIcsdiagctrl );
				}
// 22-0167,22-0361 cor <--
// -----------------------------------
			//	diag_setattr( IDC_ICSDIAGCTRL1, Sell31Ex2[cnt].Index, &attr, FALSE, CIcsdiagctrl );
...
// 返送値	0		：	正常終了
//-----------------------------------------------------------------------------
int CShinSell31Ex2::DispJudge( int clm, int line )
{
	int st = 0;
	KKVAL*	pK = (*m_pTaxListData)->NRec.KUval;
...
// 返送値	rt		：	行番号
//-----------------------------------------------------------------------------
int CShinSell31Ex2::GetLineNum( int index )
{
	int rt = -1;
...
// 返送値	rt		：	カラム番号
//-----------------------------------------------------------------------------
int CShinSell31Ex2::GetColumNum( int index )
{
	int rt = -1;
...
// 返送値	rt		：	カラム番号
//-----------------------------------------------------------------------------
int CShinSell31Ex2::GetColumNum2( int index )
{
	int rt = -1;
...
// 返送値	0		：	正常終了
//-----------------------------------------------------------------------------
int CShinSell31Ex2::GetDiagData( long index, short leng, DIAGRAM_DATA *pData, int mode )
{
// 22-0167,22-0361 cor -->
	//if ( index < 0 ) {
// -----------------------
	if(index < 0 || index == SELChk1ex2 || index == SELChk2ex2 || index == SELChk3ex2) {
// 22-0167,22-0361 cor <--
		return 0;
	}
	...
	if(( m_Idx == SEL3111Aex2 ) || ( m_Idx == SEL3112Aex2 )){	// 仮受消費税？
		if( !(*m_pTaxListData)->Head.KHstax&0x01 ){
			DispSell31DiagData();
			return 0;
		}
	...
	int clm_pos = 0;
	if(( m_Idx == SEL3111Aex2 )||( m_Idx == SEL3112Aex2 )){
		clm_pos = 2;
	}else if(( m_Idx == SEL3111Bex2 )||( m_Idx == SEL3112Bex2 )){
		clm_pos = 1;
	}else if(( m_Idx == SEL3111Cex2 )||( m_Idx == SEL3112Cex2 )){
		clm_pos = 0;
	}else{
		clm_pos = (m_Idx - SKVMINex2) % 3;
	}
	...
	if( !mode ){
		...
		if( m_Idx == SEL31NAMEex2 ){
			...
			pK = &(*m_pTaxListData)->NRec.KUval[Lin_tmp];
			if (m_Idx == SEL3111Aex2) {
				pK->KKauto &= 0xfb;
				::ZeroMemory(pK->KKval3, 6);
			}
			else if (m_Idx == SEL3111Bex2) {
				pK->KKauto &= 0xfd;
				::ZeroMemory( pK->KKval2, 6 );
			}
			else if (m_Idx == SEL3112Aex2) {
				pK->KKauto &= 0xfb;
				::ZeroMemory(pK->KKval3, 6);
			}
			else if( m_Idx == SEL3112Bex2 ){
				pK->KKauto &= 0xfd;
				::ZeroMemory( pK->KKval2, 6 );
			}
			else{
...
// フォーカス範囲？
//-----------------------------------------------------------------------------
void CShinSell31Ex2::SetFocusRange()
{
	...
	for( int idx = 0; idx < FCS_SEL31CNTex2; idx++ ){
		m_Sell31.GetAttribute(FCS_Sell31Ex2[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsUpEnd = FCS_Sell31Ex2[idx].IDNo;
			break;
		}
	}
	for( int idx = 0; idx < FCS_SEL31CNTex2; idx++ ){
		m_Sell31.GetAttribute(FCS_Sell31Ex2_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsLeftEnd = FCS_Sell31Ex2_LR[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SEL31CNTex2-1; idx > -1 ; idx-- ){
		m_Sell31.GetAttribute(FCS_Sell31Ex2[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsDownEnd =FCS_Sell31Ex2[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SEL31CNTex2-1; idx > -1 ; idx-- ){
		m_Sell31.GetAttribute(FCS_Sell31Ex2_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsRightEnd =FCS_Sell31Ex2_LR[idx].IDNo;
			break;
		}
	}
}
...
// CShinSell31Ex2 メッセージ ハンドラー
BEGIN_EVENTSINK_MAP(CShinSell31Ex2, CSyzBaseDlg)
	ON_EVENT(CShinSell31Ex2, IDC_ICSDIAGCTRL1, 1, CShinSell31Ex2::EditONIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinSell31Ex2, IDC_ICSDIAGCTRL1, 2, CShinSell31Ex2::EditOFFIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinSell31Ex2, IDC_ICSDIAGCTRL1, 3, CShinSell31Ex2::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
// 22-0167,22-0361 add -->
	ON_EVENT(CShinSell31Ex2, IDC_ICSDIAGCTRL1, 9, CShinSell31Ex2::CheckButtonIcsdiagctrl1, VTS_I2 VTS_UNKNOWN)
// 22-0167,22-0361 add <--
END_EVENTSINK_MAP()


void CShinSell31Ex2::EditONIcsdiagctrl1(short index)
{
	m_Sell31.OffControl( FALSE );

	m_Idx = index;
	if( (m_Idx >= SKVMINex2) && (m_Idx <= SKVMAXex2) ){
		/*if( m_Idx == SKVMAX ){
			m_Lin = ( (m_Idx - SKVMIN) / 3 ) - 1;
		}*/
		if(m_Idx == SEL3111Cex2){	// インデックスの数字を変更するとき注意
			m_Lin = 10;
		}
		else if (m_Idx == SEL3112Cex2) {
			m_Lin = 11;
		}
		else{
			m_Lin = (m_Idx - SKVMINex2) / 3;
		}
	}
	else{
		if ( (m_Idx == SEL3111Aex2) || (m_Idx == SEL3111Bex2)) {
			m_Lin = 10;
		}
		else if ((m_Idx == SEL3112Aex2) || (m_Idx == SEL3112Bex2)) {
			m_Lin = 11;
		}
		else {
			m_Lin = -1;
...
void CShinSell31Ex2::EditOFFIcsdiagctrl1(short index)
{
	DIAGRAM_DATA	data;
...
void CShinSell31Ex2::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
...
BOOL CShinSell31Ex2::PreTranslateMessage(MSG* pMsg)
{
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
void CShinSell31Ex2::SetPrintInfo( CVPrint *pVprn, NPRT_PAR *pPset , SH_PRINT_CMINFO *pPrintCmInfo )
{
	ASSERT( pVprn );
	ASSERT( pPset );
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinSell31Ex2::PrintHyo1( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j;
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinSell31Ex2::PrintHyo2( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j, lin, max;
	char	buf[128], CVAL[6];
	int		len, st;
...
// インボイス_23/07/21_higuchi add -->
				m_pArith->l_print( val[5],(*m_pTaxListData)->NRec.KUval[l_cnt].KKval6, FMTX130 );
				m_pArith->l_print( val[6],(*m_pTaxListData)->NRec.KUval[l_cnt].KKval7, FMTX130 );
// インボイス_23/07/21_higuchi add <--
...
// インボイス_23/07/21_higuchi cor <--
	...
	if( fontsize == 1 ){
		return _xprn( pno, fno1_sex2, PF, 0, pDC, 0, 0 );
	}
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinSell31Ex2::PrintHyo3( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, len;
	char	buf[64], dbuf[64];
...
// 手入力サインのクリア
//-------------------------------------------------------------
int CShinSell31Ex2::ClearManualInputSign()
{
	int rt = 0;
...
// 22-0167,22-0361 add -->
void CShinSell31Ex2::CheckButtonIcsdiagctrl1(short index, LPUNKNOWN data)
{
	DIAGRAM_DATA	*idata;
	DIAGRAM_DATA	xdata;
	int				idx=0;
...
// 修正No.168690 add -->
void CShinSell31Ex2::DiagReSetPosition(void)
{
	//this->SetFocus();
}
// 修正No.168690 add <--
