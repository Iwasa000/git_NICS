// ShinSpecific53_31Ex2.cpp : 実装ファイル
//


#include "stdafx.h"
#include "ShinSpecific53_31Ex2.h"
#include "ShinSpecific53_31_exIdx2.h"
...
// CShinSpecific53_31Ex2 ダイアログ

IMPLEMENT_DYNAMIC(CShinSpecific53_31Ex2, CSpcBaseDlg)

CShinSpecific53_31Ex2::CShinSpecific53_31Ex2(CWnd* pParent /*=nullptr*/)
	: CSpcBaseDlg(IDD_DIALOG_K53_31_ex2, pParent)
	, m_wPos(0)
	, m_cmPos(0)
	, m_ScW(0)
	, m_SySgn(FALSE)
...
CShinSpecific53_31Ex2::~CShinSpecific53_31Ex2()
{
}

void CShinSpecific53_31Ex2::DoDataExchange(CDataExchange* pDX)
{
	CSpcBaseDlg::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ICSDIAG8CTRL1, m_Specific53Ex2);
}


BEGIN_MESSAGE_MAP(CShinSpecific53_31Ex2, CSpcBaseDlg)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_VSCROLL()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinSpecific53_31Ex2::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinSpecific53_31Ex2 メッセージ ハンドラー
BOOL CShinSpecific53_31Ex2::PreTranslateMessage(MSG* pMsg)
{
...
BOOL CShinSpecific53_31Ex2::OnInitDialog()
{
...
void CShinSpecific53_31Ex2::SpecificInit53Ex2()
{
	// 倍長演算指定
...
void CShinSpecific53_31Ex2::SetItemPosition()
{
	int index;

	//前回のカーソル位置の復元
	if (m_curwnd == IDC_ICSDIAG8CTRL1) {
		if ((index = m_Specific53Ex2.GetPosition()) != -1) {
			if ((index == ID503_7A) || (index == ID503_7B)) {
				index = ID503_7A;
			}
			m_Specific53Ex2.SetPosition(index);
			m_Specific53Ex2.SetFocus();
			return;
		}
	}
	...
	if ((*m_pSnHeadData)->Sn_Sign4 & 0x80) {
		if ((*m_pSnHeadData)->Sn_UPERS & 0x10) {
			m_Specific53Ex2.SetPosition(ID503_7A);
		}
		else {
			m_Specific53Ex2.SetPosition(ID503_1A);
		}
	}
	else {
	}
...
// 金額表示
//==============================
void CShinSpecific53_31Ex2::SpecificDisp53Ex2()
{
	...
	if ((*m_pSnHeadData)->Sn_UPERS & 0x10) {
		...
		diag_modify(IDC_ICSDIAG8CTRL1, 33, DIAG_MDFY_TRANSPARENT, CIcsdiagctrl);
		...
		diag_modify(IDC_ICSDIAG8CTRL1, 33, DIAG_MDFY_OPAQUE, CIcsdiagctrl);
...
// 24/02/19wd_特定収入対応 add -->
	// 12.控除対象外仕入れに係る調整額の合計額
	memmove(DiagData.data_val, (*m_pSpcListData)->stval.Sp53kei624, MONY_BUF_SIZE);
	diag_setdata(IDC_ICSDIAG8CTRL1, ID503_12A, (struct IUnknown *)&DiagData, CIcsdiagctrl);	// 6.24%
	memmove(DiagData.data_val, (*m_pSpcListData)->stval.Sp53kei78, MONY_BUF_SIZE);
...
// 24/02/19wd_特定収入対応 add <--
	...
	diag_setdata(IDC_ICSDIAG8CTRL1, ID503_13A, (struct IUnknown*)&DiagData, CIcsdiagctrl);	// 6.24%
	memmove(DiagData.data_val, (*m_pSpcListData)->stval.SpSiz78, MONY_BUF_SIZE);
	diag_setdata(IDC_ICSDIAG8CTRL1, ID503_13B, (struct IUnknown*)&DiagData, CIcsdiagctrl);	// 7.8%
}

//======================================================
// 再計算
//======================================================
void CShinSpecific53_31Ex2::Specific53Ex2_Calq(void)
{
...
// ダイアグラム初期設定
//======================================================
void CShinSpecific53_31Ex2::InitDiagAttr()
{
	if (((*m_pSnHeadData)->Sn_UPERS & 0x10) && ((*m_pSnHeadData)->Sn_Sign4 & 0x01)) {
		int idx = 0;
		for (int i = 0; i < 2; i++) {
	...
	if (!((*m_pSnHeadData)->Sn_Sign4 & 0x80) || ((*m_pSnHeadData)->Sn_Sign4 & 0x01) || !((*m_pSnHeadData)->Sn_UPERS & 0x10)) {
		//m_Specific53Ex2.ModifyItem(80, DIAG_MDFY_TRANSPARENT);	// 24/02/27wd_特定収入対応 del
		m_Specific53Ex2.ModifyItem(86, DIAG_MDFY_TRANSPARENT);		// 24/02/27wd_特定収入対応 add
	}
	else {
		//m_Specific53Ex2.ModifyItem(80, DIAG_MDFY_OPAQUE);	// 24/02/27wd_特定収入対応 del
		m_Specific53Ex2.ModifyItem(86, DIAG_MDFY_OPAQUE);	// 24/02/27wd_特定収入対応 add
	}

	m_Specific53Ex2.EnableDelete(0);
}
...
//			sign		:		変更サイン
//=========================================================
void CShinSpecific53_31Ex2::ChangeColor(unsigned short id, short Index, int sign)
{
...
void CShinSpecific53_31Ex2::OnSize(UINT nType, int cx, int cy)
{
	if (m_initfg) {
	...
	if (m_initfg) {
		...
		GetWindowRect(&a);
		m_Specific53Ex2.GetRect(BottomIndex, (LPUNKNOWN)&b);
...
HBRUSH CShinSpecific53_31Ex2::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CShinSpecific53_31Ex2::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
BEGIN_EVENTSINK_MAP(CShinSpecific53_31Ex2, CSpcBaseDlg)
	ON_EVENT(CShinSpecific53_31Ex2, IDC_ICSDIAG8CTRL1, 1, CShinSpecific53_31Ex2::EditONIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific53_31Ex2, IDC_ICSDIAG8CTRL1, 2, CShinSpecific53_31Ex2::EditOFFIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific53_31Ex2, IDC_ICSDIAG8CTRL1, 3, CShinSpecific53_31Ex2::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()


void CShinSpecific53_31Ex2::EditONIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinSpecific53_31Ex2::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if (!((*m_pSnHeadData)->Sn_UPERS & 0x10)) {
		return;
...
void CShinSpecific53_31Ex2::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if (nChar == 0) {
		return;
	...
	if ((wTerm == VK_RETURN) || (wTerm == VK_F3) || (wTerm == VK_RIGHT)) {
		m_SySgn = TRUE;
		m_Specific53Ex2.SetNextPosition();
		m_SySgn = FALSE;
	}
	else if ((wTerm == VK_TAB) || (wTerm == VK_LEFT) || (wTerm == VK_F2)) {
		m_SySgn = TRUE;
		m_Specific53Ex2.SetPrevPosition();
		m_SySgn = FALSE;
	}
...
void CShinSpecific53_31Ex2::OnBnClickedButton1()
{
