...
{
public:
	CH31TaxListData();   // 標準コンストラクタ
	virtual ~CH31TaxListData();
...
// 230718Aw -->
	// DB からデータ読込み
	int GetData2( CDBNpSub *pDBNpSub, CSnHeadData *pSnHeadData, int sn_seq );
	// DB へデータ書込み
	int SetData2( CDBNpSub *pDBNpSub, CSnHeadData *pSnHeadData, int sn_seq );
// 230718Aw <--
...
//2016.06.22 INSERT END
	...
	// ８％→１０％へのコンバージョン
	int GetPastData( CDBNpSub *pDBNpSub, CSnHeadData *pSnHeadData, int sn_seq );
	// 課税取引金額計算表コンバージョン
	int GetPastData2(CDBNpSub *pDBNpSub, CSnHeadData *pSnHeadData, int sn_seq);

	// コンバージョンが必要かどうか
	int IsNeedOldDataCnv( CDBNpSub *pZmsub, int sn_seq );
...
private:
	void GetKazeiKikan( CSnHeadData *pSnHeadData, long &symd, long &eymd );
