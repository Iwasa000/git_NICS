...
#include "ShinSpecific52_31_exIdx.h"
#include "TaxationList31View.h"

// 定義
const int	BottomIndex = 26;	// 最下行のダイアグインデックス
const int	ID502_11[] = { ID502_11A, ID502_11B, 0 };


// CShinSpecific52_31Ex ダイアログ

IMPLEMENT_DYNAMIC(CShinSpecific52_31Ex, CSpcBaseDlg)

CShinSpecific52_31Ex::CShinSpecific52_31Ex(CWnd* pParent /*=nullptr*/) 
	: CSpcBaseDlg(IDD_DIALOG_K52_31_ex, pParent)
	, m_wPos(0)
	, m_ScW(0)
	, m_cmPos(0)
	, m_SySgn ( FALSE )
...
CShinSpecific52_31Ex::~CShinSpecific52_31Ex()
{
}

void CShinSpecific52_31Ex::DoDataExchange(CDataExchange* pDX)
{
	CSpcBaseDlg::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ICSDIAG8CTRL1, m_Specific52Ex);
}

BEGIN_MESSAGE_MAP(CShinSpecific52_31Ex, CSpcBaseDlg)
	ON_WM_VSCROLL()
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()

// CShinSpecific52_31Ex メッセージ ハンドラー
BOOL CShinSpecific52_31Ex::PreTranslateMessage(MSG* pMsg)
{
...
BOOL CShinSpecific52_31Ex::OnInitDialog()
{
...
//初期設定
void CShinSpecific52_31Ex::SpecificInit52Ex()
{
	// 倍長演算指定
...
void CShinSpecific52_31Ex::SetItemPosition()
{
	int index;

	//前回のカーソルの位置を復元
	if ( m_curwnd == IDC_ICSDIAG8CTRL1 ) {
		if ( ( index = m_Specific52Ex.GetPosition() ) != -1 ) {
			m_Specific52Ex.SetPosition (index);
			m_Specific52Ex.SetFocus();
			return ;
		}
	}
	...
	if ( (*m_pSnHeadData)->Sn_Sign4 & 0x80 ) {
		if ( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) {
			m_Specific52Ex.SetPosition ( ID502_11A );
		}
		else {
			m_Specific52Ex.SetPosition( ID502_1A ) ;
		}
	}
	else{
	}
...
//	画面表示
//-----------------------------------------
void CShinSpecific52_31Ex::SpecificDisp52Ex()
{
	...
	if ( ( ( *m_pSnHeadData )->IsUseSemiUriRatio() ) || ( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) ) {
		...
		diag_modify ( IDC_ICSDIAG8CTRL1, 45, DIAG_MDFY_TRANSPARENT, CIcsdiagctrl );
		...
		diag_modify ( IDC_ICSDIAG8CTRL1, 45, DIAG_MDFY_OPAQUE, CIcsdiagctrl );
	...
	diag_setdata(IDC_ICSDIAG8CTRL1, ID502_17A, (LPUNKNOWN)&DiagData, CIcsdiagctrl);
	memmove( DiagData.data_val, (*m_pSpcListData)->stval.SpSiz78, MONY_BUF_SIZE );
	diag_setdata(IDC_ICSDIAG8CTRL1, ID502_17B, (LPUNKNOWN)&DiagData, CIcsdiagctrl);
}

//======================================================
// ダイアグラム初期設定
//======================================================
void CShinSpecific52_31Ex::InitDiagAttr()
{
	if ( ( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) && ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) ) {
		int idx = 0;
		for ( int i = 0; i < 2; i++ ) {
	...
	if ( !( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) || ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) || !( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) ) {
		m_Specific52Ex.ModifyItem ( 114, DIAG_MDFY_TRANSPARENT );
	}
	else {
		m_Specific52Ex.ModifyItem ( 114, DIAG_MDFY_OPAQUE );
	}

	m_Specific52Ex.EnableDelete ( 0 );
}
...
//			sign		:		変更サイン
//=========================================================
void CShinSpecific52_31Ex::ChangeColor ( unsigned short id, short Index, int sign )
{
...
void CShinSpecific52_31Ex::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	SCROLLINFO	SI;
...
void CShinSpecific52_31Ex::OnSize(UINT nType, int cx, int cy)
{
	if (m_initfg) {
	...
	if (m_initfg) {
		...
		GetWindowRect(&a);
		m_Specific52Ex.GetRect(BottomIndex, (LPUNKNOWN)&b);
...
HBRUSH CShinSpecific52_31Ex::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BEGIN_EVENTSINK_MAP(CShinSpecific52_31Ex, CSyzBaseDlg)
	ON_EVENT(CShinSpecific52_31Ex, IDC_ICSDIAG8CTRL1, 1, CShinSpecific52_31Ex::EditONIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific52_31Ex, IDC_ICSDIAG8CTRL1, 2, CShinSpecific52_31Ex::EditOFFIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific52_31Ex, IDC_ICSDIAG8CTRL1, 3, CShinSpecific52_31Ex::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinSpecific52_31Ex::EditONIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinSpecific52_31Ex::EditOFFIcsdiag8ctrl1(short index)
{
	if ( !( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) ) {
		return;
	}
...
void CShinSpecific52_31Ex::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	if ( nChar == 0 ) {
		return;
	}
	...
	if ( ( wTerm == VK_RETURN ) || ( wTerm == VK_F3 ) || ( wTerm == VK_RIGHT ) ) {
		m_SySgn = TRUE;
		m_Specific52Ex.SetNextPosition();
		m_SySgn = FALSE;
	}
	else if ( ( wTerm == VK_TAB ) || ( wTerm == VK_LEFT ) || ( wTerm == VK_F2 ) ) {
		m_SySgn = TRUE;
		m_Specific52Ex.SetPrevPosition();
		m_SySgn = FALSE;
	}
