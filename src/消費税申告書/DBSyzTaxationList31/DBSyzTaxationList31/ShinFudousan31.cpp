...
// CShinFudousan31 ダイアログ

IMPLEMENT_DYNAMIC(CShinFudousan31, CSyzBaseDlg)

CShinFudousan31::CShinFudousan31(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinFudousan31::IDD, pParent)
	, m_pParent(pParent)
	, m_Idx(0)
	, m_Lin(0)
	, m_Clm(0)
...
CShinFudousan31::~CShinFudousan31()
{
}

void CShinFudousan31::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFudousan31, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinFudousan31 メッセージ ハンドラ

BOOL CShinFudousan31::OnInitDialog()
{
...
// ダイアグラム初期属性セット
//-----------------------------------------------------------------------------
void CShinFudousan31::InitDiagAttr()
{
	...
	while( 1 ){
		...
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			//Flg = Fudousan31[cnt].AllUnConnect;
			Flg = ( pIndex + cnt )->AllUnConnect;
		}
		else{
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				//Flg = Fudousan31[cnt].UnConnect;
				Flg = ( pIndex + cnt )->UnConnect;
				if ( ( *m_pSnHeadData )->SVmzsw != 1 ) {
					//if( ( Fudousan31[cnt].Index % 6 ) == 1 ){
				...
				//Flg = Fudousan31[cnt].Connect;
				Flg = ( pIndex + cnt )->Connect;
			}
		}
		//ATRIB_MOD( IDC_ICSDIAGCTRL1, Fudousan31[cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, Fudousan31[cnt].Connect );
...
// 斜線の表示/非表示
//====================================================
void CShinFudousan31::InitDiagonalLines()
{
	long flg = 0; 
	if ( ( *m_pSnHeadData )->m_DispTabSgn & 0x1000 ) {
	...
	for ( short index = FDIAGONALLINE_S; index <= FDIAGONALLINE_E; index++ ) {
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinFudousan31::DispList( int mode )
{
...
// 引数	dlin	：	
//-----------------------------------------------------------------------------
void CShinFudousan31::DispFudousan31DiagData( long dlin )
{
	//'14.08.21
	short	idx, i/*, j*/;
	long	lin;
...
// 返送値	0	：	
//-----------------------------------------------------------------------------
int CShinFudousan31::GetDiagData( long index, short leng, DIAGRAM_DATA *pData, int mode )
{
	//'14.08.21
	if( index < 0 ){
		return 0;
...
BEGIN_EVENTSINK_MAP(CShinFudousan31, CSyzBaseDlg)
ON_EVENT(CShinFudousan31, IDC_ICSDIAGCTRL1, 1, CShinFudousan31::EditONIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinFudousan31, IDC_ICSDIAGCTRL1, 2, CShinFudousan31::EditOFFIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinFudousan31, IDC_ICSDIAGCTRL1, 3, CShinFudousan31::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFudousan31::EditONIcsdiagctrl1(short index)
{
...
void CShinFudousan31::EditOFFIcsdiagctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA	data;
...
void CShinFudousan31::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
//int CShinFudousan31::UpDownFocusMoveCheck(int Index , int Move)
int CShinFudousan31::FocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	BOOL end_flg = FALSE;
	...
	if ( ( *m_pSnHeadData )->m_DispTabSgn & 0x1000 ) {
		cnt = R02_FCS_SHINFUDOUSAN31CNT;
		pInfo = R02_FCS_ShinFudousan31;
		pInfo_LR = R02_FCS_ShinFudousan31_LR;
	}
	else {
		cnt = FCS_SHINFUDOUSAN31CNT;
		pInfo = FCS_ShinFudousan31;
		pInfo_LR = FCS_ShinFudousan31_LR;
	}
	...
	if(( Move == 0 )||( Move == 1 )){
		...
		//	if(Index == FCS_ShinFudousan31[i].IDNo){
		//		break;
		//	}
		//}
		//idx = FCS_ShinFudousan31[i].IDNo;//現在ポジションセット
		for ( i = 0; i < cnt; i++ ) {
		...
		//for(i = 0 ; i < FCS_SHINFUDOUSAN31CNT; i++){
		//	if(Index == FCS_ShinFudousan31_LR[i].IDNo){
		//		break;
		//	}
		//}
		//idx = FCS_ShinFudousan31_LR[i].IDNo;//現在ポジションセット
		for ( i = 0; i < cnt; i++ ) {
	...
	if(Move == 0){//下方向
		...
		//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31[j].IDNo ,  (LPUNKNOWN)&DA);
		//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
		//		idx = FCS_ShinFudousan31[j].IDNo;
		//		break;
		//	}
		//}
		for ( j = i + 1; j < cnt; j++ ) {
		...
		//for(j = i + 1 ; j < FCS_SHINFUDOUSAN31CNT; j++){
		//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31_LR[j].IDNo ,  (LPUNKNOWN)&DA);
		//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
		//		idx = FCS_ShinFudousan31_LR[j].IDNo;
		//		break;
		//	}
		//}
		for ( j = i + 1; j < cnt; j++ ) {
...
void CShinFudousan31::SetFocusRange()
{
	...
	if ( ( *m_pSnHeadData )->m_DispTabSgn & 0x1000 ) {
		...
		cnt = FCS_SHINFUDOUSAN31CNT;
		pInfo = FCS_ShinFudousan31_LR;
		pInfo_LR = FCS_ShinFudousan31_LR;
	}

	//for( int idx = 0; idx < FCS_SHINFUDOUSAN31CNT; idx++ ){
	//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31[idx].IDNo ,  (LPUNKNOWN)&DA);
	//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
	//		m_FcsUpEnd = FCS_ShinFudousan31[idx].IDNo;
	//		break;
	//	}
	//}
	...
	//for( int idx = 0; idx < FCS_SHINFUDOUSAN31CNT; idx++ ){
	//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
	//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
	//		m_FcsLeftEnd = FCS_ShinFudousan31_LR[idx].IDNo;
	//		break;
	//	}
	//}
	for ( int idx = 0; idx < cnt; idx++ ) {
	...
	//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31[idx].IDNo ,  (LPUNKNOWN)&DA);
	//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
	//		m_FcsDownEnd = FCS_ShinFudousan31[idx].IDNo;
	//		break;
	//	}
	//}
	...
	//	m_Fudousan31.GetAttribute(FCS_ShinFudousan31_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
	//	if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
	//		m_FcsRightEnd = FCS_ShinFudousan31_LR[idx].IDNo;
	//		break;
	//	}
	//}
	for ( int idx = cnt -1; idx > -1; idx-- ) {
...
BOOL CShinFudousan31::PreTranslateMessage(MSG* pMsg)
{
...
//		pPset	：	印刷情報
//-----------------------------------------------------------------------------
void CShinFudousan31::SetPrintInfo( CVPrint *pVprn, NPRT_PAR *pPset , SH_PRINT_CMINFO *pPrintCmInfo )
{
	ASSERT( pVprn );
	ASSERT( pPset );
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFudousan31::PrintHyo1( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j;
...
//-----------------------------------------------------------------------------
//int CShinFudousan31::PrintHyo1( int pno, int fno, CDC *pDC, char *pBp )
int CShinFudousan31::PrintHyo2( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j, lin, max;
	char	buf[128], CVAL[6];
	int		len;
...
//-----------------------------------------------------------------------------
//int CShinFudousan31::PrintHyo2( int pno, int fno, CDC *pDC, char *pBp )
int CShinFudousan31::PrintHyo3( int pno, int fno, CDC *pDC, char *pBp )
{
...
// 初期ポジションセット ('14.08.08)
//-----------------------------------------------------------------------------
void CShinFudousan31::SetItemPosition()
{
	int index;
	...
	this->SetFocus();
	if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
		m_Fudousan31.SetPosition(KFUDVAL01A);
	}
	else{
		m_Fudousan31.SetPosition(KFUDKNAME03);
	}
	SetRedrawScroll( 0 );
...
// 引数　：clm…列　line…行
//-----------------------------------------------------------------------------
int CShinFudousan31::DispJudge( int clm, int line )
{
	int st = 0;
	int j = 0;
...
// 手入力サインのクリア
int CShinFudousan31::ClearManualInputSign()
{
	int rt = 0;
...
//		1		:		旧税率 ( 6.3% )欄
//=====================================================
int CShinFudousan31::CheckSVmzsw ( int index )
{
	int ret = 0;
...
// 修正No.168690 add -->
void CShinFudousan31::DiagReSetPosition(void)
{
	this->SetFocus();
}
// 修正No.168690 add <--
