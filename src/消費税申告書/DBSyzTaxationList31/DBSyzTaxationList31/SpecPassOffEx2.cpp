...
// SpecPassOffEx2.cpp : 実装ファイル
//

#include "stdafx.h"
#include "SpecPassOffEx2.h"

//== 定数 ==
const	int	IdTop		=	68;			// 金額表示の初期位置
const	int	IdZselCmb	=	IdTop+32;	// 税率選択コンボボックス

// CSpecPassOffEx2 ダイアログ

IMPLEMENT_DYNAMIC(CSpecPassOffEx2, ICSDialog)

CSpecPassOffEx2::CSpecPassOffEx2(CWnd* pParent /*=NULL*/)
	: ICSDialog(CSpecPassOffEx2::IDD, pParent)
	, m_pParent(pParent)
	, m_pSnHeadData(NULL)
	, m_pSpcListData(NULL)
	, m_pArith(NULL)
...
CSpecPassOffEx2::~CSpecPassOffEx2()
{
}

void CSpecPassOffEx2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CSpecPassOffEx2, ICSDialog)
END_MESSAGE_MAP()

BEGIN_EVENTSINK_MAP(CSpecPassOffEx2, ICSDialog)
	ON_EVENT(CSpecPassOffEx2, IDC_ICSDIAGCTRL1, 8, CSpecPassOffEx2::ComboSelIcsdiagctrl1, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

// CSpecPassOffEx2 メッセージ ハンドラ

BOOL CSpecPassOffEx2::OnInitDialog()
{
...
// 税率コンボボックス初期化
//-----------------------------------------------------------------------------
void CSpecPassOffEx2::InitZrtCombo()
{
	CString	zrtTitle;
...
int CSpecPassOffEx2::Disp_10per()
{
	...
	if( m_pArith->l_cmp( (*m_pSpcListData)->stval.SpTswr, WORK0 ) > 0 ){	// 特例収入割合５％以上	
		...
		if(m_ListNo == 0)		m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp51kei78, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 1)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp52kei78, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 2)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp53kei78, _T("SSS,SSS,SSS,SS9") );
		DiagData.data_disp = MBF;
...
int CSpecPassOffEx2::Disp_8perK()
{
	char			CBF[80]={0}, BUF[80]={0}, KBF[80]={0}, MBF[80]={0},
		WORK0[6]={0}, WORK1[6]={0}, WORK2[6]={0};
	char			moji[64]={0};
	...
	if( m_pArith->l_cmp( (*m_pSpcListData)->stval.SpTswr, WORK0 ) > 0 ){	// 特例収入割合５％以上	
		...
		// ⑧．控除対象仕入れに係る調整対象額の合計額
		memset(MBF, 0, sizeof(MBF));
		if(m_ListNo == 0)		m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp51kei624, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 1)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp52kei624, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 2)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp53kei624, _T("SSS,SSS,SSS,SS9") );
		DiagData.data_disp = MBF;
		diag_setdata( IDC_ICSDIAGCTRL1, IdTop+27, (struct IUnknown *)&DiagData, CIcsdiagctrl );
...
int CSpecPassOffEx2::Disp_8per()
{
	...
	if( m_pArith->l_cmp( (*m_pSpcListData)->stval.SpTswr, WORK0 ) > 0 ){	// 特例収入割合５％以上	
		...
		// ⑧．控除対象仕入れに係る調整対象額の合計額
		memset(MBF, 0, sizeof(MBF));
		if(m_ListNo == 0)		m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp51kei63, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 1)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp52kei63, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 2)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp53kei63, _T("SSS,SSS,SSS,SS9") );
		DiagData.data_disp = MBF;
		diag_setdata( IDC_ICSDIAGCTRL1, IdTop+27, (struct IUnknown *)&DiagData, CIcsdiagctrl );
...
int CSpecPassOffEx2::Disp_5per()
{
	...
	if( m_pArith->l_cmp( (*m_pSpcListData)->stval.SpTswr, WORK0 ) > 0 ){	// 特例収入割合５％以上	
		...
		// ⑧．控除対象仕入れに係る調整対象額の合計額
		memset(MBF, 0, sizeof(MBF));
		if(m_ListNo == 0)		m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp51kei4, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 1)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp52kei4, _T("SSS,SSS,SSS,SS9") );
		else if(m_ListNo == 2)	m_pArith->l_print( MBF, (*m_pSpcListData)->stval.Sp53kei4, _T("SSS,SSS,SSS,SS9") );
		DiagData.data_disp = MBF;
		diag_setdata( IDC_ICSDIAGCTRL1, IdTop+27, (struct IUnknown *)&DiagData, CIcsdiagctrl );
...
void CSpecPassOffEx2::Edit_Static_10per()
{
	DIAGRAM_DATA	DiagData;
...
void CSpecPassOffEx2::Edit_Static_8perK()
{
	DIAGRAM_DATA	DiagData;
...
void CSpecPassOffEx2::Edit_Static_8per()
{
	DIAGRAM_DATA	DiagData;
...
void CSpecPassOffEx2::Edit_Static_5per()
{
	DIAGRAM_DATA	DiagData;
...
void CSpecPassOffEx2::Disp_AllClear()
{
...
int CSpecPassOffEx2::GetZeiritsu()
{
	int ret = -1; 
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CSpecPassOffEx2::Init( CSnHeadData **pSnHeadData, CH31SpcListData **pSpcListData, CArithEx *pArith )
{
	ASSERT( pSnHeadData );
	if( pSnHeadData == NULL ){
		return -1;
...
int CSpecPassOffEx2::InitControl()
{
	// 税率コンボ初期化
	InitZrtCombo();
...
void CSpecPassOffEx2::ComboSelIcsdiagctrl1(short index, LPUNKNOWN data)
{
...
// 税率に合わせた画面切替
//-----------------------------------------------------------------------------
void CSpecPassOffEx2::ChgDispByZeiritsu()
{
	int zrt = GetZeiritsu();
	if( zrt == zeiritsu2_10per ){
		Edit_Static_10per();
		Disp_10per();
	}
	else if( zrt == zeiritsu2_8perK ){
		Edit_Static_8perK();
		Disp_8perK();
	}
	else if( zrt == zeiritsu2_8per ){
		Edit_Static_8per();
		Disp_8per();
	}
	else if( zrt == zeiritsu2_5per ){
		Edit_Static_5per();
		Disp_5per();
	}
}
