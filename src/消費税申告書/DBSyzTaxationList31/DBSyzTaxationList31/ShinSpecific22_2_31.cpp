...
#include "ShinSpecific22_2_31Idx.h"
...
IMPLEMENT_DYNAMIC(CShinSpecific22_2_31, CSpcBaseDlg)

CShinSpecific22_2_31::CShinSpecific22_2_31(CWnd* pParent /*=nullptr*/)
...
CShinSpecific22_2_31::~CShinSpecific22_2_31()
{
}

void CShinSpecific22_2_31::DoDataExchange(CDataExchange* pDX)
{
	CSpcBaseDlg::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ICSDIAG8CTRL2, m_Specific22_22);
	DDX_Control(pDX, IDC_ICSDIAG8CTRL1, m_Specific22_21);
	DDX_Control(pDX, IDC_ICSDIAG8CTRL3, m_Specific22_20);
	DDX_Control(pDX, IDC_ICSDISP8CTRL1, m_ICSDisp1);
}


BEGIN_MESSAGE_MAP(CShinSpecific22_2_31, CSpcBaseDlg)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_COMMAND(ID_BUTTON_F1, &CShinSpecific22_2_31::OnButtonF1)
	ON_COMMAND(ID_BUTTON_F2, &CShinSpecific22_2_31::OnButtonF2)
END_MESSAGE_MAP()
...
BOOL CShinSpecific22_2_31::OnInitDialog()
{
	...
	m_Specific22_21.EnableDelete(0) ;
...
void CShinSpecific22_2_31::SpecificInit22_2()
{
	// 倍長演算指定
	...
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
		m_Spage = 30;
		m_Specific22_21.SetScPageCnt( ( short )m_Spage );
	}
...
// 表示
//-----------------------------------------------------------------------------
void CShinSpecific22_2_31::SpecificDisp22_2()
{
	TSREC			TSrec;
...
// 再計算
//-----------------------------------------------------------------------------
void CShinSpecific22_2_31::Specific22_2_Calq( void )
{
	TSREC			TSrec = { 0 };
	int				rno = 0;
...
// テーブル番号の取得
//-----------------------------------------------------------------------------
int CShinSpecific22_2_31::GetTableNo ( int index )
{
	int	i= 0, j = 0;
...
int CShinSpecific22_2_31::findindex (int line )
{
...
// カラム番号の取得
//-----------------------------------------------------------------------------
int CShinSpecific22_2_31::GetColmNo ( int index )
{
...
//-----------------------------------------------------------------------------
int CShinSpecific22_2_31::GetFocusNo ( int index )
{
	int	i = 0, j = 0;
	int pageno = 0, tableno = 0, indexno = 0;
...
void CShinSpecific22_2_31::CellClear ( int index )
{
...
BOOL CShinSpecific22_2_31::PreTranslateMessage(MSG* pMsg)
{
	...
	if( pMsg->message == WM_KEYDOWN ){
		...
		if( pMsg->wParam == VK_F2 ){
			...
			m_Specific22_21.ScrollPos ( SB_PAGEUP, 0 );
		}
		else if ( pMsg->wParam == VK_NEXT ) {
			m_Specific22_21.ScrollPos ( SB_PAGEDOWN, 0 );
		}
...
BEGIN_EVENTSINK_MAP(CShinSpecific22_2_31, CSpcBaseDlg)
	ON_EVENT(CShinSpecific22_2_31, IDC_ICSDIAG8CTRL1, 1, CShinSpecific22_2_31::EditONIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific22_2_31, IDC_ICSDIAG8CTRL1, 2, CShinSpecific22_2_31::EditOFFIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific22_2_31, IDC_ICSDIAG8CTRL1, 3, CShinSpecific22_2_31::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinSpecific22_2_31::EditONIcsdiag8ctrl1(short index)
{
...
void CShinSpecific22_2_31::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	TSREC TSrec = { 0 };
...
void CShinSpecific22_2_31::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	char VK_FLG = '\0';
	short wTerm = 0;
...
void CShinSpecific22_2_31::IndexControl()
{
	int	Cnt = 0;
	while ( 1 ){
		if ( Specific22_2_31[Cnt].Index == 0 ) {
			break;
		}
		int	Flg = 0;
		// 確定時は全入力不可
		if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
			Flg = Specific22_2_31[Cnt].AllUnConnect;
		}
		else {
			if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
				Flg = Specific22_2_31[Cnt].UnConnect;
			}
			else {
				Flg = Specific22_2_31[Cnt].Connect;
			}
		}
		...
		ATRIB_MOD ( IDC_ICSDIAG8CTRL1, Specific22_2_31[Cnt].Index, Flg, ( *m_pSnHeadData )->Sn_Sign4, Specific22_2_31[Cnt].Connect );
		Cnt++;
	}

	SetFocusRange();

	m_Specific22_21.EnableDelete ( 0 );//DELETEキー１回で削除モード設定
}

void CShinSpecific22_2_31::SetItemPosition ( BOOL IsInEnd/*=FALSE*/ )
{
	int index = 0;

	// 前回ポジションの復元
	if ( m_curwnd == IDC_ICSDIAG8CTRL1 ) {
		if ( ( index = m_Specific22_21.GetPosition() ) != -1 ) {
			if ( IsInEnd ) {

				// 画面上に完全に矩形領域がある時のみフォーカスを再セット
				RECT	rc221, rc22, rc220;
				m_Specific22_22.GetRect ( 1, ( LPUNKNOWN )&rc22 );
				m_Specific22_21.GetRect ( index, ( LPUNKNOWN )&rc221 );
				m_Specific22_20.GetRect ( 1, ( LPUNKNOWN )&rc220 );

				if ( ( ( rc22.bottom < rc221.top ) && ( rc221.bottom < rc220.top ) ) ) {
					m_Specific22_21.SetPosition(index);
				}

				m_Specific22_21.SetFocus();
			}
			else {
				m_Specific22_21.SetPosition ( index );
				m_Specific22_21.SetFocus();
	...
	m_Specific22_20.SetFocus();
	m_Specific22_21.SetFocus();

	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
		m_Specific22_21.SetPosition( ID2022_1K );
	}

	SetRedrawScroll ( 0 );
}

void CShinSpecific22_2_31::SetFocusRange()
{
	...
	for ( int idx = 0; idx < FCS_SPECIFIC22CNT; idx++ ) {
		m_Specific22_21.GetAttribute ( FCS_Specific22_2_31[idx].IDNo, ( LPUNKNOWN )&DA );

		if ( ( DA.attr_bcolor == BC_CREAM ) || ( DA.attr_bcolor == BC_BLUE ) ) {
			m_FcsUpEnd = FCS_Specific22_2_31[idx].IDNo;
			break;
		}
	}

	for ( int idx = 0; idx < FCS_SPECIFIC22CNT; idx++ ) {
		m_Specific22_21.GetAttribute ( FCS_Specific22_2_31_LR[idx].IDNo, ( LPUNKNOWN )&DA );

		if ( ( DA.attr_bcolor == BC_CREAM ) || ( DA.attr_bcolor == BC_BLUE ) ) {
			m_FcsLeftEnd = FCS_Specific22_2_31_LR[idx].IDNo;
			break;
		}
	}
	...
	for ( int idx = FCS_SPECIFIC22CNT-1; idx > -1 ; idx-- ) {
		...
		if ( ( DA.attr_bcolor == BC_CREAM ) || ( DA.attr_bcolor == BC_BLUE ) ) {
			m_FcsDownEnd = FCS_Specific22_2_31[idx].IDNo;
			break;
		}
	}
	...
	for ( int idx = FCS_SPECIFIC22CNT-1; idx > -1 ; idx-- ) {
		...
		if ( ( DA.attr_bcolor == BC_CREAM ) || ( DA.attr_bcolor == BC_BLUE ) ) {
			m_FcsRightEnd = FCS_Specific22_2_31_LR[idx].IDNo;
			break;
		}
	}
}

void CShinSpecific22_2_31::OnButtonF1()
{
	// TODO: ここにコマンド ハンドラ コードを追加します。
	diag_setprev( IDC_ICSDIAG8CTRL1, CIcsdiagctrl );
}

void CShinSpecific22_2_31::OnButtonF2()
{
	// TODO: ここにコマンド ハンドラ コードを追加します。
	diag_setnext( IDC_ICSDIAG8CTRL1, CIcsdiagctrl );
}

void CShinSpecific22_2_31::OnSize(UINT nType, int cx, int cy)
{
	...
	if ( m_initfg ) {
		...
		m_Specific22_20.GetRect ( BottomIndex, ( LPUNKNOWN )&b );

	}
}

HBRUSH CShinSpecific22_2_31::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//			FALSE	：　該当税率データでない
//---------------------------------------------------------
BOOL CShinSpecific22_2_31::IsNowZrtData( TSREC *pTsrec )
{
	BOOL	rt = FALSE;
