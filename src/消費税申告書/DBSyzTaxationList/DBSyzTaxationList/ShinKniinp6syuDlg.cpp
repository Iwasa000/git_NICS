// ShinKniinp6syuDlg.cpp : 実装ファイル
//

#include "stdafx.h"
#include "ShinKniinp6syuDlg.h"
#include "ShinKniinp6syuDlgIdx.h"

// CShinKniinp6syuDlg ダイアログ

IMPLEMENT_DYNAMIC(CShinKniinp6syuDlg, CSyzBaseDlg)

CShinKniinp6syuDlg::CShinKniinp6syuDlg(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKniinp6syuDlg::IDD, pParent)
	, m_pParent(pParent)
	, m_Idx(0)
	, m_Lin(0)
	, m_Clm(0)
...
CShinKniinp6syuDlg::~CShinKniinp6syuDlg()
{
}

void CShinKniinp6syuDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKniinp6syuDlg, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinKniinp6syuDlg メッセージ ハンドラ

BOOL CShinKniinp6syuDlg::OnInitDialog()
{
...
// ダイアグラム初期属性セット
//-----------------------------------------------------------------------------
void CShinKniinp6syuDlg::InitDiagAttr()
{
	int	cnt = 0;
	int	Flg;
	while( 1 ){
		if( ShinKniinp6syu[cnt].Index == 0 ){
			break;
		}
		// 確定時は全入力不可
		if( (*m_pSnHeadData)->Sn_Sign4&0x01){
			Flg = ShinKniinp6syu[cnt].AllUnConnect;
		}
		else{
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				Flg = ShinKniinp6syu[cnt].UnConnect;
			}
			else{
				Flg = ShinKniinp6syu[cnt].Connect;
			}
		}
		ATRIB_MOD( IDC_ICSDIAG8CTRL1, ShinKniinp6syu[cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, ShinKniinp6syu[cnt].Connect );
		cnt++;
	}
...
BOOL CShinKniinp6syuDlg::PreTranslateMessage(MSG* pMsg)
{
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinKniinp6syuDlg::DispList( int mode )
{
...
// ダイアグデータ表示
//-----------------------------------------------------------------------------
void CShinKniinp6syuDlg::DispKniinpDiagData( long dlin )
{
...
// 返送値	
//-----------------------------------------------------------------------------
int CShinKniinp6syuDlg::GetDiagData( long index, short leng, DIAGRAM_DATA *data, int mode )
{
...
BEGIN_EVENTSINK_MAP(CShinKniinp6syuDlg, CSyzBaseDlg)
	ON_EVENT(CShinKniinp6syuDlg, IDC_ICSDIAG8CTRL1, 1, CShinKniinp6syuDlg::EditONIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinKniinp6syuDlg, IDC_ICSDIAG8CTRL1, 2, CShinKniinp6syuDlg::EditOFFIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinKniinp6syuDlg, IDC_ICSDIAG8CTRL1, 3, CShinKniinp6syuDlg::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinKniinp6syuDlg::EditONIcsdiag8ctrl1(short index)
{
...
void CShinKniinp6syuDlg::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA	data;
...
void CShinKniinp6syuDlg::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
//int CShinKniinp6syuDlg::UpDownFocusMoveCheck(int Index , int Move)
int CShinKniinp6syuDlg::FocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	BOOL end_flg = FALSE;
	...
	if(( Move == 0 )||( Move == 1 )){
		for(i = 0 ; i < FCS_SHINKNIINPCNT ; i++){
			if(Index == FCS_ShinKniinp6syu[i].IDNo){
				break;
			}
		}
		idx = FCS_ShinKniinp6syu[i].IDNo;//現在ポジションセット
	}else if(( Move == 2 )||( Move == 3 )){
		for(i = 0 ; i < FCS_SHINKNIINPCNT ; i++){
			if(Index == FCS_ShinKniinp6syu_LR[i].IDNo){
				break;
			}
		}
		idx = FCS_ShinKniinp6syu_LR[i].IDNo;//現在ポジションセット
	}

	//次ポジション検索
	if(Move == 0){//下方向
		for(j = i + 1 ; j < FCS_SHINKNIINPCNT ; j++){
			m_Kniinp.GetAttribute(FCS_ShinKniinp6syu[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_ShinKniinp6syu[j].IDNo;
				break;
			}
		}
	}
		...
		for(j = i + 1 ; j < FCS_SHINKNIINPCNT ; j++){
			m_Kniinp.GetAttribute(FCS_ShinKniinp6syu_LR[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_ShinKniinp6syu_LR[j].IDNo;
				break;
			}
		}
	}
...
void CShinKniinp6syuDlg::SetFocusRange()
{
	...
	for( int idx = 0; idx < FCS_SHINKNIINPCNT; idx++ ){
		m_Kniinp.GetAttribute(FCS_ShinKniinp6syu[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsUpEnd = FCS_ShinKniinp6syu[idx].IDNo;
			break;
		}
	}
	for( int idx = 0; idx < FCS_SHINKNIINPCNT; idx++ ){
		m_Kniinp.GetAttribute(FCS_ShinKniinp6syu_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsLeftEnd = FCS_ShinKniinp6syu_LR[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SHINKNIINPCNT-1; idx > -1 ; idx-- ){
		m_Kniinp.GetAttribute(FCS_ShinKniinp6syu[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsDownEnd = FCS_ShinKniinp6syu[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SHINKNIINPCNT-1; idx > -1 ; idx-- ){
		m_Kniinp.GetAttribute(FCS_ShinKniinp6syu_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsRightEnd = FCS_ShinKniinp6syu_LR[idx].IDNo;
			break;
		}
	}
}
...
// 初期ポジションセット'14.09.01
//-----------------------------------------------------------------------------
void CShinKniinp6syuDlg::SetItemPosition()
{
	int index;
...
int CShinKniinp6syuDlg::DispJudge( int clm, int line )
{
	int st = 0;
	int j = 0;
...
// 手入力サインのクリア
int CShinKniinp6syuDlg::ClearManualInputSign()
{
	int rt = 0;
