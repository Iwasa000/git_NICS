...
class CShinFudousan : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinFudousan)

public:
	CShinFudousan(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinFudousan();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_FUDOUSAN };
	...
	// ダイアグデータ表示
	void DispFudousanDiagData( long dlin );
	// カーソル移動
