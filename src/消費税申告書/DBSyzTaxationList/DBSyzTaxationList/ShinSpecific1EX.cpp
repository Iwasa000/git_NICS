...
// CShinSpecific1EX ダイアログ

IMPLEMENT_DYNAMIC(CShinSpecific1EX, CSpcBaseDlg)

CShinSpecific1EX::CShinSpecific1EX(CWnd* pParent /*=NULL*/)
	: CSpcBaseDlg(CShinSpecific1EX::IDD, pParent)
{
...
CShinSpecific1EX::~CShinSpecific1EX()
{
}

void CShinSpecific1EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinSpecific1EX, CSpcBaseDlg)
	ON_WM_SIZE()
	ON_WM_CTLCOLOR()
	ON_WM_VSCROLL()
END_MESSAGE_MAP()
...
// CShinSpecific1EX メッセージ ハンドラ

BOOL CShinSpecific1EX::OnInitDialog()
{
...
BOOL CShinSpecific1EX::PreTranslateMessage(MSG* pMsg)
{
...
void CShinSpecific1EX::OnSize(UINT nType, int cx, int cy)
{
...
HBRUSH CShinSpecific1EX::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CShinSpecific1EX::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
// 再描画
//-----------------------------------------------------------------------------
void CShinSpecific1EX::sykscroll( int mpos )
{
	int		smax;
...
// 初期設定
//-----------------------------------------------------------------------------
void CShinSpecific1EX::SpecificInit1( void )
{
	// 倍長演算指定
...
// 表示
//-----------------------------------------------------------------------------
void CShinSpecific1EX::SpecificDisp1( void )
{
...
// 再計算
//-----------------------------------------------------------------------------
void CShinSpecific1EX::Specific1_Calq( void )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
...
BEGIN_EVENTSINK_MAP(CShinSpecific1EX, CSpcBaseDlg)
	ON_EVENT(CShinSpecific1EX, IDC_ICSDIAG8CTRL1, 1, CShinSpecific1EX::EditONIcsdiag8ctrl1, VTS_I2)//yoshida150310
	ON_EVENT(CShinSpecific1EX, IDC_ICSDIAG8CTRL1, 2, CShinSpecific1EX::EditOFFIcsdiag8ctrl1, VTS_I2)
	ON_EVENT(CShinSpecific1EX, IDC_ICSDIAG8CTRL1, 3, CShinSpecific1EX::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinSpecific1EX::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinSpecific1EX::GetDiagData()
{
...
void CShinSpecific1EX::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
// 
//-----------------------------------------------------------------------------
void CShinSpecific1EX::IndexControl( void )
{
	int	Cnt = 0;
	while( 1 ){
		if( Specific1Ex[Cnt].Index == 0 ){
			break;
		}
		int	Flg;
		// 確定時は全入力不可
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			Flg = Specific1Ex[Cnt].AllUnConnect;
		}
		else{
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				Flg = Specific1Ex[Cnt].UnConnect;
			}
			else{
				Flg = Specific1Ex[Cnt].Connect;
			}
		}
		if( Flg == 2 ){
			if( ((*m_pSnHeadData)->Sn_TUMIAG&0x01) && (m_pZmSub->zvol->s_sgn1&0xc0) ){
		...
		ATRIB_MOD( IDC_ICSDIAG8CTRL1, Specific1Ex[Cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, Specific1Ex[Cnt].Connect );
		Cnt++;
	}
...
void CShinSpecific1EX::OnOK()
{
	// TODO: ここに特定なコードを追加するか、もしくは基本クラスを呼び出してください。
//	CSpcBaseDlg::OnOK();
}

void CShinSpecific1EX::OnCancel()
{
	// TODO: ここに特定なコードを追加するか、もしくは基本クラスを呼び出してください。
//	CSpcBaseDlg::OnCancel();
}

void CShinSpecific1EX::EditONIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinSpecific1EX::FocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	BOOL end_flg = FALSE;
	...
	if(( Move == 0 )||( Move == 1 )){
		for(i = 0 ; i < FCS_SPECIFIC1CNT ; i++){
			if(Index == FCS_Specific1Ex[i].IDNo){
				break;
			}
		}
		idx = FCS_Specific1Ex[i].IDNo;//現在ポジションセット
	}else if(( Move == 2 )||( Move == 3 )){
		for(i = 0 ; i < FCS_SPECIFIC1CNT ; i++){
			if(Index == FCS_Specific1Ex_LR[i].IDNo){
				break;
			}
		}
		idx = FCS_Specific1Ex_LR[i].IDNo;//現在ポジションセット
	}

	//次ポジション検索
	if(Move == 0){//下方向
		for(j = i + 1 ; j < FCS_SPECIFIC1CNT ; j++){
			m_Specific1.GetAttribute(FCS_Specific1Ex[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_Specific1Ex[j].IDNo;
				break;
			}
		}
	}
		...
		for(j = i + 1 ; j < FCS_SPECIFIC1CNT ; j++){
			m_Specific1.GetAttribute(FCS_Specific1Ex_LR[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_Specific1Ex_LR[j].IDNo;
				break;
			}
		}
	}
...
void CShinSpecific1EX::SetFocusRange()
{
	...
	for( int idx = 0; idx < FCS_SPECIFIC1CNT; idx++ ){
		m_Specific1.GetAttribute(FCS_Specific1Ex[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsUpEnd = FCS_Specific1Ex[idx].IDNo;
			break;
		}
	}
	for( int idx = 0; idx < FCS_SPECIFIC1CNT; idx++ ){
		m_Specific1.GetAttribute(FCS_Specific1Ex_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsLeftEnd = FCS_Specific1Ex_LR[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SPECIFIC1CNT-1; idx > -1 ; idx-- ){
		m_Specific1.GetAttribute(FCS_Specific1Ex[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsDownEnd = FCS_Specific1Ex[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SPECIFIC1CNT-1; idx > -1 ; idx-- ){
		m_Specific1.GetAttribute(FCS_Specific1Ex_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsRightEnd = FCS_Specific1Ex_LR[idx].IDNo;
			break;
		}
	}
}
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinSpecific1EX::SetItemPosition( BOOL IsInEnd )
{
	int index;
