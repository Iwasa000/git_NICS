// ShinJigyo.cpp : 実装ファイル
//

#include "stdafx.h"
#include "ShinJigyo.h"
#include "ShinJigyoIdx.h"


// CShinJigyo ダイアログ

IMPLEMENT_DYNAMIC(CShinJigyo, CSyzBaseDlg)

CShinJigyo::CShinJigyo(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinJigyo::IDD, pParent)
...
CShinJigyo::~CShinJigyo()
{
}

void CShinJigyo::DoDataExchange(CDataExchange* pDX)
{
	CSyzBaseDlg::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ICSDIAGCTRL1, m_Jigyo);
}


BEGIN_MESSAGE_MAP(CShinJigyo, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinJigyo メッセージ ハンドラ

BOOL CShinJigyo::OnInitDialog()
{
...
// ダイアグラム初期属性セット
//-----------------------------------------------------------------------------
void CShinJigyo::InitDiagAttr()
{
	DIAGRAM_ATTRIBUTE data;
	long dg1[] = { JIGKNAME25, JIGKNAME26, JIGKNAME27, JIGKNAME28, JIGKNAME29, JIGKNAME30, -1 };
	...
	for( int i=0;; i++ ){
		...
		// ank対応
		m_Jigyo.GetAttribute( (short)dg1[i], (LPUNKNOWN)&data );
		data.attr_editattr |= DIAG_EATTR_ZENHAN;			//DIAG_EATTR_ZENHAN ank対応
		m_Jigyo.SetAttribute( (short)dg1[i], (LPUNKNOWN)&data, 1 );
	}

	int	cnt = 0;
	while( 1 ){
		if( Jigyou[cnt].Index == 0 ){
			break;
		}
		int	Flg;
		// 確定時は全入力不可
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			Flg = Jigyou[cnt].AllUnConnect;
		}
		else{
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				Flg = Jigyou[cnt].UnConnect;
			}
			else{
				Flg = Jigyou[cnt].Connect;
			}
		}
		ATRIB_MOD( IDC_ICSDIAGCTRL1, Jigyou[cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, Jigyou[cnt].Connect );
		cnt++;
	}

	SetFocusRange();

	m_Jigyo.EnableDelete(0);//DELETEキー１回で削除モード設定'14.08.06

}

BOOL CShinJigyo::PreTranslateMessage(MSG* pMsg)
{
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinJigyo::DispList( int mode )
{
	m_Jigyo.OffControl( TRUE );
//2016.06.22 INSERT START
	(*m_Scalc)->Shz_AllCalc(1);
//2016.06.22 INSERT END
	short	i;
	for( i = 0; i < VLIN; i++ ){
		DispJigyoDiagData( i );
...
//2016.06.22 INSERT START
	int edit[] = {JIGKNAME25,JIGKNAME26,JIGKNAME27,JIGKNAME28,JIGKNAME29,JIGKNAME30};
	BOOL edit_focus = FALSE;

	for( int cnt = 0; cnt < ARRAYCOUNT(edit); cnt++ ){
		Switch_KamokuEdit( IDC_ICSDIAGCTRL1, edit[cnt] );

		if( m_Jigyo.GetPosition() == edit[cnt] ){
			edit_focus = TRUE;
		}
	}
...
//2016.06.22 INSERT END
	...
	//SetRedrawScrollToIdx(m_Jigyo.GetPosition());
}
...
// 引数	dlin	：
//-----------------------------------------------------------------------------
void CShinJigyo::DispJigyoDiagData( long dlin )
{
	short	idx, i;
	long	lin;
	if( !dlin ){
...
// 返送値
//-----------------------------------------------------------------------------
int CShinJigyo::GetDiagData( long index, short leng, DIAGRAM_DATA *pData, int mode )
{
	if( index < 0 ){
		return 0;
	}
...
//		nChar	：
//-----------------------------------------------------------------------------
void CShinJigyo::Diag1_Edit( short index, UINT flg, short nChar )
{
	short	xpos = index;
	char	VK_FLG = 0x00;
	...
	switch( nChar ){
		...
		case VK_UP:
			flg = REPLY_STAY;
			if( (index>=JIGKNAME25) && (index<=JIGKNAME30) ){	// 項目名
				if( index != JIGKNAME25 ){
					xpos -= 1;
				}
			}
...
}BEGIN_EVENTSINK_MAP(CShinJigyo, CSyzBaseDlg)
ON_EVENT(CShinJigyo, IDC_ICSDIAGCTRL1, 1, CShinJigyo::EditONIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinJigyo, IDC_ICSDIAGCTRL1, 2, CShinJigyo::EditOFFIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinJigyo, IDC_ICSDIAGCTRL1, 3, CShinJigyo::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinJigyo::EditONIcsdiagctrl1(short index)
{
	...
	*/
	...
	else if( (m_Idx>=JIGKNAME25) && (m_Idx<=JIGKNAME30) ){
		m_Lin = 25+(m_Idx-JIGKNAME25);
		m_Clm = -1;
	}
...
void CShinJigyo::EditOFFIcsdiagctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA	data;
...
void CShinJigyo::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
	...
	if( wTerm==VK_UP ){		
//		if(index == JIGVAL33C){
//			m_Jigyo.SetPrevPosition();
//		}
//		else{
////			pos = UpDownFocusMoveCheck(index , 1);
			pos = FocusMoveCheck(index , 1);
			m_Jigyo.SetPosition(pos);	
//		}
	}
	if( wTerm==VK_DOWN ){
//		if(index == JIGVAL33C){
//			m_Jigyo.SetNextPosition();
//		}
//		else{
...
/*	int nowpos = m_Jigyo.GetPosition();	// 移動先または自身のポジション
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
//int CShinJigyo::UpDownFocusMoveCheck(int Index , int Move)
int CShinJigyo::FocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	BOOL end_flg = FALSE;
	...
	//現在ポジション検索
	if(( Move == 0 )||( Move == 1 )){
		for(i = 0 ; i < FCS_SHINJIGYOUCNT ; i++){
			if(Index == FCS_ShinJigyou[i].IDNo){
				break;
			}
		}
		idx = FCS_ShinJigyou[i].IDNo;//現在ポジションセット
	}else if(( Move == 2 )||( Move == 3 )){
		for(i = 0 ; i < FCS_SHINJIGYOUCNT ; i++){
			if(Index == FCS_ShinJigyou_LR[i].IDNo){
				break;
			}
		}
		idx = FCS_ShinJigyou_LR[i].IDNo;//現在ポジションセット
	}

	//次ポジション検索
	if(Move == 0){//下方向
		for(j = i + 1 ; j < FCS_SHINJIGYOUCNT ; j++){
			m_Jigyo.GetAttribute(FCS_ShinJigyou[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_ShinJigyou[j].IDNo;
				break;
			}
		}
	}
		...
		for(j = i + 1 ; j < FCS_SHINJIGYOUCNT ; j++){
			m_Jigyo.GetAttribute(FCS_ShinJigyou_LR[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
				idx = FCS_ShinJigyou_LR[j].IDNo;
				break;
			}
		}
	}
...
void CShinJigyo::SetFocusRange()
{
	...
	for( int idx = 0; idx < FCS_SHINJIGYOUCNT; idx++ ){
		m_Jigyo.GetAttribute(FCS_ShinJigyou[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsUpEnd = FCS_ShinJigyou[idx].IDNo;
			break;
		}
	}
	for( int idx = 0; idx < FCS_SHINJIGYOUCNT; idx++ ){
		m_Jigyo.GetAttribute(FCS_ShinJigyou_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsLeftEnd = FCS_ShinJigyou_LR[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SHINJIGYOUCNT-1; idx > -1 ; idx-- ){
		m_Jigyo.GetAttribute(FCS_ShinJigyou[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsDownEnd = FCS_ShinJigyou[idx].IDNo;
			break;
		}
	}
	for( int idx = FCS_SHINJIGYOUCNT-1; idx > -1 ; idx-- ){
		m_Jigyo.GetAttribute(FCS_ShinJigyou_LR[idx].IDNo ,  (LPUNKNOWN)&DA);
		if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
			m_FcsRightEnd = FCS_ShinJigyou_LR[idx].IDNo;
			break;
		}
	}
}
...
//		pPset	：	印刷情報
//-----------------------------------------------------------------------------
void CShinJigyo::SetPrintInfo( CVPrint *pVprn, NPRT_PAR *pPset , SH_PRINT_CMINFO *pPrintCmInfo )
{
	ASSERT( pVprn );
	ASSERT( pPset );
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinJigyo::PrintHyo1( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j, lin, max;
...
// 
//-----------------------------------------------------------------------------
int CShinJigyo::PrintHyo2( int pno, int fno, CDC *pDC, char *pBp )
{
	int		i, j , len , lin , max;
	char	buf[64];
	KKVAL*	pK;
...
//		if( (j+lin) != 9 || (j+lin) != 15 || (j+lin) != 16 ){
		if( (j+lin) != KJIGLINE10 || (j+lin) != KJIGLINE16 || (j+lin) != KJIGLINE17 ){
			if( DispJudge( 1, j+lin ) == 1 ){
				m_pArith->l_print( buf, pK->KKval2, FMTX130 );
			}
		}
...
//		if( (j+lin) != 14 || (j+lin) != 17 || (j+lin) != 21 || (j+lin) != 23 ){
		if( (j+lin) != KJIGLINE15 || (j+lin) != KJIGLINE18 || (j+lin) != KJIGLINE22 || (j+lin) != KJIGLINE24){
//			if( l_test(pK->KKval1) || l_test(pK->KKval2) ){
//				m_pArith->l_print( buf, pK->KKval3, FMTX13 );
//			}
			if( DispJudge( 2, j+lin ) == 1 ){
...
// 初期ポジションセット '14.09.01
//-----------------------------------------------------------------------------
void CShinJigyo::SetItemPosition()
{
	int index;

	// 前回ポジションの復元
	if( m_curwnd == IDC_ICSDIAGCTRL1 ) {
		if( (index = m_Jigyo.GetPosition()) != -1) {
			m_Jigyo.SetPosition(index);
			m_Jigyo.SetFocus();
			return;
		}
	}
	...
	this->SetFocus();
	if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
		m_Jigyo.SetPosition(JIGVAL01A);
	}
	else{
		m_Jigyo.SetPosition(JIGKNAME25);
	}
	SetRedrawScroll( 0 );

}

int CShinJigyo::DispJudge( int clm, int line )
{
		...
		case 2:	// 課税取引金額
			switch( line ){
				...
				case 31: //計
					...
					for( j=0; j<31; j++ ){
						if((j==1)||(j==3)||(j==4)||(j==5)||(j==6)||(j==9)||(j==15)||(j==16)){
							continue;
						}
						if( m_pArith->l_test((*m_pTaxListData)->NRec.KJval[j].KKval2) ){
							st = 1;
...
// 手入力サインのクリア
int CShinJigyo::ClearManualInputSign()
{
	int rt = 0;
