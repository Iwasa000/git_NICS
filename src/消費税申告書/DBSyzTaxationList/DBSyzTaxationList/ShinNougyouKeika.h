...
class CShinNougyouKeika : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinNougyouKeika)

public:
	CShinNougyouKeika(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinNougyouKeika();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_NOUGYOU_KEIKA };
	...
	// ダイアグデータ表示
	void DispNougyouKeikaDiagData( long dlin );
	// カーソル移動
...
//	int UpDownFocusMoveCheck(int Index , int Move);
	...
	// 印刷４
	int PrintHyo4( int pno, int fno, CDC *pDC, char *pBp );
