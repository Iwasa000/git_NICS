// DBSyzTaxationList.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには DBSyzTaxationList.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE DBSyzTaxationListDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DBSyzTaxationList.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(DBSyzTaxationListDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(DBSyzTaxationListDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DBSyzTaxationList.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(DBSyzTaxationListDLL);
	}
	return 1;   // OK
}
...
extern "C" __declspec( dllexport ) HINSTANCE WINAPI GetTaxationListInstance()
{
	return 	g_hInstance;
}