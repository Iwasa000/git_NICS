...
// CPrnPrintKani_EX ダイアログ

IMPLEMENT_DYNAMIC(CPrnPrintKani_EX, ICSDialog)

CPrnPrintKani_EX::CPrnPrintKani_EX(CWnd* pParent /*=NULL*/)
	: ICSDialog(CPrnPrintKani_EX::IDD, pParent)
	, m_pSnHeadData(NULL)
	, m_pZmSub(NULL)
	, m_pShinInfo(NULL)
	, m_pPrintCmInfo(NULL)
...
CPrnPrintKani_EX::~CPrnPrintKani_EX()
{
}

void CPrnPrintKani_EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPrnPrintKani_EX, ICSDialog)
	ON_BN_CLICKED(IDC_BUTTON1, &CPrnPrintKani_EX::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CPrnPrintKani_EX::OnBnClickedButton2)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_CHECK_ALLOUT, &CPrnPrintKani_EX::OnBnClickedCheckAllout)
	ON_BN_CLICKED(IDC_CHECK_WITH_MYNO, &CPrnPrintKani_EX::OnBnClickedCheckWithMyno)
	ON_BN_CLICKED(IDC_RADIO1,&CPrnPrintKani_EX::OnBnClickedRadio1)
	ON_BN_CLICKED(IDC_RADIO2,&CPrnPrintKani_EX::OnBnClickedRadio2)
	ON_BN_CLICKED(IDC_BUTTON4, &CPrnPrintKani_EX::OnBnClickedButton4)
	ON_BN_CLICKED(IDC_BUTTON5, &CPrnPrintKani_EX::OnBnClickedButton5)
	ON_BN_CLICKED(IDC_BUTTON3, &CPrnPrintKani_EX::OnBnClickedButton3)
END_MESSAGE_MAP()


// CPrnPrintKani_EX メッセージ ハンドラ

BOOL CPrnPrintKani_EX::OnInitDialog()
{
...
BOOL CPrnPrintKani_EX::PreTranslateMessage(MSG* pMsg)
{
...
// 24/03/18_税務代理書面 cor <--
		...
		m_Diag2.SetData(CHEK21, (LPUNKNOWN)&data);
	}
	//<-----
	...
	// 中間申告第26号様式チェック
	if( m_pSnHeadData->IsMiddleProvisional() ){
		diag_setposition( IDC_ICSDIAG8CTRL2, CHEK06_K, CIcsdiagctrl );
...
// 返送値	0		：	正常終了
//-----------------------------------------------------------------------------
int CPrnPrintKani_EX::InitInfo( CSnHeadData *pSnHeadData, CDBNpSub *pZmSub, SYC_SHININFO *pShinInfo, 
...
// 【戻り値】	なし
//-----------------------------------------------------------------------------
void CPrnPrintKani_EX::SetInputItem( void )
{
	...
	while( 1 ){
		...
		switch( Flg ){
			case 1 :	// 本表・付表・第26号様式
				...
				if(( (pPrnTbl+cnt)->Index == CHEK05_1 )||( (pPrnTbl+cnt)->Index == CHEK05_2 )){
					if( m_Util.GetKazeihoushiki(m_pZmSub->zvol) == ID_ICSSH_KANNI ){
						if ( m_isNoneMTForm ) {
							Flg = 0;
							//hide = 1;
				...
				else if ( ( ( pPrnTbl + cnt )->Index == CHEK04_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK04_2 )
					   || ( ( pPrnTbl + cnt )->Index == CHEK03_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK03_2 ) ) {
					if ( m_isNoneMTForm ) {
						Flg = 0;
						//hide = 1;
					}
				}
				else if ( ( ( pPrnTbl + cnt )->Index == CHEK04_3 ) || ( ( pPrnTbl + cnt )->Index == CHEK06_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK06_2 ) ) {
...
// 24/03/18_税務代理書面 cor <--
			...
			default:	// 入力不可
				Flg = 0;
				//if ( m_isNoneMTForm ) {
				//	if ( ( ( pPrnTbl + cnt )->Index == CHEK04_2 ) || ( ( pPrnTbl + cnt )->Index == CHEK05_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK05_2 ) ) {
				//		hide = 1;
				//	}
				//}
				//else {
				//	if ( ( ( pPrnTbl + cnt )->Index == CHEK04_3 ) || ( ( pPrnTbl + cnt )->Index == CHEK06_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK06_2 ) ) {
				//		hide = 1;
				//	}
				//}
				break;
...
// 22-0720 del <--
			...
			for(ii=0; ; ii++)	{
				if(LstBusuTblK[ii].idx[0] == -1)	break;
				if(LstBusuTblK[ii].idx[0] == (pPrnTbl+cnt)->Index)	{
					for(jj=0; ; jj++)	{
						if(LstBusuTblK[ii].idx[jj] == -1)	{
							break;
						}
						flg2=Flg;
						if(!(m_pSnHeadData->Sn_TYPE&0x01))	{
						...
						if(flg2)	{
							// 入力可
							ChangeColor(IDC_ICSDIAG8CTRL2,LstBusuTblK[ii].idx[jj],1);
							m_Diag2.ModifyItem(LstBusuTblK[ii].idx[jj],DIAG_MDFY_EDIT);
							SetInputItemData(LstBusuTblK[ii].idx[jj]);

// 24/07/10_個別タブから出力 add -->
							// 課税取引金額計算表、税務代理書面の部数
							// ※特定収入計算表は簡易課税では使用しないのでif文に組まない
							if(LstBusuTblK[ii].idx[jj] == CHEK12_BUSU_K || LstBusuTblK[ii].idx[jj] == CHEK13_BUSU_K || LstBusuTblK[ii].idx[jj] == CHEK14_BUSU_K)	{
								ChangeColor(IDC_ICSDIAG8CTRL2, LstBusuTblK[ii].idx[jj], 2);
								DIAGRAM_DATA	dd;
								dd.data_imgdata = NULL;
								dd.data_check = 0;
								m_Diag2.SetData(LstBusuTblK[ii].idx[jj], (LPUNKNOWN)&dd);
								GetInputItemData(LstBusuTblK[ii].idx[jj]);
								m_Diag2.ModifyItem(LstBusuTblK[ii].idx[jj], DIAG_MDFY_READONLY);
							}
// 24/07/10_個別タブから出力 add <--
						}
						else	{
							...
							dd.data_check = 0;
// 24/07/10_個別タブから出力 cor -->
							//if(IsBusu(LstBusuTblK[ii].idx[jj]) == TRUE)	{
// ---------------------------------
							if(IsBusu(LstBusuTblK[ii].idx[jj]) == TRUE &&
								(LstBusuTblK[ii].idx[jj] != CHEK12_BUSU_K && LstBusuTblK[ii].idx[jj] != CHEK13_BUSU_K && LstBusuTblK[ii].idx[jj] != CHEK14_BUSU_K))	{
// 24/07/10_個別タブから出力 cor <--
								dd.data_edit.Format(_T("%d"),m_pPrintCmInfo->busu[GetBusuNo(LstBusuTblK[ii].idx[jj])]);
							}
							m_Diag2.SetData(LstBusuTblK[ii].idx[jj],(LPUNKNOWN)&dd);
							GetInputItemData(LstBusuTblK[ii].idx[jj]);
							m_Diag2.ModifyItem(LstBusuTblK[ii].idx[jj],DIAG_MDFY_READONLY);
...
// 入力項目設定（サブ）
//-----------------------------------------------------------------------------
int CPrnPrintKani_EX::SetInputItemData( short idx )
{
	DIAGRAM_DATA dd;
	dd.data_imgdata = NULL;
	dd.data_check = 0;
	...
	switch( idx )	{
		...
		case CHEK03_1:	// 付表５－１（１）
			if(m_pSnHeadData->Sn_PrintOutSgn & 0x40)	{
				dd.data_check = 1;
			}
			break;
		case CHEK03_2:	// 付表５－１(２)
			if(m_pSnHeadData->Sn_PrintOutSgn & 0x100)	{
				dd.data_check = 1;
			}
			break;
		case CHEK06_1:	// 付表５－３（１）
			if(m_pSnHeadData->Sn_PrintOutSgn & 0x8000000)	{
				dd.data_check = 1;
			}
			break;
		case CHEK06_2:	// 付表５－３(２)
			if(m_pSnHeadData->Sn_PrintOutSgn & 0x10000000)	{
				dd.data_check = 1;
			}
			break;
		case CHEK04_1:	// 付表４－１
...
// 返送値	
//-----------------------------------------------------------------------------
int CPrnPrintKani_EX::GetInputItemData( short idx )
{
	DIAGRAM_DATA	dd;
	DIAGRAM_DATA	dd2;
	dd.data_imgdata = NULL;
	...
	switch(idx)	{
		...
		case CHEK03_1:	// 付表５－１（１）
			...
			break;
		case CHEK03_2:	// 付表５－１（２）
			m_pSnHeadData->Sn_PrintOutSgn &= ~0x100;
			if(dd.data_check)	{
				m_pSnHeadData->Sn_PrintOutSgn |= 0x100;
			}
			break;
		case CHEK06_1:	// 付表５－３（１）
			...
			break;
		case CHEK06_2:	// 付表５－３（２）
			m_pSnHeadData->Sn_PrintOutSgn &= ~0x10000000;
			if(dd.data_check)	{
				m_pSnHeadData->Sn_PrintOutSgn |= 0x10000000;
			}
			break;
		...
		case CHEK05_1:	// 付表５－２（１）
			...
			break;
		case CHEK05_2:	// 付表５－２（２）
			m_pSnHeadData->Sn_PrintOutSgn &= ~0x200;
			if(dd.data_check)	{
				m_pSnHeadData->Sn_PrintOutSgn |= 0x200;
			}
			break;
		...
		case CHEK14_K:	// 税務代理書面　個別指定用
			...
			if(dd.data_check)	{
		 	...
		 	break;
		//case CHEK15_K:	// 課税売上高計算表
		//	m_pSnHeadData->Sn_PrintOutSgn &= ~0x80000;
		//	if(dd.data_check)	{
		//		m_pSnHeadData->Sn_PrintOutSgn |= 0x80000;
		//	}
		// 	break;
		case CHEK18_K:	// 第２６号様式A4ヨコ控用
			...
			break;
		//case CHEK19:	// 事業別売上高
		//	m_pSnHeadData->Sn_PrintOutSgn &= ~0x2000000;
		//	if(dd.data_check)	{
		//		m_pSnHeadData->Sn_PrintOutSgn |= 0x2000000;
		//	}
		//	break;
...
// 印刷用情報のセット
//-----------------------------------------------------------------------------
//void CPrnPrintKani_EX::SetPrintInfo()			// 24/07/10_個別タブから出力 del
void CPrnPrintKani_EX::SetPrintInfo(int seltb)	// 24/07/10_個別タブから出力 add
{
	// 初期設定
	char	work[6] = {0};
...
// 24/07/10_個別タブから出力 add <--
			...
			switch( Flg ){
				case 1 :	// 本表・付表・第26号様式
					Flg = 1;
					if ( ( ( pPrnTbl + cnt )->Index == CHEK04_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK04_2 )
						   || ( ( pPrnTbl + cnt )->Index == CHEK03_1 ) || ( ( pPrnTbl + cnt )->Index == CHEK03_2 ) ) {
						if ( m_isNoneMTForm ) {
							Flg = 0;
						}
...
// 22-0720 del <--
					...
					case CHEK06_2:	// 付表５－３（２）
						if(m_pSnHeadData->Sn_PrintOutSgn & 0x10000000)	{
							//m_pPrintCmInfo->PRtable[Put] = 0x19;
							BusuCheckAndPut(0x19,cur);
						}
						break;
					case CHEK04_1:	// 付表４－１
						if(m_pSnHeadData->Sn_PrintOutSgn & 0x10)	{
					...
					case CHEK05_1:	// 付表５－２（１）
						if(m_pSnHeadData->Sn_PrintOutSgn & 0x80)	{
							//m_pPrintCmInfo->PRtable[Put] = 0x74;
							BusuCheckAndPut(0x74,cur);
						}
						break;
					case CHEK05_2:	// 付表５－２（２）
						if(m_pSnHeadData->Sn_PrintOutSgn & 0x200)	{
							//m_pPrintCmInfo->PRtable[Put] = 0x75;
							BusuCheckAndPut(0x75,cur);
						}
						break;
					case CHEK06_K:	// 第２６号様式
						if(m_pSnHeadData->Sn_PrintOutSgn & 0x800)	{
					...
					case CHEK14_K:	// 税務代理書面　個別指定用  
						...
						if((m_pSnHeadData->Sn_PrintOutSgn & 0x80000000) || (seltb & 0x04))	{	// 24/07/10_個別タブから出力 del
// 24/03/18_税務代理書面 cor -->
...
//								m_pPrintCmInfo->Fhy6Tp = Put + 1;
					...
					case CHEK22_K: //特例１
						if(m_pShinInfo->ActViewVer == ID_VER_SYZ31)	{
							CH31HyoView	*pH31View = NULL;
							pH31View = (CH31HyoView *)(((CDBSyzShinMainApp*)AfxGetApp())->GetDocumentTemplate()->GetViewWnd(1));
							//int idx = (pPrnTbl + cnt)->Index - CHEK22_K;
							...
							if (pH31View) {
								...
								//	*pTokurei_Mx[idx] = pH31View->GetPageNumTokurei(idx);
									for (int i = 0; i != pH31View->GetPageNumTokurei(0); i++) {
								//		m_pPrintCmInfo->PRtable[Put] = Table[idx];
										//m_pPrintCmInfo->PRtable[Put] = 0x76;
										BusuCheckAndPut(0x76,cur,i);
									}
								}
							}
...
BEGIN_EVENTSINK_MAP(CPrnPrintKani_EX, ICSDialog)
	ON_EVENT(CPrnPrintKani_EX, IDC_ICSDIAG8CTRL2, 2, CPrnPrintKani_EX::EditOFFIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CPrnPrintKani_EX, IDC_ICSDIAG8CTRL2, 1, CPrnPrintKani_EX::EditONIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CPrnPrintKani_EX, IDC_ICSDIAG8CTRL2, 3, CPrnPrintKani_EX::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CPrnPrintKani_EX, IDC_ICSDIAG8CTRL2, 9, CPrnPrintKani_EX::CheckButtonIcsdiag8ctrl2, VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CPrnPrintKani_EX, IDC_ICSDIAG8CTRL2, 8, CPrnPrintKani_EX::ComboSelIcsdiag8ctrl2, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CPrnPrintKani_EX::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	GetInputItemData( index );
}

void CPrnPrintKani_EX::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
}

void CPrnPrintKani_EX::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
	...
	switch(wTerm)	{
		...
		case VK_F2:
			while(1)	{
				...
				if(CursorPosKan[yy][xx] == -1)	{
					continue;
				}
				diag_getattr(IDC_ICSDIAG8CTRL2,CursorPosKan[yy][xx],&da,CIcsdiagctrl);
...
void CPrnPrintKani_EX::CheckButtonIcsdiag8ctrl2(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	GetInputItemData( index );
	// 白紙
...
// 24/03/22_背景色 add -->
		...
		if(index == CHEK12_K || index == CHEK13_K || index == CHEK14_K)	{
...
void CPrnPrintKani_EX::ComboSelIcsdiag8ctrl2(short index, LPUNKNOWN data)
{
	DIAGRAM_DATA *pdata = (DIAGRAM_DATA*)data;
...
void CPrnPrintKani_EX::ComboSelIcsdiag8ctrl2Ex( short pIndex, DIAGRAM_DATA* pData )
{
	DIAGRAM_DATA dd;
...
//-----------------------------------------------------------------------------
BOOL CPrnPrintKani_EX::ChoiceItem( int pSw )
{
	BOOL			brv=TRUE;
	SH_ITEMINDEX	*pPrnTbl;
	short			idx=0;
...
//					if( ((pPrnTbl+cnt)->Index == CHEK20) && !(m_Util.GetKazeihoushiki(m_pZmSub->zvol) == ID_ICSSH_KANNI) ){
					if( (((pPrnTbl+cnt)->Index == CHEK05_1)||(pPrnTbl+cnt)->Index == CHEK05_2) && !(m_Util.GetKazeihoushiki(m_pZmSub->zvol) == ID_ICSSH_KANNI) ){
						Flg = 0;
					}
				}
				break;
...
//		sign	：
//-----------------------------------------------------------------------------
void CPrnPrintKani_EX::ChangeColor( unsigned short id, short index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 24/03/25_168691 add -->
			if(index == CHEK12_K || index == CHEK13_K || index == CHEK14_K)	{
				...
				if(dd.data_check != 0)	{
					int	sw=0;
					if(index == CHEK12_K)		sw=0;
					else if(index == CHEK13_K)	sw=1;
					else						sw=2;
// 24/04/01_168704 cor -->
...
void CPrnPrintKani_EX::OnBnClickedButton1()
{
...
void CPrnPrintKani_EX::OnBnClickedButton3()
{
	int tab_num = 1;
...
void CPrnPrintKani_EX::OnBnClickedButton4()
{
	int tab_num = 1;
	CWnd*	p1 = GetParent();
	CWnd*	p2 = p1->GetParent();
	...
	((CPrnReportDlg*)p2)->m_PrnTab.SetCurSel(tab_num);
	((CPrnReportDlg*)p2)->PostMessage(WM_ACTIVE_SETFORCUS, 0, 0);
	((CPrnReportDlg*)p2)->SetFocusTab(1);//課税取引
	
}


void CPrnPrintKani_EX::OnBnClickedButton5()
{
	int tab_num = 1;
	CWnd*	p1 = GetParent();
	CWnd*	p2 = p1->GetParent();
...
// ボタン制御
void CPrnPrintKani_EX::Button3Control(void)
{
//	BOOL			syzSgn = FALSE;
//	long			eymd = 0;
//	DIAGRAM_DATA	dd;
...
//
//		ChangeColor(IDC_ICSDIAG8CTRL2, CHEK13_K, 2);
//		m_Diag2.ModifyItem(CHEK13_K, DIAG_MDFY_READONLY);
//		ChangeColor(IDC_ICSDIAG8CTRL2, CHEK13_BUSU_K, 2);
//		m_Diag2.ModifyItem(CHEK13_BUSU_K, DIAG_MDFY_READONLY);
//		return;
//	}
//
//	GetDlgItem(IDC_BUTTON3)->ShowWindow(SW_SHOW);
//
//	ChangeColor(IDC_ICSDIAG8CTRL2, CHEK13_K, 1);
//	m_Diag2.ModifyItem(CHEK13_K, DIAG_MDFY_EDIT);
//	ChangeColor(IDC_ICSDIAG8CTRL2, CHEK13_BUSU_K, 1);
//	m_Diag2.ModifyItem(CHEK13_BUSU_K, DIAG_MDFY_EDIT);
//
//	// 課税取引・旧税率ありなしの判定
...
//	}
//	// チェックの状態を取得
////	m_Diag2.GetData(CHEK17, (LPUNKNOWN)&dd);

	GetDlgItem(IDC_BUTTON3)->ShowWindow(SW_HIDE);
}
// 24/02/19wd_特定収入対応 add <--koko

void CPrnPrintKani_EX::Button4Control(void)
{
	//BOOL			syzSgn = FALSE;
	//long			eymd = 0;
	//DIAGRAM_DATA	dd;
	...
	//	ChangeColor(IDC_ICSDIAG8CTRL2, CHEK12_K, 2);
	//	m_Diag2.ModifyItem(CHEK12_K, DIAG_MDFY_READONLY);
	//	ChangeColor(IDC_ICSDIAG8CTRL2, CHEK12_BUSU_K, 2);
	//	m_Diag2.ModifyItem(CHEK12_BUSU_K, DIAG_MDFY_READONLY);
	//	return;
	//}
	//GetDlgItem(IDC_BUTTON4)->ShowWindow(SW_SHOW);
	//ChangeColor(IDC_ICSDIAG8CTRL2, CHEK12_K, 1);
	//m_Diag2.ModifyItem(CHEK12_K, DIAG_MDFY_EDIT);
	//ChangeColor(IDC_ICSDIAG8CTRL2, CHEK12_BUSU_K, 1);
	//m_Diag2.ModifyItem(CHEK12_BUSU_K, DIAG_MDFY_EDIT);
...
void CPrnPrintKani_EX::Button5Control(void)
{
// 24/03/18_税務代理書面 cor -->
	//GetDlgItem(IDC_BUTTON5)->ShowWindow(SW_SHOW);

	////ChangeColor(IDC_ICSDIAG8CTRL2, CHEK14_K, 1);
	////m_Diag2.ModifyItem(CHEK14_K, DIAG_MDFY_EDIT);
	////ChangeColor(IDC_ICSDIAG8CTRL2, CHEK14_BUSU_K, 1);
	//m_Diag2.ModifyItem(CHEK14_BUSU_K, DIAG_MDFY_EDIT);
// -----------------------------
// 修正No.168698 cor -->
...
void CPrnPrintKani_EX::OnCancel()
{
	// TODO: ここに特定なコードを追加するか、もしくは基本クラスを呼び出してください。
...
HBRUSH CPrnPrintKani_EX::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CPrnPrintKani_EX::OnBnClickedCheckAllout()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
...
void CPrnPrintKani_EX::OnBnClickedCheckWithMyno()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
...
void CPrnPrintKani_EX::OnBnClickedRadio1()
{
// 160103 add -->
	if(!(m_pSnHeadData->Sn_TYPE & 0x01))	{
		// ＯＣＲ用紙に印刷するから変更された場合
		...
		// 直前のチェック状態に戻す
		if(m_pSnHeadData->Sn_PrintOutSgn2 & 0x04)	{
			// 白紙のときにチェックが付いていなければ「提出用カラー」のみONにする
			if( !(m_pSnHeadData->Sn_PrintOutSgn2 & 0x08) && 
				!(m_pSnHeadData->Sn_PrintOutSgn2 & 0x10) )	{
				m_pSnHeadData->Sn_PrintOutSgn2 |= 0x08;
			}
...
void CPrnPrintKani_EX::OnBnClickedRadio2()
{
	if(m_swBeForked)	{
...
// 返送値	BOOL	：	TRUE:部数である
//-----------------------------------------------------------------------------
BOOL CPrnPrintKani_EX::IsBusu( int pIdx )
{
	int	ii=0;
	BOOL	brv=FALSE;

	for(ii=0; ; ii++)	{
		if(LstBusuIdxKan[ii].dsp_no == -1)	{
			break;
		}
		if(LstBusuIdxKan[ii].dsp_no == pIdx)	{
			brv=TRUE;
			break;
		}
	}
...
// 返送値	int		：	部数の配列番号
//-----------------------------------------------------------------------------
int CPrnPrintKani_EX::GetBusuNo( int pIdx )
{
	int	ii=0,ret=-1;
...
//-----------------------------------------------------------------------------
BOOL CPrnPrintKani_EX::IsCheckOn( int pIdx )
{
	BOOL			brv=FALSE;
	int				ii=0,jj=0;
	DIAGRAM_DATA	dd;
	...
	for(ii=0; ; ii++)	{
		...
		for(jj=1; ; jj++)	{
			...
			if(LstBusuTblK[ii].idx[jj] == pIdx)	{
				m_Diag2.GetData(LstBusuTblK[ii].idx[0],(LPUNKNOWN)&dd);
				if(dd.data_check != 0)	{
					brv=TRUE;
				}
				break;
...
// 返送値	int		：	最大部数
//-----------------------------------------------------------------------------
int CPrnPrintKani_EX::GetMaxBusu( void )
{
	int				ret=0;
	int				cnt=0;
	int				ii=0,jj=0,kk=0;
			...
			for(ii=0; ; ii++)	{
				...
				if(LstBusuTblK[ii].idx[0] == (pPrnTbl+cnt)->Index)	{
					for(jj=1; ; jj++)	{
						...
						for(kk=0; ; kk++)	{
							if(LstBusuIdxKan[kk].dsp_no == -1)	{
								break;
							}
							if(LstBusuIdxKan[kk].dsp_no == LstBusuTblK[ii].idx[jj])	{
// 24/07/10_個別タブから出力 cor -->
								////m_Diag2.GetData(LstBusuIdxKan[kk].dsp_no,(LPUNKNOWN)&dd);
								////if(ret < _tstoi(dd.data_edit))	{
								////	ret = _tstoi(dd.data_edit);
								////}
								//if(ret < m_pPrintCmInfo->busu[GetBusuNo(LstBusuTblK[ii].idx[jj])])	{
								//	ret = m_pPrintCmInfo->busu[GetBusuNo(LstBusuTblK[ii].idx[jj])];
								//}
// ---------------------------------
								if(LstBusuTblK[ii].idx[jj] == CHEK12_BUSU_K)	{		// 課税取引金額計算表の部数
								...
								else if(LstBusuTblK[ii].idx[jj] == CHEK14_BUSU_K)	{	// 税務代理書面の部数
...
//-----------------------------------------------------------------------------
void CPrnPrintKani_EX::BusuCheckAndPut( unsigned char pPno, int pSno, int pPage/*=1*/ )
{
	bool			brv=false;
	bool			rsw=false;
	int				ii=0,jj=0;
	...
	if(!(m_pSnHeadData->Sn_TYPE&0x01))	{
		...
		for(ii=0; ; ii++)	{
			...
			for(jj=0; ; jj++)	{
				if(LstBusuIdxKan[ii].prt_no[jj] == -1)	{
					break;
				}
				if(LstBusuIdxKan[ii].prt_no[jj] == (int)pPno)	{
					if(m_pPrintCmInfo->busu[GetBusuNo(LstBusuIdxKan[ii].dsp_no)] >= pSno)	{
						brv = TRUE;
					}
					rsw = true;
					break;
...
// 返送値	なし
//-----------------------------------------------------------------------------
void CPrnPrintKani_EX::BusuCheckAndPut3( unsigned char pPno, int pSno )
{
	int			key=0;
...
// 返送値	なし
//-----------------------------------------------------------------------------
void CPrnPrintKani_EX::BusuCheckAndPut4( unsigned char pPno, int pSno )
{
	int			key=0;
...
// ---------------------------------------------------------------------
// 24/03/25_168691 cor -->
//void CPrnPrintKani_EX::BackColorControl( short pInd, DIAGRAM_DATA* pData )
// -----------------------
void CPrnPrintKani_EX::BackColorControl( short pInd, DIAGRAM_DATA* pData/*=NULL*/ )
// 24/03/25_168691 cor <--
{
	DIAGRAM_ATTRIBUTE	da;
	int					sw=0;
...
// 24/03/25_168691 cor <--
	...
	// チェックが付いた場合、選択可能なチェックがすべてＯＮか調べる
		...
		if(pInd == CHEK12_K)		sw=0;
		else if(pInd == CHEK13_K)	sw=1;
		else						sw=2;
...
// 【戻り値】	なし
// --------------------------------------------------------------------------------
void CPrnPrintKani_EX::BackColorControl2( int pSw )
{
	DIAGRAM_DATA		dd;
	DIAGRAM_ATTRIBUTE	da;
	int					sw=0;
	...
	dd.data_imgdata = NULL;
	if(pSw == 0)		ind=CHEK12_K;
	else if(pSw == 1)	ind=CHEK13_K;
	else				ind=CHEK14_K;
