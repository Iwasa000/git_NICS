...
IMPLEMENT_DYNAMIC(CDlgJobEnd, ICSDialog)

CDlgJobEnd::CDlgJobEnd(CWnd* pParent /*=nullptr*/)
	: ICSDialog(IDD_DIALOG_JOBEND, pParent)
{
	m_nRet = -1;
}

CDlgJobEnd::~CDlgJobEnd()
{
}

void CDlgJobEnd::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgJobEnd, ICSDialog)
	ON_BN_CLICKED(IDC_BUTTON1, &CDlgJobEnd::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CDlgJobEnd::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON3, &CDlgJobEnd::OnBnClickedButton3)
END_MESSAGE_MAP()
...
void CDlgJobEnd::OnBnClickedButton1()
{
...
// 処理終了ボタン（業務を終了する）
void CDlgJobEnd::OnBnClickedButton2()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	// 戻り値をセット
	m_nRet = IDNO;
...
// 処理に戻るボタン（終了処理を中断）
void CDlgJobEnd::OnBnClickedButton3()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	// 戻り値をセット
...
//************************************************
int	CDlgJobEnd::ShowDialog( LPCTSTR title, LPCTSTR text, LPCTSTR btn1text, LPCTSTR btn2text, LPCTSTR btn3text, UINT type, UINT help, short point, CWnd* pParent)
{
	// 引数をメンバ変数に保持
	m_strTitle = title;
	m_strText = text;
...
void CDlgJobEnd::ResetDlgWindow( CRect rcOrg, CRect rcResize )
{
