...
class CPrnPrint_EX : public ICSDialog
{
	DECLARE_DYNAMIC(CPrnPrint_EX)

public:
	CPrnPrint_EX(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CPrnPrint_EX();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_PRN_PRINT_EX };
...
// 240308_h_160357 cor <--
	...
	// 出力指定の状態
	EnumIdPrintSelState	m_PrselState;	// 24/07/10_個別タブから出力 add
...
public:
	...
	afx_msg void OnBnClickedButton3();	// 24/02/19wd_特定収入対応 add
...
public:
	...
	void BusuCheckAndPut2(unsigned char pPno, int pSno, int pPage=1);	// 24/02/26wd_特定収入対応 add
...
// 修正No.160356,168678 add -->
	//LRESULT OnMyBackColorChange(WPARAM w, LPARAM l);
// 修正No.160356,168678 add <--
