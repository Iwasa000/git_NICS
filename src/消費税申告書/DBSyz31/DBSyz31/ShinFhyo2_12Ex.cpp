...
IMPLEMENT_DYNAMIC(CShinFhyo2_12Ex, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;


CShinFhyo2_12Ex::CShinFhyo2_12Ex(CWnd* pParent /*=nullptr*/)
	: CSyzBaseDlg(CShinFhyo2_12Ex::IDD, pParent)
	, m_pParent ( pParent )
	...
	, m_SpcSgn ( FALSE )
{
	...
	//----> 20230120
	memset ( F212_27A, '\0', sizeof ( F212_27A ) );
	memset ( F212_27B, '\0', sizeof ( F212_27B ) );
	memset ( F212_27C, '\0', sizeof ( F212_27C ) );
	memset ( F212_27X, '\0', sizeof ( F212_27X ) );

	memset ( F212_28A, '\0', sizeof ( F212_28A ) );
	memset ( F212_28B, '\0', sizeof ( F212_28B ) );
	memset ( F212_28C, '\0', sizeof ( F212_28C ) );
	memset ( F212_28X, '\0', sizeof ( F212_28X ) );
	//<---- 20230120
	...
	F212_10c = F212_12c = F212_19c = F212_20c = F212_23c = F212_28c = 0;
...
CShinFhyo2_12Ex::~CShinFhyo2_12Ex()
{
}

void CShinFhyo2_12Ex::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo2_12Ex, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinFhyo2_12Ex::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CShinFhyo2_12Ex::OnBnClickedButton2)
END_MESSAGE_MAP()

BEGIN_EVENTSINK_MAP(CShinFhyo2_12Ex, CSyzBaseDlg)
	ON_EVENT(CShinFhyo2_12Ex, IDC_ICSDIAG8CTRL1, 1, CShinFhyo2_12Ex::EditONIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CShinFhyo2_12Ex, IDC_ICSDIAG8CTRL1, 2, CShinFhyo2_12Ex::EditOFFIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CShinFhyo2_12Ex, IDC_ICSDIAG8CTRL1, 3, CShinFhyo2_12Ex::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinFhyo2_12Ex, IDC_ICSDIAG8CTRL1, 9, CShinFhyo2_12Ex::CheckButtonIcsdiag8ctrl2, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()
// ShinFhyo2_12 メッセージ ハンドラー


BOOL CShinFhyo2_12Ex::OnInitDialog()
{
...
void CShinFhyo2_12Ex::OnSize(UINT nType, int cx, int cy)
{
...
void CShinFhyo2_12Ex::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinFhyo2_12Ex::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CShinFhyo2_12Ex::PreTranslateMessage(MSG* pMsg)
{
...
void CShinFhyo2_12Ex::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinFhyo2_12Ex::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinFhyo2_12Ex::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if ( nChar == 0 ) {
		return;
	...
	if ( wTerm == VK_DELETE ) {
		...
		case ID212_28A:
			F212_28c &= ~0x08;
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328201" ) );
			// 各入力の表示
			F212data_all ( 28 );
			m_Fhyo2_12Diag01.SetPosition ( ID212_28A );
			break;

		case ID212_28B:
			F212_28c &= ~0x80;
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328202" ) );
			// 各入力の表示
			F212data_all ( 28 );
			m_Fhyo2_12Diag01.SetPosition ( ID212_28B );
			break;

		case ID212_28C:
			F212_28c &= ~0x800;
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328203" ) );
			// 各入力の表示
			F212data_all ( 28 );
			m_Fhyo2_12Diag01.SetPosition ( ID212_28C );
			break;
		//<----20230120
...
void CShinFhyo2_12Ex::CheckButtonIcsdiag8ctrl2(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
//			-1			：	エラー
//=========================================================
int CShinFhyo2_12Ex::InitInfo ( CH31SnFhyo2_02Data **pSnFhyo2_02Data, CSnHeadData **pSnHeadData, CH31SnTanaData **pSnTanaData , CArithEx *pArith, SYC_SHININFO *pShinInfo, CSyzSyukei **pSyzSyukei )
{
...
int CShinFhyo2_12Ex::ReadData()
{
	...
	memmove ( F212_27A, ( *m_pSnFhyo2_02Data )->Sn_F202_27A, MONY_BUF_SIZE );
	memmove ( F212_27B, ( *m_pSnFhyo2_02Data )->Sn_F202_27B, MONY_BUF_SIZE );
	memmove ( F212_27C, ( *m_pSnFhyo2_02Data )->Sn_F202_27C, MONY_BUF_SIZE );
	memmove ( F212_27X, ( *m_pSnFhyo2_02Data )->Sn_F202_27X, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	memmove ( F212_28A, ( *m_pSnFhyo2_02Data )->Sn_F202_28A, MONY_BUF_SIZE );
	memmove ( F212_28B, ( *m_pSnFhyo2_02Data )->Sn_F202_28B, MONY_BUF_SIZE );
	memmove ( F212_28C, ( *m_pSnFhyo2_02Data )->Sn_F202_28C, MONY_BUF_SIZE );
	memmove ( F212_28X, ( *m_pSnFhyo2_02Data )->Sn_F202_28X, MONY_BUF_SIZE );
	F212_28c = ( *m_pSnFhyo2_02Data )->Sn_F202_28sw;
	//<---- 20230120
...
// 読込みデータを集計モジュールへ
//=========================================================
void CShinFhyo2_12Ex::ReadData2()
{
	if ((*m_pSnHeadData)->Sn_Sign4 & 0x80) {//非連動の場合
		...
		//---->20230120
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328101" ), F212_27A );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328102" ), F212_27B );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328103" ), F212_27C );

		if ( F212_28c & 0x08 ) {
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328201" ), F212_28A );
		}
		else {
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328201" ) );
		}
		...
		if ( F212_28c & 0x80 ) {
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328202" ), F212_28B );
		}
		else {
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328202" ) );
		}
		...
		if ( F212_28c & 0x800 ) {
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328203" ), F212_28C );
		}
		else {
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328203" ) );
		}
...
int CShinFhyo2_12Ex::WriteData()
{
	...
	//----> 20230120
	// 11.適格請求書発行事業者～…支払い対価の額(税込み)
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_27A, F212_27A, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_27B, F212_27B, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_27C, F212_27C, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_27X, F212_27X, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_28A, F212_28A, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_28B, F212_28B, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_28C, F212_28C, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_02Data )->Sn_F202_28X, F212_28X, MONY_BUF_SIZE );
	( *m_pSnFhyo2_02Data )->Sn_F202_28sw = F212_28c;
	//<---- 20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_12Ex::MemoryReget()
{
	// 入力ロック時は集計しない
	...
	for ( int l = 0; l < cnt; l++ ) {
		...
		for ( int m = 0; m < incnt; m++ ) {
			...
			//---->20230120
			else if ( money[l][m].code == _T( "328101" ) ) {
				memmove ( F212_27A, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328102" ) ) {
				memmove ( F212_27B, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328103" ) ) {
				memmove ( F212_27C, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328100" ) ) {
				memmove ( F212_27X, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328201" ) ) {
				memmove ( F212_28A, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328202" ) ) {
				memmove ( F212_28B, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328203" ) ) {
				memmove ( F212_28C, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328200" ) ) {
				memmove ( F212_28X, money[l][m].arith, MONY_BUF_SIZE );
			}
			//<----20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_12Ex::SyukeiDataSet ( int cID )
{
	char val[MONY_BUF_SIZE] = { 0 };
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	//----> 20230120
	case ID212_27A:
		memmove ( F212_27A, val, MONY_BUF_SIZE );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328101" ), F212_27A );
		break;
	case ID212_27B:
		memmove ( F212_27B, val, MONY_BUF_SIZE );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328102" ), F212_27B );
		break;
	case ID212_27C:
		memmove ( F212_27C, val, MONY_BUF_SIZE );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328103" ), F212_27C );
		break;
	case ID212_28A:
		if ( m_pArith->l_cmp ( F212_28A, val ) ) {
			F212_28c |= 0x08;
			memmove ( F212_28A, val, MONY_BUF_SIZE );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328201" ), F212_28A );
		}
		break;
	case ID212_28B:
		if ( m_pArith->l_cmp ( F212_28B, val ) ) {
			F212_28c |= 0x80;
			memmove ( F212_28B, val, MONY_BUF_SIZE );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328202" ), F212_28B );
		}
		break;
	case ID212_28C:
		if ( m_pArith->l_cmp ( F212_28C, val ) ) {
			F212_28c |= 0x800;
			memmove ( F212_28C, val, MONY_BUF_SIZE );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328203" ), F212_28C );
		}
		break;
	//<---- 20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_12Ex::InitDiagAttr()
{
	int cnt = 0;
...
//============================================================
int CShinFhyo2_12Ex::ChkTkkzIdx ( short idx )
{
	int rSt = 0;
...
// 画面出力
//=========================================================
void CShinFhyo2_12Ex::DispFh2_12()
{
	...
	for( int i = ID212_1A; i <= ID212_28X; i++ ){
		dsp_cls ( IDC_ICSDIAG8CTRL1, ( short )i );
	}
	...
	diag_setdata ( IDC_ICSDIAG8CTRL1, KAZEIKIKAN_S, ( struct IUnknown* )&DiagData, CIcsdiagctrl );

	yy = mm = dd = 0;
	( *m_pSnHeadData )->GetYmdDataGen ( ID_ICSSH_KAZEIKIKAN_TO, &yy, &mm, &dd );
	sprintf_s ( buf, sizeof ( buf ), _T( "%02x.%02x.%02x" ), ( yy & 0xff ), ( mm & 0xff ), ( dd & 0xff ) );
	DiagData.data_disp = ( char* )buf;
	diag_setdata ( IDC_ICSDIAG8CTRL1, KAZEIKIKAN_E, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	...
	DiagData.data_disp = ( char* )buf;
	diag_setdata ( IDC_ICSDIAG8CTRL1, SHIMEI, ( struct IUnknown* )&DiagData, CIcsdiagctrl );

	// 課税売上高の5億円超えチェック
	ChangeColor ( IDC_ICSDIAG8CTRL1, MILLION, 1 );
	ChangeColor ( IDC_ICSDIAG8CTRL1, MILLION2, 1 );
	diag_clear ( IDC_ICSDIAG8CTRL2, MILLION_CHK, TRUE, CIcsdiagctrl );
	CheckOver500Million();
...
//			Index		：	ダイアグラム内インデックス
//=========================================================
void CShinFhyo2_12Ex::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//			Index		：	ダイアグラム内インデックス
//=========================================================
void CShinFhyo2_12Ex::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
// 課税売上高5億円越えチェック
//=========================================================
void CShinFhyo2_12Ex::CheckOver500Million()
{
	DIAGRAM_ATTRIBUTE DA;
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	if ( *m_pShinInfo->pOver500MillionSw == 1 ) {	// ON:９５％ルール改正対応期間で５億円超え
		...
		if ( ( *m_pSnHeadData )->IsKobetuSiireAnbun() == FALSE ) {
			...
			//ChangeColor ( IDC_ICSDIAG8CTRL1, MILLION2, 5 );
			color = BC_GREEN_L;
			memset ( buf, '\0', sizeof ( buf ) );
			...
			DiagData.data_disp = ( char* )buf;
			diag_setdata ( IDC_ICSDIAG8CTRL2, MILLION_CHK, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
		}
	}
	...
	if ( ( *m_pSnHeadData )->IsKobetuSiireAnbun() == FALSE ) {
		...
		diag_getattr ( IDC_ICSDIAG8CTRL1, MILLION2, &DA, CIcsdiagctrl );
		DA.attr_bcolor = color;
		diag_setattr ( IDC_ICSDIAG8CTRL1, MILLION2, &DA, TRUE, CIcsdiagctrl );
	}
}
...
//						FF3000　にセットした値
//=========================================================
int CShinFhyo2_12Ex::CheckOver500MillionSub ( int &MilSw, char *pVAL, char *pValCnv )
{
	int		nflg = 0;
	char	flg[MONY_BUF_SIZE] ={ 0 };
...
//		atno		：		項目番号
//=========================================================
void CShinFhyo2_12Ex::F212data_all ( int atno )
{
	switch( atno ){
	case 0:
	case 1:
...
// 課税売上(税抜き)表示
//=========================================================
void CShinFhyo2_12Ex::F212data_1()
{
...
// 課税資産の譲渡等の対価の額
//=========================================================
void CShinFhyo2_12Ex::F212data_4()
{
...
// 資産の譲渡等の対価の額
//=========================================================
void CShinFhyo2_12Ex::F212data_7()
{
...
// 課税売上割合
//=========================================================
void CShinFhyo2_12Ex::F212data_8()
{
	...
	if ( !( ( *m_pSnHeadData )->Sn_EXP100 & 0x01 ) ) {
		if ( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) {
			...
			if ( ( *m_pSnHeadData )->IsUseSemiUriRatio() ) {
				...
				diag_setdata ( IDC_ICSDIAG8CTRL1, KAZEIURIWARI, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
...
// 課税仕入れに係る支払対価の額(税込み)
//=========================================================
void CShinFhyo2_12Ex::F212data_9()
{
...
// 課税仕入れに係る消費税額
//=========================================================
void CShinFhyo2_12Ex::F212data_10()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
...
// 特定課税仕入れに係る支払対価の額
//=========================================================
void CShinFhyo2_12Ex::F212data_11()
{
...
// 特定課税仕入れに係る消費税額
//=========================================================
void CShinFhyo2_12Ex::F212data_12()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
		if ( ( *m_pSnHeadData )->IsNeedTkkzDisp() ) {
...
// 課税貨物に係る消費税額
//=========================================================
void CShinFhyo2_12Ex::F212data_13()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_14()
{
...
// 課税仕入れ等の税額の合計額
//=========================================================
void CShinFhyo2_12Ex::F212data_15()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_16()
{
...
// 15のうち、課税売上げにのみ要するもの
//=========================================================
void CShinFhyo2_12Ex::F212data_17()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_18()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_19()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_20()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_21()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_22()
{
...
// 控除対象仕入税額がプラスの時
//=========================================================
void CShinFhyo2_12Ex::F212data_23()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
...
// 控除対象仕入税額がマイナスの時
//=========================================================
void CShinFhyo2_12Ex::F212data_24()
{
...
// 貸倒回収に係る消費税額
//=========================================================
void CShinFhyo2_12Ex::F212data_25()
{
...
//=========================================================
void CShinFhyo2_12Ex::F212data_26()
{
...
//=========================================================
// 課税仕入れに係る消費税額  20230120
//=========================================================
void CShinFhyo2_12Ex::F212data_27()
{
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_27A, F212_27A );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_27B, F212_27B );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_27C, F212_27C );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_27X, F212_27X );
}

//=========================================================
// 課税仕入れに係る消費税額  20230120
//=========================================================
void CShinFhyo2_12Ex::F212data_28()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
		ChangeColor ( IDC_ICSDIAG8CTRL1, ID212_28A, ( F212_28c & 0x08 ) ? 3 : 0 );
		ChangeColor ( IDC_ICSDIAG8CTRL1, ID212_28B, ( F212_28c & 0x80 ) ? 3 : 0 );
		ChangeColor ( IDC_ICSDIAG8CTRL1, ID212_28C, ( F212_28c & 0x800 ) ? 3 : 0 );
	}

	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_28A, F212_28A );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_28B, F212_28B );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_28C, F212_28C );
	dsp_prs ( IDC_ICSDIAG8CTRL1, ID212_28X, F212_28X );
}

//=========================================================
// 課税売上割合を変更
//=========================================================
void CShinFhyo2_12Ex::ChangeF212data_8()
{
	// 課税売上割合に準ずる割合を有効
	// 課税売上割合が９５％未満
	if ( ( *m_pSnHeadData )->IsUseSemiUriRatio() || ( F212_8EX < (*m_pSnHeadData)->m_UriRatioBorder ) ) {
...
// 棚卸データの加減算を行う
//=========================================================
void CShinFhyo2_12Ex::TanaDataAdd()
{
...
// 表示金額取得
//=========================================================
void CShinFhyo2_12Ex::GetAllDiagData()
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	//----> 20230120
	// 11.適格請求書発行事業者～…支払い対価の額(税込み)
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_27A, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_27A, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_27B, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_27B, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_27C, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_27C, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_27X, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_27X, DiagData.data_val, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_28A, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_28A, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_28B, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_28B, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_28C, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_28C, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL1, ID212_28X, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F212_28X, DiagData.data_val, MONY_BUF_SIZE );
	//<---- 20230120
...
// ボタン表示変更
//=========================================================
void CShinFhyo2_12Ex::ChangeBtnDisp()
{
...
// 初期ポジションセット
//===================================================
void CShinFhyo2_12Ex::SetItemPosition()
{
	int index;
...
//			Move		：	0 = 下方向, 1 = 上方向
//=========================================================
int CShinFhyo2_12Ex::UpDownFocusMoveCheck ( int Index, int Move )
{
	int idx = 0;
	int i = 0, j = 0;
...
//			sign		:		変更サイン
//=========================================================
void CShinFhyo2_12Ex::ModifyDiagInput ( short index, int sign )
{
...
//			sign		:		変更サイン
//=========================================================
void CShinFhyo2_12Ex::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 棚卸調整額入力
//=========================================================
void CShinFhyo2_12Ex::OnBnClickedButton1()
{
...
// 特定収入の簡易計算式表示
//=========================================================
void CShinFhyo2_12Ex::OnBnClickedButton2()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
//===================================================
void CShinFhyo2_12Ex::F212calq_spc ( SYZTOSPC_DATA SpcData )
{
	if ( *( m_pShinInfo->pSgSpc ) == 1 ) {
...
//	FALSE		:		なし
//=========================================================
BOOL CShinFhyo2_12Ex::IsManualInputSign()
{
	BOOL ret = FALSE;
	if ( !( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) || ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) ) {
		// 連動時、入力ロック中はチェックしない
		...
		if ( ( F212_10c & 0x888 ) || ( F212_12c & 0x800 ) || ( F212_19c & 0x888 ) ||
			 ( F212_20c & 0x888 ) || ( F212_23c & 0x888 ) || ( F212_28c & 0x888 ) ) {
			ret = TRUE;
		}
	}
