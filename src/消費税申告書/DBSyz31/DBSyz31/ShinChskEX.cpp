...
CShinChskEX::CShinChskEX(CWnd* pParent /*=nullptr*/)
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinChskEX::InitInfo(CSnHeadData *pSnHeadData, CH31SnChskData *pSnChskData, CH31SnHonpyoData *pSnHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo)
{
	ASSERT(pSnHeadData);
	if (pSnHeadData == NULL) {
		return -1;
...
//	m_pSnHeadData->GetYmdData(ID_ICSSH_KAZEIKIKAN_FROM, &yymmdd[0], &yymmdd[1], &yymmdd[2]);
	...
	if( gengo == ID_ICSSH_REIWA_GENGO ){
		dgda.data_disp = SYZ31_REIWA_G_STR;
	}
	else if( gengo == ID_ICSSH_HEISEI_GENGO ){
		dgda.data_disp = SYZ31_HEISEI_G_STR;
	}
	diag_setdata(IDC_ICSDIAGCTRL1, x, &dgda, CIcsdiagctrl);
...
void CShinChskEX::SetDiagData()
{
	...
	m_Util.DiagOcxIniz(&dgda);
	memset(yymmdd, '\0', sizeof(yymmdd));
	m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &yymmdd[1], &yymmdd[2], &yymmdd[3] , &nengo);
	yymmdd[0] = (char)nengo;
	memmove(dgda.data_day, yymmdd, 4);
	diag_setdata(IDC_ICSDIAGCTRL1, x, &dgda, CIcsdiagctrl);
	x = CHUZKITA;
	m_Util.DiagOcxIniz(&dgda);
	memset(yymmdd, '\0', sizeof(yymmdd));
	nengo = 0;
	m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, &yymmdd[1], &yymmdd[2], &yymmdd[3] , &nengo);
	yymmdd[0] = (char)nengo;
	memmove(dgda.data_day, yymmdd, 4);
	diag_setdata(IDC_ICSDIAGCTRL1, x, &dgda, CIcsdiagctrl);
	...
	nengo = 0;
	memset(yymmdd, '\0', sizeof(yymmdd));
	m_pSnChskData->GetYmdDataGen(ID_SYUZEI_DECIDE_DAY, &yymmdd[1], &yymmdd[2], &yymmdd[3] , &nengo);
	yymmdd[0] = (char)nengo;
	memmove(dgda.data_day, yymmdd, 4);
	diag_setdata(IDC_ICSDIAGCTRL1, x, &dgda, CIcsdiagctrl);
...
void CShinChskEX::CalqSyz(char flg)
{
	...
	if ((flg == 0x01) || (flg == 0x02)) {
		...
		//--->'19.10.25
		if (m_pSnHeadData->Sn_KDAYS >= ICS_SH_10PER_REFORM_DAY) {
			m_Util.percent(ans, num1, 13, 78, 0);			

			/* 税シス第2949号の回答より、予定申告の計算方法は変更なし
			char val22[MONY_BUF_SIZE] = { 0 };
			char val78[MONY_BUF_SIZE] = { 0 };
			char val100[MONY_BUF_SIZE] = { 0 };
			m_pArith->l_input ( val22, "22" );
			m_pArith->l_input ( val78, "78" );
			m_pArith->l_input ( val100, "100000000000" );

			l_pardotnet ( ans, val100, val22, val78 );
			l_pardotnet ( ans, num1, ans, val100 );
			*/
		}
		else {
			m_Util.percent(ans, num1, 8, 63, 0);
		...
		//-------
//		m_Util.percent(ans, num1, 13, 78, 0);	// １０％（８％軽減分は考慮されていない）
		//<------
...
void CShinChskEX::EditOFFIcsdiagctrl1(short index)
{
	...
	char	dbuf[128] = { 0 };
	char	*buff;
	char	val[6] = { 0 }, num[6] = { 0 }, sday[4] = { 0 }, eday[4] = { 0 };
	int		len, tuki;
	int		st = 0x00;
	char	Str[128] = { 0 };
	int		Length = 0;
	...
	if (!(index == CHUZKJI) && !(index == CHUZKITA) && !(index == CHUSNMD)) {//yoshida190814
		diag_deledit(IDC_ICSDIAGCTRL1, CIcsdiagctrl);
	}
	...
	case CHUZKJI:		// 前課税期間 自
		memmove(sday, xdata.data_day, 4);
		memset(eday, '\0', sizeof(eday));
		m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &eday[1], &eday[2], &eday[3] , &nengo);
		eday[0] = char(nengo);

		//yoshida190814 
		//平成と令和をまたいでる時の判定がおかしい。２６でも同じ問題あって西暦に変えてチェックしている。西暦に変えてチェック

		// 元号番号+年月で和暦→西暦変換が行われているため、年月日で返還するよう修正
		// 和暦→西暦変換
		//voldate.db_datecnvGen(sday[0], (UCHAR*)sday, &se_gengo, (UCHAR*)seSday, 1, 0);
		//voldate.db_datecnvGen(eday[0], (UCHAR*)eday, &se_gengo, (UCHAR*)seEday, 1, 0);

		voldate.db_datecnvGen(sday[0], (UCHAR*)&sday[1], &se_gengo, (UCHAR*)seSday, 1, 0);
		voldate.db_datecnvGen(eday[0], (UCHAR*)&eday[1], &se_gengo, (UCHAR*)seEday, 1, 0);

		if (_CheckYYMMDD(seSday, seEday, 0) == 0) {//yoshida190814
//		if (_CheckYYMMDD(sday, eday, 0) == 0) {
			memmove(xdata.data_day, sday, 4);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 0, sday[1] , (int)sday[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 1, sday[2] , (int)sday[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 2, sday[3] , (int)sday[0]);
		}
		else {
			memset(yymmdd, '\0', sizeof(yymmdd));
			if ( xdata.data_day[0] == 0 ) {
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_FROM, 0, yymmdd[0], ( int )yymmdd[0] );
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_FROM, 1, yymmdd[0], ( int )yymmdd[0] );
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_FROM, 2, yymmdd[0], ( int )yymmdd[0] );
			...
			//yoshida190814
			//00年00月00日の対処
			if (yymmdd[1] == 0 || yymmdd[2] == 0 || yymmdd[3] == 0) {
				//memset(yymmdd, '\0', sizeof(yymmdd));
				return;
			}

			memmove(xdata.data_day, yymmdd, 4);
		}
		break;
	case CHUZKITA:		// 前課税期間 至
		m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &sday[0], &sday[1], &sday[2] , &nengo);
		sday[0] = (char)nengo;
		memmove(eday, xdata.data_day, 4);

		//yoshida190814 平成と令和をまたいでる時の判定がおかしい。２６でも同じ問題あり。西暦に変えてチェック
		// 和暦→西暦変換
		voldate.db_datecnvGen(sday[0], (UCHAR*)&sday[1], &se_gengo, (UCHAR*)seSday, 1, 0);
		voldate.db_datecnvGen(eday[0], (UCHAR*)&eday[1], &se_gengo, (UCHAR*)seEday, 1, 0);
		...
		if (_CheckYYMMDD(seSday, seEday, 1) == 0) {
			memmove(xdata.data_day, eday, 4);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 0, eday[1] , (int)eday[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 1, eday[2] , (int)eday[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 2, eday[3] , (int)eday[0]);
		}
		else {
			memset(yymmdd, '\0', sizeof(yymmdd));
			if ( xdata.data_day[0] == 0 ) {
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_TO, 0, yymmdd[0], ( int )yymmdd[0] );
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_TO, 1, yymmdd[0], ( int )yymmdd[0] );
				m_pSnChskData->SetYmdDataGen ( ID_ZENKI_KAZEIKIKAN_TO, 2, yymmdd[0], ( int )yymmdd[0] );
	...
	case CHUSNMD:		// 修正・更正・決定の年月日
		if ((xdata.data_day[0] & 0xff) == 0xff) {
			memset(xdata.data_day, '\0', 4);
		}
		memset(yymmdd, '\0', sizeof(yymmdd));
		if ( xdata.data_day[0] == 0 ) {
			m_pSnChskData->SetYmdDataGen ( ID_SYUZEI_DECIDE_DAY, 0, yymmdd[0], ( int )yymmdd[0] );
			m_pSnChskData->SetYmdDataGen ( ID_SYUZEI_DECIDE_DAY, 1, yymmdd[0], ( int )yymmdd[0] );
			m_pSnChskData->SetYmdDataGen ( ID_SYUZEI_DECIDE_DAY, 2, yymmdd[0], ( int )yymmdd[0] );
	...
	case CHUZEINEN:	// 提出年月日
		...
		// 令和の期間かチェック
		if ( m_Util.IsValidReiwaYmd ( ID_ICSSH_REIWA_GENGO, xdata.data_day[0], xdata.data_day[1], xdata.data_day[2] ) == false ) {
			memset ( xdata.data_day, '\0', 4 );
		}

		// 平成固定になっているため、令和固定に変換
		...
		m_pSnHeadData->SetYmdDataGen(ID_ICSSH_PRESENT_DATE, 0, xdata.data_day[0], 5);
		m_pSnHeadData->SetYmdDataGen(ID_ICSSH_PRESENT_DATE, 1, xdata.data_day[1], 5);
		m_pSnHeadData->SetYmdDataGen(ID_ICSSH_PRESENT_DATE, 2, xdata.data_day[2], 5);
		break;
	case CHUZEISYO:	// 税務署名
		memset(Str, '\0', sizeof(Str));
...
void CShinChskEX::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	...
	char	dbuf[128];
	char	*buff;
	char	val[6] = { 0 }, num[6] = { 0 }, sday[4] = { 0 }, eday[4] = { 0 }, yymmdd[4] = {0};
	int		len, tuki;
	short	xpos;
...
//			if (_CheckYYMMDD(sday, eday, 0) == 0) {
				...
				memmove(xdata.data_day, sday, 4);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 0, sday[1],sday[0]);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 1, sday[2],sday[0]);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 2, sday[3],sday[0]);
			}
			else {
				if ( xdata.data_day[0] != 0 ) {
					m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &xdata.data_day[1], &xdata.data_day[2], &xdata.data_day[3] , &nengo);
					xdata.data_day[0] = (char)nengo;
				}
			}
		}
		else if (nChar == VK_DELETE) {
			// Deleteキーを押下したときに、元号が消えてしまうためクリア処理カット
			//flg = REPLY_DELT;
			flg = REPLY_OK;
			//memset(xdata.data_day, '\0', 4);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 0, xdata.data_day[1],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 1, xdata.data_day[2],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, 2, xdata.data_day[3],xdata.data_day[0]);
			//diag_clear(IDC_ICSDIAGCTRL1, index, TRUE, CIcsdiagctrl);
			xpos = index;
		}
		break;
	case CHUZKITA:		// 前課税期間 至
		if (nChar == VK_RETURN) {
			m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &sday[1], &sday[2], &sday[3],&nengo);
			sday[0] = (char)nengo;
			memmove(eday, xdata.data_day, 4);
			...
			if (_CheckYYMMDD(seSday, seEday, 1) == 0) {//yoshida190814
//			if (_CheckYYMMDD(sday, eday, 1) == 0) {

				memmove(xdata.data_day, eday, 4);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 0, eday[1],eday[0]);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 1, eday[2],eday[0]);
				m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 2, eday[3],eday[0]);
			}
			else {
				if ( xdata.data_day[0] != 0 ) {
					memset(yymmdd, '\0', sizeof(yymmdd));
					m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_FROM, &yymmdd[1], &yymmdd[2], &yymmdd[3],&nengo);
					yymmdd[0] = (char)nengo;
					if (yymmdd[0] || yymmdd[1] || yymmdd[2] || yymmdd[3]) {
						sw2 = 0x01;
					}
					m_pSnChskData->GetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, &xdata.data_day[1], &xdata.data_day[2], &xdata.data_day[3],&nengo);
					if ( nengo != 0 ) {
						xdata.data_day[0] = (char)nengo;
					}
			...
			flg = REPLY_OK;
			//memset(xdata.data_day, '\0', 4);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 0, xdata.data_day[1],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 1, xdata.data_day[2],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_ZENKI_KAZEIKIKAN_TO, 2, xdata.data_day[3],xdata.data_day[0]);
			//diag_clear(IDC_ICSDIAGCTRL1, index, TRUE, CIcsdiagctrl);
			xpos = index;
		}
		break;
	case CHUSNMD:		// 修正・更正・決定の年月日
		if (nChar == VK_RETURN) {
			m_pSnChskData->SetYmdDataGen(ID_SYUZEI_DECIDE_DAY, 0, xdata.data_day[1],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_SYUZEI_DECIDE_DAY, 1, xdata.data_day[2],xdata.data_day[0]);
			m_pSnChskData->SetYmdDataGen(ID_SYUZEI_DECIDE_DAY, 2, xdata.data_day[3],xdata.data_day[0]);
		}
		else if (nChar == VK_DELETE) {
			// Deleteキーを押下したときに、元号が消えてしまうためクリア処理カット
			//flg = REPLY_DELT;
...
int CShinChskEX::_CheckYYMMDD(char *sday, char *eday, char sw)
{
	...
	if (sw == 0) {
		flg = Date_DateCheck((BYTE*)&ssday[1]);
	}
	else {
		if ((!eeday[0]) && (!eeday[1]) && (!eeday[2])) {
			Date_GetEymd((BYTE*)eeday, (BYTE*)ssday);
		}
		if ((flg = Date_DateCheck((BYTE*)&eeday[1])) == 0) {
			flg = -1;
			if (Date_KymdCheck((BYTE*)ssday, (BYTE*)eeday) >= 0) {
				if ((eeday[1] == 0x2) && (eeday[2] >= 0x28)) {
					BYTE	ymd[4];
					memmove(ymd, eeday, 3);
					shin_datecnv(ymd[0], ymd, 1);
					Date_SetFebEday(ymd);
...
BOOL CShinChskEX::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN) {
		...
		if ((m_curwnd == IDC_ICSDIAGCTRL1)) {
			...
			//kindわからないのでID絞り。日付部分
			if (result == CHUZKJI || result == CHUZKITA || result == CHUSNMD) {	
				//yoshida190814
				//元号選択箇所では左右キーを無効化
