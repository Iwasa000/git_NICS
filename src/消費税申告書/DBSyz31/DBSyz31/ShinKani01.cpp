...
IMPLEMENT_DYNAMIC(CShinKani01, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinKani01::CShinKani01(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKani01::IDD, pParent)
	, m_pParent ( pParent )
...
CShinKani01::~CShinKani01()
{
}

void CShinKani01::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKani01, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinKani01::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinKani01 メッセージ ハンドラ

BOOL CShinKani01::OnInitDialog()
{
...
void CShinKani01::OnSize(UINT nType, int cx, int cy)
{
...
void CShinKani01::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinKani01::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//		-1				:	エラー
//=============================================================
int CShinKani01::InitInfo ( CSnHeadData **pSnHeadData, CH31SnHonpyoData **pSnHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CSyzSyukei **pSyzSyukei )
{
	ASSERT ( pSnHeadData );
	if ( pSnHeadData == NULL ) {
		return -1;
...
void CShinKani01::ChgDiagAttr()
{
	...
	while ( 1 ) {
		...
		if ( m_pParent && ( ( CH31HyoView* )m_pParent )->IsNoDeliForm() ) {
			if ( Kani1[cnt].Index == ID2080 ) {
				Flg = 0;
			}
		}
...
// 220412 del -->
		//// 個人事業者の場合は「翌年以降送付不要」非表示
		//if (Kani1[cnt].Index == ID2080) {
		//	m_Kani01Diag01.ModifyItem(Kani1[cnt].Index, (Flg ? DIAG_MDFY_OPAQUE : DIAG_MDFY_TRANSPARENT));	// チェックボックス
		//	m_Kani01Diag01.ModifyItem(23, (Flg ? DIAG_MDFY_OPAQUE : DIAG_MDFY_TRANSPARENT));				// 「翌年以降送付不要」
		//}
// 220412 del <--
// 220412 add -->
		// 個人・法人に関係なく「翌年以降送付不要」を非表示
		if (Kani1[cnt].Index == ID2080) {
			m_Kani01Diag01.ModifyItem(Kani1[cnt].Index,DIAG_MDFY_TRANSPARENT);	// チェックボックス
			m_Kani01Diag01.ModifyItem(23,DIAG_MDFY_TRANSPARENT);				// 「翌年以降送付不要」
		}
// 220412 add <--
		cnt++;
	}
...
void CShinKani01::DispKani()
{
	...
	if ( ( ( *m_pSnHeadData )->Sn_ITIREN[0] & 0xff ) != 0xff ) {
		...
		DiagData.data_edit = buf;
		m_Kani01Diag01.SetData ( ID2070, ( struct IUnknown* )&DiagData );
	}
	...
	m_Kani01Diag01.SetData ( ID2080, ( struct IUnknown* )&DiagData );
	...
	if ( ( yy & 0xff ) && ( mm & 0xff ) && ( dd & 0xff ) ) {
		...
		m_Kani01Diag01.SetData ( ID2103, ( struct IUnknown* )&DiagData );
	...
	if ( !( ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) % 2 ) ) {
		...
		if ( ( yy & 0xff ) && ( mm & 0xff ) && ( dd & 0xff ) ) {
			...
			m_Kani01Diag01.SetData ( ID2133, ( struct IUnknown* )&DiagData );
		}
		yy = mm = dd = 0;
...
// 初期データ表示
//===========================================
void CShinKani01::DispInitKaniUpper()
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		Mony		:		表示金額
//=======================================================
void CShinKani01::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	if ( Id == IDC_ICSDIAG8CTRL2 ) {
		switch ( Index ) {
			case ID3XM2:	// 消費税額
				...
				/*
				if ( ( ( *m_pSnHeadData )->SVmzsw != 1 ) && ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0 ) ) {
					if ( !( m_M1c & 0x08 ) ) {
						ChangeColor ( IDC_ICSDIAG8CTRL2, ID3XM2, 0 );
					}
					else {
						ChangeColor ( IDC_ICSDIAG8CTRL2, ID3XM2, 2 );
					}
				}
				*/
				break;
...
//		Index		:		ダイアグラム内インデックス
//=======================================================
void CShinKani01::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		atno	:	0 = 全データ
//=================================
void CShinKani01::Disp_Kanidata ( int atno )
{
	switch ( atno ) {
	case 0:		// 全
	case 1:		// 課税標準額
...
// 差引納付税額
//====================================================
void CShinKani01::Disp_Kanidata14()
{
...
// 差引納付譲渡割額
//====================================================
void CShinKani01::Disp_Kanidata25()
{
	switch ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) {
	case 1:		// 確定・中間
	case 2:
...
//		sign	:		サイン
//=====================================================
void CShinKani01::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 終了時のフォーカス移動
//==========================================
void CShinKani01::MoveFocusInEnd()
{
	// データを確定させるためにフォーカス移動
...
// マイナンバーコントロール
//=========================================
void CShinKani01::MoveDiagMNEdit()
{
	if ( m_pParent && m_pParent->GetSafeHwnd() ) {
		if ( ( ( ( CH31HyoView* )m_pParent )->GetShRevType() >= ID_ICSSH_MYNUMBER_REVTYPE ) || ( ( *m_pSnHeadData )->Sn_KDAYE >= 20160401 ) ) {
			if ( ( *m_pSnHeadData )->IsSoleProprietor() == TRUE ) {	// 個人
...
// 入力位置のセット
//=============================
void CShinKani01::SetItemPosition()
{
	int index = 0;
...
// 税務署入力にフォーカスをセット
//===================================
void CShinKani01::SetTaxOfficeInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
...
// 入力位置を保存
//===================================
void CShinKani01::SaveInputPosition()
{
	m_curInPos = -1;
...
// 入力位置を復元
//===================================
void CShinKani01::RestoreInputPosition()
{
	if ( m_curInPos == -1 ) {
		return;
	}
...
BEGIN_EVENTSINK_MAP(CShinKani01, CSyzBaseDlg)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL1, 1, CShinKani01::EditONIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL2, 1, CShinKani01::EditONIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL3, 1, CShinKani01::EditONIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL1, 2, CShinKani01::EditOFFIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL2, 2, CShinKani01::EditOFFIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL3, 2, CShinKani01::EditOFFIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL1, 3, CShinKani01::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL2, 3, CShinKani01::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01, IDC_ICSDIAG8CTRL3, 3, CShinKani01::TerminationIcsdiag8ctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()


void CShinKani01::EditONIcsdiag8ctrl1(short index)
{
...
void CShinKani01::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01::EditONIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	char yymmdd[3] = { 0 };
	char buf[512] = { 0 };
	...
	switch ( index ) {
		...
		case ID2001:	// 提出月
			if ( DiagData.data_day[0] != 0 ) {
				...
				//if ( DiagDataYY.data_day[0] ) {
				//	( *m_pSnHeadData )->GetYmdDataGen ( ID_ICSSH_PRESENT_DATE, &tmpYmd[0], &tmpYmd[1], &tmpYmd[2] );
				//	if ( DiagDataYY.data_day[0] != tmpYmd[0] ) {
				//		DiagDataYY.data_day[0] = tmpYmd[0];
				//		m_Kani01Diag01.SetData ( ID2000, ( struct IUnknown* )&DiagDataYY  );
				//	}
				//}
...
void CShinKani01::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinKani01::EditOFFIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinKani01::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	char buff[64] = { 0 };
	int len = 0;
		...
		if ( index == ID2082 ) {
			if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
				if ( ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) % 2 ) {
					...
					nowdiag = 2;
					/*  確定申告のときは課税標準額は入力不可では？　修正依頼669
					if ( !( *m_pSnHeadData )->SVmzsw ) {
						// 経過措置なし
						m_Kani01Diag02.SetPosition (ID3XM10);	// 課税標準額
						nowdiag = 2;
					}
					else {
						// 経過措置あり
...
void CShinKani01::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01::TerminationIcsdiag8ctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
BOOL CShinKani01::PreTranslateMessage(MSG* pMsg)
{
...
void CShinKani01::OnBnClickedButton1()
{
...
// 集計セット
//=================================
int CShinKani01::SyukeiDataSet ( int cID )
{
	DIAGRAM_DATA DiagData;
...
// 付表データ表示
//============================================
void CShinKani01::Disp_FhyoData()
{
	char CkMy[MONY_BUF_SIZE] = { 0 };
	...
	//-----
//	m_pArith->l_div ( CkMy, KURI1, M1000 );
	//<----
	...
	m_pArith->l_div(CkMy, CkMy, M1000);
	//-----
//	m_pArith->l_div ( CkMy, KURI2, M1000 );
	//<----
	...
	m_pArith->l_div(CkMy, CkMy, M1000);
	//-----
//	m_pArith->l_div ( CkMy, KURI3, M1000 );
	//<----
	...
	m_pArith->l_div(CkMy, CkMy, M1000);
	//-----
//	m_pArith->l_div ( CkMy, KURI4, M1000);
	//<-----
	...
	m_pArith->l_div(CkMy, CkMy, M1000);
	//-----
//	m_pArith->l_div ( CkMy, KURI5, M1000 );
	//<----
	...
	m_pArith->l_div(CkMy, CkMy, M1000);
	//-----
//	m_pArith->l_div ( CkMy, KURI6, M1000 );
	//<----
...
//		pers	:	金額
//=================================================
void CShinKani01::dsp_uws ( unsigned short Id, short Index, unsigned short pers )
{
	if ( pers ) {
		char buf[128] = { 0 };
		DIAGRAM_DATA DiagData;
...
// 還付金融機関入力にフォーカスセット
//======================================
void CShinKani01::SetKanpuKinyuInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
