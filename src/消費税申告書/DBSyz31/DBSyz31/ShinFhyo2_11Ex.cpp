...
IMPLEMENT_DYNAMIC(CShinFhyo2_11Ex, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;


CShinFhyo2_11Ex::CShinFhyo2_11Ex(CWnd* pParent /*=nullptr*/)
	: CSyzBaseDlg(CShinFhyo2_11Ex::IDD, pParent)
	, m_pParent ( pParent )
	...
	, m_UwSgn ( TRUE )
{
	...
	//----> 20230120
	memset ( F211_27X, '\0', sizeof ( F211_27X ) );
	memset ( F211_27D, '\0', sizeof ( F211_27D ) );
	memset ( F211_27E, '\0', sizeof ( F211_27E ) );
	memset ( F211_27F, '\0', sizeof ( F211_27F ) );

	memset ( F211_28X, '\0', sizeof ( F211_28X ) );
	memset ( F211_28D, '\0', sizeof ( F211_28D ) );
	memset ( F211_28E, '\0', sizeof ( F211_28E ) );
	memset ( F211_28F, '\0', sizeof ( F211_28F ) );
	//<---- 20230120
	...
	F211_10c = F211_12c = F211_19c = F211_20c = F211_23c = F211_28c = 0;
...
CShinFhyo2_11Ex::~CShinFhyo2_11Ex()
{
}

void CShinFhyo2_11Ex::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo2_11Ex, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinFhyo2_11Ex::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CShinFhyo2_11Ex::OnBnClickedButton2)
END_MESSAGE_MAP()

BEGIN_EVENTSINK_MAP(CShinFhyo2_11Ex, CSyzBaseDlg)
	ON_EVENT(CShinFhyo2_11Ex, IDC_ICSDIAG8CTRL2, 1, CShinFhyo2_11Ex::EditONIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CShinFhyo2_11Ex, IDC_ICSDIAG8CTRL2, 2, CShinFhyo2_11Ex::EditOFFIcsdiag8ctrl2, VTS_I2)
	ON_EVENT(CShinFhyo2_11Ex, IDC_ICSDIAG8CTRL2, 3, CShinFhyo2_11Ex::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinFhyo2_11Ex, IDC_ICSDIAG8CTRL2, 9, CShinFhyo2_11Ex::CheckButtonIcsdiag8ctrl2, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()
// ShinFhyo2_11 メッセージ ハンドラー


BOOL CShinFhyo2_11Ex::OnInitDialog()
{
...
void CShinFhyo2_11Ex::OnSize(UINT nType, int cx, int cy)
{
...
void CShinFhyo2_11Ex::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinFhyo2_11Ex::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CShinFhyo2_11Ex::PreTranslateMessage(MSG* pMsg)
{
...
void CShinFhyo2_11Ex::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinFhyo2_11Ex::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	...
	case ID211_13D:
		...
		if ( m_pArith->l_cmp ( F211_13D, CkMy ) ) {
			if ( ( ( ( *m_pSnHeadData )->IsKobetuSiireAnbun() == TRUE ) 
				...
				Msg += _T( "個別対応方式欄の19～21の金額を見直してください。" );
...
void CShinFhyo2_11Ex::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if ( nChar == 0 ) {
		return;
...
//---->20230120
		case ID211_28D:
			F211_28c &= ~0x80;
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328402" ) );
			// 各入力の表示
			f211data_all ( 28 );
			m_Fhyo2_11Diag02.SetPosition ( ID211_28D );
			break;

		case ID211_28E:
			F211_28c &= ~0x800;
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328403" ) );
			// 各入力の表示
			f211data_all ( 28 );
			m_Fhyo2_11Diag02.SetPosition ( ID211_28E );
			break;
//<----20230120
	...
	if ( pWnd->GetDlgCtrlID() == IDC_BUTTON1 ) {
		nowpos = ID211_16F;	// 14Fのポジションをセット
	}
	else {
...
void CShinFhyo2_11Ex::CheckButtonIcsdiag8ctrl2(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
//			-1			：	エラー
//=========================================================
int CShinFhyo2_11Ex::InitInfo ( CH31SnFhyo2_01Data **pSnFhyo2_01Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CH31SnTanaData **pSnTanaData, CSyzSyukei **pSyzSyukei )
{
...
int CShinFhyo2_11Ex::ReadData()
{
	...
	//----> 20230120
	// 11.適格請求書発行事業者～…支払い対価の額(税込み)
	memmove ( F211_27X, ( *m_pSnFhyo2_01Data )->Sn_F201_27X, MONY_BUF_SIZE );
	memmove ( F211_27D, ( *m_pSnFhyo2_01Data )->Sn_F201_27D, MONY_BUF_SIZE );
	memmove ( F211_27E, ( *m_pSnFhyo2_01Data )->Sn_F201_27E, MONY_BUF_SIZE );
	memmove ( F211_27F, ( *m_pSnFhyo2_01Data )->Sn_F201_27F, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	memmove ( F211_28X, ( *m_pSnFhyo2_01Data )->Sn_F201_28X, MONY_BUF_SIZE );
	memmove ( F211_28D, ( *m_pSnFhyo2_01Data )->Sn_F201_28D, MONY_BUF_SIZE );
	memmove ( F211_28E, ( *m_pSnFhyo2_01Data )->Sn_F201_28E, MONY_BUF_SIZE );
	memmove ( F211_28F, ( *m_pSnFhyo2_01Data )->Sn_F201_28F, MONY_BUF_SIZE );
	F211_28c = ( *m_pSnFhyo2_01Data )->Sn_F201_28sw;
	//<---- 20230120
	...
	// 18.課税売上高が5億円以下、課税売上割合が95%以上の場合
	...
	// 23.課税売上割合変動時の調整対象固定資産に係る消費税額の調整額
	...
	// 25.居住用賃貸建物を課税賃貸用に供した(譲渡した)場合の加算額
...
// 読込みデータを集計モジュールへ
//=========================================================
void CShinFhyo2_11Ex::ReadData2()
{
	...
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
		...
		//---->20230120
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328302" ), F211_27D );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328303" ), F211_27E );

		if ( F211_28c & 0x80 ) {
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328402" ), F211_28D );
		}
		else {
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328402" ) );
		}
		if ( F211_28c & 0x800 ) {
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328403" ), F211_28E );
		}
		else {
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328403" ) );
		}
		...
		if ( ( *m_pSnHeadData )->SVmzsw == 1 ) {
			...
			//---->20230120
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328301" ) );
			( *m_pSyzSyukei )->ResetShinkokuData ( _T( "328401" ) );
			//<----20230120
			...
			//---->20230120
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328301" ), mony );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328401" ), mony );
			//<----20230120
...
int CShinFhyo2_11Ex::WriteData()
{
	...
	//----> 20230120
	// 11.適格請求書発行事業者～…支払い対価の額(税込み)
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_27X, F211_27X, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_27D, F211_27D, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_27E, F211_27E, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_27F, F211_27F, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_28X, F211_28X, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_28D, F211_28D, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_28E, F211_28E, MONY_BUF_SIZE );
	memmove ( ( *m_pSnFhyo2_01Data )->Sn_F201_28F, F211_28F, MONY_BUF_SIZE );
	( *m_pSnFhyo2_01Data )->Sn_F201_28sw = F211_28c;
	//<---- 20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_11Ex::MemoryReget()
{
	// 入力ロック時は集計しない
	...
	for ( int l = 0; l < cnt; l++ ) {
		...
		for ( int m = 0; m < incnt; m++ ) {
			...
			//----> 20230120
			else if ( money[l][m].code == _T( "328301" ) ) {
				memmove ( F211_27X, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328302" ) ) {
				memmove ( F211_27D, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328303" ) ) {
				memmove ( F211_27E, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328300" ) ) {
				memmove ( F211_27F, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328401" ) ) {
				memmove ( F211_28X, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328402" ) ) {
				memmove ( F211_28D, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328403" ) ) {
				memmove ( F211_28E, money[l][m].arith, MONY_BUF_SIZE );
			}
			else if ( money[l][m].code == _T( "328400" ) ) {
				memmove ( F211_28F, money[l][m].arith, MONY_BUF_SIZE );
			}
			//<---- 20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_11Ex::SyukeiDataSet ( int cID )
{
	char val[MONY_BUF_SIZE] = { 0 };
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//----> 20230120
	case ID211_27D:
		memmove ( F211_27D, val, MONY_BUF_SIZE );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328302" ), F211_27D );
		break;
	case ID211_27E:
		memmove ( F211_27E, val, MONY_BUF_SIZE );
		( *m_pSyzSyukei )->SetShinkokuData ( _T( "328303" ), F211_27E );
		break;
	case ID211_28D:
		if ( m_pArith->l_cmp ( F211_28D, val ) ) {
			F211_28c |= 0x80;
			memmove ( F211_28D, val, MONY_BUF_SIZE );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328402" ), F211_28D );
		}
		break;
	case ID211_28E:
		if ( m_pArith->l_cmp ( F211_28E, val ) ) {
			F211_28c |= 0x800;
			memmove ( F211_28E, val, MONY_BUF_SIZE );
			( *m_pSyzSyukei )->SetShinkokuData ( _T( "328403" ), F211_28E );
		}
		break;
//<---- 20230120
...
//			0			：	正常終了
//=========================================================
int CShinFhyo2_11Ex::InitDiagAttr()
{
	...
	while ( 1 ) {
		...
		if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
			...
			if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) {
				...
				int st = ChkTkkzIdx ( Fhyo2_11[cnt].Index );
				if ( st == 1 ) {
					if ( ( *m_pSnHeadData )->IsNeedTkkzDisp() ) {
						Flg = 1;
					}
...
//============================================================
int CShinFhyo2_11Ex::ChkTkkzIdx ( short idx )
{
	int rSt = 0;
...
// 画面出力
//=========================================================
void CShinFhyo2_11Ex::DispFh2_11()
{
	...
	for( int i = ID211_1D; i <= ID211_28F; i++ ){
		dsp_cls ( IDC_ICSDIAG8CTRL2, ( short )i );
	}
	...
	diag_setdata ( IDC_ICSDIAG8CTRL2, KAZEIKIKAN_S, ( struct IUnknown* )&DiagData, CIcsdiagctrl );

	yy = mm = dd = 0;
	( *m_pSnHeadData )->GetYmdDataGen ( ID_ICSSH_KAZEIKIKAN_TO, &yy, &mm, &dd );
	sprintf_s ( buf, sizeof ( buf ), _T( "%02x.%02x.%02x" ), ( yy & 0xff ), ( mm & 0xff ), ( dd & 0xff ) );
	DiagData.data_disp = ( char* )buf;
	diag_setdata ( IDC_ICSDIAG8CTRL2, KAZEIKIKAN_E, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	...
	DiagData.data_disp = ( char* )buf;
	diag_setdata ( IDC_ICSDIAG8CTRL2, SHIMEI, ( struct IUnknown* )&DiagData, CIcsdiagctrl );

	// 課税売上高の5億円超えチェック
	ChangeColor ( IDC_ICSDIAG8CTRL2, MILLION, 1 );
	ChangeColor ( IDC_ICSDIAG8CTRL2, MILLION2, 1 );
	diag_clear ( IDC_ICSDIAG8CTRL1, MILLION_CHK, TRUE, CIcsdiagctrl );
	CheckOver500Million();
...
//			Index		：	ダイアグラム内インデックス
//=========================================================
void CShinFhyo2_11Ex::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//			Index		：	ダイアグラム内インデックス
//=========================================================
void CShinFhyo2_11Ex::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
// 課税売上高5億円越えチェック
//=========================================================
void CShinFhyo2_11Ex::CheckOver500Million()
{
	DIAGRAM_ATTRIBUTE DA;
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	if ( *m_pShinInfo->pOver500MillionSw == 1 ) {	// ON:９５％ルール改正対応期間で５億円超え
		...
		if ( ( *m_pSnHeadData )->IsKobetuSiireAnbun() == FALSE ) {
			...
			//ChangeColor ( IDC_ICSDIAG8CTRL2, MILLION2, 5 );
			color = BC_GREEN_L;
			memset ( buf, '\0', sizeof ( buf ) );
			...
			DiagData.data_disp = ( char* )buf;
			diag_setdata ( IDC_ICSDIAG8CTRL1, MILLION_CHK, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
		}
	}
	...
	if ( ( *m_pSnHeadData )->IsKobetuSiireAnbun() == FALSE ) {
		...
		diag_getattr ( IDC_ICSDIAG8CTRL2, MILLION2, &DA, CIcsdiagctrl );
		DA.attr_bcolor = color;
		diag_setattr ( IDC_ICSDIAG8CTRL2, MILLION2, &DA, TRUE, CIcsdiagctrl );
	}
}
...
//						FF3000　にセットした値
//=========================================================
int CShinFhyo2_11Ex::CheckOver500MillionSub ( int &MilSw, char *pVAL, char *pValCnv )
{
	int		nflg = 0;
	char	flg[MONY_BUF_SIZE] ={ 0 };
	...
	CH31HyoView *pView = ( CH31HyoView* )m_pParent;

	CString tag, chtag;
	tag.Format( _T( "FFF00000" ));

	if( ( pView->IsIncludeMenzeiKikan() ) && ((( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) || (( *m_pSnHeadData )->Sn_Sign4 & 0x01 )) ) {
		// 非連動もしくは入力ロックかつ免税の期間を含む
		pView->GetMony( tag, chtag, tmpVAL );
	}
	else {
		// 入力ロック時は集計しない
		...
		if( pView->IsIncludeMenzeiKikan() ) {
			// 免税の期間が含まれている場合は足し合わせる
			long symd = 0, eymd = 0;
			char money[MONY_BUF_SIZE] = { 0 };

			pView->GetKazeiKikan( symd, eymd );
			pView->MenzeiKikanSyukei( symd, eymd, money );
			m_pArith->l_add( tmpVAL, tmpVAL, money );
		}
	}

	// データクラスにセットしておく
	(( CH31HyoView* )m_pParent )->SetMony( tag, chtag, tmpVAL );
...
//		atno		：		項目番号
//=========================================================
void CShinFhyo2_11Ex::f211data_all ( int atno )
{
	switch( atno ){
	case 0:
	case 1:
...
// 課税売上(税抜き)表示
//=========================================================
void CShinFhyo2_11Ex::f211data_1()
{
...
// 免税売上額
//=========================================================
void CShinFhyo2_11Ex::f211data_2()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_3()
{
...
// 課税資産の譲渡等の対価の額
//=========================================================
void CShinFhyo2_11Ex::f211data_4_5()
{
...
// 非課税売上額
//=========================================================
void CShinFhyo2_11Ex::f211data_6()
{
...
// 資産の譲渡等の対価の額
//=========================================================
void CShinFhyo2_11Ex::f211data_7()
{
...
// 課税売上割合
//=========================================================
void CShinFhyo2_11Ex::f211data_8()
{
		...
		//else{
			if ( ( *m_pSnHeadData )->Sn_UPERS & 0x10 ) {
				...
				if ( ( *m_pSnHeadData )->IsUseSemiUriRatio() ) {
					...
					diag_setdata ( IDC_ICSDIAG8CTRL2, KAZEIURIWARI, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
					ChangeColor ( IDC_ICSDIAG8CTRL2, ID211_8F, 1 );
				}
				else {
					memset ( buf, '\0', sizeof ( buf ) );
...
// 課税仕入れに係る支払対価の額(税込み)
//=========================================================
void CShinFhyo2_11Ex::f211data_9()
{
...
// 課税仕入れに係る消費税額
//=========================================================
void CShinFhyo2_11Ex::f211data_10()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
...
// 特定課税仕入れに係る支払対価の額
//=========================================================
void CShinFhyo2_11Ex::f211data_11()
{
...
// 特定課税仕入れに係る消費税額
//=========================================================
void CShinFhyo2_11Ex::f211data_12()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
		if ( ( *m_pSnHeadData )->IsNeedTkkzDisp() ) {
...
// 課税貨物に係る消費税額
//=========================================================
void CShinFhyo2_11Ex::f211data_13()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_14()
{
...
// 課税仕入れ等の税額の合計額
//=========================================================
void CShinFhyo2_11Ex::f211data_15()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_16()
{
...
// 17のうち、課税売上げにのみ要するもの
//=========================================================
void CShinFhyo2_11Ex::f211data_17()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_18()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_19()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_20()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_21()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_22()
{
...
// 控除対象仕入税額がプラスの時
//=========================================================
void CShinFhyo2_11Ex::f211data_23()
{
...
// 控除対象仕入税額がマイナスの時
//=========================================================
void CShinFhyo2_11Ex::f211data_24()
{
...
// 貸倒回収に係る消費税額
//=========================================================
void CShinFhyo2_11Ex::f211data_25()
{
...
//=========================================================
void CShinFhyo2_11Ex::f211data_26()
{
...
//=========================================================
// 適格請求書発行事業者～…支払い対価の額(税込み)  20230120
//=========================================================
void CShinFhyo2_11Ex::f211data_27()
{
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_27X, F211_27X );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_27D, F211_27D );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_27E, F211_27E );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_27F, F211_27F );
}

//=========================================================
// 適格請求書発行事業者～…消費税額とみなされる額  20230120
//=========================================================
void CShinFhyo2_11Ex::f211data_28()
{
	if ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
		ChangeColor ( IDC_ICSDIAG8CTRL2, ID211_28D, ( F211_28c & 0x80 ) ? 3 : 0 );
		ChangeColor ( IDC_ICSDIAG8CTRL2, ID211_28E, ( F211_28c & 0x800 ) ? 3 : 0 );
	}

	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_28X, F211_28X );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_28D, F211_28D );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_28E, F211_28E );
	dsp_prs ( IDC_ICSDIAG8CTRL2, ID211_28F, F211_28F );
}

//=========================================================
// 課税売上割合を変更
//=========================================================
void CShinFhyo2_11Ex::Changef211data_8()
{
	// 課税売上割合に準ずる割合を有効
	// 課税売上割合が９５％未満
	if ( ( *m_pSnHeadData )->IsUseSemiUriRatio() || ( F211_8EX < (*m_pSnHeadData)->m_UriRatioBorder ) ) {
...
// 棚卸データの加減算を行う
//=========================================================
void CShinFhyo2_11Ex::TanaDataAdd()
{
...
// 表示金額取得
//=========================================================
void CShinFhyo2_11Ex::GetAllDiagData()
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	//----> 20230120
	// 11.適格請求書発行事業者～…支払い対価の額(税込み)
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_27X, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_27X, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_27D, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_27D, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_27E, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_27E, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_27F, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_27F, DiagData.data_val, MONY_BUF_SIZE );

	// 12.適格請求書発行事業者～…消費税額とみなされる額
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_28X, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_28X, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_28D, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_28D, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_28E, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_28E, DiagData.data_val, MONY_BUF_SIZE );
	diag_getdata ( IDC_ICSDIAG8CTRL2, ID211_28F, ( struct IUnknown* )&DiagData, CIcsdiagctrl );
	memmove ( F211_28F, DiagData.data_val, MONY_BUF_SIZE );
	//<---- 20230120
	...
	// 16.納税義務の免除を受ける(受けない)こととなった場合における消費税額の調整額
...
//		計算した課税売上割合
//=========================================================
unsigned short CShinFhyo2_11Ex::GetCalqedKzwari()
{
...
// ボタン表示変更
//=========================================================
void CShinFhyo2_11Ex::ChangeBtnDisp()
{
...
// 初期ポジションセット
//===================================================
void CShinFhyo2_11Ex::SetItemPosition()
{
	int index;
...
//			Move		：	0 = 下方向, 1 = 上方向
//=========================================================
int CShinFhyo2_11Ex::UpDownFocusMoveCheck ( int Index, int Move )
{
	int idx = 0;
	int i = 0, j = 0;
...
//			sign		:		変更サイン
//=========================================================
void CShinFhyo2_11Ex::ModifyDiagInput ( short index, int sign )
{
...
//			sign		:		変更サイン
//=========================================================
void CShinFhyo2_11Ex::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 棚卸調整額入力
//=========================================================
void CShinFhyo2_11Ex::OnBnClickedButton1()
{
...
// 特定収入の簡易計算式表示
//=========================================================
void CShinFhyo2_11Ex::OnBnClickedButton2()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
...
//===================================================
void CShinFhyo2_11Ex::f211calq_spc ( SYZTOSPC_DATA SpcData )
{
	if ( *( m_pShinInfo->pSgSpc ) == 1 ) {
...
//	FALSE		:		なし
//=========================================================
BOOL CShinFhyo2_11Ex::IsManualInputSign()
{
	BOOL ret = FALSE;
	if ( !( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) || ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) ) {
		// 連動時、入力ロック中はチェックしない
		...
		if ( ( F211_10c & 0x880 ) || ( F211_12c & 0x800 ) || ( F211_19c & 0x880 ) ||
			 ( F211_20c & 0x880 ) || ( F211_23c & 0x880 ) || ( F211_28c & 0x880 ) ) {	// 20230120
			ret = TRUE;
		}
	}
