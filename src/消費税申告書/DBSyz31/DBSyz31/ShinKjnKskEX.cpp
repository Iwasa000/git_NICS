...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinKjnKskEX::InitInfo( CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CH31SnKskData **pSnKskData )
{
	ASSERT( pSnHeadData );
	if( pSnHeadData == NULL ){
		return -1;
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjn( int mode )
{
...
void CShinKjnKskEX::DispKsiKjnDig1()
{
	...
	for( i = DLG1TDT; i <= DLG1JDT; i++ ){
		...
		case DLG1KDT:	// 更正決定通知書日付
			//memmove( data.data_day, &(*m_pSnKskData)->KsData.KSktdt[0], 4 );	break;
		...
		case DLG1JDT:	// 更正決定通知書受理年月日
			memmove( data.data_day, &(*m_pSnKskData)->KsData.KSstdt[0], 4 );	break;
		}
		diag_setdata( IDC_ICSDIAGCTRL10, i, &data, CIcsdiagctrl );
	}
...
//		mode	：
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag1( short index, UINT flg, short nChar, int mode )
{
	short	xpos = index;
	int		sw = 0, cmb = 0;
...
//		mode	：	
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag2( short index, UINT flg, short nChar, int mode )
{
	short	xpos = index;
	int		sw = 0;
...
//		mode	：
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag3( short index, UINT flg, short nChar, int mode )
{
	short	xpos = index;
	int		sw = 0, cmb = 0;
