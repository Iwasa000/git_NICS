...
IMPLEMENT_DYNAMIC(CShinKani01EX, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinKani01EX::CShinKani01EX(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKani01EX::IDD, pParent)
	, m_pParent ( pParent )
...
CShinKani01EX::~CShinKani01EX()
{
}

void CShinKani01EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKani01EX, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinKani01EX::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinKani01EX メッセージ ハンドラ

BOOL CShinKani01EX::OnInitDialog()
{
...
void CShinKani01EX::OnSize(UINT nType, int cx, int cy)
{
...
void CShinKani01EX::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinKani01EX::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//		-1				:	エラー
//=============================================================
int CShinKani01EX::InitInfo ( CSnHeadData **pSnHeadData, CH31SnHonpyoData **pSnHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CSyzSyukei **pSyzSyukei )
{
	ASSERT ( pSnHeadData );
	if ( pSnHeadData == NULL ) {
		return -1;
...
//		0	:	正常終了
//=====================================
int CShinKani01EX::InitDiagAttr()
{
	...
	if ( ( *m_pSnHeadData )->IsSoleProprietor() == TRUE ) {	// 個人
		...
		diag_index[0][0] = KANI01EX_NUM_KOJIN1;
		diag_index[0][1] = KANI01EX_NUM_KOJIN2;
		diag_index[0][2] = KANI01EX_NUM_KOJIN3;
		diag_index[0][3] = -1;
		...
		diag_index[0][0] = KANI01EX_NUM_HOJIN1;		
		diag_index[0][1] = KANI01EX_NUM_HOJIN2;
		diag_index[0][2] = KANI01EX_NUM_HOJIN3;
		diag_index[0][3] = KANI01EX_NUM_HOJIN4;
		diag_index[0][4] = -1;
	...
	// 法人番号
	int idx_cnt = 0;
	for ( idx_cnt = KANI01EX_NUM_HOJIN1; idx_cnt <= KANI01EX_NUM_HOJINHYPHEN3; idx_cnt++ ) {
		m_Kani01Diag01.ModifyItem ( idx_cnt, nHojinDispModify );
	}

	// 個人番号
	for ( idx_cnt = KANI01EX_NUM_KOJIN1; idx_cnt <= KANI01EX_NUM_KOJINHYPHEN2; idx_cnt++ ) {
...
// ダイアグラム初期設定
//=====================================
void CShinKani01EX::ChgDiagAttr()
{
	int cnt = 0;
	while ( 1 ) {
	...
	while ( 1 ) {
		...
		if (Kani3[cnt].Index == ID4420){
			if( IsJunkakutei() ){
				m_Kani01Diag03.ModifyItem( Kani3[cnt].Index, DIAG_MDFY_READONLY );
			}
		}
		//<---- 2022/12/27
...
// 初期データ表示
//===========================================
void CShinKani01EX::DispInitKaniUpper()
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		Mony		:		表示金額
//=======================================================
void CShinKani01EX::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		Index		:		ダイアグラム内インデックス
//=======================================================
void CShinKani01EX::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		atno	:	0 = 全データ
//=================================
void CShinKani01EX::Disp_Kanidata ( int atno )
{
	switch ( atno ) {
	case 0:		// 全
	case 1:		// 課税標準額
...
// 差引納付税額
//====================================================
void CShinKani01EX::Disp_Kanidata14()
{
...
// 差引納付譲渡割額
//====================================================
void CShinKani01EX::Disp_Kanidata25()
{
	switch ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) {
	case 1:		// 確定・中間
	case 2:
...
//		sign	:		サイン
//=====================================================
void CShinKani01EX::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 終了時のフォーカス移動
//==========================================
void CShinKani01EX::MoveFocusInEnd()
{
	// データを確定させるためにフォーカス移動
...
// マイナンバーコントロール
//=========================================
void CShinKani01EX::MoveDiagMNEdit()
{
	if ( m_pParent && m_pParent->GetSafeHwnd() ) {
		if ( ( *m_pSnHeadData )->IsSoleProprietor() == TRUE ) {	// 個人
			m_ctrl_mynum.MoveDiagMNEdit ( KANI01EX_NUM_KOJIN1, FALSE );
		}
		else {
			m_ctrl_mynum.MoveDiagMNEdit ( KANI01EX_NUM_HOJIN1, FALSE );
		}
...
// 最終データ読込み
//=========================================
void CShinKani01EX::GetDiagData()
{
...
// 入力位置のセット
//=============================
void CShinKani01EX::SetItemPosition()
{
	int index = 0;
...
// 税務署入力にフォーカスをセット
//===================================
void CShinKani01EX::SetTaxOfficeInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
...
// 入力位置を保存
//===================================
void CShinKani01EX::SaveInputPosition()
{
	m_curInPos = -1;
...
// 入力位置を復元
//===================================
void CShinKani01EX::RestoreInputPosition()
{
	if ( m_curInPos == -1 ) {
		return;
	}
...
BEGIN_EVENTSINK_MAP(CShinKani01EX, CSyzBaseDlg)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL1, 1, CShinKani01EX::EditONIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL2, 1, CShinKani01EX::EditONIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL3, 1, CShinKani01EX::EditONIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL1, 2, CShinKani01EX::EditOFFIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL2, 2, CShinKani01EX::EditOFFIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL3, 2, CShinKani01EX::EditOFFIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL1, 3, CShinKani01EX::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL2, 3, CShinKani01EX::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL3, 3, CShinKani01EX::TerminationIcsdiag8ctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL1, 9, CShinKani01EX::CheckButtonIcsdiag8ctrl1, VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinKani01EX, IDC_ICSDIAG8CTRL3, 9, CShinKani01EX::CheckButtonIcsdiag8ctrl3, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()


void CShinKani01EX::EditONIcsdiag8ctrl1(short index)
{
...
void CShinKani01EX::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01EX::EditONIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01EX::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	char yymmdd[3] = { 0 };
	char buf[512] = { 0 };
...
void CShinKani01EX::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinKani01EX::EditOFFIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinKani01EX::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	char buff[64] = { 0 };
	int len = 0;
...
void CShinKani01EX::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinKani01EX::TerminationIcsdiag8ctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
BOOL CShinKani01EX::PreTranslateMessage(MSG* pMsg)
{
...
void CShinKani01EX::OnBnClickedButton1()
{
...
// 集計セット
//=================================
int CShinKani01EX::SyukeiDataSet ( int cID )
{
	DIAGRAM_DATA DiagData;
...
// 付表データ表示
//============================================
void CShinKani01EX::Disp_FhyoData()
{
	char CkMy[MONY_BUF_SIZE] = { 0 };
...
//		pers	:	金額
//=================================================
void CShinKani01EX::dsp_uws ( unsigned short Id, short Index, unsigned short pers )
{
	if ( pers ) {
		char buf[128] = { 0 };
		DIAGRAM_DATA DiagData;
...
// 還付金融機関入力にフォーカスセット
//======================================
void CShinKani01EX::SetKanpuKinyuInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
...
//===================================================
void CShinKani01EX::EnableKanpuKinyu( int sw )
{
	int id = 0;
...
//===================================================
void CShinKani01EX::CheckButtonIcsdiag8ctrl1(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	switch( index ){
		case ID2062:	// （個人の方）振替継続希望
...
// diagのチェックボックスに変更があった場合の処理
//===================================================
void CShinKani01EX::CheckButtonIcsdiag8ctrl3(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	switch( index ){
		case ID4420:	// （個人の方）公金受取口座の利用
...
//===================================================
BOOL CShinKani01EX::IsJunkakutei()
{
