...
#include "stdafx.h"
#include "ShinIppan01EX2.h"
#include "ShinIppan01IdxEX2.h"
#include "H31HyoView.h"
#include "PrePrint.h"
#include "ChkMsg.h"


// CShinIppan01EX2 ダイアログ

IMPLEMENT_DYNAMIC(CShinIppan01EX2, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinIppan01EX2::CShinIppan01EX2(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinIppan01EX2::IDD, pParent)
	, m_pSnHeadData(NULL)
...
CShinIppan01EX2::~CShinIppan01EX2()
{
}

void CShinIppan01EX2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinIppan01EX2, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinIppan01EX2::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinIppan01EX2 メッセージ ハンドラ

BOOL CShinIppan01EX2::OnInitDialog()
{
...
void CShinIppan01EX2::OnSize(UINT nType, int cx, int cy)
{
...
void CShinIppan01EX2::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinIppan01EX2::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinIppan01EX2::InitInfo(CSnHeadData **pSnHeadData, CH31SnHonpyoData **pHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CSyzSyukei **pSyzSyukei)
{
	ASSERT(pSnHeadData);
	if (pSnHeadData == NULL) {
		return -1;
...
//		-1		:	エラー
//==========================================
int CShinIppan01EX2::InitDiagAttr()
{
	...
	if ( ( *m_pSnHeadData )->IsSoleProprietor() == TRUE ) {		// 個人
		...
		diag_index[0][0] = IPPAN01EX2_NUM_KOJIN1;
		diag_index[0][1] = IPPAN01EX2_NUM_KOJIN2;
		diag_index[0][2] = IPPAN01EX2_NUM_KOJIN3;
		diag_index[0][3] = -1;
		...
		diag_index[0][0] = IPPAN01EX2_NUM_HOJIN1;
		diag_index[0][1] = IPPAN01EX2_NUM_HOJIN2;
		diag_index[0][2] = IPPAN01EX2_NUM_HOJIN3;
		diag_index[0][3] = IPPAN01EX2_NUM_HOJIN4;
		diag_index[0][4] = -1;
	...
	// 法人番号
	int idx_cnt = 0;
	for ( idx_cnt = IPPAN01EX2_NUM_HOJIN1; idx_cnt <= IPPAN01EX2_NUM_HOJINHYPHEN3; idx_cnt++ ) {
		m_Ippan01Diag01.ModifyItem ( idx_cnt, nHojinDispModify );
	}

	// 個人番号
	for ( idx_cnt = IPPAN01EX2_NUM_KOJIN1; idx_cnt <= IPPAN01EX2_NUM_KOJINHYPHEN2; idx_cnt++ ) {
...
// 集計モジュールにセット
//====================================
void CShinIppan01EX2::ReadData2()
{
...
// 金額欄表示
//=================================
void CShinIppan01EX2::DispIppan()
{
	DIAGRAM_ATTRIBUTE DA;
	int Index = 0;
...
//----->('23.05.01 Add)		(メモ)ID番号は「ShinIppan01IdxEX2.h」を参照したい。リソース完成待ち。
	// 税額控除に係る経過措置の適用（２割特例）
	DiagData.data_check = (short)((*m_pSnHeadData)->m_DispTabSgn & 0x2000);
	m_Ippan01Diag03.SetData(ID2WARI, (struct IUnknown*)&DiagData);
//<-----
...
// 本表上部表示
//============================================
void CShinIppan01EX2::DispIppanUpper()
{
	DIAGRAM_ATTRIBUTE DA;
	int Index = 0;
...
//		Index	:		ダイアグラム内インデックス
//=====================================================
void CShinIppan01EX2::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		Mony	:		金額
//=====================================================
void CShinIppan01EX2::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	if ( Id == IDC_ICSDIAG8CTRL2 ) {
		...
		case ID3XM25:
		case ID3XM26:
			if(( Index == ID3XM15 ) || ( Index == ID3XM16 )) {
				if((( CH31HyoView* )m_pParent )->IsNiwariTokurei() ) {
					// 2割特例の場合は課税資産の譲渡等の対価の額は記載不要のためセットしない
					break;
				}
			}
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinIppan01EX2::ChgDiagAttr()
{
	int	cnt;
	...
	while ( 1 ) {
		...
		if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
			...
			// 2割特例の場合は課税売上高は記載不要のため、入力も閉じる
			if( Ippan2[cnt].Index == ID3XM15 ){
...
// 終了時のフォーカス移動
//=============================
void CShinIppan01EX2::MoveFocusInEnd()
{
	// データを確定させるためにフォーカス移動
...
// 金額の最終データ読込み
//=====================================
void CShinIppan01EX2::GetDiagData()
{
...
//		atno	:	0 = 全データ
//=================================
void CShinIppan01EX2::Disp_Ippandata ( int atno )
{
	switch ( atno ) {
		case 0:		// 全
		case 1:		// 課税標準額
...
// 差引納付税額
//===============================
void CShinIppan01EX2::Disp_Ippandata14()
{
...
// 差引納付譲渡割額
//===============================
void CShinIppan01EX2::Disp_Ippandata25()
{
	switch ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) {
	case 1:		// 確定・中間
	case 2:
...
//		sign	:		変更サイン
//===================================================
void CShinIppan01EX2::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE DA;
...
//=====================================================
void CShinIppan01EX2::MoveDiagMNEdit()
{
	if ( m_pParent && m_pParent->GetSafeHwnd() ) {
		if ( ( *m_pSnHeadData )->IsSoleProprietor() == TRUE ) {	// 個人
			m_ctrl_mynum.MoveDiagMNEdit ( IPPAN01EX2_NUM_KOJIN1, FALSE );
		}
		else {
			m_ctrl_mynum.MoveDiagMNEdit ( IPPAN01EX2_NUM_HOJIN1, FALSE );
		}
		if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
...
// マイナンバーコントロール非表示
//=====================================
void CShinIppan01EX2::HideMNEdit()
{
...
// 入力位置を保存
//========================================
void CShinIppan01EX2::SaveInputPosition()
{
	m_curInPos = -1;
	int index = 0;
...
// 入力位置を復元
//========================================
void CShinIppan01EX2::RestoreInputPosition()
{
	if ( m_curInPos == -1 ) {
		return;
	}
...
BEGIN_EVENTSINK_MAP(CShinIppan01EX2, CSyzBaseDlg)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL1, 1, CShinIppan01EX2::EditONIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL2, 1, CShinIppan01EX2::EditONIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL3, 1, CShinIppan01EX2::EditONIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL1, 2, CShinIppan01EX2::EditOFFIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL2, 2, CShinIppan01EX2::EditOFFIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL3, 2, CShinIppan01EX2::EditOFFIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL1, 3, CShinIppan01EX2::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL2, 3, CShinIppan01EX2::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL3, 3, CShinIppan01EX2::TerminationIcsdiag8ctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
//----->('23.05.01 Add)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL1, 9, CShinIppan01EX2::CheckButtonIcsdiag8ctrl1, VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinIppan01EX2, IDC_ICSDIAG8CTRL3, 9, CShinIppan01EX2::CheckButtonIcsdiag8ctrl3, VTS_I2 VTS_UNKNOWN)
//<-----
END_EVENTSINK_MAP()


void CShinIppan01EX2::EditONIcsdiag8ctrl1(short index)
{
...
void CShinIppan01EX2::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinIppan01EX2::EditONIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinIppan01EX2::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinIppan01EX2::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinIppan01EX2::EditOFFIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	int Length = 0;
...
void CShinIppan01EX2::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
void CShinIppan01EX2::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
void CShinIppan01EX2::TerminationIcsdiag8ctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
BOOL CShinIppan01EX2::PreTranslateMessage(MSG* pMsg)
{
...
void CShinIppan01EX2::OnBnClickedButton1()
{
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinIppan01EX2::SetItemPosition()
{
	int index;
...
// 集計セット
//=================================
int CShinIppan01EX2::SyukeiDataSet ( int cID )
{
	DIAGRAM_DATA DiagData;
...
// 集計データ取得
//=================================
int CShinIppan01EX2::MemoryReget()
{
	// 入力ロック時は集計しない
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return 0;
	...
	if(((( CH31HyoView* )m_pParent )->IsIncludeMenzeiKikan() ) && !(( *m_pSnHeadData )->Sn_Sign4 & 0x80 )) {
		...
		(( CH31HyoView* )m_pParent )->MenzeiKikanSyukei( symd, eymd, val );

		// 課税期間の集計額を加える
		m_pArith->l_add( val, val, XM15 );

		// データクラスにセット
		tag.Format( _T( "FFF00000" ));
		(( CH31HyoView* )m_pParent )->SetMony( tag, chtag, val );
	}
...
// 税務署入力にフォーカスセット
//================================
void CShinIppan01EX2::SetTaxOfficeInput()
{
	// 確定時は全入力不可
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
...
// 還付金融機関入力にフォーカスセット
//======================================
void CShinIppan01EX2::SetKanpuKinyuInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
...
void CShinIppan01EX2::Disp_FhyoData()
{
	if( !((*m_pSnHeadData)->m_DispTabSgn & 0x2000) ){
		DIAGRAM_DATA DiagData;
		DiagData.data_imgdata = NULL;
...
//===================================================
void CShinIppan01EX2::EnableKanpuKinyu( int sw )
{
	int id = 0;
...
//===================================================
void CShinIppan01EX2::CheckButtonIcsdiag8ctrl1(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	switch( index ){
		case ID2070:	// （個人の方）振替継続希望
...
// diagのチェックボックスに変更があった場合の処理
//===================================================
void CShinIppan01EX2::CheckButtonIcsdiag8ctrl3(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	switch( index ){
		case ID4420:	// （個人の方）公金受取口座の利用
...
//===================================================
BOOL CShinIppan01EX2::IsJunkakutei()
{
