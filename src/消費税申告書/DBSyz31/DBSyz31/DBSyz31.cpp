// DBSyz31.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには DBSyz31.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE DBSyz31DLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DBSyz31.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(DBSyz31DLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(DBSyz31DLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DBSyz31.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(DBSyz31DLL);
	}
	return 1;   // OK
}
...
extern "C" __declspec( dllexport ) HINSTANCE WINAPI GetH31HyoInstance()
{
	return 	g_hInstance;
}
