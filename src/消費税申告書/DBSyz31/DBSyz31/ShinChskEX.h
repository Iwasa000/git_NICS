...
public:
	CShinChskEX(CWnd* pParent = nullptr);   // 標準コンストラクター
	virtual ~CShinChskEX();
...
// ダイアログ データ
	...
	// 初期情報のセット
	int InitInfo(CSnHeadData *pSnHeadData, CH31SnChskData *pSnChskData, CH31SnHonpyoData *pSnHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo);
	// 初期化
	int Init();
	// ダイアグラム初期属性セット
	int InitDiagAttr();
