...
#define SetCrossOver(d1,d2,l1,l2) GetDiagControl(d1.GetDlgCtrlID())->SetCrossOverNext(GetDiagControl(d2.GetDlgCtrlID()),l1,ARRAYCOUNT(l1)); \
								  GetDiagControl(d2.GetDlgCtrlID())->SetCrossOverPrev(GetDiagControl(d1.GetDlgCtrlID()),l2,ARRAYCOUNT(l2))
...
protected:
	...
	CArith arith;
//2016.02.23 UDATE START
//	CSyzDataSet* syzdata;
public:
	CSyzDataSet* syzdata;
protected:
//2016.02.23 UDATE END
	CIcsdiagctrl& diag;
...
//2016.02.23 INSERT START
	virtual int GetItemLine(int index){return 0;};
	virtual BOOL Del_Message(int index){return FALSE;};

	virtual void Line_Delete(int line_idx){return;};
	virtual void Line_Insert(int line_idx){return;};
	virtual BOOL IsLineHead(int index){return FALSE;};
//2016.02.23 INSERT END

	//---->20230704
	virtual BOOL IsTorihikiInvnoClm( int index ){ return FALSE; };
	virtual bool CheckInvno( int sgn, ZmselDBData *zd = NULL ){ return true; };
	//<----20230704
...
//2016.02.23 INSERT END
	...
	{
		if(nChar == VK_DELETE){
			if( IsEditable(index) || IsCheckBox(index) ){
				Delete(index);
			}
			return 0;
		}
...
//2016.02.23 INSERT END
		...
		if( move.*ptr == MOVE_PREV && prev_diag_control ){
			if( ( !(pSnHeadData->Sn_Sign4&0x80) ) && ( diag.GetDlgCtrlID() == IDC_ICSDIAGCTRL_MEISAI_H3) && !(pSnHeadData->Sn_KOJIN & 0x01)){
				prev_diag_control->SetPosition(PrevCrossOver[next],ptr);
				prev_diag_control->prev_diag_control->SetPos();
			}
			else{
				prev_diag_control->SetPosition(PrevCrossOver[next],ptr);
			}
		...
		if( move.*ptr == MOVE_NEXT && next_diag_control ){
			if( ( !(pSnHeadData->Sn_Sign4&0x80) ) && ( diag.GetDlgCtrlID() == IDC_ICSDIAGCTRL_TOKUSYU) && !(pSnHeadData->Sn_KOJIN & 0x01)){
				next_diag_control->SetPosition(NextCrossOver[next],ptr);
				next_diag_control->next_diag_control->SetInitPos();
			}
			else{
				next_diag_control->SetPosition(NextCrossOver[next],ptr);
			}
	...
	int IsCheckBox(int index)
	{
		// ‘®«
		CDiagAttribute DA;
		diag.GetAttribute( index, DA );
		return DA.attr_inpkind == 2;
	}

	virtual void SwapTwoRows(int row_idx1, int row_idx2) {return;}	// ‚Q‚Â‚Ìs‚Ì“ü‚ê‘Ö‚¦	(2020.02.07)
	virtual BOOL IsBottomRow(int index) {return FALSE;}			// ÅIs‚©‚ð”»•Ê		(2020.02.07)
};
