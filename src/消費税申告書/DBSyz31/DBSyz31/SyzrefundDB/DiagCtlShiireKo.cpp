...
//Åö//[12'09.19]///
	...
	if( pSnHeadData->Sn_Sign4&0x01){
		...
		// START 161130 2022.12.20 (MDCR_HHA)
		//if( refundvol.KPSstx ){
		if (refundvol.KPSstx == 1 && pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_GENSOKU) {
			// END 161130 2022.12.20 (MDCR_HHA)
			flg = 1;
	...
	// START 161130 2022.12.20 (MDCR_HHA)
	if (pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_KANNI) {
		if (!refundvol.KPSstx && !arith.l_test(syzdata->GetRec(12).KPSvl4)) {
			memmove(syzdata->GetRec(12).KPSvl4, syzdata->GetRec(12).KPSvl3, 6);
		}
...
void CDiagCtlShiireKo::Delete(int index)
{
	// START 161130 2022.12.20 (MDCR_HHA)
	//if( index == ITM_SKOJO ){
	if (index == ITM_SKOJO && pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_GENSOKU) {
		// END 161130 2022.12.20 (MDCR_HHA)
...
//---------------
// START 220929_1301 2020.10.28 (MDCR_HHA)
//	if( pUtil->GetKazeihoushiki(pZmSub->zvol) != ID_ICSSH_GENSOKU ){
//		CalqZei(syzdata->GetRec(12).KPSvl3,MnyBufMap[ITM_KGOKEI],syzdata->Getvol().KPSsgn&0x01);
//	}
// END 220929_1301 2020.10.28 (MDCR_HHA)
//---------------
