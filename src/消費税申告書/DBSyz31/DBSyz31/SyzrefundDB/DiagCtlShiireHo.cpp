...
//	}
	...
	if (pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_GENSOKU) {
		...
		SetAttribMod(ITM_KGOKEI, EDCOLOR, DIAG_MDFY_EDIT);
	}

	if (pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_KANNI) {
		if (!refundvol.KPSstx && !arith.l_test(syzdata->GetRec(9).KPSvl4)) {
			memmove(syzdata->GetRec(9).KPSvl4, syzdata->GetRec(9).KPSvl3, 6);
		}
...
void CDiagCtlShiireHo::Delete(int index)
{
	// START 161130 2022.12.20 (MDCR_HHA)
	//if( index == ITM_KGOKEI ){
	if (index == ITM_KGOKEI && pUtil->GetKazeihoushiki(pZmSub->zvol) == ID_ICSSH_GENSOKU) {
		// END 161130 2022.12.20 (MDCR_HHA)
