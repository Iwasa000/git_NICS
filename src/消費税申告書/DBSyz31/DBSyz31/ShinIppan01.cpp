...
IMPLEMENT_DYNAMIC(CShinIppan01, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinIppan01::CShinIppan01(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinIppan01::IDD, pParent)
	, m_pSnHeadData(NULL)
...
CShinIppan01::~CShinIppan01()
{
}

void CShinIppan01::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinIppan01, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinIppan01::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinIppan01 メッセージ ハンドラ

BOOL CShinIppan01::OnInitDialog()
{
...
void CShinIppan01::OnSize(UINT nType, int cx, int cy)
{
...
void CShinIppan01::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HBRUSH CShinIppan01::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinIppan01::InitInfo(CSnHeadData **pSnHeadData, CH31SnHonpyoData **pHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CSyzSyukei **pSyzSyukei)
{
	ASSERT(pSnHeadData);
	if (pSnHeadData == NULL) {
		return -1;
...
int CShinIppan01::InitDiagAttr()
{
	...
	// 個人番号・法人番号
	if ( ( ( CH31HyoView* )m_pParent )->GetShRevType() >= ID_ICSSH_TKKZSIIRE_REVTYPE ) {
		// マイナンバー対応
		int kubun = 0;
		HWND edit[10] = { NULL };
...
void CShinIppan01::DispIppan()
{
	...
	if ( ( ( *m_pSnHeadData )->Sn_ITIREN[0] & 0xff ) != 0xff ) {
		...
		DiagData.data_edit = buf;
		m_Ippan01Diag01.SetData ( ID2080, ( struct IUnknown* )&DiagData );
	}
	...
	m_Ippan01Diag01.SetData ( ID2081, ( struct IUnknown* )&DiagData );
	...
	if ( ( yy & 0xff ) &&  ( mm & 0xff ) && ( dd & 0xff ) ) {
		...
		m_Ippan01Diag01.SetData ( ID2103, ( struct IUnknown* )&DiagData );
	}
	...
	if ( !( ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) % 2 ) ) {
		...
		if ( ( yy & 0xff ) && ( mm & 0xff ) && ( dd & 0xff ) ) {
			...
			m_Ippan01Diag01.SetData ( ID2133, ( struct IUnknown* )&DiagData );
		}
...
// 本表上部表示
//============================================
void CShinIppan01::DispIppanUpper()
{
	DIAGRAM_ATTRIBUTE DA;
	int Index = 0;
...
//		Index	:		ダイアグラム内インデックス
//=====================================================
void CShinIppan01::dsp_cls ( unsigned short Id, short Index )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
//		Mony	:		金額
//=====================================================
void CShinIppan01::dsp_prs ( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
	...
	if ( Id == IDC_ICSDIAG8CTRL2 ) {
		...
		case ID3XM2:
			...
			m_Ippan01Diag02.SetData ( Index, ( struct IUnknown* )&DiagData );
			// 自動計算 / 手入力の場合
			if ( ( ( *m_pSnHeadData )->SVmzsw != 1 ) && ( ( *m_pSnHeadData )->Sn_Sign4 & 0x80 ) && ( ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) == 0x00 ) ) {
				if ( !( m_M1c & 0x08 ) ) {
					ChangeColor ( IDC_ICSDIAG8CTRL2, ID3XM2, 0 );
				}
				else {
					ChangeColor ( IDC_ICSDIAG8CTRL2, ID3XM2, 3 );
				}
			}
			break;
			*/
...
// 220412 del -->
		...
		//if (Ippan1[cnt].Index == ID2081) {
		//	m_Ippan01Diag01.ModifyItem(Ippan1[cnt].Index, (Flg ? DIAG_MDFY_OPAQUE : DIAG_MDFY_TRANSPARENT));	// チェックボックス
		//	m_Ippan01Diag01.ModifyItem(23, (Flg ? DIAG_MDFY_OPAQUE : DIAG_MDFY_TRANSPARENT));	// 「翌年以降送付不要」
		//}
// 220412 del <--
// 220412 add -->
		// 個人・法人に関係なく「翌年以降送付不要」を非表示
		if (Ippan1[cnt].Index == ID2081) {
			m_Ippan01Diag01.ModifyItem(Ippan1[cnt].Index,DIAG_MDFY_TRANSPARENT);	// チェックボックス
			m_Ippan01Diag01.ModifyItem(23,DIAG_MDFY_TRANSPARENT);					// 「翌年以降送付不要」
		}
// 220412 add <--
		cnt++;
	}
...
// 終了時のフォーカス移動
//=============================
void CShinIppan01::MoveFocusInEnd()
{
	// データを確定させるためにフォーカス移動
	if ( m_curwnd == IDC_ICSDIAG8CTRL1 ) {
		...
		//if ( m_Ippan01Diag03.GetPosition() == ID4312 ) {
...
//		atno	:	0 = 全データ
//=================================
void CShinIppan01::Disp_Ippandata ( int atno )
{
	switch ( atno ) {
		case 0:		// 全
		case 1:		// 課税標準額
...
// 差引納付税額
//===============================
void CShinIppan01::Disp_Ippandata14()
{
...
// 差引納付譲渡割額
//===============================
void CShinIppan01::Disp_Ippandata25()
{
	switch ( ( *m_pSnHeadData )->Sn_SKKBN & 0xff ) {
	case 1:		// 確定・中間
	case 2:
...
//		sign	:		変更サイン
//===================================================
void CShinIppan01::ChangeColor ( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE DA;
...
//=====================================================
void CShinIppan01::MoveDiagMNEdit()
{
...
// 入力位置を保存
//========================================
void CShinIppan01::SaveInputPosition()
{
	m_curInPos = -1;
	int index = 0;
...
// 入力位置を復元
//========================================
void CShinIppan01::RestoreInputPosition()
{
	if ( m_curInPos == -1 ) {
		return;
	}
...
BEGIN_EVENTSINK_MAP(CShinIppan01, CSyzBaseDlg)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL1, 1, CShinIppan01::EditONIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL2, 1, CShinIppan01::EditONIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL3, 1, CShinIppan01::EditONIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL1, 2, CShinIppan01::EditOFFIcsdiag8ctrl1, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL2, 2, CShinIppan01::EditOFFIcsdiag8ctrl2, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL3, 2, CShinIppan01::EditOFFIcsdiag8ctrl3, VTS_I2)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL1, 3, CShinIppan01::TerminationIcsdiag8ctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL2, 3, CShinIppan01::TerminationIcsdiag8ctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinIppan01, IDC_ICSDIAG8CTRL3, 3, CShinIppan01::TerminationIcsdiag8ctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()


void CShinIppan01::EditONIcsdiag8ctrl1(short index)
{
...
void CShinIppan01::EditONIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinIppan01::EditONIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
...
void CShinIppan01::EditOFFIcsdiag8ctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	...
	switch ( index ) {
		...
		case ID2001:	// 提出月
			if ( DiagData.data_day[0] != 0x00 ) {
				...
				//		m_Ippan01Diag01.SetData ( ID2000, ( struct IUnknown* )&DiagDataYY );
				//	}
				//}
...
void CShinIppan01::EditOFFIcsdiag8ctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinIppan01::EditOFFIcsdiag8ctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	DIAGRAM_DATA DiagData;
	int Length = 0;
...
void CShinIppan01::TerminationIcsdiag8ctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
void CShinIppan01::TerminationIcsdiag8ctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
void CShinIppan01::TerminationIcsdiag8ctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラー コードを追加します。
	if ( nChar == 0 ) {
		return;
...
// 改良依頼 18-0439 修正 del -->
			...
			//	// 前項目
			//	if ( ( wTerm == VK_TAB ) || ( wTerm == VK_LEFT ) || ( wTerm == VK_F2 ) || ( wTerm == VK_UP ) ) {
			//		m_Ippan01Diag03.SetPrevPosition();
			//	}
			//	break;
// 改良依頼 18-0439 修正 del <--
// 改良依頼 18-0439 修正 add -->
...
BOOL CShinIppan01::PreTranslateMessage(MSG* pMsg)
{
...
void CShinIppan01::OnBnClickedButton1()
{
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinIppan01::SetItemPosition()
{
	int index;
...
// 集計セット
//=================================
int CShinIppan01::SyukeiDataSet ( int cID )
{
	DIAGRAM_DATA DiagData;
...
// 税務署入力にフォーカスセット
//================================
void CShinIppan01::SetTaxOfficeInput()
{
	// 確定時は全入力不可
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
...
// 還付金融機関入力にフォーカスセット
//======================================
void CShinIppan01::SetKanpuKinyuInput()
{
	if ( ( *m_pSnHeadData )->Sn_Sign4 & 0x01 ) {
		return;
	}
...
void CShinIppan01::Disp_FhyoData()
{
	DIAGRAM_DATA DiagData;
	DiagData.data_imgdata = NULL;
