...
// コンストラクタ
//-----------------------------------------------------------------------------
CH31SnKskData::CH31SnKskData(void)
{
...
// 返送値
//-----------------------------------------------------------------------------
int CH31SnKskData::GetData( CDBNpSub *pDBNpSub, CSnHeadData *pSnHeadData, int sn_seq )
{
	ASSERT( pDBNpSub );
	ASSERT( pSnHeadData );
...
//		errmes = _T("更正の請求マスターのオープンが出来ませんでした。");
	...
	// ReaskOpen時にテーブル拡張しているためサインを立てる
	int st = TableCheck( pDBNpSub );
	if( st == ERR )	return ERR;
	if( st != 0 )	pSnHeadData->Sn_GenericSgn |= 0x100;
...
//	shin_datecnv( Vol1.EE_YMD[0], (unsigned char *)&YEAR, 0 );
	...
	if( pDBSyzAcc->m_pReask->GetRecordCount() )	{
		// データ読み込み
//		KsData.KSdata[0] = 0x04;
//		m_Util.date_to_bcd( 0x04, &KsData.KSdata[1], pDBSyzAcc->m_pReask->KSdata );							// 提出年月日
//		m_Util.ADtoJC( &(char)KsData.KSdata[0] , &(char)KsData.KSdata[1], &(char)KsData.KSdata[2], &(char)KsData.KSdata[3], pDBSyzAcc->m_pReask->KSdata );//年号取得
//		KsData.KSktdt[0] = 0x04;
//		m_Util.date_to_bcd( 0x04, &KsData.KSktdt[1], pDBSyzAcc->m_pReask->KSktdt );							// 更正決定通知書日付
//		m_Util.ADtoJC( &(char)KsData.KSktdt[0] , &(char)KsData.KSktdt[1], &(char)KsData.KSktdt[2], &(char)KsData.KSktdt[3], pDBSyzAcc->m_pReask->KSktdt );//年号取得
//		KsData.KSktsw = pDBSyzAcc->m_pReask->KSktsw;														// 0=申告 1=更正 2=決定
//		KsData.KSstdt[0] = 0x04;
//		m_Util.date_to_bcd( 0x04, &KsData.KSstdt[1], pDBSyzAcc->m_pReask->KSstdt );							// 修正申告提出年月日(又は更正決定通知書受理年月日)	(平成固定)
//		m_Util.ADtoJC( &(char)KsData.KSstdt[0] , &(char)KsData.KSstdt[1], &(char)KsData.KSstdt[2], &(char)KsData.KSstdt[3], pDBSyzAcc->m_pReask->KSstdt );//年号取得		KsData.KSjrdt[0] = 0x04;
//		m_Util.date_to_bcd( 0x04, &KsData.KSjrdt[1], pDBSyzAcc->m_pReask->KSjrdt );							// 更正決定通知書受理年月日	(平成固定)
//		m_Util.ADtoJC( &(char)KsData.KSjrdt[0] , &(char)KsData.KSjrdt[1], &(char)KsData.KSjrdt[2], &(char)KsData.KSjrdt[3], pDBSyzAcc->m_pReask->KSjrdt );//年号取得		KsData.KSjrdt[0] = 0x04;
		...
		m_Util.val_to_bin( (unsigned char *)KsData.KSval[20].KVkval, pDBSyzAcc->m_pReask->Kikakutei1 );		// 13 この請求前の既確定税額　更正の請求/新様式対応
		m_Util.val_to_bin( (unsigned char *)KsData.KSval[21].KVkval, pDBSyzAcc->m_pReask->Kikakutei2 );		// 21 この請求前の既確定譲渡割額　更正の請求/新様式対応
...
// 返送値
//-----------------------------------------------------------------------------
int CH31SnKskData::SetData( CDBNpSub *pDBNpSub, int sn_seq )
{
	ASSERT( pDBNpSub );
...
//		errmes = _T("更正の請求マスターのオープンが出来ませんでした。");
	...
	m_Util.val_to_asci( &pDBSyzAcc->m_pReask->Kikakutei1, (unsigned char *)KsData.KSval[20].KVkval );	// 13 この請求前の既確定税額　更正の請求/新様式対応
	m_Util.val_to_asci( &pDBSyzAcc->m_pReask->Kikakutei2, (unsigned char *)KsData.KSval[21].KVkval );	// 21 この請求前の既確定譲渡割額　更正の請求/新様式対応
...
// 返送値	
//-----------------------------------------------------------------------------
int CH31SnKskData::CalqBillVal( CSnHeadData *pSnHeadData )
{
	ASSERT( pSnHeadData );
	if( pSnHeadData == NULL ){
		return -1;
	...
	// 複数税率（10％、8％軽減）時は計算できないため、実額項目とする
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CH31SnKskData::SetCalqedData( int mode, SH_ANS *pAns )
{
	int		lin;
	KSIVAL	*pK = NULL;
...
//-----------------------------------------------------------------------------
// 集計済みデータのセット 更正の請求/新様式対応
//-----------------------------------------------------------------------------
// 引数	mode	：	1:現申告分の金額をセット / 0:前回申告分の金額をセット
//		pAns	：	セット対象データ
//-----------------------------------------------------------------------------
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CH31SnKskData::SetCalqedData2( int mode, SH_ANS *pAns )
{
	int		lin;
	KSIVAL	*pK1 = NULL, *pK2 = NULL;
	...
	if( mode ){
		for( int i=0; i<19; i++ ){
			...
			pK1 = &KsData.KSval[i];
			pK1->KVvflg &= 0xfd;
			memmove( pK1->KVkval, pAns[lin].val, MONY_BUF_SIZE );
		}
	}
	else{
		pK1 = &KsData.KSval[20];	// 13この請求前の既確定税額
		pK2 = &KsData.KSval[21];	// 21この請求前の既確譲渡割額
		pK1->KVvflg &= 0xfd;
		pK2->KVvflg &= 0xfd;

		CArithEx Arith( 0x16 );
		if( Arith.l_test( pAns[10].val ) > 0 ){
			memmove( pK1->KVkval, pAns[10].val, MONY_BUF_SIZE );	// 11 納付税額
		}
		else{
			Arith.l_add( pK1->KVkval, pAns[7].val, pAns[11].val );	// 8 控除不足還付税額 + 12 中間納付還付税額
			Arith.l_neg( pK1->KVkval );
		}

		if( Arith.l_test( pAns[21].val ) > 0 ){
			memmove( pK2->KVkval, pAns[21].val, MONY_BUF_SIZE );	// 22 納付譲渡割額
		}
		else{
			Arith.l_add( pK2->KVkval, pAns[18].val, pAns[22].val );	// 19 還付額 + 23 中間納付譲渡割額
			Arith.l_neg( pK2->KVkval );
		}
	}
...
// 返送値	0		：	正常終了
//-----------------------------------------------------------------------------
void CH31SnKskData::SetReaskFinancial( CSnHeadData *pSnHeadData )
{
	ASSERT( pSnHeadData );
	...
	// 公金受取口座を利用する
	if( pSnHeadData->Sn_GenericSgn & 0x400 ){
		pSnHeadData->Sn_GenericSgn |= 0x1000;
	}
	else{
		pSnHeadData->Sn_GenericSgn &= ~0x1000;
	}
...
//			-1		：	エラー
//-----------------------------------------------------------------------------
int CH31SnKskData::TableCheck( CDBNpSub *pDBNpSub )
{
	CString filter;

	filter.Format("SELECT COUNT(*) FROM sys.columns WHERE (name = 'Kikakutei1' OR name = 'Kikakutei2') AND object_id = object_id('[dbo].[reask]')");
	CRecordset	rset( pDBNpSub->m_database );
	CString		data = "";
	int			exists = 0;
	try {
		rset.Open(CRecordset::forwardOnly, filter, CRecordset::readOnly);
		if (!rset.IsEOF()) {
			rset.GetFieldValue((short)0, data);
			if (atoi(data) == 0) {	// ないとき
				exists = 0;
			}
			else {					// あるとき
