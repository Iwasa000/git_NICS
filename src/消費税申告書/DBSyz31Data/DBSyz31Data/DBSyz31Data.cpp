// DBSyz31Data.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには DBSyz31Data.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE DBSyz31DataDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DBSyz31Data.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(DBSyz31DataDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(DBSyz31DataDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DBSyz31Data.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(DBSyz31DataDLL);
	}
	return 1;   // OK
}
