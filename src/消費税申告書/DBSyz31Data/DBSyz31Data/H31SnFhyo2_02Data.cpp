...
//<---- 20230120
	...
	{ _T("AEC00088"), 999 },
	// â€ê≈â›ï®Ç…åWÇÈè¡îÔê≈
	// 3%Ç∆4%ÇÕÉ^ÉOÇ™åˆäJÇ≥ÇÍÇƒÇ¢Ç»Ç¢ÇΩÇﬂÅAì∆é©Ç…ê›íËÇ∑ÇÈ
//	{ _T("AEC00113"), 0 },
//	{ _T("AEC00114"), 1 },
	{ _T("AEC00115"), 3 },
	{ _T("AEC00120"), 999 },
	...
	{ _T("AEF00045"), 3 },
	{ _T("AEF00050"), 999 },
	// í≤êÆëŒè€å≈íËéëéYÇâ€ê≈ã∆ñ±óp(îÒâ€ê≈âcã∆óp)Ç…ì]ópÇµÇΩèÍçáÇÃí≤êÆ(â¡éZñîÇÕå∏éZ)äz
	{ _T("AEF00080"), 0 },
	{ _T("AEF00090"), 1 },
	{ _T("AEF00095"), 3 },
	{ _T("AEF00100"), 999 },
...
int CH31SnFhyo2_02Data::GetData( CDBNpSub *pDBNpSub, int sn_seq )
{
	...
	for (int i = 0; ; i++ ) {
		...
		// â€ê≈édì¸ÇÍÇ…åWÇÈéxï•Ç¢ëŒâøÇÃäz
...
//		pShLstVal->sign1 = 0;
		...
		// â€ê≈îÑè„äÑçáïœìÆéûÇÃí≤êÆëŒè€å≈íËÇ…åWÇÈè¡îÔê≈äzÇÃí≤êÆ(â¡éZñîÇÕå∏éZ)äz
		pShLstVal->AddNew();
		pShLstVal->sn_seq = sn_seq;
		pShLstVal->version = nowVersion;
		pShLstVal->hyoId = pHyoID;
...
*/
			...
			// ãèèZópí¿ë›åöï®Çâ€ê≈í¿ë›ópÇ…ãüÇµÇΩ(è˜ìnÇµÇΩ)èÍçáÇÃâ¡éZäz
...
//	Sn_F202_10sw = 0;//yoshida190909 ÉRÉÅÉìÉgâª
	Sn_F202_12sw = 0;
	Sn_F202_19sw = 0;
	Sn_F202_20sw = 0;
	Sn_F202_23sw = 0;
	//---->20230120
//	Sn_F202_28sw = 0; // Ç±Ç±Ç≈óéÇ∆Ç∑Ç∆òAìÆÅ®îÒòAìÆéûÇ…ReadData2ÇÃã‡äzÉZÉbÉgÇÃó¨ÇÍÇ…ì¸ÇÁÇ»Ç¢ÇÃÇ≈âΩÇ‡ÇµÇ»Ç¢
	//<----20230120
...
int CH31SnFhyo2_02Data::GetPastData ( CDBNpSub *pDBNpSub, int sn_seq, CSnHeadData *pSnHeadData )
{
	...
	for ( int idx = 0; ; idx++ ) {
		...
		// çTèúê≈äzÇÃí≤êÆ 6.3%ÇÕòAìÆéûÇ‡éÊìæ
		// å¬ï ëŒâûï˚éÆ â€ê≈îÑè„Ç…ÇÃÇ›óvÇ∑ÇÈÇ‡ÇÃ
		...
		// å¬ï ëŒâûï˚éÆ â€ê≈îÑè„Ç∆îÒâ€ê≈îÑÇËè„Ç∞Ç…ã§í ÇµÇƒóvÇ∑ÇÈÇ‡ÇÃ
		else if ( pShLstVal->itmId == _T( "AEE00080" ) ) {
			if ( Kobetsu ) {
				m_Util.val_to_bin ( Sn_F202_18A, pShLstVal->val );
			}
