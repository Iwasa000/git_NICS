...
#include "ShinFhyo40EXIdx.h"


// CShinFhyo40EX ダイアログ

IMPLEMENT_DYNAMIC(CShinFhyo40EX, CSyzBaseDlg)

CShinFhyo40EX::CShinFhyo40EX(CWnd* pParent /*=NULL*/)	
	: CSyzBaseDlg(CShinFhyo40EX::IDD, pParent)
	, m_pParent(pParent)
	, m_pSnHeadData(NULL)
	, m_pArith(NULL)
	, m_pShinInfo(NULL)
...
CShinFhyo40EX::~CShinFhyo40EX()
{
}

void CShinFhyo40EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo40EX, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinFhyo40EX メッセージ ハンドラ

BOOL CShinFhyo40EX::OnInitDialog()
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo40EX::InitInfo( CH26SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo , CH26SnHonpyoData **pSnHonpyoData )
{
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFhyo40EX::ReadData( int pas )
{
	// 課税標準額
...
//		Mony	：	表示金額
//-----------------------------------------------------------------------------
void CShinFhyo40EX::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinFhyo40EX::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFhyo40EX::InitDiagAttr()
{
	int	cnt = 0;
	while( 1 ){
...
BOOL CShinFhyo40EX::PreTranslateMessage(MSG* pMsg)
{
...
BEGIN_EVENTSINK_MAP(CShinFhyo40EX, CSyzBaseDlg)
	ON_EVENT(CShinFhyo40EX, IDC_ICSDIAGCTRL2, 1, CShinFhyo40EX::EditONIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinFhyo40EX, IDC_ICSDIAGCTRL2, 2, CShinFhyo40EX::EditOFFIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinFhyo40EX, IDC_ICSDIAGCTRL2, 3, CShinFhyo40EX::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFhyo40EX::EditONIcsdiagctrl2(short index)
{
...
void CShinFhyo40EX::EditOFFIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo40EX::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
// 各入力データ抽出
//-----------------------------------------------------------------------------
void CShinFhyo40EX::GetAllDiagData()
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinFhyo40EX::UpDownFocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinFhyo40EX::SetItemPosition()
{
	int index;
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinFhyo40EX::ChangeColor( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 他表の再集計による再表示
//-----------------------------------------------------------------------------
void CShinFhyo40EX::disp_by_recalq()
{
	// 控除対象仕入税額
	memmove( F44A, (*m_pSnFhyo40Data)->Sn_1F4A, MONY_BUF_SIZE );
	memmove( F44B, (*m_pSnFhyo40Data)->Sn_1F4B, MONY_BUF_SIZE );
