...
IMPLEMENT_DYNAMIC(CShinFhyo60, CSyzBaseDlg)
...
int MNCallback(void* wnd, int index, int mes_id, MSG* pMsg)
{
	if(mes_id == ID_MN_MESSAGE){	// ICSMNEditのPretranslateMessageでコールされた場合
		if( wnd && IsWindow(((CShinFhyo60*)wnd)->m_hWnd) && pMsg ){
			TRACE("MNCallback ID_MN_MESSAGE\n");
			return (((CShinFhyo60*)wnd)->OnTerminationCallBack( index, (short)pMsg->wParam, 0, NULL ));
		}
	}
	else if(mes_id == ID_MN_EDIT_ON){ // ICSMNEditのEditOnでコールされた場合
		if( wnd && IsWindow(((CShinFhyo60*)wnd)->m_hWnd) ){
			TRACE("MNCallback ID_MN_EDIT_ON\n");
			return ((CShinFhyo60*)wnd)->OnEditONCallBack( index );
		}
	}
	else if(mes_id == ID_MN_EDIT_OFF){ // ICSMNEditのEditOffでコールされた場合
		if( wnd && IsWindow(((CShinFhyo60*)wnd)->m_hWnd) ){
			TRACE("MNCallback ID_MN_EDIT_OFF\n");
	 		return ((CShinFhyo60*)wnd)->OnEditOFFCallBack( index );
		}
	}
	return FALSE;
}

CShinFhyo60::CShinFhyo60(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinFhyo60::IDD, pParent)
	, m_pSnHeadData(NULL)
	...
	, mynum_cursel(0)
{
	...
	Sn_6FSDEATH_GENGO = 0;
	memset(Sn_6FSDEATH,'\0',sizeof(Sn_6FSDEATH));

	MyNumber_DelHaiki_Clear();
}

CShinFhyo60::~CShinFhyo60()
{
}

void CShinFhyo60::DoDataExchange(CDataExchange* pDX)
{
	CSyzBaseDlg::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_ICSDIAGCTRL1, m_Fhyo60Diag);
...
BEGIN_MESSAGE_MAP(CShinFhyo60, CSyzBaseDlg)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_HSCROLL()
	ON_WM_ACTIVATE()
	ON_BN_CLICKED(IDC_MYNUM_DEL1, &CShinFhyo60::OnBnClickedMynumDel1)
	ON_BN_CLICKED(IDC_MYNUM_DEL2, &CShinFhyo60::OnBnClickedMynumDel2)
	ON_BN_CLICKED(IDC_MYNUM_DEL3, &CShinFhyo60::OnBnClickedMynumDel3)
	ON_BN_CLICKED(IDC_MYNUM_DEL4, &CShinFhyo60::OnBnClickedMynumDel4)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_SOZOKU_DEL, &CShinFhyo60::OnBnClickedSozokuDel)
	ON_BN_CLICKED(IDC_SOZOKU_INS, &CShinFhyo60::OnBnClickedSozokuIns)
END_MESSAGE_MAP()


// CShinFhyo60 メッセージ ハンドラ

BOOL CShinFhyo60::OnInitDialog()
{
...
int CShinFhyo60::InitInfo( CH28SnFhyo60Data **pSnFhyo60Data, CH26SnFhyo10Data **pSnFhyo10Data, CH26SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CH26SnHonpyoData **pSnHonpyoData, CArithEx *pArith, CDBNpSub *pZmSub, SYC_SHININFO *pShinInfo )
{
	ASSERT( pSnFhyo60Data );
	if( pSnFhyo60Data == NULL ){
		return -1;
	}
	m_pSnFhyo60Data = pSnFhyo60Data;	
...
//			-1	：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo60::Init()
{

	if( ((*m_pSnFhyo60Data)==NULL) || (((*m_pSnFhyo10Data)==NULL)&&((*m_pSnFhyo40Data)==NULL)) || ((*m_pSnHonpyoData)==NULL) ){
		return -1;
	}
	...
	if( m_pArith->l_test(XM26) > 0 ){
		for( int idx = 0; idx < SOZOKU_MAX; idx++ ){
			f6calq_9to14(idx);
			if( !(sozoku[idx].Sn_6FSNOFUKANPUsw&0x01)){
				memmove( sozoku[idx].Sn_6FSNOFUZEI, F69[idx], MONY_BUF_SIZE );
			}
			if( !(sozoku[idx].Sn_6FSNOFUKANPUsw&0x02)){
				memmove( sozoku[idx].Sn_6FSNOFUCHIHO, F610[idx], MONY_BUF_SIZE );
			}
		}
	}
...
//2018.08.21 INSERT START
	...
	if( line >= 0 ){
		m_ctrl_mynum.SetData( sozoku[line].Sn_6FSMYNUM1+sozoku[line].Sn_6FSMYNUM2+sozoku[line].Sn_6FSMYNUM3);
	}
//2018.08.21 INSERT END
	return 0;
}

void CShinFhyo60::InitMyNumberData()
{
	...
	//マイナンバーコントロールを初期化
	m_ctrl_mynum.Init(this, MN_KOJIN, (*m_pSnHeadData)->m_apno, TRUE, edit, m_Fhyo60Diag.GetSafeHwnd(), diag_index);
...
int CShinFhyo60::MyNumberCheck( int sgn )
{
	...
	for( int idx1 = 0; idx1 < SOZOKU_MAX-1; idx1++ ){
		if( sozoku[idx1].Sn_6FSMYNUM1.IsEmpty()){
			continue;
		}
		if( sozoku[idx1].Sn_6FSMYNUM1.Compare("****") == 0 ){
			continue;
		}
		for( int idx2 = idx1+1; idx2 < SOZOKU_MAX; idx2++ ){
			BOOL flg = TRUE;
			if( sozoku[idx1].Sn_6FSMYNUM1.Compare( sozoku[idx2].Sn_6FSMYNUM1 ) != 0 ){
				flg = FALSE;
			}
			if( sozoku[idx1].Sn_6FSMYNUM2.Compare( sozoku[idx2].Sn_6FSMYNUM2 ) != 0 ){
				flg = FALSE;
			}
			if( sozoku[idx1].Sn_6FSMYNUM3.Compare( sozoku[idx2].Sn_6FSMYNUM3 ) != 0 ){
				flg = FALSE;
			}
			if( flg == TRUE ){
				ret = -1;
...
int CShinFhyo60::SozokubunCheck( int sgn )
{
	int num1=0, num2=0;
	...
	for( int index = 0; index < SOZOKU_MAX; index++ ){
		...
		if(( sozoku[index].Sn_6FSBUNBO.IsEmpty() )||(atoi(sozoku[index].Sn_6FSBUNBO)==0)){
			bunbo_zero = TRUE;
		}
		BOOL bunshi_zero = FALSE;
		if(( sozoku[index].Sn_6FSBUNSHI.IsEmpty() )||(atoi(sozoku[index].Sn_6FSBUNSHI)==0)){
			bunshi_zero = TRUE;
		}
		...
		if(( bunshi_t == 0 )||( bunbo_t == 0 )){
			bunshi_t = atoi( sozoku[index].Sn_6FSBUNSHI );
			bunbo_t  = atoi( sozoku[index].Sn_6FSBUNBO );
		}else{
			bunbo = atoi( sozoku[index].Sn_6FSBUNBO );
			...
			bunshi_t *= num2;
			//bunbo*=num1;
			bunshi_t += atoi( sozoku[index].Sn_6FSBUNSHI ) * num1;
...
int CShinFhyo60::NofuKanpuGokeiCheck()
{
	int ret = 0;
	...
	if( m_pArith->l_test(F61)!=0 ){
		...
		for( int idx = 0; idx < SOZOKU_MAX; idx++ ){
			if( sozoku[idx].Sn_NO != -1 ){
				...
				sw |= sozoku[idx].Sn_6FSNOFUKANPUsw;

				if( sozoku[idx].Sn_6FSNOFUKANPUsw&0x01 ){
					m_pArith->l_add( zei_total, zei_total, sozoku[idx].Sn_6FSNOFUZEI );
				}else{
					m_pArith->l_add( zei_total, zei_total, F69[idx] );
				}
				if( sozoku[idx].Sn_6FSNOFUKANPUsw&0x02 ){
					m_pArith->l_add( chiho_total, chiho_total, sozoku[idx].Sn_6FSNOFUCHIHO );
				}else{
		...
		for( int idx = 0; idx < SOZOKU_MAX; idx++ ){
			if( sozoku[idx].Sn_NO != -1 ){
				m_pArith->l_add( zei_total, zei_total, sozoku[idx].Sn_6FSKANPUZEI );
				m_pArith->l_add( chiho_total, chiho_total, sozoku[idx].Sn_6FSKANPUCHIHO );

				if( m_pArith->l_test( sozoku[idx].Sn_6FSKANPUZEI ) != 0 ){
					sw |= 0x04;
				}
				if( m_pArith->l_test( sozoku[idx].Sn_6FSKANPUCHIHO ) != 0 ){
					sw |= 0x08;
				}
			}
		}
...
int CShinFhyo60::get_gcd( int num1, int num2 )
{
	int tmp1=0,tmp2=0;
	int joyo=0,gcd=0;
...
void CShinFhyo60::MyNumber_ButtomDisp()
{
	...
	for( int btn_cnt = 0; btn_cnt < SOZOKU_PER_PAGE; btn_cnt++ ){
		if( hide ){
			...
			// ハッシュが空なら非表示
			int size = sizeof(sozoku[m_Lin + btn_cnt].Sn_6FSHASH);

			if( MyNumber_IsHashEmpty( sozoku[m_Lin + btn_cnt].Sn_6FSHASH, size ) == FALSE ){
...
BOOL CShinFhyo60::MyNumber_IsHashEmpty( char *hash, int size )
{
	BOOL ret = TRUE;
...
BOOL CShinFhyo60::MyNumber_IsAllEmpty()
{
	BOOL ret = TRUE;

	for( int idx = 0; idx < SOZOKU_MAX; idx++){
		if( sozoku[idx].Sn_6FSMYNUM1.IsEmpty() && sozoku[idx].Sn_6FSMYNUM2.IsEmpty() && sozoku[idx].Sn_6FSMYNUM3.IsEmpty() ){
			;
		}else{
			ret = FALSE;
			break;
...
void CShinFhyo60::ResetMyNumberControl()
{
	...
	CString str;
	if( index != -1 ){
		str = sozoku[index].Sn_6FSMYNUM1+sozoku[index].Sn_6FSMYNUM2+sozoku[index].Sn_6FSMYNUM3;
	}
...
int CShinFhyo60::MyNumber_CheckFromDisp( int sgn )
{
	int accesPrm = MynoAccessPerm();										//マイナンバー権限
	int isMNAble = NTBMOD8_IsAbleToMyNumber( (*m_pSnHeadData)->m_apno, 0 );	//マイナンバー有償・無償
	if( !accesPrm||!isMNAble ){
...
BOOL CShinFhyo60::PreTranslateMessage(MSG* pMsg)
{
	...
	if( pMsg->message == WM_KEYDOWN ){
		...
		if( pMsg->wParam == VK_RETURN ){
			short result = m_Fhyo60Diag.GetPosition();
...
int CShinFhyo60::ReadData()
{

	//memmove( Sn_6FSDEATH, (*m_pSnFhyo60Data)->Sn_6FSDEATH, sizeof(Sn_6FSDEATH));
	//memmove( Sn_6FSDEATH, (*m_pSnFhyo60Data)->Sn_6FSDEATH, sizeof(Sn_6FSDEATH)-1);
	CVolDateDB	voldate;
	int	yymmdd = 0, g = 0;
	Sn_6FSDEATH_GENGO = 0;
//	m_Util.ADtoJC( &dummy, &Sn_6FSDEATH[0], &Sn_6FSDEATH[1], &Sn_6FSDEATH[2], (*m_pSnFhyo60Data)->Sn_6FSDEATH ); //年号固定(平成)
	if( (*m_pSnFhyo60Data)->Sn_6FSDEATH ){
		voldate.db_datecnvGen( 0, (*m_pSnFhyo60Data)->Sn_6FSDEATH, &Sn_6FSDEATH_GENGO, &yymmdd, 0, 0 );
	}
#ifdef _DBG_VIEW_START_
	CString	hoge;
	hoge.Format("ReadData()_Sn_6FSDEATH_GENGO:%d\n", Sn_6FSDEATH_GENGO);
	DbgViewTrace(hoge);
#endif
	//m_pArith->int_bcd( &Sn_6FSDEATH[0], g, 1 );
	//m_pArith->int_bcd( &Sn_6FSDEATH[1], yymmdd, 3 );
	m_pArith->int_bcd( &Sn_6FSDEATH, yymmdd, 3 );

	Sn_6FSDAIHYO	= (*m_pSnFhyo60Data)->Sn_6FSDAIHYO;
	Sn_6FSLIMIT		= (*m_pSnFhyo60Data)->Sn_6FSLIMIT;

	for( int cnt = 0; cnt < SOZOKU_MAX; cnt++ ){

		if( (*m_pSnFhyo60Data)->Sn_NO[cnt] == -1 ){
			continue;
		}

		sozoku[cnt].Sn_NO			= (*m_pSnFhyo60Data)->Sn_NO[cnt];
		sozoku[cnt].Sn_6FSJUSYO	= (*m_pSnFhyo60Data)->Sn_6FSJUSYO[cnt];
		sozoku[cnt].Sn_6FSFURI		= (*m_pSnFhyo60Data)->Sn_6FSFURI[cnt];
		sozoku[cnt].Sn_6FSNAME		= (*m_pSnFhyo60Data)->Sn_6FSNAME[cnt];
		memmove( sozoku[cnt].Sn_6FSHASH, (*m_pSnFhyo60Data)->Sn_6FSHASH[cnt], sizeof(sozoku[cnt].Sn_6FSHASH));
		sozoku[cnt].Sn_6FSSYOKU		= (*m_pSnFhyo60Data)->Sn_6FSSYOKU[cnt];
		sozoku[cnt].Sn_6FSTSUDUKI	= (*m_pSnFhyo60Data)->Sn_6FSTSUDUKI[cnt];

		yymmdd = 0;
		g = 0;
		if( (*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt] ){
			if( (*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt] <= 19890107 ){
//				m_Util.ADtoJC( &sozoku[cnt].Sn_6FSBIRTH[0], &sozoku[cnt].Sn_6FSBIRTH[1], &sozoku[cnt].Sn_6FSBIRTH[2], &sozoku[cnt].Sn_6FSBIRTH[3], (*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt] );
				voldate.vd_CeraToJcal( (*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt], &g, &yymmdd );
			}else{
				//db_datecnvGenは平成以降のみ対応
				CVolDateDB	voldate;
				voldate.db_datecnvGen( 0,  (*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt], &g, &yymmdd, 0, 0 );
				//wsprintf( &sozoku[cnt].Sn_6FSBIRTH[0], "%02X", yymmdd/10000 );
				//wsprintf( &sozoku[cnt].Sn_6FSBIRTH[1], "%02X", (yymmdd/100)%100 );
				//wsprintf( &sozoku[cnt].Sn_6FSBIRTH[2], "%02X", yymmdd%100 );
			}
			m_pArith->int_bcd( &sozoku[cnt].Sn_6FSBIRTH[0], g, 1 );
			m_pArith->int_bcd( &sozoku[cnt].Sn_6FSBIRTH[1], yymmdd, 3 );
		}

		sozoku[cnt].Sn_6FSTEL1		= (*m_pSnFhyo60Data)->Sn_6FSTEL1[cnt];
		sozoku[cnt].Sn_6FSTEL2		= (*m_pSnFhyo60Data)->Sn_6FSTEL2[cnt];
		sozoku[cnt].Sn_6FSTEL3		= (*m_pSnFhyo60Data)->Sn_6FSTEL3[cnt];
		sozoku[cnt].Sn_6FSHOTEI		= (*m_pSnFhyo60Data)->Sn_6FSHOTEI[cnt];
		sozoku[cnt].Sn_6FSBUNSHI	= (*m_pSnFhyo60Data)->Sn_6FSBUNSHI[cnt];
		sozoku[cnt].Sn_6FSBUNBO		= (*m_pSnFhyo60Data)->Sn_6FSBUNBO[cnt];
		memmove( sozoku[cnt].Sn_6FSKAGAKU, (*m_pSnFhyo60Data)->Sn_6FSKAGAKU[cnt], sizeof(sozoku[cnt].Sn_6FSKAGAKU) );
		memmove( sozoku[cnt].Sn_6FSNOFUZEI, (*m_pSnFhyo60Data)->Sn_6FSNOFUZEI[cnt], sizeof(sozoku[cnt].Sn_6FSNOFUZEI) );
		memmove( sozoku[cnt].Sn_6FSNOFUCHIHO, (*m_pSnFhyo60Data)->Sn_6FSNOFUCHIHO[cnt], sizeof(sozoku[cnt].Sn_6FSNOFUCHIHO) );
		memmove( sozoku[cnt].Sn_6FSKANPUZEI, (*m_pSnFhyo60Data)->Sn_6FSKANPUZEI[cnt], sizeof(sozoku[cnt].Sn_6FSKANPUZEI) );
		memmove( sozoku[cnt].Sn_6FSKANPUCHIHO, (*m_pSnFhyo60Data)->Sn_6FSKANPUCHIHO[cnt], sizeof(sozoku[cnt].Sn_6FSKANPUCHIHO) );
		sozoku[cnt].Sn_6FSNOFUKANPUsw		= (*m_pSnFhyo60Data)->Sn_6FSNOFUKANPUsw[cnt];
		sozoku[cnt].Sn_6FSGNAME		= (*m_pSnFhyo60Data)->Sn_6FSGNAME[cnt];
		sozoku[cnt].Sn_6FSGSEL		= (*m_pSnFhyo60Data)->Sn_6FSGSEL[cnt];
		sozoku[cnt].Sn_6FSSNAME		= (*m_pSnFhyo60Data)->Sn_6FSSNAME[cnt];
		sozoku[cnt].Sn_6FSSSEL		= (*m_pSnFhyo60Data)->Sn_6FSSSEL[cnt];
		sozoku[cnt].Sn_6FSYOKIN		= (*m_pSnFhyo60Data)->Sn_6FSYOKIN[cnt];
		sozoku[cnt].Sn_6FSKOUZA		= (*m_pSnFhyo60Data)->Sn_6FSKOUZA[cnt];
		sozoku[cnt].Sn_6FSKIGO1		= (*m_pSnFhyo60Data)->Sn_6FSKIGO1[cnt];
		sozoku[cnt].Sn_6FSKIGO2		= (*m_pSnFhyo60Data)->Sn_6FSKIGO2[cnt];
		sozoku[cnt].Sn_6FSYUBIN		= (*m_pSnFhyo60Data)->Sn_6FSYUBIN[cnt];
	}
...
int CShinFhyo60::WriteData()
{
	...
	//memmove( (*m_pSnFhyo60Data)->Sn_6FSDEATH, Sn_6FSDEATH, sizeof((*m_pSnFhyo60Data)->Sn_6FSDEATH));
	//m_Util.JCtoAD( &(*m_pSnFhyo60Data)->Sn_6FSDEATH, Sn_6FSDEATH[0], Sn_6FSDEATH[1], Sn_6FSDEATH[2], Sn_6FSDEATH[3] ); 
	//m_Util.JCtoAD( &(*m_pSnFhyo60Data)->Sn_6FSDEATH, 0x04, Sn_6FSDEATH[0], Sn_6FSDEATH[1], Sn_6FSDEATH[2] ); //年号固定(平成)
	CVolDateDB	voldate;
	int dummy;
	//int tmp_gengo = m_pArith->bcd_int( &Sn_6FSDEATH[0], 1 );
//	int tmp_gengo = 4; //年号固定(平成)
//	Sn_6FSDEATH_GENGO = 0;
	//int tmp_death = m_pArith->bcd_int( &Sn_6FSDEATH[1], 3 );
	int tmp_death = m_pArith->bcd_int( &Sn_6FSDEATH, 3 );
	if( tmp_death ){
		voldate.db_datecnvGen(Sn_6FSDEATH_GENGO, tmp_death, &dummy, &(*m_pSnFhyo60Data)->Sn_6FSDEATH, 1, 0 );
#ifdef _DBG_VIEW_START_
		CString	hoge;
		hoge.Format("WriteData()_Sn_6FSDEATH_GENGO:%d\n", Sn_6FSDEATH_GENGO);
		DbgViewTrace(hoge);
#endif
	}else{
		(*m_pSnFhyo60Data)->Sn_6FSDEATH = 0;
	}
	(*m_pSnFhyo60Data)->Sn_6FSDAIHYO = Sn_6FSDAIHYO;
	(*m_pSnFhyo60Data)->Sn_6FSLIMIT = Sn_6FSLIMIT;
	...
	for( int cnt = 0; cnt < SOZOKU_MAX; cnt++ ){
		...
		if( IsNULL_Column(cnt,sgn) == TRUE ){
			(*m_pSnFhyo60Data)->Sn_NO[cnt]			= -1;
		}else{
			(*m_pSnFhyo60Data)->Sn_NO[cnt]			= sozoku[cnt].Sn_NO;
		}

		(*m_pSnFhyo60Data)->Sn_6FSJUSYO[cnt]	= sozoku[cnt].Sn_6FSJUSYO;
		(*m_pSnFhyo60Data)->Sn_6FSFURI[cnt]		= sozoku[cnt].Sn_6FSFURI;
		(*m_pSnFhyo60Data)->Sn_6FSNAME[cnt]		= sozoku[cnt].Sn_6FSNAME;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSHASH[cnt], sozoku[cnt].Sn_6FSHASH, sizeof((*m_pSnFhyo60Data)->Sn_6FSHASH[cnt]));
		(*m_pSnFhyo60Data)->Sn_6FSSYOKU[cnt]	= sozoku[cnt].Sn_6FSSYOKU;
		(*m_pSnFhyo60Data)->Sn_6FSTSUDUKI[cnt]	= sozoku[cnt].Sn_6FSTSUDUKI;
		CVolDateDB	voldate;
		int dummy;
		int tmp_birth = m_pArith->bcd_int( sozoku[cnt].Sn_6FSBIRTH+1, 3 );
		int tmp_gengo = m_pArith->bcd_int( sozoku[cnt].Sn_6FSBIRTH, 1 );
		if( tmp_birth ){
			if( tmp_gengo <= 3 ){
				//m_Util.JCtoAD( &(*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt], sozoku[cnt].Sn_6FSBIRTH[0], sozoku[cnt].Sn_6FSBIRTH[1], sozoku[cnt].Sn_6FSBIRTH[2], sozoku[cnt].Sn_6FSBIRTH[3] );
				voldate.vd_JcalToCera( tmp_gengo, tmp_birth, &(*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt] );
			}else{
				voldate.db_datecnvGen( tmp_gengo, tmp_birth, &dummy, &(*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt], 1, 0 );
			}
		}else{
			(*m_pSnFhyo60Data)->Sn_6FSBIRTH[cnt] = 0;
		}

		(*m_pSnFhyo60Data)->Sn_6FSTEL1[cnt]		= sozoku[cnt].Sn_6FSTEL1;
		(*m_pSnFhyo60Data)->Sn_6FSTEL2[cnt]		= sozoku[cnt].Sn_6FSTEL2;
		(*m_pSnFhyo60Data)->Sn_6FSTEL3[cnt]		= sozoku[cnt].Sn_6FSTEL3;
		(*m_pSnFhyo60Data)->Sn_6FSHOTEI[cnt]	= sozoku[cnt].Sn_6FSHOTEI;
		(*m_pSnFhyo60Data)->Sn_6FSBUNSHI[cnt]	= sozoku[cnt].Sn_6FSBUNSHI;
		(*m_pSnFhyo60Data)->Sn_6FSBUNBO[cnt]	= sozoku[cnt].Sn_6FSBUNBO;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSKAGAKU[cnt],	sozoku[cnt].Sn_6FSKAGAKU, sizeof((*m_pSnFhyo60Data)->Sn_6FSKAGAKU[cnt])) ;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSNOFUZEI[cnt], sozoku[cnt].Sn_6FSNOFUZEI, sizeof((*m_pSnFhyo60Data)->Sn_6FSNOFUZEI[cnt])) ;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSNOFUCHIHO[cnt], sozoku[cnt].Sn_6FSNOFUCHIHO, sizeof((*m_pSnFhyo60Data)->Sn_6FSNOFUCHIHO[cnt])) ;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSKANPUZEI[cnt], sozoku[cnt].Sn_6FSKANPUZEI, sizeof((*m_pSnFhyo60Data)->Sn_6FSKANPUZEI[cnt])) ;
		memmove( (*m_pSnFhyo60Data)->Sn_6FSKANPUCHIHO[cnt], sozoku[cnt].Sn_6FSKANPUCHIHO, sizeof((*m_pSnFhyo60Data)->Sn_6FSKANPUCHIHO[cnt])) ;
		(*m_pSnFhyo60Data)->Sn_6FSNOFUKANPUsw[cnt]	= sozoku[cnt].Sn_6FSNOFUKANPUsw;
		(*m_pSnFhyo60Data)->Sn_6FSGNAME[cnt]	= sozoku[cnt].Sn_6FSGNAME;
		(*m_pSnFhyo60Data)->Sn_6FSGSEL[cnt]		= sozoku[cnt].Sn_6FSGSEL;
		(*m_pSnFhyo60Data)->Sn_6FSSNAME[cnt]	= sozoku[cnt].Sn_6FSSNAME;
		(*m_pSnFhyo60Data)->Sn_6FSSSEL[cnt]		= sozoku[cnt].Sn_6FSSSEL;
		(*m_pSnFhyo60Data)->Sn_6FSYOKIN[cnt]	= sozoku[cnt].Sn_6FSYOKIN;
		(*m_pSnFhyo60Data)->Sn_6FSKOUZA[cnt]	= sozoku[cnt].Sn_6FSKOUZA;
		(*m_pSnFhyo60Data)->Sn_6FSKIGO1[cnt]	= sozoku[cnt].Sn_6FSKIGO1;
		(*m_pSnFhyo60Data)->Sn_6FSKIGO2[cnt]	= sozoku[cnt].Sn_6FSKIGO2;
		(*m_pSnFhyo60Data)->Sn_6FSYUBIN[cnt]	= sozoku[cnt].Sn_6FSYUBIN;		
	}

	return 0;
}

void CShinFhyo60::DispFh6(int mode)
{
#ifdef _DBG_VIEW_START_
	CString	hoge;
	hoge.Format("DispFh6()_START\n");
	DbgViewTrace(hoge);
#endif
...
//2018.08.21 UPDATE END
	...
	if( isReiwaPrnt && (gengo==ID_ICSSH_REIWA_GENGO) ){
		gengoStr = SYZ26_REIWA_G_STR;
	}
	else{
		gengoStr = SYZ26_HEISEI_G_STR;
	}
...
//2018.08.21 UPDATE END
...
#ifdef _DBG_VIEW_START_
	hoge.Format("DispFh6()_Sn_6FSDEATH_GENGO：%d\n", Sn_6FSDEATH_GENGO);
	DbgViewTrace(hoge);
#endif

	if( isReiwaPrnt ){
		if( Sn_6FSDEATH_GENGO == ID_ICSSH_REIWA_GENGO ){
			gengoStr = SYZ26_REIWA_G_STR;
		}
		else if( Sn_6FSDEATH_GENGO == ID_ICSSH_HEISEI_GENGO ){
			gengoStr = SYZ26_HEISEI_G_STR;
		}
		else{
			gengoStr = SYZ26_EMPTY_G_STR;
		}
	}
	else{
		gengoStr = SYZ26_HEISEI_G_STR;
	}
	dsp_disp(ID1040_G, gengoStr);

	dsp_day( ID1040, Sn_6FSDEATH );
	dsp_edit( ID2010, Sn_6FSDAIHYO );
	dsp_check( ID3010, Sn_6FSLIMIT );
	...
	for( int cnt = 0; cnt < SOZOKU_PER_PAGE; cnt++ ){
		...
		if( idx != -1 ){
			...
			dsp_edit(	JUSYO_ID[cnt],		sozoku[idx].Sn_6FSJUSYO );
			dsp_edit(	FURI_ID[cnt],		sozoku[idx].Sn_6FSFURI );
			dsp_edit(	NAME_ID[cnt],		sozoku[idx].Sn_6FSNAME );

			dsp_edit(	MYNUM1_ID[cnt],		sozoku[idx].Sn_6FSMYNUM1 );	
			dsp_edit(	MYNUM2_ID[cnt],		sozoku[idx].Sn_6FSMYNUM2 );	
			dsp_edit(	MYNUM3_ID[cnt],		sozoku[idx].Sn_6FSMYNUM3 );

			dsp_edit(	SYOKU_ID[cnt],		sozoku[idx].Sn_6FSSYOKU );
			dsp_edit(	TSUDUKI_ID[cnt],	sozoku[idx].Sn_6FSTSUDUKI );
			dsp_day( 	BIRTH_ID[cnt],		sozoku[idx].Sn_6FSBIRTH );
			dsp_edit(	TEL1_ID[cnt],		sozoku[idx].Sn_6FSTEL1 );
			dsp_edit(	TEL2_ID[cnt],		sozoku[idx].Sn_6FSTEL2 );
			dsp_edit(	TEL3_ID[cnt],		sozoku[idx].Sn_6FSTEL3 );
			dsp_combo(	HOTEI_ID[cnt],		sozoku[idx].Sn_6FSHOTEI );
			dsp_edit(	BUNSHI_ID[cnt],		sozoku[idx].Sn_6FSBUNSHI );
			dsp_edit(	BUNBO_ID[cnt],		sozoku[idx].Sn_6FSBUNBO );
			dsp_val(	KAGAKU_ID[cnt],		sozoku[idx].Sn_6FSKAGAKU );

			char zei[MONY_BUF_SIZE]={0},chiho[MONY_BUF_SIZE]={0},gokei[MONY_BUF_SIZE]={0};
			if( m_pArith->l_test(XM26) > 0 ){
				if( sozoku[idx].Sn_6FSNOFUKANPUsw&0x01){
					memmove( zei, sozoku[idx].Sn_6FSNOFUZEI, MONY_BUF_SIZE );
					if( !((*m_pSnHeadData)->Sn_Sign4&0x01) ){
				...
				if( sozoku[idx].Sn_6FSNOFUKANPUsw&0x02){
					memmove( chiho, sozoku[idx].Sn_6FSNOFUCHIHO, MONY_BUF_SIZE );
					if( !((*m_pSnHeadData)->Sn_Sign4&0x01) ){
				...
				dsp_val( KANPUZEI_ID[cnt],	sozoku[idx].Sn_6FSKANPUZEI );
				dsp_val( KANPUCHIHO_ID[cnt],	sozoku[idx].Sn_6FSKANPUCHIHO );
				m_pArith->l_add( gokei, sozoku[idx].Sn_6FSKANPUZEI, sozoku[idx].Sn_6FSKANPUCHIHO );
			...
			dsp_edit(	GNAME_ID[cnt],		sozoku[idx].Sn_6FSGNAME );
			dsp_combo(	GSEL_ID[cnt],		sozoku[idx].Sn_6FSGSEL );
			dsp_edit(	SNAME_ID[cnt],		sozoku[idx].Sn_6FSSNAME );
			dsp_combo(	SSEL_ID[cnt],		sozoku[idx].Sn_6FSSSEL );
			dsp_edit(	YOKIN_ID[cnt],		sozoku[idx].Sn_6FSYOKIN );
			dsp_edit(	KOUZA_ID[cnt],		sozoku[idx].Sn_6FSKOUZA );
			dsp_edit(	KIGO1_ID[cnt],		sozoku[idx].Sn_6FSKIGO1 );
			dsp_edit(	KIGO2_ID[cnt],		sozoku[idx].Sn_6FSKIGO2 );
			dsp_edit(	YUBIN_ID[cnt],		sozoku[idx].Sn_6FSYUBIN );
		}else{
			if( !((*m_pSnHeadData)->Sn_Sign4&0x01) ){
				if( m_pArith->l_test(XM26) > 0 ){
					ChangeColor( IDC_ICSDIAGCTRL1, NOFUZEI_ID[cnt], 0 );
		...
		if( idx != -1 ){
			size = sizeof(sozoku[idx].Sn_6FSHASH);
			empty =  MyNumber_IsHashEmpty( sozoku[idx].Sn_6FSHASH, size );
		}else{
			empty = TRUE;
		}
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinFhyo60::FocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	...
	if(Move == 0){//下方向
		for(j = i + 1 ; j < FCS_FHYO6CNT ; j++){
			m_Fhyo60Diag.GetAttribute(FCS_Fhyo60[j].IDNo ,  (LPUNKNOWN)&DA);
		...
		for(j = i + 1 ; j < FCS_FHYO6CNT ; j++){
			m_Fhyo60Diag.GetAttribute(FCS_Fhyo60_LR[j].IDNo ,  (LPUNKNOWN)&DA);
			if(DA.attr_bcolor == BC_CREAM || DA.attr_bcolor == BC_BLUE){
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinFhyo60::ChangeColor( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
// 初期ポジションセット
void CShinFhyo60::SetItemPosition()
{
	if( mynum_cursel ){//mainframe：OnActivateから飛んでくるため、個人番号にフォーカス状態は何もしない。清水
		return;
	}
	this->SetFocus();
	int index = m_Fhyo60Diag.GetPosition();
	if( index != -1 ){
		m_Fhyo60Diag.SetPosition(index);
		m_Fhyo60Diag.SetFocus();
		return;
	}

	m_Fhyo60Diag.SetFocus();
	m_Fhyo60Diag.SetPosition(ID1040);
}

int CShinFhyo60::InitDiagAttr()
{
	...
	for( int cnt = 0; cnt < (sizeof(edit_id)/sizeof(edit_id[0])); cnt++ ){
		m_Fhyo60Diag.GetAttribute( edit_id[cnt], (LPUNKNOWN)&DA );
		DA.attr_editattr |= DIAG_EATTR_ZENHAN;
		m_Fhyo60Diag.SetAttribute( edit_id[cnt], (LPUNKNOWN)&DA, TRUE );
	}
	...
	m_Fhyo60Diag.EnableDelete(0);

	return 0;
}

void CShinFhyo60::dsp_disp( short Index, CString disp )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo60::dsp_edit( short Index, CString edit )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo60::dsp_combo( short Index, short combo )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo60::dsp_check( short Index, short check )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo60::dsp_val( short Index, char *val )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
void CShinFhyo60::dsp_day( short Index, char *day )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinFhyo60::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	...
	// 各入力データ抽出
void CShinFhyo60::GetAllDiagData()
{
...
//	memmove( Sn_6FSDEATH, DiagData.data_day, sizeof(Sn_6FSDEATH));
	memmove(tmpWaYmd, DiagData.data_day, sizeof(tmpWaYmd));
	m_Util.CnvNoGengoToGengoWareki(tmpWaYmd, &tmpGengo, tmpCnvWaYmd);
	Sn_6FSDEATH_GENGO = tmpGengo;
#ifdef _DBG_VIEW_START_
	CString	hoge;
	hoge.Format("GetAllDiagData()_Sn_6FSDEATH_GENGO:%d\n", Sn_6FSDEATH_GENGO);
	DbgViewTrace(hoge);
#endif
	memmove(Sn_6FSDEATH, tmpCnvWaYmd, sizeof(Sn_6FSDEATH));
	DIAGRAM_DATA_clear( &DiagData );
	diag_getdata( IDC_ICSDIAGCTRL1, ID2010, (struct IUnknown *)&DiagData, CIcsdiagctrl );
	Sn_6FSDAIHYO = DiagData.data_edit;
	DIAGRAM_DATA_clear( &DiagData );
	diag_getdata( IDC_ICSDIAGCTRL1, ID3010, (struct IUnknown *)&DiagData, CIcsdiagctrl );
	Sn_6FSLIMIT = DiagData.data_check;

	for( int idx = 0; idx < SOZOKU_PER_PAGE; idx++ ){

		DIAGRAM_DATA_clear( &DiagData );
		diag_getdata( IDC_ICSDIAGCTRL1, JUSYO_ID[idx]		, (struct IUnknown *)&DiagData, CIcsdiagctrl );
		sozoku[idx+m_Lin].Sn_6FSJUSYO = DiagData.data_edit;

		DIAGRAM_DATA_clear( &DiagData );
		diag_getdata( IDC_ICSDIAGCTRL1, FURI_ID[idx]		, (struct IUnknown *)&DiagData, CIcsdiagctrl );
		sozoku[idx+m_Lin].Sn_6FSFURI = DiagData.data_edit;

		DIAGRAM_DATA_clear( &DiagData );
		diag_getdata( IDC_ICSDIAGCTRL1, NAME_ID[idx]		, (struct IUnknown *)&DiagData, CIcsdiagctrl );
		sozoku[idx+m_Lin].Sn_6FSNAME = DiagData.data_edit;

		if(( m_Fhyo60Diag.GetPosition() == MYNUM1_ID[idx] )||
			( m_Fhyo60Diag.GetPosition() == MYNUM2_ID[idx] )||
			( m_Fhyo60Diag.GetPosition() == MYNUM3_ID[idx] )){
		...
		if( m_pArith->l_test(XM26) > 0 ){
			...
			if( sozoku[idx+m_Lin].Sn_6FSNOFUKANPUsw&0x01 ){
				cmp = m_pArith->l_cmp( sozoku[idx+m_Lin].Sn_6FSNOFUZEI, DiagData.data_val );
			}else{
				cmp = m_pArith->l_cmp( F69[idx+m_Lin], DiagData.data_val );
			}
			if( cmp != 0 ){
				sozoku[idx+m_Lin].Sn_6FSNOFUKANPUsw |= 0x01;
				memmove( tmp, DiagData.data_val, sizeof(tmp) );
				m_pArith->l_div( tmp, tmp, PW1 );
				m_pArith->l_mul( tmp, tmp, PW1 );
				memmove( sozoku[idx+m_Lin].Sn_6FSNOFUZEI, tmp, sizeof(sozoku[idx+m_Lin].Sn_6FSNOFUZEI) );
			}
			...
			if( sozoku[idx+m_Lin].Sn_6FSNOFUKANPUsw&0x02 ){
				cmp = m_pArith->l_cmp( sozoku[idx+m_Lin].Sn_6FSNOFUCHIHO, DiagData.data_val );
			}else{
				cmp = m_pArith->l_cmp( F610[idx+m_Lin], DiagData.data_val );
			}
			if( cmp != 0 ){
				sozoku[idx+m_Lin].Sn_6FSNOFUKANPUsw |= 0x02;
				memmove( tmp, DiagData.data_val, sizeof(tmp) );
				m_pArith->l_div( tmp, tmp, PW1 );
				m_pArith->l_mul( tmp, tmp, PW1 );
				memmove( sozoku[idx+m_Lin].Sn_6FSNOFUCHIHO, tmp, sizeof(sozoku[idx+m_Lin].Sn_6FSNOFUCHIHO) );
			}
		}
...
void CShinFhyo60::MoveFocus_FromMyNum()
{
	for(int idx = 0; idx < SOZOKU_PER_PAGE; idx++ ){
		if(( m_Fhyo60Diag.GetPosition() == MYNUM1_ID[idx] )||
			( m_Fhyo60Diag.GetPosition() == MYNUM2_ID[idx] )||
			( m_Fhyo60Diag.GetPosition() == MYNUM3_ID[idx] )){
				...
				if( m_ctrl_mynum.CheckNumber( sozoku[idx+m_Lin].Sn_6FSMYNUM1+sozoku[idx+m_Lin].Sn_6FSMYNUM2+sozoku[idx+m_Lin].Sn_6FSMYNUM3) == 1 ){
					DispFh6();
					m_Fhyo60Diag.SetPosition(FocusMoveCheck(MYNUM3_ID[idx] , 0));
				}
		}
	}
}
...
void CShinFhyo60::MoveWindow_SozokuBar()
{
	if( !m_bar ){
		return;
	}
...
/////////////////////////////////////////////////
BOOL CShinFhyo60::IsNULL_Column( int line_idx, char sgn )
{
	BOOL ret = TRUE;

	CString str;

	str = sozoku[line_idx].Sn_6FSJUSYO;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	str = sozoku[line_idx].Sn_6FSFURI;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	str = sozoku[line_idx].Sn_6FSNAME;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	for( int idx = 0; idx < sizeof(sozoku[line_idx].Sn_6FSHASH); idx++ ){
		if( sozoku[line_idx].Sn_6FSHASH[idx] != '\0' ){
			return FALSE;	
		}
	}
	str = sozoku[line_idx].Sn_6FSSYOKU;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	str = sozoku[line_idx].Sn_6FSTSUDUKI;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBIRTH[0] != '\0' ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBIRTH[1] != '\0' ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBIRTH[2] != '\0' ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBIRTH[3] != '\0' ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSTEL1.IsEmpty() == FALSE ){
		return FALSE;	
	}
	if( sozoku[line_idx].Sn_6FSTEL2.IsEmpty() == FALSE ){
		return FALSE;	
	}
	if( sozoku[line_idx].Sn_6FSTEL3.IsEmpty() == FALSE ){
		return FALSE;	
	}
	if( sozoku[line_idx].Sn_6FSHOTEI != 0 ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBUNSHI.IsEmpty() == FALSE ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSBUNBO.IsEmpty() == FALSE ){
		return FALSE;
	}
	if( m_pArith->l_test( sozoku[line_idx].Sn_6FSKAGAKU ) != 0 ){
		return FALSE;
	}
	if( !(sgn&0x01) ){
		if( m_pArith->l_test( sozoku[line_idx].Sn_6FSNOFUZEI ) != 0 ){
			return FALSE;
		}
		if( m_pArith->l_test( sozoku[line_idx].Sn_6FSNOFUCHIHO ) != 0 ){
			return FALSE;
		}
	}
	if( !(sgn&0x02) ){
		if( m_pArith->l_test( sozoku[line_idx].Sn_6FSKANPUZEI ) != 0 ){
			return FALSE;
		}
		if( m_pArith->l_test( sozoku[line_idx].Sn_6FSKANPUCHIHO ) != 0 ){
			return FALSE;
		}
	}
	str = sozoku[line_idx].Sn_6FSGNAME;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSGSEL != 0 ){
		return FALSE;
	}
	str = sozoku[line_idx].Sn_6FSSNAME;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSSSEL != 0 ){
		return FALSE;
	}
	str = sozoku[line_idx].Sn_6FSYOKIN;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
	if( sozoku[line_idx].Sn_6FSKOUZA.IsEmpty() == FALSE ){
		return FALSE;	
	}
	if( sozoku[line_idx].Sn_6FSKIGO1.IsEmpty() == FALSE ){
		return FALSE;	
	}
	if( sozoku[line_idx].Sn_6FSKIGO2.IsEmpty() == FALSE ){
		return FALSE;	
	}
	str = sozoku[line_idx].Sn_6FSYUBIN;
	str.Replace(_T("　"),"");
	if( str.Trim().GetLength() != 0 ){
		return FALSE;
	}
...
BEGIN_EVENTSINK_MAP(CShinFhyo60, CSyzBaseDlg)
ON_EVENT(CShinFhyo60, IDC_ICSDIAGCTRL1, 1, CShinFhyo60::EditONIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinFhyo60, IDC_ICSDIAGCTRL1, 2, CShinFhyo60::EditOFFIcsdiagctrl1, VTS_I2)
ON_EVENT(CShinFhyo60, IDC_ICSDIAGCTRL1, 3, CShinFhyo60::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinFhyo60, IDC_ICSDIAGCTRL1, 8, CShinFhyo60::ComboSelIcsdiagctrl1, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFhyo60::EditOFFIcsdiagctrl1(short index)
{
...
int CShinFhyo60::OnTerminationCallBack(short index, short nChar, short length, LPUNKNOWN data)
{
	...
	if( ((CH26HyoView*)m_pParent)->IsEndStart() ){
		return 0;
	}	
	...
	if( wTerm==VK_TAB ){
		...
		if( IsLeftEnd(index)&&(index==ID_UPPER_LEFT)&&(m_Lin!=0) ){ //左上端andNOT最初ページ
			F60DataSet(index);
			m_Fhyo60Diag.SetPosition(ID_LOWER_LEFT);
			m_Lin--;
			...
			m_Fhyo60Diag.SetPosition(pos);
		}
	}
	if((wTerm==VK_UP)||(wTerm==VK_F2)){
		if( IsComboBox(index)&&(wTerm!=VK_F2)){
			m_Fhyo60Diag.ComboDropDown( (short)index, TRUE );
			return 0;
		}
		pos = FocusMoveCheck(index , 1);
		if( IsLeftEnd(index)&&(index==ID_UPPER_LEFT)&&(m_Lin!=0) ){ //左上端andNOT最初ページ
	...
	if(wTerm==VK_RETURN){
		...
		if( IsRightEnd(index)&&(pos==index)&&(m_Lin!=SOZOKU_MAX-SOZOKU_PER_PAGE) ){ //右下端andNOT最終ページ
			F60DataSet(index);
			m_Fhyo60Diag.SetPosition(ID_UPPER_RIGHT);
			m_Lin++;
			m_bar.SetScrollPos(m_Lin);			//スクロールバーの移動
			scroll = TRUE;
	...
	if(wTerm==VK_RIGHT){
		if( IsRightEnd(index) ){
			...
			if( line < SOZOKU_MAX - 1){
				BOOL empty = MyNumber_IsHashEmpty( sozoku[line+1].Sn_6FSHASH, sizeof(sozoku[line+1].Sn_6FSHASH));
	...
	if( wTerm == VK_DELETE ){
		...
		switch(index){
			...
			case ID5133:
			case ID5134:
				sozoku[line].Sn_6FSNOFUKANPUsw &= ~0x01;
				memmove( sozoku[line].Sn_6FSNOFUZEI, F69[line], MONY_BUF_SIZE );
				DispFh6();
				break;
			case ID5141:
			case ID5142:
			case ID5143:
			case ID5144:
				sozoku[line].Sn_6FSNOFUKANPUsw &= ~0x02;
				memmove( sozoku[line].Sn_6FSNOFUCHIHO, F610[line], MONY_BUF_SIZE );
				DispFh6();
				break;
			case ID5071:
			case ID5072:
...
//2018.08.21 UPDATE END
			...
			sozoku[line].Sn_6FSJUSYO = tmp_str3;
	...
	if( IsNeedAllSel(pos) ){
		m_Fhyo60Diag.SetPosition(pos);
		m_Fhyo60Diag.SetCursorPos( pos, -1 );
	}

	int nowpos = m_Fhyo60Diag.GetPosition();	// 移動先または自身のポジション
...
//　　　　　str…編集対象文字列
////////////////////////////////////////////////////////////////////
void CShinFhyo60::Insert_kaigyo( int length, int line_max, CString *str )
{
...
//			false	：	全選択は不要
//-----------------------------------------------------------------------------
bool CShinFhyo60::IsNeedAllSel( int nID )
{
	bool bRt = false;
...
int CShinFhyo60::OnEditONCallBack(short index)
{
	if( ((CH26HyoView*)m_pParent)->IsEndStart() ){
		return 0;
	}
	...
	m_Fhyo60Diag.GetRect( index, (LPUNKNOWN)&crect_diag );
	this->GetWindowRect( &crect_window );
	m_bar.GetWindowRect( &crect_bar );
	if(( crect_window.bottom < 0 )||(crect_window.top < 0)
		||(crect_window.left < 0 )||(crect_window.right < 0 )){
...
int CShinFhyo60::OnEditOFFCallBack(short index)
{
	mynum_cursel = 0;
...
int CShinFhyo60::F60DataSet( int index )
{
	...
	DIAGRAM_DATA_clear( &DiagData );
	m_Fhyo60Diag.GetData( (int)index, (struct IUnknown *)&DiagData );
...
//			memmove(Sn_6FSDEATH, DiagData.data_day, sizeof(Sn_6FSDEATH));
			...
			Sn_6FSDEATH_GENGO = tmpGengo;
#ifdef _DBG_VIEW_START_
			hoge.Format("F60DataSet()_Sn_6FSDEATH_GENGO:%d\n", Sn_6FSDEATH_GENGO);
			DbgViewTrace(hoge);
#endif
			memmove( Sn_6FSDEATH, tmpCnvWaYmd, sizeof(Sn_6FSDEATH));
			redisp = TRUE;
		...
		case 	ID5013:
		case 	ID5014:
			sozoku[line_idx].Sn_6FSJUSYO = DiagData.data_edit;
			break;
		case	ID5021:
		case	ID5022:
		case	ID5023:
		case	ID5024:
			sozoku[line_idx].Sn_6FSFURI = DiagData.data_edit;
			break;
		case	ID5031:
		case	ID5032:
		case	ID5033:
		case	ID5034:
			sozoku[line_idx].Sn_6FSNAME = DiagData.data_edit;
			break;
		case	ID5041_1:
		case	ID5041_2:
		case	ID5041_3:
		...
		case	ID5044_3:
			...
			if( ret == 0 ){
				...
				sozoku[line_idx].Sn_6FSMYNUM1.Format(tmpMNAry.GetAt(0));
				sozoku[line_idx].Sn_6FSMYNUM2.Format(tmpMNAry.GetAt(1));
				sozoku[line_idx].Sn_6FSMYNUM3.Format(tmpMNAry.GetAt(2));
		...
		case	ID5053:	
		case	ID5054:
			sozoku[line_idx].Sn_6FSSYOKU = DiagData.data_edit;
			break;
		case	ID5061:	
		case	ID5062:	
		case	ID5063:	
		case	ID5064:	
			sozoku[line_idx].Sn_6FSTSUDUKI = DiagData.data_edit;
			break;		
		case	ID5071:	
		case	ID5072:	
		case	ID5073:	
		case	ID5074:	
			memmove( sozoku[line_idx].Sn_6FSBIRTH, DiagData.data_day, sizeof(sozoku[line_idx].Sn_6FSBIRTH) );
			redisp = TRUE;
			break;
		case	ID5081_1:
		case	ID5082_1:
		case	ID5083_1:
		case	ID5084_1:
			sozoku[line_idx].Sn_6FSTEL1 = DiagData.data_edit;
			break;		
		case	ID5081_2:
		case	ID5082_2:
		case	ID5083_2:
		case	ID5084_2:
			sozoku[line_idx].Sn_6FSTEL2 = DiagData.data_edit;
			break;		
		case	ID5081_3:
		case	ID5082_3:
		case	ID5083_3:
		case	ID5084_3:
			sozoku[line_idx].Sn_6FSTEL3 = DiagData.data_edit;
			break;		
		case	ID5091:
		case	ID5092:
		case	ID5093:
		case	ID5094:
			sozoku[line_idx].Sn_6FSHOTEI = DiagData.data_combo;
			break;
		case	ID5101:
		case	ID5102:
		case	ID5103:
		case	ID5104:
			if( atoi( DiagData.data_edit ) == 0 ){
				sozoku[line_idx].Sn_6FSBUNSHI.Empty();
			}else{
				// 頭の0をカット
				//sozoku[line_idx].Sn_6FSBUNSHI = DiagData.data_edit;
				sozoku[line_idx].Sn_6FSBUNSHI.Format ( _T( "%d" ), atoi ( DiagData.data_edit ) );
			}
			redisp = TRUE;
			break;
		case	ID5111:
		...
		case	ID5114:
			if( atoi( DiagData.data_edit ) == 0 ){
				sozoku[line_idx].Sn_6FSBUNBO.Empty();
			}else{
				// 頭の0をカット
				//sozoku[line_idx].Sn_6FSBUNBO = DiagData.data_edit;
				sozoku[line_idx].Sn_6FSBUNBO.Format ( _T( "%d" ), atoi ( DiagData.data_edit ) );
			}
			redisp = TRUE;
			break;
		case	ID5121:
		...
		case	ID5123:
		case	ID5124:
			memmove( sozoku[line_idx].Sn_6FSKAGAKU, DiagData.data_val, sizeof(sozoku[line_idx].Sn_6FSKAGAKU) );
			break;
		...
		case	ID5134:
			...
			if( sozoku[line_idx].Sn_6FSNOFUKANPUsw&0x01 ){
				cmp = m_pArith->l_cmp( sozoku[line_idx].Sn_6FSNOFUZEI, tmp );
			}else{
				cmp = m_pArith->l_cmp( F69[line_idx], tmp );
			}
			if( cmp != 0 ){
				sozoku[line_idx].Sn_6FSNOFUKANPUsw |= 0x01;
				ChangeColor( IDC_ICSDIAGCTRL1, index, 3 );
				memmove( sozoku[line_idx].Sn_6FSNOFUZEI, tmp, sizeof(sozoku[line_idx].Sn_6FSNOFUZEI) );
			}
			redisp = TRUE;
		...
		case	ID5144:
			...
			m_pArith->l_div( tmp, tmp, PW1 );
			m_pArith->l_mul( tmp, tmp, PW1 );
			if( sozoku[line_idx].Sn_6FSNOFUKANPUsw&0x02){
				cmp = m_pArith->l_cmp( sozoku[line_idx].Sn_6FSNOFUCHIHO, tmp );
			}else{
				cmp = m_pArith->l_cmp( F610[line_idx], tmp );
			}
			if( cmp != 0 ){
				sozoku[line_idx].Sn_6FSNOFUKANPUsw |= 0x02;
				ChangeColor( IDC_ICSDIAGCTRL1, index, 3 );
				memmove( sozoku[line_idx].Sn_6FSNOFUCHIHO, tmp, sizeof(sozoku[line_idx].Sn_6FSNOFUCHIHO) );
			}
			redisp = TRUE;
		...
		case	ID5163:
		case	ID5164:
			memmove( sozoku[line_idx].Sn_6FSKANPUZEI, DiagData.data_val, sizeof(sozoku[line_idx].Sn_6FSKANPUZEI) );
			redisp = TRUE;
			break;
		...
		case	ID5173:
		case	ID5174:
			memmove( sozoku[line_idx].Sn_6FSKANPUCHIHO, DiagData.data_val, sizeof(sozoku[line_idx].Sn_6FSKANPUCHIHO) );
			redisp = TRUE;
			break;
		...
		case	ID5193_1:
		case	ID5194_1:
			sozoku[line_idx].Sn_6FSGNAME = DiagData.data_edit;
			break;
		case	ID5191_2:
		case	ID5192_2:
		case	ID5193_2:
		case	ID5194_2:
			sozoku[line_idx].Sn_6FSGSEL = DiagData.data_combo;
			break;
		case	ID5201_1:
		case	ID5202_1:
		case	ID5203_1:
		case	ID5204_1:
			sozoku[line_idx].Sn_6FSSNAME = DiagData.data_edit;
			break;
		case	ID5201_2:
		case	ID5202_2:
		case	ID5203_2:
		case	ID5204_2:
			sozoku[line_idx].Sn_6FSSSEL = DiagData.data_combo;
			break;
		case	ID5211:
		case	ID5212:
		case	ID5213:
		case	ID5214:
			sozoku[line_idx].Sn_6FSYOKIN = DiagData.data_edit;
			break;
		case	ID5221:
		case	ID5222:
		case	ID5223:
		case	ID5224:
			sozoku[line_idx].Sn_6FSKOUZA = DiagData.data_edit;
			break;
		case	ID5231_1:
		case	ID5232_1:
		case	ID5233_1:
		case	ID5234_1:
			sozoku[line_idx].Sn_6FSKIGO1 = DiagData.data_edit;
			break;
		case	ID5231_2:
		case	ID5232_2:
		case	ID5233_2:
		case	ID5234_2:
			sozoku[line_idx].Sn_6FSKIGO2 = DiagData.data_edit;
			break;
		case	ID5241:
		case	ID5242:
		case	ID5243:
		case	ID5244:
			sozoku[line_idx].Sn_6FSYUBIN = DiagData.data_edit;
			break;
		default:
			break;
	}
...
////////////////////////////////////////////////////////////////////////////////////
int CShinFhyo60::GetLineNumber_FromIndex( int index, int sgn )
{
	int ret = -1;
...
BOOL CShinFhyo60::IsLeftEnd(int index)
{
	BOOL ret = FALSE;
...
BOOL CShinFhyo60::IsRightEnd(int index)
{
	BOOL ret = FALSE;
...
BOOL CShinFhyo60::IsComboBox(int index)
{
	BOOL ret = FALSE;
...
BOOL CShinFhyo60::IsMyNumberEdit(int index)
{
	BOOL ret = FALSE;
...
int CShinFhyo60::GetIndex_FromLine( int line_num )
{
	int ret = -1;
...
void CShinFhyo60::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CShinFhyo60::OnSize(UINT nType, int cx, int cy)
{
...
void CShinFhyo60::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: ここにメッセージ ハンドラ コードを追加するか、既定の処理を呼び出します。
	OnVScrollCallBack( nSBCode, nPos, pScrollBar);
}

int CShinFhyo60::OnVScrollCallBack(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	int focus = 0;
	if( m_Focus == 0 ){
		m_ctrl_mynum.MoveDiagMNEdit(MYNUM1_1,FALSE);
...
void CShinFhyo60::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CShinFhyo60::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	OnTerminationCallBack(index, nChar, length, data);
}

int CShinFhyo60::TerminationMyNumber(short index, short nChar, short length, LPUNKNOWN data)
{
			...
			case ID5044_1:
				if(( wTerm == VK_TAB )||( wTerm == VK_UP )||( wTerm == VK_F2 )){
					...
					if( IsLeftEnd(index)&&(m_Lin!=0) ){
						...
						int line = GetLineNumber_FromIndex(index);
						if( MyNumber_IsHashEmpty(sozoku[line].Sn_6FSHASH, sizeof(sozoku[line].Sn_6FSHASH)) ){
							pos = index;
						}else{
			...
			case ID5044_3:
				if( wTerm == VK_UP ){
					...
					if( IsRightEnd(index)&&(m_Lin+SOZOKU_PER_PAGE<=SOZOKU_MAX-1) ){
						...
						int line = GetLineNumber_FromIndex(index);
						sozoku[line].Sn_6FSMYNUM1.Format("%s", str.Left(4) );
						sozoku[line].Sn_6FSMYNUM2.Format("%s", str.Mid(4, 4) );
						sozoku[line].Sn_6FSMYNUM3.Format("%s", str.Right(4) );
...
void CShinFhyo60::EditONIcsdiagctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
// 終了処理
void CShinFhyo60::EndJob()
{
	NTBMOD8_EndProc();
}

void CShinFhyo60::ComboSelIcsdiagctrl1(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
}

void CShinFhyo60::Init_sozoku( int index )
{
//	sozoku[index].Sn_SEQ = -1;
	sozoku[index].Sn_NO = -1;
	//sozoku[index].Sn_NO = index;
	sozoku[index].Sn_6FSJUSYO.Empty();
	sozoku[index].Sn_6FSFURI.Empty();
	sozoku[index].Sn_6FSNAME.Empty();
	//sozoku[index].Sn_6FSHASH.Empty();
	memset( sozoku[index].Sn_6FSHASH, '\0', sizeof( sozoku[index].Sn_6FSHASH ));
	sozoku[index].Sn_6FSMYNUM1.Empty();
	sozoku[index].Sn_6FSMYNUM2.Empty();
	sozoku[index].Sn_6FSMYNUM3.Empty();
	sozoku[index].Sn_6FSSYOKU.Empty();
	sozoku[index].Sn_6FSTSUDUKI.Empty();
	memset( sozoku[index].Sn_6FSBIRTH, '\0', sizeof( sozoku[index].Sn_6FSBIRTH ));
	sozoku[index].Sn_6FSTEL1.Empty();
	sozoku[index].Sn_6FSTEL2.Empty();
	sozoku[index].Sn_6FSTEL3.Empty();
	sozoku[index].Sn_6FSHOTEI = 0;
	sozoku[index].Sn_6FSBUNSHI.Empty();
	sozoku[index].Sn_6FSBUNBO.Empty();
	//m_pArith->l_clear( sozoku[index].Sn_6FSKAGAKU );
	memset( sozoku[index].Sn_6FSKAGAKU, '\0', sizeof( sozoku[index].Sn_6FSKAGAKU ));
	memset( sozoku[index].Sn_6FSNOFUZEI, '\0', sizeof( sozoku[index].Sn_6FSNOFUZEI ));
	memset( sozoku[index].Sn_6FSNOFUCHIHO, '\0', sizeof( sozoku[index].Sn_6FSNOFUCHIHO ));
	memset( sozoku[index].Sn_6FSKANPUZEI, '\0', sizeof( sozoku[index].Sn_6FSKANPUZEI ));
	memset( sozoku[index].Sn_6FSKANPUCHIHO, '\0', sizeof( sozoku[index].Sn_6FSKANPUCHIHO ));
	sozoku[index].Sn_6FSNOFUKANPUsw = 0;
	sozoku[index].Sn_6FSGNAME.Empty();
	sozoku[index].Sn_6FSGSEL = 0;
	sozoku[index].Sn_6FSSNAME.Empty();
	sozoku[index].Sn_6FSSSEL = 0;
	sozoku[index].Sn_6FSYOKIN.Empty();
	sozoku[index].Sn_6FSKOUZA.Empty();
	sozoku[index].Sn_6FSKIGO1.Empty();
	sozoku[index].Sn_6FSKIGO2.Empty();
	sozoku[index].Sn_6FSYUBIN.Empty();
	
	return;
}

// マイナンバー更新(廃棄・削除・追加)を一括で
int CShinFhyo60::MyNumber_Update()
{
...
// 削除履歴書き込み
int CShinFhyo60::MyNumber_DeleteHistory()
{
		...
		if(( ret == 0 )||( ret == 1 )){
			...
			if( ret == 1 ){
				for( int idx2 = 0; idx2 < SOZOKU_MAX; idx2++ ){
					if( mynum.Compare(sozoku[idx2].Sn_6FSMYNUM1+sozoku[idx2].Sn_6FSMYNUM2+sozoku[idx2].Sn_6FSMYNUM3) == 0 ){
						//削除後再度入力→履歴は残さない
						ReEnter = TRUE;
						break;
					}
...
// マイナンバー廃棄
int CShinFhyo60::MyNumber_Haiki()
{
...
// マイナンバー更新
int CShinFhyo60::MyNumber_Write()
{

	for( int idx = 0; idx < SOZOKU_MAX; idx++ ){
		CString my_num;
		my_num = sozoku[idx].Sn_6FSMYNUM1+sozoku[idx].Sn_6FSMYNUM2+sozoku[idx].Sn_6FSMYNUM3;
		...
		NOHISTTBL	nhtbl;
		NTBMOD8_InitNoHistoryTbl( 0, &nhtbl );
		wsprintf( nhtbl.tgt_char, "%s", sozoku[idx].Sn_6FSNAME );
...
//2018.08.21 INSERT END
		...
		if( ret == 0 ){ //該当の個人番号あり
			memmove( sozoku[idx].Sn_6FSHASH, ndtbl.arrow, sizeof(sozoku[idx].Sn_6FSHASH) );

			if( strlen( ndtbl.nam2 ) == 0 ){
				nhtbl.user_play = NTBMOD_USER_PLAY_UPDATE|NTBMOD_USER_PLAY_APPLI;
				wsprintf( ndtbl.nam2, "%s", sozoku[idx].Sn_6FSNAME );
		...
		wsprintf( ndtbl.nam2, "%s", sozoku[idx].Sn_6FSNAME );	//氏名
...
void CShinFhyo60::MyNumber_DelHaiki_Clear()
{
	for(int cnt = 0; cnt < SOZOKU_MAX; cnt++){
...
void CShinFhyo60::MyNumber_Change()
{
	...
	for( int idx = 0; idx < SOZOKU_MAX; idx++ ){
		...
		memmove( ndtbl.arrow, sozoku[idx].Sn_6FSHASH, sizeof( ndtbl.arrow ));
		...
		if( NTBMOD8_GetNoDataTbl( 0, &ndtbl ) == 0 ){
			...
			if( MynoAccessPerm() == TRUE ){
				sozoku[idx].Sn_6FSMYNUM1.Format("%s", mynum_tmp.Left(4) );
				sozoku[idx].Sn_6FSMYNUM2.Format("%s", mynum_tmp.Mid(4, 4) );
				sozoku[idx].Sn_6FSMYNUM3.Format("%s", mynum_tmp.Right(4) );
			}else{
				sozoku[idx].Sn_6FSMYNUM1.Format("****");
				sozoku[idx].Sn_6FSMYNUM2.Format("****");
				sozoku[idx].Sn_6FSMYNUM3.Format("****");
			}
		}
	}
}
void CShinFhyo60::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
...
////////////////////////////////////////////////////////////////
int CShinFhyo60::MynumDel( int line )
{
	// 上手くん環境なら不可
	if( GET_EntrySign() != 0 ){
		return -1;
	...
	if( MyNumber_IsHashEmpty( sozoku[m_Lin+line].Sn_6FSHASH, sizeof(sozoku[m_Lin+line].Sn_6FSHASH))){
		return -1;
	}

	NODATATBL ndtbl = {0};
	memmove( ndtbl.arrow, sozoku[m_Lin+line].Sn_6FSHASH, sizeof(ndtbl.arrow));
	...
	if( sgn == RET_NODEL_H ){ //削除　ハッシュのみ削除
		memmove( m_MyNum_Delete[m_MyNum_Delete_idx], sozoku[m_Lin+line].Sn_6FSHASH, sizeof(m_MyNum_Delete[m_MyNum_Haiki_idx]) );
		m_MyNum_Delete_idx++;
		m_MyNum_Delete_Shimei.Add( sozoku[m_Lin+line].Sn_6FSNAME );
	}else
	if( sgn == RET_NODEL_DB ){ //廃棄　個人番号削除
		memmove( m_MyNum_Haiki[m_MyNum_Haiki_idx], sozoku[m_Lin+line].Sn_6FSHASH, sizeof(m_MyNum_Delete[m_MyNum_Haiki_idx]) );
		m_MyNum_Haiki_idx++;
		m_MyNum_Haiki_Shimei.Add( sozoku[m_Lin+line].Sn_6FSNAME );
	}
	memset( sozoku[m_Lin+line].Sn_6FSHASH, '\0', sizeof(sozoku[m_Lin+line].Sn_6FSHASH));
	sozoku[m_Lin+line].Sn_6FSMYNUM1.Empty();
	sozoku[m_Lin+line].Sn_6FSMYNUM2.Empty();
	sozoku[m_Lin+line].Sn_6FSMYNUM3.Empty();
...
void CShinFhyo60::OnBnClickedMynumDel1()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	MynumDel(0);
}
void CShinFhyo60::OnBnClickedMynumDel2()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	MynumDel(1);
}

void CShinFhyo60::OnBnClickedMynumDel3()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	MynumDel(2);
}

void CShinFhyo60::OnBnClickedMynumDel4()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	MynumDel(3);
}

HBRUSH CShinFhyo60::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CShinFhyo60::DIAGRAM_DATA_clear( DIAGRAM_DATA *DiagData )
{
...
void CShinFhyo60::OnBnClickedSozokuDel()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	int index = GetLineNumber_FromIndex( m_Focus );
	if( index == -1 ){
	...
	BOOL empty = MyNumber_IsHashEmpty( sozoku[index].Sn_6FSHASH, sizeof(sozoku[index].Sn_6FSHASH) );
	...
	m_Fhyo60Diag.SetPosition(m_Focus);

}

void CShinFhyo60::OnBnClickedSozokuIns()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	int index = GetLineNumber_FromIndex( m_Focus );
	if( index == -1 ){
