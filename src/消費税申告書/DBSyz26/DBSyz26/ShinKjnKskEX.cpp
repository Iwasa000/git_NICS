...
#include "ShinKjnKskEXIdx.h"

// CShinKjnKskEX ダイアログ

IMPLEMENT_DYNAMIC(CShinKjnKskEX, CSyzBaseDlg)

CShinKjnKskEX::CShinKjnKskEX(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKjnKskEX::IDD, pParent)
	, m_pSnHeadData(NULL)
	, m_pSnKskData(NULL)
	, m_pArith(NULL)
	, m_pShinInfo(NULL)
...
CShinKjnKskEX::~CShinKjnKskEX()
{
}

void CShinKjnKskEX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKjnKskEX, CSyzBaseDlg)
	ON_COMMAND(ID_BUTTON_F2, &CShinKjnKskEX::OnButtonF2)
	ON_WM_HSCROLL()
	ON_WM_SHOWWINDOW()
	ON_WM_VSCROLL()
	ON_WM_SIZE()
...
// CShinKjnKskEX メッセージ ハンドラ

BOOL CShinKjnKskEX::OnInitDialog()
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinKjnKskEX::InitInfo( CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CH26SnKskData **pSnKskData )
{
	ASSERT( pSnHeadData );
	if( pSnHeadData == NULL ){
		return -1;
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinKjnKskEX::Init()
{
	InitDiagAttr();

	//[Y] 平成23年10月2日より前の場合に許可
	int		gengo=0;
	char	yymmdd[3]={0};
	(*m_pSnHeadData)->GetYmdDataGen( ID_ICSSH_KAZEIKIKAN_FROM, &yymmdd[0], &yymmdd[1], &yymmdd[2], &gengo );
	if( (gengo==4/*平成*/) && (m_pArith->l_jsgn(yymmdd, AMENDK, 3)<0) ){
		DIAGRAM_DATA	data;
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinKjnKskEX::InitDiagAttr()
{
...
//2016.02.22 DELETE END
		...
		diag_index[0][0] = KJNKSK_NUM_KOJIN1;		
		diag_index[0][1] = KJNKSK_NUM_KOJIN2;
		diag_index[0][2] = KJNKSK_NUM_KOJIN3;
		diag_index[0][3] = -1;

		nKojinDispModify = DIAG_MDFY_OPAQUE|DIAG_MDFY_READONLY;

		//個人番号
		for( int idx_cnt=KJNKSK_NUM_KOJIN1; idx_cnt<=KJNKSK_NUM_KOJINHYPHEN2; idx_cnt++ ){
...
//2015.12.08 INSERT END
	...
	(*m_pSnKskData)->CalqBillVal( (*m_pSnHeadData) );	// 経過措置あり→なしに変更した場合に再計算が必要なため。
	DispKsiKjn( 0 );
	...
	while( 1 ){
		...
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			...
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				if( (KjnKsk2[cnt].Index == 71) || (KjnKsk2[cnt].Index == 72) ){
	...
	int		gengo=0;
	char	yymmdd[3]={0};
	(*m_pSnHeadData)->GetYmdDataGen( ID_ICSSH_KAZEIKIKAN_FROM, &yymmdd[0], &yymmdd[1], &yymmdd[2], &gengo );//[Y]
	if( (gengo==4/*平成*/) && (m_pArith->l_jsgn(yymmdd, AMENDK, 3)<0) ){
		cnt = 0;
		while( 1 ){
...
/*	if( isReiwaPrnt ){
		...
		diag_setdata(IDC_ICSDIAGCTRL10, DLG1TDG, &DiagData, CIcsdiagctrl);
	}
	else{
		// 提出年月日
		m_Util.DiagOcxIniz(&DiagData);
...
*/
	...
	if( isReiwaPrnt ){
		...
		if ( ( *m_pSnKskData )->KsData.KSdata[0] == ID_ICSSH_HEISEI_GENGO ) {
			sprintf_s ( buf, sizeof ( buf ), _T( "%s" ), SYZ26_HEISEI_G_STR);
		}
		else {
			sprintf_s ( buf, sizeof ( buf ), _T( "%s" ), SYZ26_REIWA_G_STR);
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjn( int mode/*=0*/ )
{
...
//2015.10.20 INSERT END
	...
	for( i = DLG1SDT; i <= DLG1EDT; i++ ){
		...
		//if ( isReiwaPrnt ) {
		//	sprintf_s(tmpbuf, sizeof(tmpbuf), _T("%s"), SYZ26_REIWA_G_STR);
		//}
		//else{
			if ( gengo == ID_ICSSH_REIWA_GENGO ) {
				...
				sprintf_s(tmpbuf, sizeof(tmpbuf), _T("%s"), SYZ26_HEISEI_G_STR);
			}
		//}
...
// データ表示
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjnDig1()
{
	DIAGRAM_DATA	data;
	int		i, ln;
	char	buf[128];
...
//2015.12.08 INSERT START
			...
			//		m_ctrl_mynum.MoveDiagMNEdit(KJNKSK_NUM_KOJIN1);
...
// データ表示
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjnDig2()
{
	DIAGRAM_DATA	data;
	int		i, idx;
...
// データ表示
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjnDig3()
{
	DIAGRAM_DATA	data;
	int		i;
...
// データ表示
//-----------------------------------------------------------------------------
void CShinKjnKskEX::DispKsiKjnDigZi()
{
	ASSERT( m_pShinInfo );
...
// 返送値
//-----------------------------------------------------------------------------
int	CShinKjnKskEX::DataSetDig1(long index, short leng, DIAGRAM_DATA *data, int mode )
{
	if( index < 0 ){
		return 0;
	}
...
//				memmove( (*m_pSnHeadData)->Sn_TELNO1, data->data_edit, 6 );
				break;
			case DLG1_TL2:	// 電話番号
				Length = (int) data->data_edit.GetLength();
...
//				memmove( &(*m_pSnKskData)->KsData.KSdata[1], data->data_day, 3 );
				...
				if ( ( ( tmpWaYmd[0] != 0x00 ) && ( tmpWaYmd[1] != 0x00 ) && ( tmpWaYmd[2] != 0x00 ) ) &&
					 ( ( tmpWaYmd[0] != 0xff ) && ( tmpWaYmd[1] != 0xff ) && ( tmpWaYmd[2] != 0xff ) ) ) {
					if( isReiwaPrnt ){
						tmpGengo = ID_ICSSH_REIWA_GENGO;
						memmove(tmpCnvWaYmd, tmpWaYmd, sizeof(tmpWaYmd));
...
// 返送値	
//-----------------------------------------------------------------------------
int	CShinKjnKskEX::DataSetDig2( long index, short leng, DIAGRAM_DATA *data, int mode )
{
	if( index < 0 ){
		return 0;
	}
...
// 返送値	
//-----------------------------------------------------------------------------
int	CShinKjnKskEX::DataSetDig3( long index, short leng, DIAGRAM_DATA *data, int mode )
{
	if( index < 0 ){
		return 0;
	}
...
// 返送値
//-----------------------------------------------------------------------------
int CShinKjnKskEX::GetTaxationOffice()
{
	DIAGRAM_DATA	dgda;
...
//		mode	：
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag1( short index, UINT flg, short nChar, int mode/*=0*/ )
{
	short	xpos = index;
	int		sw = 0, cmb = 0;
...
//		mode	：	
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag2( short index, UINT flg, short nChar, int mode/*=0*/ )
{
	short	xpos = index;
	int		sw = 0;
...
//		mode	：
//-----------------------------------------------------------------------------
int CShinKjnKskEX::MoveCurselDiag3( short index, UINT flg, short nChar, int mode/*=0*/ )
{
	short	xpos = index;
	int		sw = 0, cmb = 0;
	...
	if( sw == 1 ){
		int		gengo=0;
		char	yymmdd[3]={0};
		(*m_pSnHeadData)->GetYmdDataGen( ID_ICSSH_KESSAN_FROM, &yymmdd[0], &yymmdd[1], &yymmdd[2], &gengo );
		if( (gengo==4/*平成*/) && (m_pArith->l_jsgn(yymmdd, AMENDK, 3)<0) ){
			CRect	rc;
...
BEGIN_EVENTSINK_MAP(CShinKjnKskEX, CSyzBaseDlg)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL10, 3, CShinKjnKskEX::TerminationIcsdiagctrl10, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL10, 1, CShinKjnKskEX::EditONIcsdiagctrl10, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL10, 2, CShinKjnKskEX::EditOFFIcsdiagctrl10, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL10, 8, CShinKjnKskEX::ComboSelIcsdiagctrl10, VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL20, 3, CShinKjnKskEX::TerminationIcsdiagctrl20, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL20, 1, CShinKjnKskEX::EditONIcsdiagctrl20, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL20, 2, CShinKjnKskEX::EditOFFIcsdiagctrl20, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL30, 3, CShinKjnKskEX::TerminationIcsdiagctrl30, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL30, 1, CShinKjnKskEX::EditONIcsdiagctrl30, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL30, 2, CShinKjnKskEX::EditOFFIcsdiagctrl30, VTS_I2)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL30, 8, CShinKjnKskEX::ComboSelIcsdiagctrl30, VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL40, 8, CShinKjnKskEX::ComboSelIcsdiagctrl40, VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL40, 3, CShinKjnKskEX::TerminationIcsdiagctrl40, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKjnKskEX, IDC_ICSDIAGCTRL40, 1, CShinKjnKskEX::EditONIcsdiagctrl40, VTS_I2)
END_EVENTSINK_MAP()

void CShinKjnKskEX::TerminationIcsdiagctrl10(short index, short nChar, short length, LPUNKNOWN data)
{
...
void CShinKjnKskEX::EditONIcsdiagctrl10(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::EditOFFIcsdiagctrl10(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::ComboSelIcsdiagctrl10(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::TerminationIcsdiagctrl20(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinKjnKskEX::EditONIcsdiagctrl20(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::EditOFFIcsdiagctrl20(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::TerminationIcsdiagctrl30(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinKjnKskEX::EditONIcsdiagctrl30(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::EditOFFIcsdiagctrl30(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::ComboSelIcsdiagctrl30(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::ComboSelIcsdiagctrl40(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
	...
	diag_getdata( IDC_ICSDIAGCTRL40, DLG0CMB, &dataCombo, CIcsdiagctrl );
...
void CShinKjnKskEX::TerminationIcsdiagctrl40(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
//		index	：
//-----------------------------------------------------------------------------
void CShinKjnKskEX::ChkScroll( WORD id, short index )
{
	RECT	drect, wrect;
	long	sz, cnt;
	int		bpos;
...
BOOL CShinKjnKskEX::PreTranslateMessage(MSG* pMsg)
{
...
// 初期化（データクリア）
//-----------------------------------------------------------------------------
void CShinKjnKskEX::ClearKskKjn()
{
	CString	str;
	int		ans;
...
//-----------------------------------------------------------------------------
void CShinKjnKskEX::SetItemPosition( int IndexNo )
{
	int	Index;
	if( IndexNo ){
		Index = IndexNo;
...
void CShinKjnKskEX::EditONIcsdiagctrl40(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( m_IsStartEnd ){
		return;
...
void CShinKjnKskEX::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CShinKjnKskEX::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CShinKjnKskEX::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: ここにメッセージ ハンドラ コードを追加するか、既定の処理を呼び出します。
	MoveDiagMNEdit();
...
void CShinKjnKskEX::OnSize(UINT nType, int cx, int cy)
{
...
void CShinKjnKskEX::MoveDiagMNEdit()
{
	if( m_pParent && m_pParent->GetSafeHwnd() ){
//2016.02.22 DELETE START
//		if( ((CH26HyoView*)m_pParent)->GetShRevType() >= ID_ICSSH_MYNUMBER_REVTYPE ){
//2016.02.22 DELETE END
			m_ctrl_mynum.MoveDiagMNEdit( KJNKSK_NUM_KOJIN1,FALSE );
...
//-----------------------------------------------------------------------------
void CShinKjnKskEX::SetKanpuKinyuInput()
{
	// 確定時は全入力不可
	if( ((*m_pSnHeadData)->Sn_Sign4&0x01) ){
		return;
...
//-----------------------------------------------------------------------------
void CShinKjnKskEX::MoveFocusInEnd()
{
	// データを確定させる為にフォーカス移動
