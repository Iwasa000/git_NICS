// ShinIppan.cpp : 実装ファイル
//

#include "stdafx.h"
#include "ShinIppan.h"
#include "ShinIppanIdx.h"
...
IMPLEMENT_DYNAMIC(CShinIppan, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinIppan::CShinIppan(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinIppan::IDD, pParent)
	, m_kanisw(0)
	, m_DpSw(0)
	, m_M1c(0)
	, m_pParent(pParent)
...
CShinIppan::~CShinIppan()
{
}

void CShinIppan::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinIppan, CSyzBaseDlg)
	ON_WM_CTLCOLOR()
	ON_WM_VSCROLL()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BUTTON1, &CShinIppan::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinIppan メッセージ ハンドラ

BOOL CShinIppan::OnInitDialog()
{
...
BOOL CShinIppan::PreTranslateMessage(MSG* pMsg)
{
...
HBRUSH CShinIppan::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CShinIppan::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CShinIppan::OnSize(UINT nType, int cx, int cy)
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinIppan::InitInfo( CH26SnHonpyoData **pSnHonpyoData, CH26SnFhyo10Data **pSnFhyo10Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo )
{
...
void CShinIppan::DispIppanUpper()
{
	DIAGRAM_ATTRIBUTE	DA;
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinIppan::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
...
//		Mony	：	表示金額
//-----------------------------------------------------------------------------
void CShinIppan::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	char			M1000[MONY_BUF_SIZE] = {0};
...
// 終了処理
//-----------------------------------------------------------------------------
void CShinIppan::EndJob()
{
}

BEGIN_EVENTSINK_MAP(CShinIppan, CSyzBaseDlg)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL1, 2, CShinIppan::EditOFFIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL2, 2, CShinIppan::EditOFFIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL3, 2, CShinIppan::EditOFFIcsdiagctrl3, VTS_I2)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL1, 3, CShinIppan::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL2, 3, CShinIppan::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL3, 3, CShinIppan::TerminationIcsdiagctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL1, 1, CShinIppan::EditONIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL2, 1, CShinIppan::EditONIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinIppan, IDC_ICSDIAGCTRL3, 1, CShinIppan::EditONIcsdiagctrl3, VTS_I2)
END_EVENTSINK_MAP()

void CShinIppan::EditOFFIcsdiagctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinIppan::EditOFFIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA	DiagData;
...
void CShinIppan::EditOFFIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA	DiagData;
	int				Length;
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinIppan::SetItemPosition()
{
	int index;
...
void CShinIppan::OnBnClickedButton1()
{
...
void CShinIppan::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinIppan::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinIppan::TerminationIcsdiagctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinIppan::EditONIcsdiagctrl1(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinIppan::EditONIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
void CShinIppan::EditONIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinIppan::ChangeColor( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
//-----------------------------------------------------------------------------
void CShinIppan::ShiftHonpyoFocus()
{
...
//-----------------------------------------------------------------------------
void CShinIppan::SetTaxOfficeInput()
{
	// 確定時は全入力不可
	if( ((*m_pSnHeadData)->Sn_Sign4&0x01) ){
		return;
...
// 入力位置を保存 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinIppan::SaveInputPosition()
{
	m_curInPos = -1;
...
// 入力位置を復元 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinIppan::RestoreInputPosition()
{
	if( m_curInPos == -1 ){
		return;
	}
...
//-----------------------------------------------------------------------------
void CShinIppan::MoveFocusInEnd()
{
	// データを確定させる為にフォーカス移動
	if( m_curwnd == IDC_ICSDIAGCTRL1 ){
...
//-----------------------------------------------------------------------------
void CShinIppan::SetKanpuKinyuInput()
{
	// 確定時は全入力不可
	if( ((*m_pSnHeadData)->Sn_Sign4&0x01) ){
		return;
