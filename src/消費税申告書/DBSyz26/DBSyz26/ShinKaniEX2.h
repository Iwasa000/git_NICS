...
class CShinKaniEX2 : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinKaniEX2)

public:
	CShinKaniEX2(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinKaniEX2();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_KANI_EX2 };

		// 初期情報のセット
	int InitInfo( CH28SnHonpyoData **pSnHonpyoData, CH27SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo );
	// 初期化
	int Init();
	// ダイアグラム初期属性セット
	int InitDiagAttr();
...
//== 帳票データ ==
	...
	//== 帳票データ本体 ==
	CH28SnHonpyoData	**m_pSnHonpyoData;
	CH27SnFhyo10Data	**m_pSnFhyo40Data;		// 付表４データ
