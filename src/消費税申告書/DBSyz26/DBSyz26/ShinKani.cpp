...
IMPLEMENT_DYNAMIC(CShinKani, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinKani::CShinKani(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKani::IDD, pParent)
	, m_pParent(pParent)
...
void CShinKani::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKani, CSyzBaseDlg)
	ON_BN_CLICKED(IDC_BUTTON1, &CShinKani::OnBnClickedButton1)
END_MESSAGE_MAP()


// CShinKani メッセージ ハンドラ

BOOL CShinKani::OnInitDialog()
{
...
BOOL CShinKani::PreTranslateMessage(MSG* pMsg)
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinKani::InitInfo( CH26SnHonpyoData **pSnHonpyoData, CH26SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo )
{
...
// 初期データ表示
//-----------------------------------------------------------------------------
void CShinKani::DispInitKaniUpper()
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	char			buf[512] = {0};
...
//		Mony	：	表示金額
//-----------------------------------------------------------------------------
void CShinKani::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	char			MNY[MONY_BUF_SIZE] = {0};
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinKani::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
...
// 終了処理
//-----------------------------------------------------------------------------
void CShinKani::EndJob()
{
	// エディット中だと正しく終了しない為
}

BEGIN_EVENTSINK_MAP(CShinKani, CSyzBaseDlg)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL2, 1, CShinKani::EditONIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL2, 2, CShinKani::EditOFFIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL2, 3, CShinKani::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL3, 1, CShinKani::EditONIcsdiagctrl3, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL3, 2, CShinKani::EditOFFIcsdiagctrl3, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL3, 3, CShinKani::TerminationIcsdiagctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL4, 1, CShinKani::EditONIcsdiagctrl4, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL4, 2, CShinKani::EditOFFIcsdiagctrl4, VTS_I2)
	ON_EVENT(CShinKani, IDC_ICSDIAGCTRL4, 3, CShinKani::TerminationIcsdiagctrl4, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinKani::EditONIcsdiagctrl2(short index)
{
...
void CShinKani::EditOFFIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKani::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
//	char	Str[128] = {0};
	char buff[64] = {0};
...
void CShinKani::EditONIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
void CShinKani::EditOFFIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKani::TerminationIcsdiagctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKani::EditONIcsdiagctrl4(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
void CShinKani::EditOFFIcsdiagctrl4(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKani::TerminationIcsdiagctrl4(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinKani::OnBnClickedButton1()
{
...
//-----------------------------------------------------------------------------
//Kanisyo_calq
void CShinKani::calq_by_OtherChange( int ksw )
{
	m_DpSw = -1;
	m_kanisw = ksw;
	ReadData();
...
// 引数	no	：	
//-----------------------------------------------------------------------------
void CShinKani::SetItemPosition(int no )
{
	int index;
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinKani::ChangeColor( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
//-----------------------------------------------------------------------------
void CShinKani::SetTaxOfficeInput()
{
	if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
		return;
	}
...
// 入力位置を保存 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinKani::SaveInputPosition()
{
	m_curInPos = -1;
...
// 入力位置を復元 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinKani::RestoreInputPosition()
{
	if( m_curInPos == -1 ){
		return;
	}
...
//-----------------------------------------------------------------------------
// 還付金融機関入力にフォーカスセット('15.02.03)
//-----------------------------------------------------------------------------
void CShinKani::SetKanpuKinyuInput()
{
	if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
		return;
	}
