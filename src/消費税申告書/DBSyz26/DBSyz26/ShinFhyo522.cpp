...
IMPLEMENT_DYNAMIC(CShinFhyo522, CSyzBaseDlg)

CShinFhyo522::CShinFhyo522(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinFhyo522::IDD, pParent)
...
CShinFhyo522::~CShinFhyo522()
{
}

void CShinFhyo522::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo522, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinFhyo522 メッセージ ハンドラ

BOOL CShinFhyo522::OnInitDialog()
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo522::InitInfo( CH26SnFhyo50Data **pSnFhyo52Data, CH26SnFhyo10Data **pSnFhyo40Data, CH26SnHonpyoData **pSnHonpyoData, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CShinFhyo52 *pShinFhyo52, CDBNpSub *pZmSub )
{
	...
	SH_52_LINKED_INFO	LinkedInfo = {0};
	...
	LinkedInfo.pF5218T = &pF5218T;
	LinkedInfo.pF5218A = &pF5218A;
	LinkedInfo.pF5218B = &pF5218B;
	LinkedInfo.pF5218C = &pF5218C;
	LinkedInfo.pF5218c = &pF5218c;
	LinkedInfo.pF5219T = &pF5219T;
	LinkedInfo.pF5219A = &pF5219A;
	LinkedInfo.pF5219B = &pF5219B;
	LinkedInfo.pF5219C = &pF5219C;
	LinkedInfo.pF5219c = &pF5219c;
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFhyo522::ReadData( int upck )
{
...
// 画面出力（非連動）
//-----------------------------------------------------------------------------
void CShinFhyo522::DispFh522_Unconnect()
{
	if( (*m_ptypeCnt) < 2 ){
		*m_pSzsw = 0;
	}
	...
	Arith.l_print( moji, pF5220A, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, pF5220B, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, pF5220C, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, pF5220T, _T("sss,sss,sss,sss,ss9") );
#endif

	// イ 原則計算
	dsp_prs( IDC_ICSDIAGCTRL1, ID5218A, pF5218A );
	dsp_prs( IDC_ICSDIAGCTRL1, ID5218B, pF5218B );
	dsp_prs( IDC_ICSDIAGCTRL1, ID5218C, pF5218C );
	dsp_prs( IDC_ICSDIAGCTRL1, ID5218T, pF5218T );

	// ロ-イ 特例計算
	for( int i=ID5219A; i<=ID5219T; i++ ){
...
//		Mony	：	
//-----------------------------------------------------------------------------
void CShinFhyo522::dsp_prs( unsigned short Id, short Index, char *Mony )
{
...
//		Index	：	
//-----------------------------------------------------------------------------
void CShinFhyo522::dsp_cls( unsigned short Id, short Index )
{
	if( !m_DpSw ){	// 再集計のみ（表示無し）
...
// 返送値
//-----------------------------------------------------------------------------
int CShinFhyo522::IsProvideEditCell( short Idx )
{
	int	rt = 0;
...
//		}
//---------------
		if( (ID5218A<=Idx) && (Idx<=ID5218C) ){
			rt = 0;
		}
		else if( (Idx==ID5218T) && !isAddtype ){
			rt = 0;
		}
//---------------
	}
...
//			}
//---------------
			if( (ID5219A<=Idx) && (Idx<=ID5219C) ){
				rt = 0;
			}
			else if( (Idx==ID5219T) && !isAddtype ){
				rt = 0;
			}
//---------------
		}
...
BEGIN_EVENTSINK_MAP(CShinFhyo522, CSyzBaseDlg)
	ON_EVENT(CShinFhyo522, IDC_ICSDIAGCTRL1, 1, CShinFhyo522::EditONIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinFhyo522, IDC_ICSDIAGCTRL1, 2, CShinFhyo522::EditOFFIcsdiagctrl1, VTS_I2)
	ON_EVENT(CShinFhyo522, IDC_ICSDIAGCTRL1, 3, CShinFhyo522::TerminationIcsdiagctrl1, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFhyo522::EditONIcsdiagctrl1(short index)
{
...
void CShinFhyo522::EditOFFIcsdiagctrl1( short index )
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//--> '14.10.17 INS START
		case ID5218T :	// 18. 特例適用･合計 仕入税額(期間計)
			if( GetRelInfoByDiagIdx(index, &ptmpMony) ){
				break;
			}
			if( m_IsDeleteKeyIn ){
...
/*		case ID5219T :	// 19. 特例適用･合計 仕入税額(期間計)
		...
		case ID5219A :	// 19. 特例適用･合計 仕入税額(4%分)	
		case ID5219B :
		case ID5219C :
...
//-----------------------------------------------------------------------------
void CShinFhyo522::TrunOnInputSign( short index )
{
	int	*pTmpSw = NULL;
	int	leftMove = 0;
...
void CShinFhyo522::TrunOnInputSign( short index )
{
	int	*pTmpSw = NULL;
	int	leftMove = 0;
	if( (index==ID5218A) || (index==ID5218B) || (index==ID5218C) || (index==ID5218T) ){
		pTmpSw = pF5218c;
		leftMove = index - ID5218A;
	}
	else if( (index==ID5219A) || (index==ID5219B) || (index==ID5219C) || (index==ID5219T) ){
		pTmpSw = pF5219c;
		leftMove = index - ID5219A;
	}
	else if( (index==ID5220A) || (index==ID5220B) || (index==ID5220C) || (index==ID5220T) ){
		pTmpSw = pF5220c;
		leftMove = index - ID5220A;
	}
...
void CShinFhyo522::TerminationIcsdiagctrl1(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
//<-- '14.06.10 INS END
		...
		if( (ID5218A<=index) && (index<=ID5229T) ){
//-- '14.10.17 --
//			if( (index==ID5218A) || (index==ID5218B) || (index==ID5218C) ){
//---------------
			if( (index==ID5218A) || (index==ID5218B) || (index==ID5218C) || (index==ID5218T) ){
...
//			-1	：	該当なし
//-----------------------------------------------------------------------------
int CShinFhyo522::GetRelInfoByDiagIdx( short Idx, char **pRtMony )
{
	if( Idx == ID5218A ){
		*pRtMony = pF5218A;
	}
...
//		sign	：
//-----------------------------------------------------------------------------
void CShinFhyo522::ChangeColor( unsigned short id, short index, int sign )
{
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinFhyo522::UpDownFocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
...
//			-1		：	該当無し
//-----------------------------------------------------------------------------
int CShinFhyo522::GetAbleInputPosition( int index )
{
	//現在ポジション検索
	int	rtIdx = -1;
	int stIdx = 0;
...
BOOL CShinFhyo522::PreTranslateMessage(MSG* pMsg)
{
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinFhyo522::SetItemPosition()
{
	int index;
...
/*-------------*/
	int setIdx = GetAbleInputPosition( ID5218A );
	if( setIdx != -1 ){
