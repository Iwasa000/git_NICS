...
class CShinFhyo52 : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinFhyo52)

public:
	CShinFhyo52(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinFhyo52();
...
/*-------------*/
	...
	//== 付表５－２（２）との連動用 ==
	// 連動情報の取得
	void GetLinkedData( SH_52_LINKED_INFO *pLinkedInfo );
...
private:
	...
	SH_FH5_MONY			m_Fh5Mony;					// 付表5の金額フィールド管理用('14.06.10)
	...
	int					F520c,						// 
						...
						F525c;						// 11.第五種 再計算スイッチ
													...
													//[Y]（２）消費税額の明細用
	int					F5212c,						// 
						F5213c,						//  13.第一種 再計算スイッチ
						F5214c,						//  14.第二種 再計算スイッチ
						F5215c,						//  15.第三種 再計算スイッチ
						F5216c,						//  16.第四種 再計算スイッチ
						F5217c;						//  17.第五種 再計算スイッチ
													// 手入力サイン	□□□□ ■□□■ ■□□■ ■□□■
	...
	int					m_dspSkjSw;					// 仕入控除税額計算の、どの行が表示されているか（'14.06.11）
													// □□□□ □□□□  □□□□ □□□□  □□□□ ■■■■  ■■■■ ■■■■
													//  D0：18(原則計算)
													//  D1：19(特例計算：1種類の事業で75％以上)
													//  D2：20(特例計算：2種類の事業で75％以上)
													//  D3：21(〃)
													//  D4：22(〃)
													//  D5：23(〃)
													//  D6：24(〃)
