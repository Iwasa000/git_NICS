...
class CShinIppan : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinIppan)

public:
	CShinIppan(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinIppan();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_IPPAN };

	// 初期情報のセット
	int InitInfo( CH26SnHonpyoData **pSnHonpyoData, CH26SnFhyo10Data **pSnFhyo10Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo );
	// 初期化
	int Init();
	// 初期ポジションセット
...
//	static	int		DpSw = 0;
	int					m_DpSw;					//
//static	char	WORK0[6], WORK1[6], M1c, dm;
	char				m_M1c;					// 
