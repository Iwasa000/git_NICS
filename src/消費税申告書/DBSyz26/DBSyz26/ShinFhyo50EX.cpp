...
IMPLEMENT_DYNAMIC(CShinFhyo50EX, CSyzBaseDlg)

CShinFhyo50EX::CShinFhyo50EX(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinFhyo50EX::IDD, pParent)
	, m_typeCnt(0)
	, m_pParent(pParent)
	, m_FrSw(0)
	, m_DpSw(0)
	...
	, m_pZmSub(NULL)
{
	...
	memset( F531T, '\0', sizeof(F531T) );
	memset( F531A, '\0', sizeof(F531A) );
	memset( F531B, '\0', sizeof(F531B) );
	memset( F531C, '\0', sizeof(F531C) );

	memset( F532T, '\0', sizeof(F532T) );
	memset( F532A, '\0', sizeof(F532A) );
	memset( F532B, '\0', sizeof(F532B) );
	memset( F532C, '\0', sizeof(F532C) );

	memset( F533T, '\0', sizeof(F533T) );
	memset( F533A, '\0', sizeof(F533A) );
	memset( F533B, '\0', sizeof(F533B) );
	memset( F533C, '\0', sizeof(F533C) );

	memset( F534T, '\0', sizeof(F534T) );
	memset( F534A, '\0', sizeof(F534A) );
	memset( F534B, '\0', sizeof(F534B) );
	memset( F534C, '\0', sizeof(F534C) );

	memset( F535T, '\0', sizeof(F535T) );
	memset( F535A, '\0', sizeof(F535A) );
	memset( F535B, '\0', sizeof(F535B) );
	memset( F535C, '\0', sizeof(F535C) );

	memset( F536T, '\0', sizeof(F536T) );
	memset( F536A, '\0', sizeof(F536A) );
	memset( F536B, '\0', sizeof(F536B) );
	memset( F536C, '\0', sizeof(F536C) );

	memset( F537T, '\0', sizeof(F537T) );
	memset( F537A, '\0', sizeof(F537A) );
	memset( F537B, '\0', sizeof(F537B) );
	memset( F537C, '\0', sizeof(F537C) );

	F51UPb = F52UPb = F53UPb = F54UPb = F55UPb = F56UPb = 0;
	SZsw = SBsw = CQsw = 0x00;

//	F50c = F51c = F52c = F53c = F54c = F55c;
	F520sw = F521sw = F522sw = F523sw = F524sw = F525sw = F526sw = F527sw = F528sw = F529sw = F530sw = F531sw = F532sw = F533sw = F534sw = F535sw = F536sw = 0;

	F56sw = 0;
	F513sw = F514sw = F515sw = F516sw = F517sw = F518sw = F519sw = 0;

	m_curwnd = -1;
}

CShinFhyo50EX::~CShinFhyo50EX()
{
}

void CShinFhyo50EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo50EX, CSyzBaseDlg)
END_MESSAGE_MAP()


// CShinFhyo50EX メッセージ ハンドラ

BOOL CShinFhyo50EX::OnInitDialog()
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo50EX::InitInfo( CH27SnFhyo50Data **pSnFhyo50Data, CH27SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CH27SnHonpyoData **pSnHonpyoData, CArithEx *pArith, CDBNpSub *pZmSub, SYC_SHININFO *pShinInfo )
{
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFhyo50EX::InitDiagAttr()
{
	int	cnt = 0;
	while( 1 ){
		if( Fhyo5EX[cnt].Index == 0 ){
			break;
		}
		int	Flg;
		// 確定時は全入力不可
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			Flg = Fhyo5EX[cnt].AllUnConnect;
		}
		else{
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				Flg = Fhyo5EX[cnt].UnConnect;
			}
			else{
				Flg = Fhyo5EX[cnt].Connect;
			}
		}
		ATRIB_MOD( IDC_ICSDIAGCTRL2, Fhyo5EX[cnt].Index, Flg, (*m_pSnHeadData)->Sn_Sign4, Fhyo5EX[cnt].Connect );
		cnt++;
	}
	...
	else if( F530sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID530T, 3 );
	}
	else if( F531sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID531T, 3 );
	}
	else if( F532sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID532T, 3 );
	}
	else if( F533sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID533T, 3 );
	}
	else if( F534sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID534T, 3 );
	}
	else if( F535sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID535T, 3 );
	}
	else if( F536sw & ChkFlg ){
		ChangeColor( IDC_ICSDIAGCTRL2,  ID536T, 3 );
	}
...
BOOL CShinFhyo50EX::PreTranslateMessage(MSG* pMsg)
{
...
//			-1	：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo50EX::ReadData( int upck )
{
	m_typeCnt = 0;
...
//	memmove( F51B,  pSyzShin->Snd.Sn_1F4B, MONY_BUF_SIZE );
	...
	memmove( F514T, (*m_pSnFhyo50Data)->Sn_5FUZT1, MONY_BUF_SIZE );
	memmove( F514A, (*m_pSnFhyo50Data)->Sn_5FUZK1A, MONY_BUF_SIZE );
	memmove( F514B, (*m_pSnFhyo50Data)->Sn_5FUZK1B, MONY_BUF_SIZE );
	memmove( F514C, (*m_pSnFhyo50Data)->Sn_5FUZK1C, MONY_BUF_SIZE );
	F514sw = (*m_pSnFhyo50Data)->Sn_5FUZK1sw;
	...
	memmove( F515T, (*m_pSnFhyo50Data)->Sn_5FUZT2, MONY_BUF_SIZE );
	memmove( F515A, (*m_pSnFhyo50Data)->Sn_5FUZK2A, MONY_BUF_SIZE );
	memmove( F515B, (*m_pSnFhyo50Data)->Sn_5FUZK2B, MONY_BUF_SIZE );
	memmove( F515C, (*m_pSnFhyo50Data)->Sn_5FUZK2C, MONY_BUF_SIZE );
	F515sw = (*m_pSnFhyo50Data)->Sn_5FUZK2sw;
	...
	memmove( F516T, (*m_pSnFhyo50Data)->Sn_5FUZT3, MONY_BUF_SIZE );
	memmove( F516A, (*m_pSnFhyo50Data)->Sn_5FUZK3A, MONY_BUF_SIZE );
	memmove( F516B, (*m_pSnFhyo50Data)->Sn_5FUZK3B, MONY_BUF_SIZE );
	memmove( F516C, (*m_pSnFhyo50Data)->Sn_5FUZK3C, MONY_BUF_SIZE );
	F516sw = (*m_pSnFhyo50Data)->Sn_5FUZK3sw;
	...
	memmove( F517T, (*m_pSnFhyo50Data)->Sn_5FUZT4, MONY_BUF_SIZE );
	memmove( F517A, (*m_pSnFhyo50Data)->Sn_5FUZK4A, MONY_BUF_SIZE );
	memmove( F517B, (*m_pSnFhyo50Data)->Sn_5FUZK4B, MONY_BUF_SIZE );
	memmove( F517C, (*m_pSnFhyo50Data)->Sn_5FUZK4C, MONY_BUF_SIZE );
	F517sw = (*m_pSnFhyo50Data)->Sn_5FUZK4sw;
	...
	memmove( F518T, (*m_pSnFhyo50Data)->Sn_5FUZT5, MONY_BUF_SIZE );
	memmove( F518A, (*m_pSnFhyo50Data)->Sn_5FUZK5A, MONY_BUF_SIZE );
	memmove( F518B, (*m_pSnFhyo50Data)->Sn_5FUZK5B, MONY_BUF_SIZE );
	memmove( F518C, (*m_pSnFhyo50Data)->Sn_5FUZK5C, MONY_BUF_SIZE );
	F518sw = (*m_pSnFhyo50Data)->Sn_5FUZK5sw;

	// 第六種事業課税売上高
	memset( F512A, '\0', MONY_BUF_SIZE );
	memset( F512B, '\0', MONY_BUF_SIZE );
	memmove( F512C, (*m_pSnFhyo50Data)->Sn_5FURI6C, MONY_BUF_SIZE );
	memmove( F512T, F512C, MONY_BUF_SIZE );
	if( m_pArith->l_cmp((*m_pSnHonpyoData)->Sn_KURI6, F512T) ){
		memmove( (*m_pSnHonpyoData)->Sn_KURI6, F512C, MONY_BUF_SIZE );
	}
	...
	// 第六種事業別課税売上に係る消費税額	
	F56UPb = (*m_pSnFhyo50Data)->Sn_5FUP6;
	memmove( F519T, (*m_pSnFhyo50Data)->Sn_5FUZT6, MONY_BUF_SIZE );
	memmove( F519A, (*m_pSnFhyo50Data)->Sn_5FUZK6A, MONY_BUF_SIZE );
	memmove( F519B, (*m_pSnFhyo50Data)->Sn_5FUZK6B, MONY_BUF_SIZE );
	memmove( F519C, (*m_pSnFhyo50Data)->Sn_5FUZK6C, MONY_BUF_SIZE );
	F519sw = (*m_pSnFhyo50Data)->Sn_5FUZK6sw;
	...
	m_pArith->l_add( WORK0, WORK0, F512C );
//--> '15.02.10 INS START
...
//<-- '15.02.10 INS END
	...
	memset( F513A, '\0', MONY_BUF_SIZE );
	memset( F513B, '\0', MONY_BUF_SIZE );
	memmove( F513C, (*m_pSnFhyo50Data)->Sn_5FUZKTC, MONY_BUF_SIZE );
	memmove( F513T, (*m_pSnFhyo50Data)->Sn_5FUZTT, MONY_BUF_SIZE );
	F513sw = (*m_pSnFhyo50Data)->Sn_5FUZKTsw;
	...
	memmove( F520T, (*m_pSnFhyo50Data)->Sn_5FSZTG, MONY_BUF_SIZE );
	memmove( F520A, (*m_pSnFhyo50Data)->Sn_5FSZKGA, MONY_BUF_SIZE );
	memmove( F520B, (*m_pSnFhyo50Data)->Sn_5FSZKGB, MONY_BUF_SIZE );
	memmove( F520C, (*m_pSnFhyo50Data)->Sn_5FSZKGC, MONY_BUF_SIZE );

	// 特例計算を適用する場合の控除対象仕入税額
	if( (*m_pSnFhyo50Data)->Sn_5FSZsw ){
		memmove( F521T, (*m_pSnFhyo50Data)->Sn_5FSZTT, MONY_BUF_SIZE );
		memmove( F521A, (*m_pSnFhyo50Data)->Sn_5FSZKTA, MONY_BUF_SIZE );
		memmove( F521B, (*m_pSnFhyo50Data)->Sn_5FSZKTB, MONY_BUF_SIZE );
		memmove( F521C, (*m_pSnFhyo50Data)->Sn_5FSZKTC, MONY_BUF_SIZE );
	}
	else{
	...
	F520sw = (*m_pSnFhyo50Data)->Sn_5FGKsw;

	F521sw = (*m_pSnFhyo50Data)->Sn_5FTKsw;//0x0800
	...
	F530sw = (*m_pSnFhyo50Data)->Sn_5FSZT30sw;
	F531sw = (*m_pSnFhyo50Data)->Sn_5FSZT31sw;
	F532sw = (*m_pSnFhyo50Data)->Sn_5FSZT32sw;
	F533sw = (*m_pSnFhyo50Data)->Sn_5FSZT33sw;
	F534sw = (*m_pSnFhyo50Data)->Sn_5FSZT34sw;
	F535sw = (*m_pSnFhyo50Data)->Sn_5FSZT35sw;
	F536sw = (*m_pSnFhyo50Data)->Sn_5FSZT36sw;
	...
	memmove( F530T, (*m_pSnFhyo50Data)->Sn_5FSZT30C, MONY_BUF_SIZE );
	memmove( F531T, (*m_pSnFhyo50Data)->Sn_5FSZT31C, MONY_BUF_SIZE );
	memmove( F532T, (*m_pSnFhyo50Data)->Sn_5FSZT32C, MONY_BUF_SIZE );
	memmove( F533T, (*m_pSnFhyo50Data)->Sn_5FSZT33C, MONY_BUF_SIZE );
	memmove( F534T, (*m_pSnFhyo50Data)->Sn_5FSZT34C, MONY_BUF_SIZE );
	memmove( F535T, (*m_pSnFhyo50Data)->Sn_5FSZT35C, MONY_BUF_SIZE );
	memmove( F536T, (*m_pSnFhyo50Data)->Sn_5FSZT36C, MONY_BUF_SIZE );
...
//	memmove( (*m_pSnFhyo50Data)->Sn_5FURITC, F56C,  6 );
	...
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZTT, F513T, 6 );
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZKTC, F512C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZKTC, F513T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZKTsw = F513sw;
	// 第一種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI1,  F57T,  6  );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI1C,  F57T,  6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT1, F514T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK1sw = F514sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK1C, F513C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK1C, F514T, 6 );
	// 第二種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI2,  F58T,  6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI2C,  F58T,  6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT2, F515T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK2sw = F515sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK2C, F514C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK2C, F515T, 6 );
	// 第三種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI3,  F59T,  6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI3C,  F59T,  6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT3, F516T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK3sw = F516sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK3C, F515C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK3C, F516T, 6 );
	// 第四種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI4,  F510T, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI4C, F510T, 6 );	
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT4, F517T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK4sw = F517sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK4C, F516C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK4C, F517T, 6 );
	// 五種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI5,  F511T, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI5C, F511T, 6 );	
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT5, F518T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK5sw = F518sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK5C, F517C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK5C, F518T, 6 );
	
	// 六種事業課税売上高
	memmove( (*m_pSnHonpyoData)->Sn_KURI6,  F512T, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FURI6C, F512T, 6 );	
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZT6, F519T, 6 );
	(*m_pSnFhyo50Data)->Sn_5FUZK6sw = F519sw;
//	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK5C, F517C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FUZK6C, F519T, 6 );

	// 原則計算を適用する場合の控除対象仕入税額
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZTG, F520T, 6 );
//	memmove( (*m_pSnFhyo50Data)->Sn_5FSZKGC, F518C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZKGC, F520T, 6 );

	// 特例計算を適用する場合の控除対象仕入税額
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZTT, F521T, 6 );
//	memmove( (*m_pSnFhyo50Data)->Sn_5FSZKTC, F519C, 6 );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZKTC, F521T, 6 );
	// 種類の事業の専業者の場合
	...
	if( CQsw & 0x0f ){
		...
		if( !(*m_pSnHeadData)->Sn_TOKUR ){
			memmove( (*m_pSnFhyo40Data)->Sn_1F4C, F520T, 6 );
		}
		else{
			memmove( (*m_pSnFhyo40Data)->Sn_1F4C, F537T, 6 );
		}
	}
	...
	(*m_pSnFhyo50Data)->Sn_5FGKsw = F520sw;

	(*m_pSnFhyo50Data)->Sn_5FTKsw = F521sw;
	...
	(*m_pSnFhyo50Data)->Sn_5FSZT30sw = F530sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT31sw = F531sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT32sw = F532sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT33sw = F533sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT34sw = F534sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT35sw = F535sw;
	(*m_pSnFhyo50Data)->Sn_5FSZT36sw = F536sw;
	...
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT30C, F530T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT31C, F531T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT32C, F532T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT33C, F533T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT34C, F534T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT35C, F535T, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo50Data)->Sn_5FSZT36C, F536T, MONY_BUF_SIZE );
...
void CShinFhyo50EX::DispFh5()
{
	...
	diag_setdata( IDC_ICSDIAGCTRL2, 145, (struct IUnknown *)&DiagData, CIcsdiagctrl );

	yy = mm = dd = 0;
	(*m_pSnHeadData)->GetYmdDataGen( ID_ICSSH_KAZEIKIKAN_TO, &yy, &mm, &dd );
	sprintf_s( buf, sizeof( buf ), _T("%02x.%02x.%02x"), (yy&0xff), (mm&0xff), (dd&0xff) );
	DiagData.data_disp = (char *)buf;
	diag_setdata( IDC_ICSDIAGCTRL2, 146, (struct IUnknown *)&DiagData, CIcsdiagctrl );
	...
	DiagData.data_disp = (char *)buf;
	diag_setdata( IDC_ICSDIAGCTRL2, 147, (struct IUnknown *)&DiagData, CIcsdiagctrl );
	...
	diag_modify( IDC_ICSDIAGCTRL2, ID518T, DIAG_MDFY_EDIT, CIcsdiagctrl );
}
...
//		Mony	：	表示金額
//-----------------------------------------------------------------------------
void CShinFhyo50EX::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinFhyo50EX::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinFhyo50EX::ChangeColor( unsigned short id, short index, int sign )
{
	if( (*m_pSnHeadData)->IsInUnconnectInput(m_pZmSub) == FALSE ){
		return;
	}
...
//		Sign	：	
//-----------------------------------------------------------------------------
void CShinFhyo50EX::dsp_uws( unsigned short Id, short Index, unsigned short Pers, short Sign )
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
	char				buf[512] = {0};
...
// 各入力データ抽出
//-----------------------------------------------------------------------------
void CShinFhyo50EX::GetAllDiagData()
{
	DIAGRAM_DATA		DiagData;
	DiagData.data_imgdata = NULL;
...
BEGIN_EVENTSINK_MAP(CShinFhyo50EX, CSyzBaseDlg)
	ON_EVENT(CShinFhyo50EX, IDC_ICSDIAGCTRL2, 1, CShinFhyo50EX::EditONIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinFhyo50EX, IDC_ICSDIAGCTRL2, 2, CShinFhyo50EX::EditOFFIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinFhyo50EX, IDC_ICSDIAGCTRL2, 3, CShinFhyo50EX::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFhyo50EX::EditONIcsdiagctrl2(short index)
{
...
void CShinFhyo50EX::EditOFFIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
//				F50c |= 0x01;
		...
		case	ID513T	:	// 12. 事業別課税売上に係る消費税額合計（期間計）
...
//				F51c |= 0x01;
		...
		case	ID514T	:	//	14. 事業別課税売上に係る消費税額･第一種（期間計）
			memset( CkMy, '\0', MONY_BUF_SIZE );
...
//				F54c |= 0x01;
		...
		case	ID517T	:	// 17. 事業別課税売上に係る消費税額･第四種（期間計）
			memset( CkMy, '\0', MONY_BUF_SIZE );
...
//				F55c |= 0x01;
			...
			memmove( F512T, CkMy, MONY_BUF_SIZE );
			// 各入力の計算表示
			f5calq_all( 1 );
			// 入力直後の特例計算サイン
			Tokurei50();
		...
		case	ID519T	:	// 19. 事業別課税売上に係る消費税額･第六種（期間計）
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F519T, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID519T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F519T, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F519T, CkMy) ){
				CQsw &= 0x0f;
				F519sw |= 0x800;
			}
			else{
...
//				F518c |= 0x01;
		...
		case	ID521T	:	// 21. 特例適用･合計 仕入税額(4%分)	
			memset( CkMy, '\0', MONY_BUF_SIZE );
		...
		case	ID530T	:	// 30.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 10 ){
				memmove( CkMy, F530T, MONY_BUF_SIZE );
				memmove( CkVl, F530T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID530T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(CkMy, CkVl) ){
				...
				if( m_pArith->l_test(F54S) >= 0 ){
					F530sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID530T, 3 );
					memmove( F530T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID531T	:	// 31.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 11 )	{
				memmove( CkMy, F531T, MONY_BUF_SIZE );
				memmove( CkVl, F531T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID531T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				...
				if( m_pArith->l_test(F54S) >= 0 ){
					...
					F531sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID531T, 3 );
					memmove( F531T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID532T	:	// 32.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 12 )	{
				memmove( CkMy, F532T, MONY_BUF_SIZE );
				memmove( CkVl, F532T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID532T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				SZsw = ( char ) ( m_pArith->l_test( CkMy ) ? 12 : 0 );
				if( m_pArith->l_test(F54S) >= 0 ){
					AllTkSwClear();
					F532sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID532T, 3 );
					memmove( F532T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID533T	:	// 33.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 13 )	{
				memmove( CkMy, F533T, MONY_BUF_SIZE );
				memmove( CkVl, F533T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID533T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				SZsw = ( char ) ( m_pArith->l_test( CkMy ) ? 13 : 0 );
				if( m_pArith->l_test(F54S) >= 0 ){
					AllTkSwClear();
					F533sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID533T, 3 );
					memmove( F533T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID534T	:	// 34.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 14 )	{
				memmove( CkMy, F534T, MONY_BUF_SIZE );
				memmove( CkVl, F534T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID534T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				SZsw = ( char ) ( m_pArith->l_test( CkMy ) ? 14 : 0 );
				if( m_pArith->l_test(F54S) >= 0 ){
					AllTkSwClear();
					F534sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID534T, 3 );
					memmove( F534T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID535T	:	// 35.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 15 )	{
				memmove( CkMy, F535T, MONY_BUF_SIZE );
				memmove( CkVl, F535T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID535T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				SZsw = ( char ) ( m_pArith->l_test( CkMy ) ? 15 : 0 );
				if( m_pArith->l_test(F54S) >= 0 ){
					AllTkSwClear();
					F535sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID535T, 3 );
					memmove( F535T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
		...
		case	ID536T	:	// 36.
			...
			memset( CkVl, '\0', MONY_BUF_SIZE );
			if( (SZsw&0xff) == 16 )	{
				memmove( CkMy, F536T, MONY_BUF_SIZE );
				memmove( CkVl, F536T, MONY_BUF_SIZE );
			}
			diag_getdata( IDC_ICSDIAGCTRL2, ID536T, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp( CkMy, CkVl ) ) {
				SZsw = ( char ) ( m_pArith->l_test( CkMy ) ? 16 : 0 );
				if( m_pArith->l_test(F54S) >= 0 ){
					AllTkSwClear();
					F536sw |= 0x800;
					ChangeColor( IDC_ICSDIAGCTRL2,  ID536T, 3 );
					memmove( F536T, CkMy, MONY_BUF_SIZE );
					// 入力直後の特例計算サイン
					Tokurei50();
				}
				// 各入力の計算表示
...
void CShinFhyo50EX::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
//			// 入力直後の特例計算サイン
			...
			// 各入力の計算表示
			f5dsp_tk();
			m_Fhyo50Diag02.SetPosition( ID520T );
		}
		else if( index == ID56T ){
			F56sw &= ~0x800;
			f5calq_all( 3 );
		...
		else if( index == ID518T ){
			F518sw &= ~0x800;
			F518sw |= 0x100;
			f5calq_all( 3 );
			m_Fhyo50Diag02.SetPosition( ID518T );
		}
		else if( index == ID519T ){
			F519sw &= ~0x800;
			F519sw |= 0x100;
			f5calq_all( 3 );
			m_Fhyo50Diag02.SetPosition( ID519T );
		}
		else if( index >= ID521T && index <= ID536T ){
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinFhyo50EX::UpDownFocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
	...
	//現在ポジション検索
	for(i = 0 ; i < FCS_FHYO5EXCNT ; i++){
		if(Index == FCS_Fhyo5EX[i].IDNo){
			break;
		}
	}

	idx = FCS_Fhyo5EX[i].IDNo;//現在ポジションセット

	//次ポジション検索
	if(Move == 0){//下方向
		for(j = i + 1 ; j < FCS_FHYO5EXCNT ; j++){
			m_Fhyo50Diag02.GetAttribute(FCS_Fhyo5EX[j].IDNo ,  (LPUNKNOWN)&DA);
			if( (DA.attr_bcolor==BC_CREAM) || (DA.attr_bcolor==BC_BLUE) ){
				idx = FCS_Fhyo5EX[j].IDNo;
				break;
			}
		}
	}
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinFhyo50EX::SetItemPosition()
{
	int index;
...
void CShinFhyo50EX::AllTkSwClear(){
...
int CShinFhyo50EX::TkSwCheck(int idx){
...
//-----------------------------------------------------------------------------
void CShinFhyo50EX::MoveFocusInTaxTenki()
{
	m_Fhyo50Diag02.SetPosition( ID56T );
