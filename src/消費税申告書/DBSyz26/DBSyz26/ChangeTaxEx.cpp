...
//			-1			ÅF	ÉGÉâÅ[
//-----------------------------------------------------------------------------
int CChangeTaxEx::InitInfo( CH26SnFhyo20Data *pSnFhyo20Data, CSnHeadData *pSnHeadData, CH26SnTanaData *pSnTanaData, CArithEx *pArith )
{
...
//					m_pSnHeadData->SVsign |= 0x10;	// ê≈çûÇ›
			...
			// éüçÄñ⁄
			if( (wTerm==VK_RETURN) || (wTerm==VK_F3) || (wTerm == VK_DOWN )){//[Y]
				diag_setposition( IDC_ICSDIAGCTRL1, IDMNK63K, CIcsdiagctrl );
			}
			// ëOçÄñ⁄Ç÷
			if( (wTerm==VK_TAB) || (wTerm==VK_F2)  || (wTerm == VK_UP )){//[Y]
				diag_setposition( IDC_ICSDIAGCTRL1, IDCOMBO, CIcsdiagctrl );
			}
			break;				
