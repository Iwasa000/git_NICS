...
//	shin_datecnv( Vol1.EE_YMD[0], (unsigned char *)&YEAR, 0 );
	...
	if( pDBSyzAcc->m_pReask->GetRecordCount() )	{
		...
		if( pDBSyzAcc->m_pReask->KSdata != 0 ){
			voldate.db_datecnvGen( 0, pDBSyzAcc->m_pReask->KSdata, &dummy, &yymmdd, 0, 0 );
			Arith.int_bcd( &KsData.KSdata[1], yymmdd, 3 );
		}

		KsData.KSktdt[0] = 0x04;
		//m_Util.date_to_bcd( 0x04, &KsData.KSktdt[1], pDBSyzAcc->m_pReask->KSktdt );						// 更正決定通知書日付
		if( pDBSyzAcc->m_pReask->KSktdt != 0 ){
			yymmdd=0;
			voldate.db_datecnvGen( 0, pDBSyzAcc->m_pReask->KSktdt, &dummy, &yymmdd, 0, 0 );
			Arith.int_bcd( &KsData.KSktdt[1], yymmdd, 3 );
		}
		...
		if( pDBSyzAcc->m_pReask->KSstdt != 0 ){
			yymmdd=0;
			voldate.db_datecnvGen( 0, pDBSyzAcc->m_pReask->KSstdt, &dummy, &yymmdd, 0, 0 );
			Arith.int_bcd( &KsData.KSstdt[1], yymmdd, 3 );
		}

		KsData.KSjrdt[0] = 0x04;
		//m_Util.date_to_bcd( 0x04, &KsData.KSjrdt[1], pDBSyzAcc->m_pReask->KSjrdt );						// 更正決定通知書受理年月日	(平成固定)
		if( pDBSyzAcc->m_pReask->KSjrdt != 0 ){
			yymmdd=0;
			voldate.db_datecnvGen( 0, pDBSyzAcc->m_pReask->KSjrdt, &dummy, &yymmdd, 0, 0 );
			Arith.int_bcd( &KsData.KSjrdt[1], yymmdd, 3 );
		}
...
//		errmes = _T("更正の請求マスターのオープンが出来ませんでした。");
	...
	if(( KsData.KSdata[1] != 0 )&&( KsData.KSdata[2] != 0 )&&( KsData.KSdata[3] != 0 )){
		data[0] = KsData.KSdata[1];
		data[1] = KsData.KSdata[2];
		data[2] = KsData.KSdata[3];
		yymmdd = Arith.bcd_int( data, 3 );
		int test = 0;
		voldate.db_datecnvGen( 4, yymmdd, &dummy, &test, 1, 0 );
		pDBSyzAcc->m_pReask->KSdata = test;
	}
	//m_Util.date_to_bin( 0, &pDBSyzAcc->m_pReask->KSktdt, &KsData.KSktdt[1] );							// 更正決定通知書日付
	if(( KsData.KSktdt[1] != 0 )&&( KsData.KSktdt[2] != 0 )&&( KsData.KSktdt[3] != 0 )){
		//memset( yymmdd, '\0', sizeof(yymmdd));
//		voldate.db_datecnvGen( 4, (unsigned char*)KsData.KSktdt[1], &dummy, yymmdd, 1, 0 );
//		pDBSyzAcc->m_pReask->KSktdt = Arith.bcd_int( yymmdd, 4 );
	}
	...
	if(( KsData.KSstdt[1] != 0 )&&( KsData.KSstdt[2] != 0 )&&( KsData.KSstdt[3] != 0 )){
		//memset( yymmdd, '\0', sizeof(yymmdd));
//		voldate.db_datecnvGen( 4, (unsigned char*)KsData.KSstdt[1], &dummy, yymmdd, 1, 0 );
//		pDBSyzAcc->m_pReask->KSstdt = Arith.bcd_int( yymmdd, 4 );
	}

	//m_Util.date_to_bin( 0, &pDBSyzAcc->m_pReask->KSjrdt, &KsData.KSjrdt[1] );							// 更正決定通知書受理年月日	(平成固定)]
	if(( KsData.KSjrdt[1] != 0 )&&( KsData.KSjrdt[2] != 0 )&&( KsData.KSjrdt[3] != 0 )){
		//memset( yymmdd, '\0', sizeof(yymmdd));
//		voldate.db_datecnvGen( 4, (unsigned char*)KsData.KSjrdt[1], &dummy, yymmdd, 1, 0 );
//		pDBSyzAcc->m_pReask->KSjrdt = Arith.bcd_int( yymmdd, 4 );
	}
