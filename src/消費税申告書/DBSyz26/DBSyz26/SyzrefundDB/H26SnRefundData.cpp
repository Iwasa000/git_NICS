...
//		errmes =  _T("主な固定資産等の取得の領域を確保できませんでした。");
	...
	if( pDBSyzAcc->m_pRefundvol->GetRecordCount() == 0 )	{
		...
		RefundVol.KPSnen = yy;
		RefundVol.KPSnGengo = 4;													// 特殊事情用 年分 (個人のみ)　元号番号追加  2020.01.14 kasai
...
//		errmes =  pDBSyzAcc->GetReFundVolErrMsg();
	...
	// --> 2020.01.14 kasai  特殊事情用 年分 (個人のみ)　元号番号追加 '20.01.07
	if( (pDBSyzAcc->m_pRefundvol->KPSndm == 0) || (pDBSyzAcc->m_pRefundvol->KPSndm == 5) ){
		// バージョンアップ後でデータセットされてない時、令和元号をセット
		// 元号番号が５(令和)の場合は４(平成)に書きかえる。
		RefundVol.KPSnGengo = 4;																				
	}
	else{
...
//					errmes =  pDBSyzAcc->GetReFundVolErrMsg();
			...
			m_Util.date_to_bcdGen( 0x04, &RefundRec.KPSgengo, (unsigned char*)RefundRec.KPSday, pDBSyzAcc->m_pRefundrec->KPSday );				// 譲渡年月日
...
//		errmes =  pDBSyzAcc->GetReFundVolErrMsg();
	...
	pDBSyzAcc->m_pRefundvol->KPSndm = RefundVol.KPSnGengo;														// 特殊事情用 年分 (個人のみ) 元号番号 2020.01.14 kasai
