...
// ‚PD‰ÛÅ•W€Šz
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_1()
{
	unsigned char	WORK0[MONY_BUF_SIZE];

	// ‚P‚O‚O‚OˆÈ‰º‚ÌØ‚èŽÌ‚Ä
	m_Util.l_calq( XM1, XM1, 0 );
	dsp_prs( IDC_ICSDIAGCTRL3, ID3XM1, XM1 );
	if( !(*m_pSnHeadData)->SVmzsw ) {
		// ‰ÛÅ•W€Šz
		memset( WORK0, '\0', MONY_BUF_SIZE );
		m_Util.l_calq( (char *)WORK0, XM1, 0 );
		dsp_prs( IDC_ICSDIAGCTRL4, ID4KM3, (char *)WORK0 );
	}
}
...
// ‚QDÁ”ïÅŠz
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_2()
{
	if( (m_M1c&0x01) && !(m_M1c&0x08) ){//[Y]
//	if( m_M1c || !m_pArith->l_test(XM2) ){
...
// ‚VDTœÅŠz¬Œv
//-----------------------------------------------------------------------------
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_7( int sw )
{
	m_pArith->l_add( XM7, XM4, XM5 );
	m_pArith->l_add( XM7, XM7, XM6 );
	if( !sw ){
		dsp_cls( IDC_ICSDIAGCTRL3, ID3XM7 );
		dsp_prs( IDC_ICSDIAGCTRL3, ID3XM7, XM7 );
	}
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_89( int sw )
{
	memset( XM8, '\0', MONY_BUF_SIZE );
	memset( XM9, '\0', MONY_BUF_SIZE );
	if( (*m_pSnHeadData)->SVmzsw ){
//		if( m_pArith->l_test(pSyzShin->Snd.Sn_1F14C) < 0 ){
		if( m_pArith->l_test((*m_pSnFhyo40Data)->Sn_1F10T) < 0 ){
			if( ((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2 ){	// ’†ŠÔ\ˆÈŠO
				memmove( XM8, (*m_pSnFhyo40Data)->Sn_1F10T, MONY_BUF_SIZE );
				m_pArith->l_neg( XM8 );
			}
			// Tœ•s‘«ŠÒ•tÅŠz‚Ì•\Ž¦
			if( !sw ){
				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM8, XM8 );
			}
			// ŒÀŠETœ‘OÅŠz‚ÌÁ‹Ž
			memset( XM9, '\0', MONY_BUF_SIZE );
			if( !sw ){
				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM9, XM9 );
			}
		}
		else {
...
//				memmove( XM9, pSyzShin->Snd.Sn_3F3, MONY_BUF_SIZE );
				...
				memmove( XM9, (*m_pSnFhyo40Data)->Sn_1F10T, MONY_BUF_SIZE );
			}
			// ‚P‚O‚OˆÈ‰º‚ÌØ‚èŽÌ‚Ä
...
// ‚P‚OD’†ŠÔ”[•tÅŠz
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_10()
{
	// ‚P‚O‚OˆÈ‰º‚ÌØ‚èŽÌ‚Ä
	m_Util.l_calq( XM10, XM10, 0x10 );
	dsp_prs( IDC_ICSDIAGCTRL3, ID3XM10, XM10);
}

//-----------------------------------------------------------------------------
// ‚P‚PD”[•tÅŠzi‚X|‚P‚OjE‚P‚QD’†ŠÔ”[•tŠÒ•t
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_1112( int sw )
{
	...
	if( m_pArith->l_test(XM11) < 0 ){
		...
		if( !sw ){
			// ’†ŠÔ”[•tÅŠz‚Ì•\Ž¦
			dsp_prs( IDC_ICSDIAGCTRL3, ID3XM12, XM12 );
			// ‚P‚PD”[•tÅŠzi‚X|‚P‚Oj‚ÌÁ‹Ž
			memset( XM11, '\0', 6 );
			dsp_prs( IDC_ICSDIAGCTRL3, ID3XM11, XM11 );
		}
	}
	else {
		// ‚P‚O‚OˆÈ‰º‚ÌØ‚èŽÌ‚Ä
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
int CShinKaniEX2::calqx_14( int sw )
{
	...
	// ·ˆø‚«”[•tÅŠzE•\Ž¦
	if( !sw ){
		dsp_prs( IDC_ICSDIAGCTRL3, ID3XM14, XM14 );
	}
	return TRUE;
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_17( int sw )
{
	...
	if( 0 ){
		...
		if( ((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2 ){	// ’†ŠÔ\ˆÈŠO
			if( (*m_pSnHeadData)->SVmzsw ){
				...
				m_pArith->l_add( WORK0 , (*m_pSnFhyo40Data)->Sn_1F8B , (*m_pSnFhyo40Data)->Sn_1F8C );//8
				m_pArith->l_add( WORK1 , (*m_pSnFhyo40Data)->Sn_1F9B , (*m_pSnFhyo40Data)->Sn_1F9C );//9
...
/*				//Tœ•s‘«ŠÒ•tÅŠz‚É‚Íƒ}ƒCƒiƒX‚Ìê‡‚Ì‚Ý•\Ž¦‚·‚é
				if( m_pArith->l_test((*m_pSnFhyo40Data)->Sn_1F10T) < 0 ){
					memmove( XM17, (*m_pSnFhyo40Data)->Sn_1F10T, MONY_BUF_SIZE );
					m_pArith->l_neg( XM17 );
				}
*/			}
			else{
			...
			// Tœ•s‘«ŠÒ•tÅŠz‚Ì•\Ž¦
			if( !sw ){
				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM17, XM17 );
			}
		}
	}
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_18( int sw )
{
...
//		if( (m_pArith->l_test((*m_pSnFhyo40Data)->Sn_1F10T)<0) && !(((*m_pSnHeadData)->Sn_SKKBN&0xff)%2) ){
//			;
//		}
//		else {		// ŠÒ•t’†ŠÔ\ ˆÈŠO
			if( (*m_pSnHeadData)->SVmzsw ){
...
//					memmove( XM18, pSyzShin->Snd.Sn_3F6,   MONY_BUF_SIZE );
					...
					if( m_pArith->l_test((*m_pSnFhyo40Data)->Sn_1F10T) >= 0 ){
						memmove( XM18, (*m_pSnFhyo40Data)->Sn_1F10T, MONY_BUF_SIZE );
					}
				}
*/			}
			else{
			...
			// Tœ•s‘«ŠÒ•tÅŠz‚Ì•\Ž¦
			if( !sw ){
				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM18, XM18 );
			}
	//	}
	}
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_1920( int sw )
{
	memset( XM19,  '\0', MONY_BUF_SIZE );
	memset( XM20, '\0', MONY_BUF_SIZE );

	unsigned char	WORK0[MONY_BUF_SIZE] = {0};
	if( 1 ){
		char	WVAL[MONY_BUF_SIZE] = {0};
		if( (*m_pSnHeadData)->SVmzsw ){
			memmove( WVAL, (*m_pSnFhyo40Data)->Sn_1F10T, MONY_BUF_SIZE );  
		}
		else{
...
//			if( !sw ){
//				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM19, XM19 );
//			}
//			if( !sw ){
//				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM20, XM20 );
//			}
//
//			if( (*m_pSnHeadData)->SVmzsw ){
//				if( m_pArith->l_test((*m_pSnHonpyoData)->Sn_GENKAI) ){
...
//					m_Util.percent( WORK0, (char*)(*m_pSnFhyo40Data)->Sn_1F8B, 7, 25, 0 );
//					m_Util.percent( WORK1, (char*)(*m_pSnFhyo40Data)->Sn_1F9B, 7, 25, 0 );
//					m_Util.percent( WORK2, (char*)(*m_pSnFhyo40Data)->Sn_1F8C, 8, 17, 0 );
//					m_Util.percent( WORK3, (char*)(*m_pSnFhyo40Data)->Sn_1F9C, 8, 17, 0 );
...
//			}
//			else{
//				m_pArith->l_input( WORK0, _T("250") );
//				if( !sw ){
//					dsp_prs( IDC_ICSDIAGCTRL3, ID3XM19, XM19 );
//				}
//				if( m_pArith->l_test(XM17) ){
//					m_Util.percent( XM19, (char*)XM17, 8, 17, 0 );
//					if( !sw ){
//						dsp_prs( IDC_ICSDIAGCTRL3, ID3XM19, XM19 );
//					}
//				}
...
//	if( !(((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2) && pSyzShin->N_ver != 2 ){
	if( 0 ){
		...
		dsp_prs( IDC_ICSDIAGCTRL3, ID3XM21, XM21 );
	}
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_2223( int sw )
{
	...
	if( 0 ){
		...
		if( m_pArith->l_test( XM22 ) < 0 ){
			if( !(((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2) ){	// ’†ŠÔ\
				memset( XM22, '\0', MONY_BUF_SIZE );
				if( !sw ){
					dsp_prs( IDC_ICSDIAGCTRL3, ID3XM23, XM23 );
				}
			}
			else{
				m_pArith->l_neg( XM22 );
			...
			if( !sw ){
				dsp_prs( IDC_ICSDIAGCTRL3, ID3XM22, XM22 );
			}
		}
		else{
			// ‚P‚O‚OˆÈ‰º‚ÌØ‚èŽÌ‚Ä
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
int CShinKaniEX2::calqx_25( int sw )
{
	// ’†ŠÔŠŽ‰ü³“ú‘OŠJŽn‚Í’n•ûÁ”ïÅ“ü—Í–³‚µ
//	if( !(((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2) && pSyzShin->N_ver != 2 ){
	if( 0 ){
		...
		if( !sw ){
			dsp_prs( IDC_ICSDIAGCTRL3, ID3XM25, XM25 );
		}
	}
	return TRUE;
}
...
// ˆø”	sw	F	•\Ž¦Ø‘ÖƒXƒCƒbƒ`
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_26( int sw )
{
	char	W0[MONY_BUF_SIZE];
	...
	if( !sw ){
		dsp_prs( IDC_ICSDIAGCTRL3, ID3XM26, XM26 );
	}
}
...
//		sw		F	
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_all( int atno, int sw )
{
	while( 1 ){
		switch( atno ){
			case	1 :
...
// •Ê•\‚ÌŒvŽZ
//-----------------------------------------------------------------------------
void CShinKaniEX2::calqx_otherlists()
{
	if( m_pParent ){
