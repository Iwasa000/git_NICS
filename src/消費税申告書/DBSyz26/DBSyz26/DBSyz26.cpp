// DBSyz26.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには DBSyz26.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE DBSyz26DLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DBSyz26.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(DBSyz26DLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(DBSyz26DLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DBSyz26.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(DBSyz26DLL);
	}
	return 1;   // OK
}
...
extern "C" __declspec( dllexport ) HINSTANCE WINAPI GetH26HyoInstance()
{
	return 	g_hInstance;
}