...
// 事業区分別の合計消費税額 （期間計）
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_gts( int sg )
{
...
// 事業区分別の合計消費税額 （4%分）
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_gks()
{
	char	sw;
...
//---------------
	// 3%分
	m_pArith->l_clear( F5213A );
	if( m_pArith->l_test(F5214A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5214A );
	}
	if( m_pArith->l_test(F5215A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5215A );
	}
	if( m_pArith->l_test(F5216A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5216A );
	}
	if( m_pArith->l_test(F5217A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5217A );
	}
	if( m_pArith->l_test(F5218A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5218A );
	}
	if( m_pArith->l_test(F5219A) > 0 ){
		m_pArith->l_add( F5213A, F5213A, F5219A );
	}

	// 4%分
	m_pArith->l_clear( F5213B );
	if( m_pArith->l_test(F5214B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5214B );
	}
	if( m_pArith->l_test(F5215B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5215B );
	}
	if( m_pArith->l_test(F5216B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5216B );
	}
	if( m_pArith->l_test(F5217B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5217B );
	}
	if( m_pArith->l_test(F5218B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5218B );
	}
	if( m_pArith->l_test(F5219B) > 0 ){
		m_pArith->l_add( F5213B, F5213B, F5219B );
	}
	// 6.3%分
	m_pArith->l_clear( F5213C );
	if( m_pArith->l_test(F5214C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5214C );
	}
	if( m_pArith->l_test(F5215C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5215C );
	}
	if( m_pArith->l_test(F5216C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5216C );
	}
	if( m_pArith->l_test(F5217C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5217C );
	}
	if( m_pArith->l_test(F5218C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5218C );
	}
	if( m_pArith->l_test(F5219C) > 0 ){
		m_pArith->l_add( F5213C, F5213C, F5219C );
	}
//---------------
...
//---------------
	...
	if( m_pArith->l_test(F5214T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5214T );
	}
	if( m_pArith->l_test(F5215T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5215T );
	}
	if( m_pArith->l_test(F5216T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5216T );
	}
	if( m_pArith->l_test(F5217T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5217T );
	}
	if( m_pArith->l_test(F5218T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5218T );
	}
	if( m_pArith->l_test(F5219T) > 0 ){
		m_pArith->l_add( F5213T, F5213T, F5219T );
	}
//---------------
...
// 事業別課税売上割合の表示
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_uws()
{
	// 課税期間計
	...
	dsp_uws( IDC_ICSDIAGCTRL2, ID5212TE, (*m_pSnHonpyoData)->Sn_UP6, 1 );
...
// 第一種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku1()
{
	char	WORK0[MONY_BUF_SIZE]={0};
...
//<-- '14.10.07 INS END
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5214c&0x08) ){
			...
			if( (F521c&0x01) || (m_pArith->l_test(F527A)&&(m_pArith->l_test(F5214A)==0)) ){
				memset( F5214A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5214A, F527A, 1, 30, 0 );
				F521c &= ~0x0f;
			}
		}
		...
		if( !(F5214c&0x80) ){//[Y]
			...
			if( (F521c&0x10) || (m_pArith->l_test(F527B)&&(m_pArith->l_test(F5214B)==0)) ){
				memset( F5214B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5214B, F527B, 1, 40, 0 );
				F521c &= ~0xf0;
			}
		}
		...
		if( !(F5214c&0x800) ){
			...
			if( (F521c&0x100) || (m_pArith->l_test(F527C)&&(m_pArith->l_test(F5214C)==0)) ){
				memset( F5214C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5214C, F527C, 1, 63, 0 );
				F521c &= ~0xf00;
			}
		}
	}
...
// 第二種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku2()
{
	char	WORK0[MONY_BUF_SIZE]={0};
...
//<-- '14.10.07 INS END
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5215c&0x08) ){//[Y]
			...
			if( (F522c&0x01) || (m_pArith->l_test(F528A)&&(m_pArith->l_test(F5215A)==0)) ){
				memset( F5215A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5215A, F528A, 1, 30, 0 );
				F522c &= ~0x0f;
			}
		}
		if( !(F5215c&0x80) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5215B, 0 );
			if( (F522c&0x10) || (m_pArith->l_test(F528B)&&(m_pArith->l_test(F5215B)==0)) ){
				memset( F5215B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5215B, F528B, 1, 40, 0 );
				F522c &= ~0xf0;
			}
		}
		if( !(F5215c&0x800) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5215C, 0 );
			if( (F522c&0x100) || (m_pArith->l_test(F528C)&&(m_pArith->l_test(F5215C)==0)) ){
				memset( F5215C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5215C, F528C, 1, 63, 0 );
				F522c &= ~0xf00;
			}
		}
	}
...
// 第三種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku3()
{
	char	WORK0[MONY_BUF_SIZE]={0};
...
//<-- '14.10.07 INS END
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5216c&0x08) ){//[Y]
			...
			if( (F523c&0x01) || (m_pArith->l_test(F529A)&&(m_pArith->l_test(F5216A)==0)) ){
				memset( F5216A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5216A, F529A, 1, 30, 0 );
				F523c &= ~0x0f;
			}
		}
		...
		if( !(F5216c&0x80) ){//[Y]
			...
			if( (F523c&0x10) || (m_pArith->l_test(F529B)&&(m_pArith->l_test(F5216B)==0)) ){
				memset( F5216B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5216B, F529B, 1, 40, 0 );
				F523c &= ~0xf0;
			}
		}
		...
		if( !(F5216c&0x800) ){//[Y]
			...
			if( (F523c&0x100) || (m_pArith->l_test(F529C)&&(m_pArith->l_test(F5216C)==0)) ){
				memset( F5216C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5216C, F529C, 1, 63, 0 );
				F523c &= ~0xf00;
			}
		}
	}
...
// 第四種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku4()
{
	char	WORK0[MONY_BUF_SIZE]={0};
...
//<-- '14.10.07 INS END
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5217c&0x08) ){//[Y]
			...
			if( (F524c&0x01) || (m_pArith->l_test(F5210A)&&(m_pArith->l_test(F5217A)==0)) ){
				memset( F5217A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5217A, F5210A, 1, 30, 0 );
				F524c &= ~0x0f;
			}
		}
		if( !(F5217c&0x80) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5217B, 0 );
			if( (F524c&0x10) || (m_pArith->l_test(F5210B)&&(m_pArith->l_test(F5217B)==0)) ){
				memset( F5217B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5217B, F5210B, 1, 40, 0 );
				F524c &= ~0xf0;
			}
		}
		if( !(F5217c&0x800) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5217C, 0 );
			if( (F524c&0x100) || (m_pArith->l_test(F5210C)&&(m_pArith->l_test(F5217C)==0)) ){
				memset( F5217C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5217C, F5210C, 1, 63, 0 );
				F524c &= ~0xf00;
			}
		}
	}
...
// 第五種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku5()
{
	char	WORK0[MONY_BUF_SIZE]={0};
...
//<-- '14.10.07 INS END
	...
	dsp_cls( IDC_ICSDIAGCTRL2, ID5218A );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5218B );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5218C );
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5218c&0x08) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218A, 0 );
			if( (F525c&0x01) || (m_pArith->l_test(F5211A)&&(m_pArith->l_test(F5218A)==0)) ){
				memset( F5218A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5218A, F5211A, 1, 30, 0 );
				F525c &= ~0x0f;
			}
		}
		if( !(F5218c&0x80) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218B, 0 );
			if( (F525c&0x10) || (m_pArith->l_test(F5211B)&&(m_pArith->l_test(F5218B)==0)) ){
				memset( F5218B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5218B, F5211B, 1, 40, 0 );
				F525c &= ~0xf0;
			}
		}
		if( !(F5218c&0x800) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218C, 0 );
			if( (F525c&0x100) || (m_pArith->l_test(F5211C)&&(m_pArith->l_test(F5218C)==0)) ){
				memset( F5218C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5218C, F5211C, 1, 63, 0 );
				F525c &= ~0xf00;
			}
		}
	}
...
//--> '14.10.07 INS START
	if( m_pArith->l_test(F5218T) < 0 ){
		m_pArith->l_clear(F5218T);
	}
//<-- '14.10.07 INS END

	dsp_prs( IDC_ICSDIAGCTRL2, ID5218A, F5218A );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5218B, F5218B );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5218C, F5218C );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5218T, F5218T );

	//[Y]入力色変更
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){
		if( F5218c&0x08 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218A, 3 );
		}
		if( F5218c&0x80 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218B, 3 );
		}
		if( F5218c&0x800 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5218C, 3 );
		}
	}
...
// 第六種事業
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku6()
{
	// 横計
	m_pArith->l_add( F5212T, F5212A, F5212B );
	m_pArith->l_add( F5212T, F5212T, F5212C );
//--> '14.10.07 INS START
	if( m_pArith->l_test(F5212T) < 0 ){
		m_pArith->l_clear(F5212T);
	}
//<-- '14.10.07 INS END

	dsp_cls( IDC_ICSDIAGCTRL2, ID5212A );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5212B );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5212C );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5212T );
	...
	// 税額計算
	dsp_cls( IDC_ICSDIAGCTRL2, ID5219T );
	
	if( m_pArith->l_test(F5212T) == 0 ){
		memset( F5219T, '\0', MONY_BUF_SIZE );
	}
		
	dsp_cls( IDC_ICSDIAGCTRL2, ID5219A );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5219B );
	dsp_cls( IDC_ICSDIAGCTRL2, ID5219C );
	if( m_pArith->l_test(F5212A) == 0 ){
		memset( F5219A, '\0', MONY_BUF_SIZE );
		F5219c &= ~0x08;
	}
	if( m_pArith->l_test(F5212B) == 0 ){
		memset( F5219B, '\0', MONY_BUF_SIZE );
		F5219c &= ~0x80;
	}
	if( m_pArith->l_test(F5212C) == 0 ){
	...
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
		if( !(F5219c&0x08) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219A, 0 );
			if( (F526c&0x01) || (m_pArith->l_test(F5212A)&&(m_pArith->l_test(F5219A)==0)) ){
				memset( F5219A, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5219A, F5212A, 1, 30, 0 );
				F526c &= ~0x0f;
			}
		}
		if( !(F5219c&0x80) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219B, 0 );
			if( (F526c&0x10) || (m_pArith->l_test(F5212B)&&(m_pArith->l_test(F5219B)==0)) ){
				memset( F5219B, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5219B, F5212B, 1, 40, 0 );
				F526c &= ~0xf0;
			}
		}
		if( !(F5219c&0x800) ){//[Y]
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219C, 0 );
			if( (F526c&0x100) || (m_pArith->l_test(F5212C)&&(m_pArith->l_test(F5219C)==0)) ){
				memset( F5219C, '\0', MONY_BUF_SIZE );
				m_Util.percent( F5219C, F5212C, 1, 63, 0 );
				F526c &= ~0xf00;
			}
		}
	}
...
//--> '14.10.07 INS START
	if( m_pArith->l_test(F5219T) < 0 ){
		m_pArith->l_clear(F5219T);
	}
//<-- '14.10.07 INS END

	dsp_prs( IDC_ICSDIAGCTRL2, ID5219A, F5219A );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5219B, F5219B );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5219C, F5219C );
	dsp_prs( IDC_ICSDIAGCTRL2, ID5219T, F5219T );

	//[Y]入力色変更
	if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){
		if( F5219c&0x08 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219A, 3 );
		}
		if( F5219c&0x80 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219B, 3 );
		}
		if( F5219c&0x800 ){
			ChangeColor( IDC_ICSDIAGCTRL2, ID5219C, 3 );
		}
	}
...
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_ku( int tk_clqsw/*=1*/ )
{
	char kani = 0x00;
//	m_typeCnt = 0;
...
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_all( int atno, int tk_clqsw/*=1*/ )
{
...
// 各事業の課税売上高の横計
//----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_gsubtotal()
{
//--> '14.10.07 INS START
	m_typeCnt = 0;
//<-- '14.10.07 INS END
...
//	if( m_pArith->l_test(F5212T) ){
//--------------
	if( m_pArith->l_test(F5212A) || m_pArith->l_test(F5212B) || m_pArith->l_test(F5212C) ){
//--------------
		m_typeCnt++;
	}
	if( m_pArith->l_test(F5212T) < 0 ){
	...
	if( !(F520c&0x08) ){
		...
		m_pArith->l_add( F526A, F526A, F5212A );
		if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
	...
	if( !(F520c&0x80) ){
		...
		m_pArith->l_add( F526B, F526B, F5212B );
		if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
	...
	if( !(F520c&0x800) ){
		...
		m_pArith->l_add( F526C, F526C, F5212C );
		if( ((*m_pSnHeadData)->Sn_Sign4&0x80) && (((*m_pSnHeadData)->Sn_Sign4&0x01)==0x00) ){//[Y]
...
///////////////////
	...
	char			kani = 0x00;	// □□■■ ■■■■
									// D0:第１ ,D1:第２, D2:第３, D3:第４, D4:第５, D4:第６
...
//--> '14.06.28 INS START
			...
			F5230c = 0;
			memset( F5231T, '\0', MONY_BUF_SIZE );	
			memset( F5231A, '\0', MONY_BUF_SIZE );
			memset( F5231B, '\0', MONY_BUF_SIZE );
			memset( F5231C, '\0', MONY_BUF_SIZE );
			F5231c = 0;
			memset( F5232T, '\0', MONY_BUF_SIZE );	
			memset( F5232A, '\0', MONY_BUF_SIZE );
			memset( F5232B, '\0', MONY_BUF_SIZE );
			memset( F5232C, '\0', MONY_BUF_SIZE );
			F5232c = 0;
			memset( F5233T, '\0', MONY_BUF_SIZE );	
			memset( F5233A, '\0', MONY_BUF_SIZE );
			memset( F5233B, '\0', MONY_BUF_SIZE );
			memset( F5233C, '\0', MONY_BUF_SIZE );
			F5233c = 0;
			memset( F5234T, '\0', MONY_BUF_SIZE );	
			memset( F5234A, '\0', MONY_BUF_SIZE );
			memset( F5234B, '\0', MONY_BUF_SIZE );
			memset( F5234C, '\0', MONY_BUF_SIZE );
			F5234c = 0;
			memset( F5235T, '\0', MONY_BUF_SIZE );	
			memset( F5235A, '\0', MONY_BUF_SIZE );
			memset( F5235B, '\0', MONY_BUF_SIZE );
			memset( F5235C, '\0', MONY_BUF_SIZE );
			F5235c = 0;
			memset( F5236T, '\0', MONY_BUF_SIZE );	
			memset( F5236A, '\0', MONY_BUF_SIZE );
			memset( F5236B, '\0', MONY_BUF_SIZE );
			memset( F5236C, '\0', MONY_BUF_SIZE );
			F5236c = 0;
			memset( F5237T, '\0', MONY_BUF_SIZE );	
			memset( F5237A, '\0', MONY_BUF_SIZE );
			memset( F5237B, '\0', MONY_BUF_SIZE );
			memset( F5237C, '\0', MONY_BUF_SIZE );
//<-- '14.06.28 INS END
...
//<-- '14.06.19 INS END
	...
	if( m_pArith->l_cmp(F5220T, F5221T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5222T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5223T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5224T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5225T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5226T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5227T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5228T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5229T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5230T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5231T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5232T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5233T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5234T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5235T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
	if( m_pArith->l_cmp(F5220T, F5236T) < 0 ){
		(*m_pSnHeadData)->Sn_TOKUR = 1;
		return;
	}
...
// 他表からの計算
//-----------------------------------------------------------------------------
void CShinFhyo52EX::calq_by_OtherChange(int sw)
{
	m_DpSw = -1;
	if( sw ){
		ReadData( 1 );
...
int CShinFhyo52EX::calq_CfrOfSales( char ssg )
{
...
//	l_defn( 0x16 );
	...
	// 第６種事業課税売上
	memmove( m6, F5212T, MONY_BUF_SIZE );
...
//	else		memmove( tt, F526T, MONY_BUF_SIZE );
	...
	if ( m_pArith->l_test(tt) == 0 ){
		if( (*m_pSnHonpyoData)->Sn_UP1 || (*m_pSnHonpyoData)->Sn_UP2 || (*m_pSnHonpyoData)->Sn_UP3 || (*m_pSnHonpyoData)->Sn_UP4 || (*m_pSnHonpyoData)->Sn_UP5 || (*m_pSnHonpyoData)->Sn_UP6 ){
			st = 1;
		}
		else{
			st = 0;
...
// 返送値	：	構成比
//-----------------------------------------------------------------------------
int CShinFhyo52EX::upersnt( char *mm, char *tt )
{
...
//---------------
			sprintf_s( WORK0, sizeof(WORK0), _T("%d"), ( 1000 - (*m_pSnHeadData)->SVsper[5] ) );
			memset( PW0, '\0', MONY_BUF_SIZE );
			m_pArith->l_input( PW0, WORK0 );
			if( m_pArith->l_test(WORK1) ){
				m_Util.l_6calq( WORK1, WORK1, PW0, PW1 );
...
//-- '15.02.03 --
//			if( m_pArith->l_test(val) && m_pArith->l_test(F5213T) ){
//---------------
			if( m_pArith->l_test(val) &&
				(((judge_skj_calq()==0)&&m_pArith->l_test(F5213T)) || ((judge_skj_calq()==1)&&m_pArith->l_test(pValBunbo))) ){
//---------------
...
int CShinFhyo52EX::f52_urikbn()
{
	if( m_typeCnt == 1 ){		// 単一事業
		...
		else if( m_pArith->l_test(F5213T) ){
			m_tkSw |= 0x20;
		}
	}
...
//---------------
			...
			if( f52_toku75(F526T, F527T,  F5212T) ){
				m_tkSw |= 0x001000;
			}
			if( f52_toku75(F526T, F528T,  F529T) ){
				m_tkSw |= 0x002000;
			...
			if( f52_toku75(F526T, F528T,  F5212T) ){
				m_tkSw |= 0x010000;
			}
			if( f52_toku75(F526T, F529T,  F5210T) ){
				m_tkSw |= 0x020000;
			...
			if( f52_toku75(F526T, F529T,  F5212T) ){
				m_tkSw |= 0x080000;
			}
			if( f52_toku75(F526T, F5210T, F5211T) ){
				m_tkSw |= 0x100000;
			}
			if( f52_toku75(F526T, F5210T, F5212T) ){
				m_tkSw |= 0x200000;
			}
			if( f52_toku75(F526T, F5211T, F5212T) ){
				m_tkSw |= 0x400000;
			}
			if( F5222c || F5223c || F5224c || F5225c || F5226c || F5227c || F5228c || F5229c || F5230c || F5231c || F5232c || F5233c || F5234c || F5235c || F5236c){
				if( F5222c && ( m_tkSw & 0x000100 ) ){
					m_tkSw = 0x000100;
				}
				else if( F5223c && ( m_tkSw & 0x000200 )){
					m_tkSw = 0x000200;
				}
				else if( F5224c && ( m_tkSw & 0x000400 )){
					m_tkSw = 0x000400;
				}
				else if( F5225c && ( m_tkSw & 0x000800 )){
					m_tkSw = 0x000800;
				}
				else if( F5226c && ( m_tkSw & 0x001000 )){
					m_tkSw = 0x001000;
				}
				else if( F5227c && ( m_tkSw & 0x002000 )){
					m_tkSw = 0x002000;
				}
				else if( F5228c && ( m_tkSw & 0x004000 )){
					m_tkSw = 0x004000;
				}
				else if( F5229c && ( m_tkSw & 0x008000 )){
					m_tkSw = 0x008000;
				}
				else if( F5230c && ( m_tkSw & 0x010000 )){
					m_tkSw = 0x010000;
				}
				else if( F5231c && ( m_tkSw & 0x020000 )){
					m_tkSw = 0x020000;
				}
				else if( F5232c && ( m_tkSw & 0x040000 )){
					m_tkSw = 0x040000;
				}
				else if( F5233c && ( m_tkSw & 0x080000 )){
					m_tkSw = 0x080000;
				}
				else if( F5234c && ( m_tkSw & 0x100000 )){
					m_tkSw = 0x100000;
				}
				else if( F5235c && ( m_tkSw & 0x200000 )){
					m_tkSw = 0x200000;
				}
				else if( F5236c && ( m_tkSw & 0x400000 )){
					m_tkSw = 0x400000;
				}
			}
		}
...
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_skj_gn( char *dest, int ztype )
{
...
//--> '14.10.17 INS START
	...
	if( m_pArith->l_test(pVal18) < 0 ){
		pVal18 = zeroVal;
	}
	if( m_pArith->l_test(pVal19) < 0 ){
		pVal19 = zeroVal;
	}
//<-- '14.10.17 INS END
...
/*-------------*/
		if( dbVal4 != dbVal13 ){
			...
			ans = ((dbVal14*900.0)+(dbVal15*800.0)+(dbVal16*700.0)+(dbVal17*600.0)+(dbVal18*500.0)+(dbVal19*400.0)) / 1000.0;
		}
//---------------
/*-------------*/
	}
...
//		deno	：	分母
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_skj_tk( char *dest, const char *src, const char *num1, const char *rate1, const char *num2, const char *rate2, const char *deno )
{
...
//			0	：	75%以上でない
//-----------------------------------------------------------------------------
int CShinFhyo52EX::f52_toku75( char *Base, char *Add1, char *Add2  )
{
	int		st = 0;
...
// 原則・特例：最有利になる表示位置を取得
//-----------------------------------------------------------------------------
int CShinFhyo52EX::f52calq_tk_for_disp()
{
...
//<-- '15.02.23 INS END
			...
			switch( i ){
				case 14 :
					if( j==0 ){
						if( m_pArith->l_test(F5218A) > 0 ){
							memmove( val1, F5218A, MONY_BUF_SIZE );
							memmove( nume1, F5218A, MONY_BUF_SIZE );
						}
					}
					else if( j==1 ){
						if( m_pArith->l_test(F5218B) > 0 ){
							memmove( val1, F5218B, MONY_BUF_SIZE );
							memmove( nume1, F5218B, MONY_BUF_SIZE );
						}
					}
					else if( j==2 ){
						if( m_pArith->l_test(F5218C) > 0 ){
							memmove( val1, F5218C, MONY_BUF_SIZE );
							memmove( nume1, F5218C, MONY_BUF_SIZE );
						}
					}
					per1 = (*m_pSnHeadData)->SVsper[4];
					break;
				...
				case 12 :
					if( j==0 ){
						if( m_pArith->l_test(F5217A) > 0 ){
							memmove( val1, F5217A, MONY_BUF_SIZE );
							memmove( nume1, F5217A, MONY_BUF_SIZE );
						}
					}
					else if( j==1 ){
						if( m_pArith->l_test(F5217B) > 0 ){
							memmove( val1, F5217B, MONY_BUF_SIZE );
							memmove( nume1, F5217B, MONY_BUF_SIZE );
						}
					}
					else if( j==2 ){
						if( m_pArith->l_test(F5217C) > 0 ){
							memmove( val1, F5217C, MONY_BUF_SIZE );
							memmove( nume1, F5217C, MONY_BUF_SIZE );
						}
					}
					per1 = (*m_pSnHeadData)->SVsper[3];
					break;
			...
			else if( (i==2) || (i==6) || (i==9) ){//60%
				per2 = (*m_pSnHeadData)->SVsper[3];
			}
			else if( (i==3) || (i==7) || (i==10) || (i==12) ){//50%
				per2 = (*m_pSnHeadData)->SVsper[4];
			}
			else{//40%
			...
			if( j==0 ){
				m_pArith->l_sub( val2, F5213A, val1 );
				memmove( nume2, val2, MONY_BUF_SIZE );
			}
			else if( j==1 ){
				m_pArith->l_sub( val2, F5213B, val1 );
				memmove( nume2, val2, MONY_BUF_SIZE );
			}
			else if( j==2 ){
				m_pArith->l_sub( val2, F5213C, val1 );
				memmove( nume2, val2, MONY_BUF_SIZE );
			}
			...
			if( m_pArith->l_test(val1) ){
				if( j==0 ){
					f52calq_skj_tk( WORK0, F524A, nume1, rate1, nume2, rate2, F5213A );
				}
				else if( j==1 ){
					f52calq_skj_tk( WORK0, F524B, nume1, rate1, nume2, rate2, F5213B );
				}
				else if( j==2 ){
					f52calq_skj_tk( WORK0, F524C, nume1, rate1, nume2, rate2, F5213C );
				}
			}
			else{
				memset( WORK0, '\0', MONY_BUF_SIZE );
	...
	if( m_typeCnt > 1 ){
//-- '15.01.27 --
//		if( m_pArith->l_test(F5220T) ){
//			m_dspSkjSw |= 0x01;
//		}
//---------------
		if( m_pArith->l_test(F5220A) || m_pArith->l_test(F5220B) || m_pArith->l_test(F5220C) ){
			m_dspSkjSw |= 0x01;
		}
//---------------
		if( m_pArith->l_test(max_valttl) ){
			long	onbit = 0x02<<(maxSign-21);
			m_dspSkjSw |= onbit;
		}
	}
...
// 引数	tk_clqsw	：	特例計算の優先順位を再設定するかどうか（1:する, 0:しない）
//-----------------------------------------------------------------------------
int CShinFhyo52EX::f52calq_tk_all( int tk_clqsw/*=1*/ )
{
	char			val1[MONY_BUF_SIZE], val2[MONY_BUF_SIZE], WORK1[MONY_BUF_SIZE], PW0[MONY_BUF_SIZE], WORK0[MONY_BUF_SIZE];
	char			*VAL;
	char			nume1[MONY_BUF_SIZE], nume2[MONY_BUF_SIZE], rate1[MONY_BUF_SIZE], rate2[MONY_BUF_SIZE];
			...
			switch( i ){
				case 14 :
					if( j==0 ){
						...
						if( m_pArith->l_test(F5218T) > 0 ){
							memmove( val1, F5218T, MONY_BUF_SIZE );
							memmove( nume1, F5218T, MONY_BUF_SIZE );
						}
					}
					per1 = (*m_pSnHeadData)->SVsper[4];
					break;
				...
				case 12 :
					if( j==0 ){
						...
						if( m_pArith->l_test(F5217T) > 0 ){
							memmove( val1, F5217T, MONY_BUF_SIZE );
							memmove( nume1, F5217T, MONY_BUF_SIZE );
						}
					}
					per1 = (*m_pSnHeadData)->SVsper[3];
					break;
...
//---------------
			switch( i ){
				case 0 :// 80%
					per2 = (*m_pSnHeadData)->SVsper[1];	// 1000 - 200
					break;
				case 1 :// 70%
				case 5 :
					per2 = (*m_pSnHeadData)->SVsper[2];	// 1000 - 300
					break;
				case 2 :// 60%
				case 6 : 
				case 9 : 
					per2 = (*m_pSnHeadData)->SVsper[3];	// 1000 - 400
					break;
				case 3 :// 50%
				case 7 : 
				case 10 :
				case 12 :
					per2 = (*m_pSnHeadData)->SVsper[4];	// 1000 - 500
					break;
				case 4 :// 40%
				case 8 :
				case 11 :
				case 13 :
				case 14 :
					per2 = (*m_pSnHeadData)->SVsper[5];	// 1000 - 600
					break;
				default:
					break;
			}
...
//--> '14.10.17 INS START
				...
				m_pArith->l_sub( val2, F5213T, val1 );
				memmove( nume2, val2, MONY_BUF_SIZE );
			}
//<-- '14.10.17 INS END
...
//--> '14.10.17 INS START
					...
					f52calq_skj_tk( WORK0, F524T, nume1, rate1, nume2, rate2, F5213T );
				}
//<-- '14.10.17 INS END
			}
			else{
...
//---------------
	if( isAddtype ){
		if( m_tkSw&0x10000 ){
			m_pArith->l_add( F5230T, F5230A, F5230B );
			m_pArith->l_add( F5230T, F5230T, F5230C );
		}
		else{
...
//---------------
	if( isAddtype ){
		if( m_tkSw&0x20000 ){
			m_pArith->l_add( F5231T, F5231A, F5231B );
			m_pArith->l_add( F5231T, F5231T, F5231C );
		}
		else{
	...
	if( isAddtype ){
		if( m_tkSw&0x40000 ){
			m_pArith->l_add( F5232T, F5232A, F5232B );
			m_pArith->l_add( F5232T, F5232T, F5232C );
		}
		else{
	...
	if( isAddtype ){
		if( m_tkSw&0x80000 ){
			m_pArith->l_add( F5233T, F5233A, F5233B );
			m_pArith->l_add( F5233T, F5233T, F5233C );
		}
		else{
	...
	if( isAddtype ){
		if( m_tkSw&0x100000 ){
			m_pArith->l_add( F5234T, F5234A, F5234B );
			m_pArith->l_add( F5234T, F5234T, F5234C );
		}
		else{
	...
	if( isAddtype ){
		if( m_tkSw&0x200000 ){
			m_pArith->l_add( F5235T, F5235A, F5235B );
			m_pArith->l_add( F5235T, F5235T, F5235C );
		}
		else{
	...
	if( isAddtype ){
		if( m_tkSw&0x400000 ){
			m_pArith->l_add( F5236T, F5236A, F5236B );
			m_pArith->l_add( F5236T, F5236T, F5236C );
		}
		else{
...
//<-- '14.06.11 INS END
	...
	if( m_dspSkjSw&0x02 ){
		...
		memmove( WORK1, F5230T, MONY_BUF_SIZE );
		pValA = F5230A;
		pValB = F5230B;
		pValC = F5230C;
		pValT = F5230T;
		...
		memmove( WORK1, F5231T, MONY_BUF_SIZE );
		pValA = F5231A;
		pValB = F5231B;
		pValC = F5231C;
		pValT = F5231T;
	...
	else if( m_dspSkjSw&0x1000 ){
		memmove( WORK1, F5232T, MONY_BUF_SIZE );
		pValA = F5232A;
		pValB = F5232B;
		pValC = F5232C;
		pValT = F5232T;
	...
	else if( m_dspSkjSw&0x2000 ){
		memmove( WORK1, F5233T, MONY_BUF_SIZE );
		pValA = F5233A;
		pValB = F5233B;
		pValC = F5233C;
		pValT = F5233T;
	...
	else if( m_dspSkjSw&0x4000 ){
		memmove( WORK1, F5234T, MONY_BUF_SIZE );
		pValA = F5234A;
		pValB = F5234B;
		pValC = F5234C;
		pValT = F5234T;
	...
	else if( m_dspSkjSw&0x8000 ){
		memmove( WORK1, F5235T, MONY_BUF_SIZE );
		pValA = F5235A;
		pValB = F5235B;
		pValC = F5235C;
		pValT = F5235T;
	...
	else if( m_dspSkjSw&0x10000 ){
		memmove( WORK1, F5236T, MONY_BUF_SIZE );
		pValA = F5236A;
		pValB = F5236B;
		pValC = F5236C;
		pValT = F5236T;
...
//-- '15.01.27 --
//		if( m_pArith->l_cmp(F5220T, pValT) < 0 ){
//			// 特例計算
//			memmove( F5237A, pValA, MONY_BUF_SIZE );
//			memmove( F5237B, pValB, MONY_BUF_SIZE );
//			memmove( F5237C, pValC, MONY_BUF_SIZE );
//			memmove( F5237T, pValT, MONY_BUF_SIZE );
//			(*m_pSnHeadData)->Sn_TOKUR = 1;
//		}
//		else{
//			// 原則計算
//			memmove( F5237A, F5220A, MONY_BUF_SIZE );
//			memmove( F5237B, F5220B, MONY_BUF_SIZE );
//			memmove( F5237C, F5220C, MONY_BUF_SIZE );
//			memmove( F5237T, F5220T, MONY_BUF_SIZE );
//			(*m_pSnHeadData)->Sn_TOKUR = 0;
//		}
//---------------
		...
		else if( m_pArith->l_cmp(F5220T, pValT) == 0 ){
			// 原則計算と特例計算の比較
			char	genSum[MONY_BUF_SIZE]={0}, tkSum[MONY_BUF_SIZE]={0};
			m_pArith->l_add( genSum, F5220A, F5220B );
			m_pArith->l_add( genSum, genSum, F5220C );
...
//<-- '14.06.11 INS END
	...
	Arith.l_print( moji, F5222A, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, F5222B, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, F5222C, _T("sss,sss,sss,sss,ss9") );
	Arith.l_print( moji, F5222T, _T("sss,sss,sss,sss,ss9") );
#endif
...
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_one_skj_gen( int rIdx, int cIdx )
{
	if( cIdx == 0 ){
		f52calq_skj_gn( F5220A, cIdx );
	}
	else if( cIdx == 1 ){
		f52calq_skj_gn( F5220B, cIdx );
	}
	else if( cIdx == 2 ){
		f52calq_skj_gn( F5220C, cIdx );
	}
//--> '14.10.17 INS START
	else if( cIdx == 3 ){
		f52calq_skj_gn( F5220T, cIdx );
	}
//<-- '14.10.17 INS END
...
//		pRate2	：
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_get_clqfield( int rIdx, int cIdx, char *pVal1, char *pValttl, char *pRate1, char *pRate2 )
{
...
//---------------
	...
	if( (2<=rIdx) && (rIdx<=6) ){
		...
		memmove( valttl, m_Fh5Mony.pF5213[tmpIdx], MONY_BUF_SIZE );
		nRate1 = (*m_pSnHeadData)->SVsper[0];
	}
	else if( (7<=rIdx) && (rIdx<=10) ){
		memmove( val1, m_Fh5Mony.pF5215[tmpIdx], MONY_BUF_SIZE );
		memmove( valttl, m_Fh5Mony.pF5213[tmpIdx], MONY_BUF_SIZE );
		nRate1 = (*m_pSnHeadData)->SVsper[1];
	}
	else if( (11<=rIdx) && (rIdx<=13) ){
		memmove( val1, m_Fh5Mony.pF5216[tmpIdx], MONY_BUF_SIZE );
		memmove( valttl, m_Fh5Mony.pF5213[tmpIdx], MONY_BUF_SIZE );
		nRate1 = (*m_pSnHeadData)->SVsper[2];
	}
	else if( (14<=rIdx) && (rIdx<=15) ){
		memmove( val1, m_Fh5Mony.pF5217[tmpIdx], MONY_BUF_SIZE );
		memmove( valttl, m_Fh5Mony.pF5213[tmpIdx], MONY_BUF_SIZE );
		nRate1 = (*m_pSnHeadData)->SVsper[3];
	}
	else if( rIdx == 16 ){
		memmove( val1, m_Fh5Mony.pF5218[tmpIdx], MONY_BUF_SIZE );
		memmove( valttl, m_Fh5Mony.pF5213[tmpIdx], MONY_BUF_SIZE );
		nRate1 = (*m_pSnHeadData)->SVsper[4];
	}
//---------------
	nRate1 = 1000 - nRate1;
	...
	else if( (rIdx==4) || (rIdx==8) || (rIdx==11) ){
		nRate2 = (*m_pSnHeadData)->SVsper[3];
	}
	else if( (rIdx==5) || (rIdx==9) || (rIdx==12) || (rIdx==14) ){
		nRate2 = (*m_pSnHeadData)->SVsper[4];
	}
	else if( (rIdx==6) || (rIdx==10) || (rIdx==13) || (rIdx==15) || (rIdx==16) ){
		nRate2 = (*m_pSnHeadData)->SVsper[5];
	}
	nRate2 = 1000 - nRate2;
...
//-----------------------------------------------------------------------------
// 引数	rIdx	：	行インデックス	(0:⑳, 1:21, 2～16:22～36)
//		cIdx	：	列インデックス	(0:3%, 1:5%, 2:8%)
//		pVal	：	計算結果金額の取得先
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_one_skj_tk75over( int rIdx, int cIdx, char *pVal )
{
...
//---------------
		...
		if( cIdx == 0 ){
			memmove( F5230A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5230B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5230C, WORK0, MONY_BUF_SIZE );
		}
//--> '14.10.17 INS START
		else if( cIdx == 3 ){
			memmove( F5230T, WORK0, MONY_BUF_SIZE );
		}
//<-- '14.10.17 INS END
//-- '14.10.17 --
//		m_pArith->l_add( F5228T, F5228A, F5228B );
...
//---------------
		...
		if( cIdx == 0 ){
			memmove( F5231A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5231B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5231C, WORK0, MONY_BUF_SIZE );
		}
//--> '14.10.17 INS START
		else if( cIdx == 3 ){
			memmove( F5231T, WORK0, MONY_BUF_SIZE );
		}
//<-- '14.10.17 INS END
//-- '14.10.17 --
//		m_pArith->l_add( F5229T, F5229A, F5229B );
...
//---------------
		...
		if( cIdx == 0 ){
			memmove( F5232A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5232B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5232C, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 3 ){
			memmove( F5232T, WORK0, MONY_BUF_SIZE );
		}
		if( isAddtype ){
			m_pArith->l_add( F5232T, F5232A, F5232B );
			m_pArith->l_add( F5232T, F5232T, F5232C );
		...
		if( cIdx == 0 ){
			memmove( F5233A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5233B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5233C, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 3 ){
			memmove( F5233T, WORK0, MONY_BUF_SIZE );
		}
		if( isAddtype ){
			m_pArith->l_add( F5233T, F5233A, F5233B );
			m_pArith->l_add( F5233T, F5233T, F5233C );
		...
		if( cIdx == 0 ){
			memmove( F5234A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5234B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5234C, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 3 ){
			memmove( F5234T, WORK0, MONY_BUF_SIZE );
		}
		if( isAddtype ){
			m_pArith->l_add( F5234T, F5234A, F5234B );
			m_pArith->l_add( F5234T, F5234T, F5234C );
		...
		if( cIdx == 0 ){
			memmove( F5235A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5235B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5235C, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 3 ){
			memmove( F5235T, WORK0, MONY_BUF_SIZE );
		}
		if( isAddtype ){
			m_pArith->l_add( F5235T, F5235A, F5235B );
			m_pArith->l_add( F5235T, F5235T, F5235C );
		...
		if( cIdx == 0 ){
			memmove( F5236A, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 1 ){
			memmove( F5236B, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 2 ){
			memmove( F5236C, WORK0, MONY_BUF_SIZE );
		}
		else if( cIdx == 3 ){
			memmove( F5236T, WORK0, MONY_BUF_SIZE );
		}
		if( isAddtype ){
			m_pArith->l_add( F5236T, F5236A, F5236B );
			m_pArith->l_add( F5236T, F5236T, F5236C );
...
//		pVal	：	計算結果金額の取得先
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_one_skj( int rIdx, int cIdx, char *pVal )
{
	if( rIdx == 0 ){					// 原則計算
	...
	else if( (2<=rIdx) && (rIdx<=16) ){	// 特例計算(2種類で75%以上)
...
//		cIdx	：	列インデックス	(0:3%, 1:5%, 2:8%)
//-----------------------------------------------------------------------------
void CShinFhyo52EX::f52calq_one_skj_one_tk75over( int rIdx, int cIdx )
{
	unsigned short	per1=0;
	if( (m_tkSw&0xff) == 0x01 ){
		per1 = (*m_pSnHeadData)->SVsper[0];
...
//			0	：	D列はA+B+C
//-----------------------------------------------------------------------------
int CShinFhyo52EX::judge_skj_calq()
{
	// 貸倒回収あり？
