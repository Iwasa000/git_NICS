...
class CShinFhyo50 : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinFhyo50)

public:
	CShinFhyo50(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinFhyo50();
...
// ダイアログ データ
	...
	// 初期情報のセット
	int InitInfo( CH26SnFhyo50Data **pSnFhyo50Data, CH26SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CH26SnHonpyoData **pSnHonpyoData, CArithEx *pArith, CDBNpSub *pZmSub, SYC_SHININFO *pShinInfo );
	// 初期化
	int Init();
	// ダイアグラム初期属性セット
	int InitDiagAttr();
...
private:
	...
	char				F512T[MONY_BUF_SIZE],		// 12. 事業別課税売上に係る消費税額合計（期間計）
	...
	char				F513T[MONY_BUF_SIZE],		// 13. 事業別課税売上に係る消費税額･第一種（期間計）
	...
	char				F514T[MONY_BUF_SIZE],		// 14. 事業別課税売上に係る消費税額･第二種（期間計）
	...
	char				F515T[MONY_BUF_SIZE],		// 15. 事業別課税売上に係る消費税額･第三種（期間計）
	...
	char				F516T[MONY_BUF_SIZE],		// 16. 事業別課税売上に係る消費税額･第四種（期間計）
	...
	char				F517T[MONY_BUF_SIZE],		// 17. 事業別課税売上に係る消費税額･第五種（期間計）
	...
	char				F518T[MONY_BUF_SIZE],		// 18. 原則計算適用･合計 仕入税額（期間計）
						F518A[MONY_BUF_SIZE],		// 18. 　　　　　　　 〃　　　　 （3%分）
						F518B[MONY_BUF_SIZE],		// 18. 　　　　　　　 〃　　　　 （4%分）
						F518C[MONY_BUF_SIZE];		// 18. 　　　　　　　 〃　　　　 （6.3%分）

	char				F519T[MONY_BUF_SIZE],		// 19. 特例適用･合計 仕入税額(期間計)
	...
	char				F520T[MONY_BUF_SIZE],		// 20. 特例適用･合計 仕入税額(期間計)
	...
	char				F530T[MONY_BUF_SIZE],		// 29. 特例適用･合計 仕入税額(期間計)
						F530A[MONY_BUF_SIZE],		// 29. 　　　　　　　　〃　　(3%分)　　　
						F530B[MONY_BUF_SIZE],		// 29. 　　　　　　　　〃　　(4%分)　
						F530C[MONY_BUF_SIZE];		// 29. 　　　　　　　　〃　　(6.3%分)　

	//== 帳票データ本体 ==
	CH26SnFhyo50Data	**m_pSnFhyo50Data;			// 付表５
