...
// 引数	index	：	インデックス
//-----------------------------------------------------------------------------
void CShinFhyo522EX::f52calq_sum1( short index )
{
...
// 引数	index	：	インデックス
//-----------------------------------------------------------------------------
void CShinFhyo522EX::f52calq_sum2( short index )
{
...
// 引数	index	：	インデックス
//-----------------------------------------------------------------------------
void CShinFhyo522EX::f52calq_sum3( short index )
{
	...
	m_pArith->l_add( pF5230T, pF5230A, pF5230B );
	m_pArith->l_add( pF5230T, pF5230T, pF5230C );
	if( (index==ID5230A) || (index==ID5230B) || (index==ID5230C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5230T, pF5230T );
	}

	m_pArith->l_add( pF5231T, pF5231A, pF5231B );
	m_pArith->l_add( pF5231T, pF5231T, pF5231C );
	if( (index==ID5231A) || (index==ID5231B) || (index==ID5231C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5231T, pF5231T );
	}

	m_pArith->l_add( pF5232T, pF5232A, pF5232B );
	m_pArith->l_add( pF5232T, pF5232T, pF5232C );
	if( (index==ID5232A) || (index==ID5232B) || (index==ID5232C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5232T, pF5232T );
	}

	m_pArith->l_add( pF5233T, pF5233A, pF5233B );
	m_pArith->l_add( pF5233T, pF5233T, pF5233C );
	if( (index==ID5233A) || (index==ID5233B) || (index==ID5233C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5233T, pF5233T );
	}

	m_pArith->l_add( pF5234T, pF5234A, pF5234B );
	m_pArith->l_add( pF5234T, pF5234T, pF5234C );
	if( (index==ID5234A) || (index==ID5234B) || (index==ID5234C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5234T, pF5234T );
	}

	m_pArith->l_add( pF5235T, pF5235A, pF5235B );
	m_pArith->l_add( pF5235T, pF5235T, pF5235C );
	if( (index==ID5235A) || (index==ID5235B) || (index==ID5235C) ){
		dsp_prs( IDC_ICSDIAGCTRL1, ID5235T, pF5235T );
	}

	m_pArith->l_add( pF5236T, pF5236A, pF5236B );
	m_pArith->l_add( pF5236T, pF5236T, pF5236C );
	if( (index==ID5236A) || (index==ID5236B) || (index==ID5236C) ){
...
// 最も有利な方式を選択
//-----------------------------------------------------------------------------
void CShinFhyo522EX::f52calq_sel( short index )
{
	if( (*m_ptypeCnt) < 2 ){
		*m_pSzsw = 0;
		return;
...
//	int		maxSign = 19;
	...
	if( ((*m_pdspSkjSw)&0x01) && (((*m_pdspSkjSw)&~0x01)==0) ){	// 原則計算のみ
		dsp_prs( IDC_ICSDIAGCTRL1, ID5237A, pF5220A );
		dsp_prs( IDC_ICSDIAGCTRL1, ID5237B, pF5220B );
		dsp_prs( IDC_ICSDIAGCTRL1, ID5237C, pF5220C );
		dsp_prs( IDC_ICSDIAGCTRL1, ID5237T, pF5220T );
	}
		...
		if( (*m_pdspSkjSw)&0x02 ){
			...
			memmove( WORK1, pF5230T, MONY_BUF_SIZE );
			pValA = pF5230A;
			pValB = pF5230B;
			pValC = pF5230C;
			pValT = pF5230T;
			...
			memmove( WORK1, pF5231T, MONY_BUF_SIZE );
			pValA = pF5231A;
			pValB = pF5231B;
			pValC = pF5231C;
			pValT = pF5231T;
			...
			memmove( WORK1, pF5232T, MONY_BUF_SIZE );
			pValA = pF5232A;
			pValB = pF5232B;
			pValC = pF5232C;
			pValT = pF5232T;
			...
			memmove( WORK1, pF5233T, MONY_BUF_SIZE );
			pValA = pF5233A;
			pValB = pF5233B;
			pValC = pF5233C;
			pValT = pF5233T;
			...
			memmove( WORK1, pF5234T, MONY_BUF_SIZE );
			pValA = pF5234A;
			pValB = pF5234B;
			pValC = pF5234C;
			pValT = pF5234T;
			...
			memmove( WORK1, pF5235T, MONY_BUF_SIZE );
			pValA = pF5235A;
			pValB = pF5235B;
			pValC = pF5235C;
			pValT = pF5235T;
			...
			memmove( WORK1, pF5236T, MONY_BUF_SIZE );
			pValA = pF5236A;
			pValB = pF5236B;
			pValC = pF5236C;
			pValT = pF5236T;
		...
		// 判定
//-- '15.01.27 --
//		if( m_pArith->l_cmp(pF5220T, WORK1) < 0 ){
//			dsp_prs( IDC_ICSDIAGCTRL1, ID5237A, pValA );
//			dsp_prs( IDC_ICSDIAGCTRL1, ID5237B, pValB );
//			dsp_prs( IDC_ICSDIAGCTRL1, ID5237C, pValC );
//			dsp_prs( IDC_ICSDIAGCTRL1, ID5237T, pValT );
//			(*m_pSnHeadData)->Sn_TOKUR = 1;
//		}
//		else{
//			dsp_prs( IDC_ICSDIAGCTRL1, ID5237A, pF5220A );
...
//---------------
		...
		else if( m_pArith->l_cmp(pF5220T, WORK1) == 0 ){
			// 原則計算と特例計算の比較
			char	genSum[MONY_BUF_SIZE]={0}, tkSum[MONY_BUF_SIZE]={0};
			m_pArith->l_add( genSum, pF5220A, pF5220B );
			m_pArith->l_add( genSum, genSum, pF5220C );
...
//-------------------------------------------------------------------
int CShinFhyo522EX::f52_toku75( char *Base, char *Add1, char *Add2  )
{
