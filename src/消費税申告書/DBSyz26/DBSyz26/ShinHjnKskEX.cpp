...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinHjnKskEX::InitInfo( CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo, CH26SnKskData **pSnKskData )
{
	ASSERT( pSnHeadData );
	if( pSnHeadData == NULL ){
		return -1;
...
//2015.12.08 INSERT END
	...
	while( 1 ) {
		...
		if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
			...
			if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
				if( (HjnKsk2[cnt].Index == 71) || (HjnKsk2[cnt].Index == 72) ){
...
/*	if( isReiwaPrnt ){
		...
		diag_setdata(IDC_ICSDIAGCTRL10, DLG1PRD, &DiagData, CIcsdiagctrl);
	}
	else{
		// 提出年月日
		m_Util.DiagOcxIniz(&DiagData);
...
// 引数	mode	：	
//-----------------------------------------------------------------------------
void CShinHjnKskEX::DispKsiHjn( int mode/*=0*/ )
{
...
void CShinHjnKskEX::DispKsiHjnDig1()
{
	...
	for( i = DLG1TDT; i <= DLG1KSW; i++ ){
		...
		if( i == DLG1KDT ){
			m_Util.DiagOcxIniz(&data);
			diag_clear(IDC_ICSDIAGCTRL10, DLG1KDG, TRUE, CIcsdiagctrl);
			...
			//}
			data.data_disp = buf;
			diag_setdata(IDC_ICSDIAGCTRL10, DLG1KDG, &data, CIcsdiagctrl);
		}
	}
}
...
void CShinHjnKskEX::DispKsiHjnDig3()
{
	...
	diag_clear(IDC_ICSDIAGCTRL30, DLG3SDG, TRUE, CIcsdiagctrl);
	m_Util.DiagOcxIniz(&data);
	memset(buf, '\0', sizeof(buf));
	...
	//}
	data.data_disp = buf;
	diag_setdata(IDC_ICSDIAGCTRL30, DLG3SDG, &data, CIcsdiagctrl);

	// 更正決定通知書受理年月日 元号
	diag_clear(IDC_ICSDIAGCTRL30, DLG3JDG, TRUE, CIcsdiagctrl);
	m_Util.DiagOcxIniz(&data);
	memset(buf, '\0', sizeof(buf));
	...
	//}
	data.data_disp = buf;
	diag_setdata(IDC_ICSDIAGCTRL30, DLG3JDG, &data, CIcsdiagctrl);
}
...
//		mode
//-----------------------------------------------------------------------------
int CShinHjnKskEX::MoveCurselDiag1( short index, UINT flg, short nChar, int mode/*=0*/ )
{
	short	xpos = index;
	int		sw = 0, cmb = 0;
...
int CShinHjnKskEX::MoveCurselDiag2( short index, UINT flg, short nChar, int mode/*=0*/ )
{
	short	xpos = index;
	int		sw = 0;
