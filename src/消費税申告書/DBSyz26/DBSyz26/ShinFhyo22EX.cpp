...
IMPLEMENT_DYNAMIC(CShinFhyo22EX, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinFhyo22EX::CShinFhyo22EX(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinFhyo22EX::IDD, pParent)
	...
	, m_pSnHonpyoData(NULL)
{
	...
	memset( F2214A, '\0', sizeof(F2214A) );
	memset( F2214B, '\0', sizeof(F2214B) );
	memset( F2214C, '\0', sizeof(F2214C) );
	...
	memset( F2217AEX, '\0', sizeof(F2217AEX) );
	memset( F2217BEX, '\0', sizeof(F2217BEX) );
	memset( F2217CEX, '\0', sizeof(F2217CEX) );
	memset( F2218AEX, '\0', sizeof(F2218AEX) );
	memset( F2218BEX, '\0', sizeof(F2218BEX) );
	memset( F2218CEX, '\0', sizeof(F2218CEX) );
	...
	memset( F2222C, '\0', sizeof(F2223C) );
	memset( F2223A, '\0', sizeof(F2223A) );
	memset( F2223B, '\0', sizeof(F2223B) );
	memset( F2223C, '\0', sizeof(F2223C) );
	memset( F2224A, '\0', sizeof(F2224A) );
	memset( F2224B, '\0', sizeof(F2224B) );
	memset( F2224C, '\0', sizeof(F2224C) );
	memset( F2225A, '\0', sizeof(F2225A) );
	memset( F2225B, '\0', sizeof(F2225B) );
	memset( F2225C, '\0', sizeof(F2225C) );
	...
	F229c = F2211c = F2213c = F2219c = F2220c = F2223c = 0;

	m_curwnd = -1;
}

CShinFhyo22EX::~CShinFhyo22EX()
{
}

void CShinFhyo22EX::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinFhyo22EX, CSyzBaseDlg)
	ON_BN_CLICKED(IDC_BUTTON1, &CShinFhyo22EX::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CShinFhyo22EX::OnBnClickedButton2)
END_MESSAGE_MAP()


// CShinFhyo22 メッセージ ハンドラ

BOOL CShinFhyo22EX::OnInitDialog()
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo22EX::InitInfo( CH28SnFhyo20Data **pSnFhyo22Data, CSnHeadData **pSnHeadData, CH26SnTanaData **pSnTanaData, CH28SnFhyo10Data **pSnFhyo10Data, CH28SnHonpyoData **pSnHonpyoData, CArithEx *pArith, SYC_SHININFO *pShinInfo )
{
...
// ボタン表示変更
//-----------------------------------------------------------------------------
void CShinFhyo22EX::ChangeBtnDisp()
{
...
// 返送値	0	：	正常終了
//-----------------------------------------------------------------------------
int CShinFhyo22EX::InitDiagAttr()
{
	int	cnt = 0;
	while( 1 ){
...
//--> '15.08.20 INS START
				int st = ChkTkkzIdx( Fhyo22[cnt].Index );
				if( st == 1 ){
...
//			-1	：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo22EX::ReadData( int pas )
{
	// 課税標準額
	...
	F229c = (*m_pSnFhyo22Data)->Sn_2F9sw;

	// 特定課税仕入れに係る支払対価の額
	if( (*m_pSnHeadData)->IsNeedTkkzDisp() ){
		memset( F2210A, '\0', MONY_BUF_SIZE );
		memset( F2210B, '\0', MONY_BUF_SIZE );
		memmove( F2210C, (*m_pSnFhyo22Data)->Sn_2F10C, MONY_BUF_SIZE );
	}
	else{
		memset( F2210A, '\0', MONY_BUF_SIZE );
		memset( F2210B, '\0', MONY_BUF_SIZE );
	...
	// 特定課税仕入れに係る消費税額
	if( (*m_pSnHeadData)->IsNeedTkkzDisp() ){
		memset( F2211A, '\0', MONY_BUF_SIZE );
		memset( F2211B, '\0', MONY_BUF_SIZE );
		memmove( F2211C, (*m_pSnFhyo22Data)->Sn_2F11C, MONY_BUF_SIZE );
		F2211c = (*m_pSnFhyo22Data)->Sn_2F11sw;
	}
	else{
		memset( F2211A, '\0', MONY_BUF_SIZE );
		memset( F2211B, '\0', MONY_BUF_SIZE );
	...
	// 課税貨物に係る消費税額
	memmove( F2213A, (*m_pSnFhyo22Data)->Sn_2F12A, MONY_BUF_SIZE );
	memmove( F2213B, (*m_pSnFhyo22Data)->Sn_2F12B, MONY_BUF_SIZE );
	memmove( F2213C, (*m_pSnFhyo22Data)->Sn_2F12C, MONY_BUF_SIZE );
	// (免税)控除対象調整税額
	if( (*m_pSnTanaData)->Sn_MENZsw&0x10 ){
		if( (*m_pSnHeadData)->IsKobetuSiireAnbun() ){
			char	WORK0[MONY_BUF_SIZE] = {0};
			m_pArith->l_add( F2214A , (*m_pSnTanaData)->Sn_menzz[0] , (*m_pSnTanaData)->Sn_menzz[1] );
			m_pArith->l_add( F2214A , F2214A , (*m_pSnTanaData)->Sn_menzz[2] );
			m_pArith->l_add( F2214B , (*m_pSnTanaData)->Sn_mnz40[0] , (*m_pSnTanaData)->Sn_mnz40[1] );
			m_pArith->l_add( F2214B , F2214B , (*m_pSnTanaData)->Sn_mnz40[2] );
			m_pArith->l_add( F2214C , (*m_pSnTanaData)->Sn_mnz63[0] , (*m_pSnTanaData)->Sn_mnz63[1] );
			m_pArith->l_add( F2214C , F2214C , (*m_pSnTanaData)->Sn_mnz63[2] );
		}
		else{
			memmove( F2214A, (*m_pSnTanaData)->Sn_MENZZ, MONY_BUF_SIZE );
			memmove( F2214B, (*m_pSnTanaData)->Sn_MNZ40, MONY_BUF_SIZE );
			memmove( F2214C, (*m_pSnTanaData)->Sn_MNZ63, MONY_BUF_SIZE );
		}
		
		if( (*m_pSnTanaData)->Sn_MENZsw&0x01 ){
			m_pArith->l_neg( F2214A );
			m_pArith->l_neg( F2214B );
			m_pArith->l_neg( F2214C );
		}
	}
	else{
		memset( F2214A, '\0', MONY_BUF_SIZE );
		memset( F2214B, '\0', MONY_BUF_SIZE );
		memset( F2214C, '\0', MONY_BUF_SIZE );
	}

	// 課税売上に係る仕入税額
	memmove( F2217A, (*m_pSnFhyo22Data)->Sn_2F16A, MONY_BUF_SIZE );
	memmove( F2217B, (*m_pSnFhyo22Data)->Sn_2F16B, MONY_BUF_SIZE );
	memmove( F2217C, (*m_pSnFhyo22Data)->Sn_2F16C, MONY_BUF_SIZE );
	// 共通の売上に係る仕入税額
	memmove( F2218A, (*m_pSnFhyo22Data)->Sn_2F17A, MONY_BUF_SIZE );
	memmove( F2218B, (*m_pSnFhyo22Data)->Sn_2F17B, MONY_BUF_SIZE );
	memmove( F2218C, (*m_pSnFhyo22Data)->Sn_2F17C, MONY_BUF_SIZE );

	// 課税売上に係る仕入税額
	memmove( F2217AEX, (*m_pSnFhyo22Data)->Sn_2F16A, MONY_BUF_SIZE );
	memmove( F2217BEX, (*m_pSnFhyo22Data)->Sn_2F16B, MONY_BUF_SIZE );
	memmove( F2217CEX, (*m_pSnFhyo22Data)->Sn_2F16C, MONY_BUF_SIZE );
	// 共通の売上に係る仕入税額
	memmove( F2218AEX, (*m_pSnFhyo22Data)->Sn_2F17A, MONY_BUF_SIZE );
	memmove( F2218BEX, (*m_pSnFhyo22Data)->Sn_2F17B, MONY_BUF_SIZE );
	memmove( F2218CEX, (*m_pSnFhyo22Data)->Sn_2F17C, MONY_BUF_SIZE );

	F2219c = F2220c = 0x00;
	// 個別対応方式控除仕入税額
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F18A) ){
		memmove( F2219A, (*m_pSnFhyo22Data)->Sn_2F18A, MONY_BUF_SIZE );
		F2219c |= 0x10;
	}
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F18B) ){
		memmove( F2219B, (*m_pSnFhyo22Data)->Sn_2F18B, MONY_BUF_SIZE );
		F2219c |= 0x01;
	}
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F18C) ){
		memmove( F2219C, (*m_pSnFhyo22Data)->Sn_2F18C, MONY_BUF_SIZE );
		F2219c |= 0x100;
	}
	// 比例配分方式控除仕入税額
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F19A) ){
		memmove( F2220A, (*m_pSnFhyo22Data)->Sn_2F19A, MONY_BUF_SIZE );
		F2220c |= 0x10;
	}
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F19B) ){
		memmove( F2220B, (*m_pSnFhyo22Data)->Sn_2F19B, MONY_BUF_SIZE );
		F2220c |= 0x01;
	}
	if( m_pArith->l_test((*m_pSnFhyo22Data)->Sn_2F19C) ){
		memmove( F2220C, (*m_pSnFhyo22Data)->Sn_2F19C, MONY_BUF_SIZE );
		F2220c |= 0x100;
	}
	// 調整対象固定資産に係る調整税額
	memmove( F2221A, (*m_pSnFhyo22Data)->Sn_2F20A, MONY_BUF_SIZE );
	memmove( F2221B, (*m_pSnFhyo22Data)->Sn_2F20B, MONY_BUF_SIZE );
	memmove( F2221C, (*m_pSnFhyo22Data)->Sn_2F20C, MONY_BUF_SIZE );
	// 調整対象固定資産を課税業務用に転用した調整額
	memmove( F2222A, (*m_pSnFhyo22Data)->Sn_2F21A, MONY_BUF_SIZE );
	memmove( F2222B, (*m_pSnFhyo22Data)->Sn_2F21B, MONY_BUF_SIZE );
	memmove( F2222C, (*m_pSnFhyo22Data)->Sn_2F21C, MONY_BUF_SIZE );

	// 控除対象仕入税額
	F2223c = (*m_pSnFhyo22Data)->Sn_2F22sw;
	if( F2223c & 0x01 ){
		memmove( F2223A, (*m_pSnFhyo22Data)->Sn_2F22A, MONY_BUF_SIZE );
	}
	else{
		memmove( F2223A, (*m_pSnFhyo10Data)->Sn_1F4A, MONY_BUF_SIZE );
	}
	if( F2223c & 0x10 ){
		memmove( F2223B, (*m_pSnFhyo22Data)->Sn_2F22B, MONY_BUF_SIZE );
	}
	else{
		memmove( F2223B, (*m_pSnFhyo10Data)->Sn_1F4B, MONY_BUF_SIZE );
	}
	if( F2223c & 0x100 ){
		memmove( F2223C, (*m_pSnFhyo22Data)->Sn_2F22C, MONY_BUF_SIZE );
	}
	else{
		memmove( F2223C, (*m_pSnFhyo10Data)->Sn_1F4C, MONY_BUF_SIZE );
	}

	// 貸倒回収に係る税額
	if( pas == 1 ){
		memmove( FS25A, (*m_pSnFhyo22Data)->Sn_2F24A, MONY_BUF_SIZE );
		memmove( FS25B, (*m_pSnFhyo22Data)->Sn_2F24B, MONY_BUF_SIZE );
		memmove( FS25C, (*m_pSnFhyo22Data)->Sn_2F24C, MONY_BUF_SIZE );
	}
	memmove( F2225A, (*m_pSnFhyo22Data)->Sn_2F24A, MONY_BUF_SIZE );
	memmove( F2225B, (*m_pSnFhyo22Data)->Sn_2F24B, MONY_BUF_SIZE );
	memmove( F2225C, (*m_pSnFhyo22Data)->Sn_2F24C, MONY_BUF_SIZE );

	// 控除過大調整税額	･ 貸倒回収に係る消費税額
	m_pArith->l_sub( F2224A, (*m_pSnFhyo10Data)->Sn_1F3A, F2225A );
	m_pArith->l_sub( F2224B, (*m_pSnFhyo10Data)->Sn_1F3B, F2225B );
...
//			-1	：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo22EX::WriteData( int pas )
{
	int		st = 0;
	// 課税標準額
	...
	(*m_pSnFhyo22Data)->Sn_2F9sw = F229c;
	...
	// 特定課税仕入れに係る消費税額
	memmove( (*m_pSnFhyo22Data)->Sn_2F11A, F2211A,  MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F11B, F2211B,  MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F11C, F2211C,  MONY_BUF_SIZE );
	(*m_pSnFhyo22Data)->Sn_2F11sw = F2211c;

	//== 未使用 ==
	// 課税貨物に係る支払対価の額
//	memmove( (*m_pSnFhyo22Data)->Sn_2F10A, F2212A, MONY_BUF_SIZE );
//	memmove( (*m_pSnFhyo22Data)->Sn_2F10B, F2212B, MONY_BUF_SIZE );
//	memmove( (*m_pSnFhyo22Data)->Sn_2F10C, F2212C, MONY_BUF_SIZE );

	// 課税貨物に係る消費税額
	memmove( (*m_pSnFhyo22Data)->Sn_2F12A, F2213A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F12B, F2213B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F12C, F2213C, MONY_BUF_SIZE );

	// 課税売上に係る仕入税額
	memmove( (*m_pSnFhyo22Data)->Sn_2F16A, F2217A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F16B, F2217B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F16C, F2217C, MONY_BUF_SIZE );
	// 共通の売上に係る仕入税額
	memmove( (*m_pSnFhyo22Data)->Sn_2F17A, F2218A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F17B, F2218B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F17C, F2218C, MONY_BUF_SIZE );
	...
	if( F2219c & 0x10 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F18A, F2219A, MONY_BUF_SIZE );
	}
	if( F2219c & 0x01 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F18B, F2219B, MONY_BUF_SIZE );
	}
	if( F2219c & 0x100 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F18C, F2219C, MONY_BUF_SIZE );
	}
	// 比例配分方式控除仕入税額
	memset( (*m_pSnFhyo22Data)->Sn_2F19A, '\0', MONY_BUF_SIZE );
	memset( (*m_pSnFhyo22Data)->Sn_2F19B, '\0', MONY_BUF_SIZE );
	memset( (*m_pSnFhyo22Data)->Sn_2F19C, '\0', MONY_BUF_SIZE );
	if( F2220c & 0x10 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F19A, F2220A, MONY_BUF_SIZE );
	}
	if( F2220c & 0x01 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F19B, F2220B, MONY_BUF_SIZE );
	}
	if( F2220c & 0x100 ){
		memmove( (*m_pSnFhyo22Data)->Sn_2F19C, F2220C, MONY_BUF_SIZE );
	}

	// 調整対象固定資産に係る調整税額
	memmove( (*m_pSnFhyo22Data)->Sn_2F20A, F2221A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F20B, F2221B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F20C, F2221C, MONY_BUF_SIZE );
	// 調整対象固定資産を課税業務用に転用した調整額
	memmove( (*m_pSnFhyo22Data)->Sn_2F21A, F2222A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F21B, F2222B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F21C, F2222C, MONY_BUF_SIZE );
	// 控除対象仕入税額
	if( F2223c & 0x01 ){
		if( m_pArith->l_test(F2223A) ){
			memmove( (*m_pSnFhyo22Data)->Sn_2F22A, F2223A, MONY_BUF_SIZE );
		}
	}
	else{
		memset( (*m_pSnFhyo22Data)->Sn_2F22A, '\0', MONY_BUF_SIZE );
	}
	if( F2223c & 0x10 ){
		if( m_pArith->l_test(F2223B) ){
			memmove( (*m_pSnFhyo22Data)->Sn_2F22B, F2223B, MONY_BUF_SIZE );
		}
	}
	else{
		memset( (*m_pSnFhyo22Data)->Sn_2F22B, '\0', MONY_BUF_SIZE );
	}
	if( F2223c & 0x100 ){
		if( m_pArith->l_test(F2223C) ){
			memmove( (*m_pSnFhyo22Data)->Sn_2F22C, F2223C, MONY_BUF_SIZE );
		}
	}
	else{
		memset( (*m_pSnFhyo22Data)->Sn_2F22C, '\0', MONY_BUF_SIZE );
	}
	(*m_pSnFhyo22Data)->Sn_2F22sw = F2223c;

	memmove( (*m_pSnFhyo10Data)->Sn_1F4A, F2223A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo10Data)->Sn_1F4B, F2223B, MONY_BUF_SIZE );	
	memmove( (*m_pSnFhyo10Data)->Sn_1F4C, F2223C, MONY_BUF_SIZE );

	// 控除過大調整税額	･ 貸倒回収に係る消費税額
	if( (pas==1) && (m_pArith->l_cmp(FS25A, F2225A)||m_pArith->l_cmp(FS25B, F2225B)||m_pArith->l_cmp(FS25C, F2225C)) ){
		st = 1;
	}

	m_pArith->l_add( (*m_pSnFhyo10Data)->Sn_1F3A, F2224A, F2225A );
	m_pArith->l_add( (*m_pSnFhyo10Data)->Sn_1F3B, F2224B, F2225B );
	m_pArith->l_add( (*m_pSnFhyo10Data)->Sn_1F3C, F2224C, F2225C );

	memmove( (*m_pSnFhyo22Data)->Sn_2F24A, F2225A, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F24B, F2225B, MONY_BUF_SIZE );
	memmove( (*m_pSnFhyo22Data)->Sn_2F24C, F2225C, MONY_BUF_SIZE );

	return st;
}

BOOL CShinFhyo22EX::PreTranslateMessage(MSG* pMsg)
{
...
//<-- '15.08.28 INS END
			...
			if( (pMsg->wParam==VK_F3) || (pMsg->wParam==VK_RIGHT) || (pMsg->wParam==VK_DOWN) || ((pMsg->wParam==VK_TAB)&&!(VK_FLG&0x80)) ){//[Y]
				if( *m_pShinInfo->pOver500MillionSw != 1 &&  F22UWS >= (*m_pSnHeadData)->m_UriRatioBorder ){	//[Y]5億円以下かつ 課税売上割合が９５％以上の場合
					diag_setposition( IDC_ICSDIAGCTRL2, ID2221A, CIcsdiagctrl );
				}
				else{
					if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){//非連動の場合
						if( (*m_pSnHeadData)->IsKobetuSiireAnbun() ){	// 個別対応
							diag_setposition( IDC_ICSDIAGCTRL2, ID2217A, CIcsdiagctrl );
						}
						else{
							diag_setposition( IDC_ICSDIAGCTRL2, ID2220A, CIcsdiagctrl );
						}
					}
					else{
						diag_setposition( IDC_ICSDIAGCTRL2, ID2221A, CIcsdiagctrl );
			...
			// 前項目へ
			if( (pMsg->wParam==VK_LEFT) || (pMsg->wParam==VK_F2) || (pMsg->wParam==VK_UP) || ((pMsg->wParam==VK_TAB)&&(VK_FLG&0x80)) ){//[Y]
				if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){//非連動の場合
					diag_setposition( IDC_ICSDIAGCTRL2, ID2213C, CIcsdiagctrl );
				}
				else{
					diag_setposition( IDC_ICSDIAGCTRL2, ID2222C, CIcsdiagctrl );
				}
				return TRUE;
			}
		}
...
// 
//-----------------------------------------------------------------------------
void CShinFhyo22EX::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	if( !m_DpSw ){	// 再集計のみ（表示無し）[04'10.26]
		...
		switch( Index ) {
			...
			case ID2210C:		// 10. 特定課税仕入れに係る支払対価の額

			case ID2211C:		// 11. 特定課税仕入れに係る消費税

			case ID2213A:		// 12. 課税貨物に係る消費税額
			case ID2213B:		// 12. 課税貨物に係る消費税額
			case ID2213C:		// 12. 課税貨物に係る消費税額

			case ID2214A:		// 13. 調整税額
			case ID2214B:		// 13. 調整税額
			case ID2214C:		// 13. 調整税額

			case ID2217A:		// 16. 課税売上に係る仕入税額
			case ID2217B:		// 16. 課税売上に係る仕入税額
			case ID2217C:		// 16. 課税売上に係る仕入税額

			case ID2218A:		// 17. 共通の売上に係る仕入税額
			case ID2218B:		// 17. 共通の売上に係る仕入税額
			case ID2218C:		// 17. 共通の売上に係る仕入税額

			case ID2219A:		// 18. 個別対応方式控除仕入税額
			case ID2219B:		// 18. 個別対応方式控除仕入税額
			case ID2219C:		// 18. 個別対応方式控除仕入税額

			case ID2220A:		// 19. 比例配分方式控除仕入税額
			case ID2220B:		// 19. 比例配分方式控除仕入税額
			case ID2220C:		// 19. 比例配分方式控除仕入税額
								   
			case ID2221A:		// 20. 調整対象固定資産に係る調整税額
			case ID2221B:		// 20. 調整対象固定資産に係る調整税額
			case ID2221C:		// 20. 調整対象固定資産に係る調整税額
								   
			case ID2222A:		// 21. 調整対象固定資産を課税業務用に転用した調整額
			case ID2222B:		// 21. 調整対象固定資産を課税業務用に転用した調整額
			case ID2222C:		// 21. 調整対象固定資産を課税業務用に転用した調整額
								   
			case ID2223A: 		// 23. 差引控除対象仕入税額
			case ID2223B:		// 23. 差引控除対象仕入税額
			case ID2223C:		// 23. 差引控除対象仕入税額

			case ID2225A:		// 24. 貸倒回収に係る消費税額
			case ID2225B:		// 24. 貸倒回収に係る消費税額
			case ID2225C:		// 24. 貸倒回収に係る消費税額
...
//
//-----------------------------------------------------------------------------
void CShinFhyo22EX::dsp_cls( unsigned short Id, short Index )
{
	if( !m_DpSw ){	// 再集計のみ（表示無し）[04'10.26]
...
// 
//-----------------------------------------------------------------------------
void CShinFhyo22EX::ChangeColor( unsigned short id, short index, int sign )
{
...
// 
//-----------------------------------------------------------------------------
void CShinFhyo22EX::ModifyDiagInput( short index, int sign )
{
...
BEGIN_EVENTSINK_MAP(CShinFhyo22EX, CSyzBaseDlg)
ON_EVENT(CShinFhyo22EX, IDC_ICSDIAGCTRL2, 1, CShinFhyo22EX::EditONIcsdiagctrl2, VTS_I2)
ON_EVENT(CShinFhyo22EX, IDC_ICSDIAGCTRL2, 2, CShinFhyo22EX::EditOFFIcsdiagctrl2, VTS_I2)
ON_EVENT(CShinFhyo22EX, IDC_ICSDIAGCTRL2, 3, CShinFhyo22EX::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
ON_EVENT(CShinFhyo22EX, IDC_ICSDIAGCTRL2, 9, CShinFhyo22EX::CheckButtonIcsdiagctrl2, VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()

void CShinFhyo22EX::EditONIcsdiagctrl2(short index)
{
	...
	switch( index ){
		...
		case ID2214B :
				...
				if( m_DlSw ){
					...
					diag_setposition( IDC_ICSDIAGCTRL2, ID2213B, CIcsdiagctrl );
				}
...
void CShinFhyo22EX::EditOFFIcsdiagctrl2(short index)
{
	...
	// 12. 調整税額
	if( (index!=ID2214A) && (index!=ID2214B) ){
		if( index > ID2214B ){
			m_DlSw = 1;
		}
		else{
	...
	switch( index ){
				...
				*m_pShinInfo->pOver500MillionSw = (*m_pSnHeadData)->CheckTaxationSales( VAL, ValCnv, m_pShinInfo->bil_stno, m_pShinInfo->bil_dtno );
				if( *m_pShinInfo->pOver500MillionSw == 1 ){	// ON:９５％ルール改正対応期間で５億円超え
					...
					if( !((*m_pSnHeadData)->Sn_Sign4&0x10) && ((*m_pSnHeadData)->m_s_sgn4&0x80) ){
						// 課税売上に係る仕入税額
						memmove( F2217A, F2217AEX, MONY_BUF_SIZE );
						memmove( F2217B, F2217BEX, MONY_BUF_SIZE );
						memmove( F2217C, F2217CEX, MONY_BUF_SIZE );

						// 共通の売上に係る仕入税額
						memmove( F2218A, F2218AEX, MONY_BUF_SIZE );
						memmove( F2218B, F2218BEX, MONY_BUF_SIZE );
						memmove( F2218C, F2218CEX, MONY_BUF_SIZE );
					}
					(*m_pSnHeadData)->Sn_Sign4 |= 0x10;
					if( (*m_pSnHeadData)->IsKobetuSiireAnbun() == FALSE ){
		...
		case ID2210C :		// 10. 特定課税仕入れに係る支払対価の額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2210C, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2210C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2210C, CkMy) ){
				if(!(F2211c&0x800)){//[Y]手入力されていない場合のみサインたてる
					F2211c |= 0x100;
				}
			}
			memmove( F2210C, CkMy, MONY_BUF_SIZE );

			// 各入力の計算表示
			f22calq_all( 15 );
			break;

		case ID2211C :		// 11. 特定課税仕入れに係る消費税額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			...
			memmove( F2211C, CkMy, MONY_BUF_SIZE );
		...
		case ID2213A:		// 12. 課税貨物に係る消費税額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2213A, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2213A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2213A, DiagData.data_val, MONY_BUF_SIZE );
			if( m_pArith->l_cmp(F2213A, CkMy) ){
		...
		case ID2213B:		// 12. 課税貨物に係る消費税額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2213B, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2213B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2213B, DiagData.data_val, MONY_BUF_SIZE );
			if( m_pArith->l_cmp(F2213B, CkMy) ){
		...
		case ID2213C:		// 12. 課税貨物に係る消費税額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2213C, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2213C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2213C, DiagData.data_val, MONY_BUF_SIZE );
			if( m_pArith->l_cmp(F2213C, CkMy) ){
				if( (((*m_pSnHeadData)->IsKobetuSiireAnbun()==TRUE)&&(F22UWS<(*m_pSnHeadData)->m_UriRatioBorder)) || ((*m_pSnHeadData)->IsUseSemiUriRatio()==TRUE) ){		// 個別対応で95％未満の場合
					CString		Msg;
					Msg.Empty();
					Msg = _T("課税貨物に係る消費税額が変更されました。\n");
		...
		case ID2214A:		// 11. 調整税額
		case ID2214B:		// 11. 調整税額
		case ID2214C:		// 11. 調整税額

			break;

		case ID2217A:		// 14. 課税売上に係る仕入税額
			...
			memmove( F2217AEX, DiagData.data_val, MONY_BUF_SIZE ); 
			// 各入力の計算表示
			f22calq_all( 8 );
			break;

		case ID2217B:		// 14. 課税売上に係る仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memmove( F2217BEX, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 8 );
			break;

		case ID2217C:		// 14. 課税売上に係る仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memmove( F2217CEX, DiagData.data_val, MONY_BUF_SIZE ); 
		...
		case ID2218A:		// 15. 共通の売上に係る仕入税額
			...
			memmove( F2218AEX, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 9 );
			break;

		case ID2218B:		// 15. 共通の売上に係る仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memmove( F2218BEX, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 9 );
			break;

		case ID2218C:		// 15. 共通の売上に係る仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memmove( F2218CEX, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 9 );
			break;

		case ID2219A:		// 16. 個別対応方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2219A, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2219A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2219A, CkMy) ){
				F2219c |= 0x10;
			}
			memmove( F2219A, CkMy, MONY_BUF_SIZE );

			// 各入力の計算表示
			f22calq_all( 10 );
			break;

		case ID2219B:		// 16. 個別対応方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2219B, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2219B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2219B, CkMy) ){
				F2219c |= 0x01;
			}
			memmove( F2219B, CkMy, MONY_BUF_SIZE );

			// 各入力の計算表示
			f22calq_all( 10 );
			break;

		case ID2219C:		// 16. 個別対応方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==FALSE) ){
					break;
				}
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2219C, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2219C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2219C, CkMy) ){
				F2219c |= 0x100;
			}
			memmove( F2219C, CkMy, MONY_BUF_SIZE );

			// 各入力の計算表示
			f22calq_all( 10 );
			break;

		case ID2220A:		// 17. 比例配分方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2220A, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2220A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2220A, CkMy) ){
				F2220c |= 0x10;
			}
			memmove( F2220A, CkMy, MONY_BUF_SIZE );

			// 各入力の計算表示
			f22calq_all( 11 );
			break;

		case ID2220B:		// 17. 比例配分方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==TRUE) ){
					break;
				}
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2220B, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2220B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2220B, CkMy) ){
				F2220c |= 0x01;
			}
			memmove( F2220B, CkMy, MONY_BUF_SIZE );
			// 各入力の計算表示
			f22calq_all( 11 );
			break;

		case ID2220C:		// 17. 比例配分方式控除仕入税額
			if( (*m_pSnHeadData)->IsUseSemiUriRatio() == FALSE ){
				if( (F22UWS>=(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsKobetuSiireAnbun()==TRUE) ){
					break;
				}
			...
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2220C, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2220C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2220C, CkMy) ){
				F2220c |= 0x100;
			}
			memmove( F2220C, CkMy, MONY_BUF_SIZE );
			// 各入力の計算表示
			f22calq_all( 11 );
			break;

		case ID2221A:		// 18. 調整対象固定資産に係る調整税額
		...
		case ID2221B:		// 18. 調整対象固定資産に係る調整税額
		...
		case ID2221C:		// 18. 調整対象固定資産に係る調整税額
		...
		case ID2222A:		// 20. 調整対象固定資産を課税業務用に転用した調整額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2222A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2222A, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 13 );
			break;

		case ID2222B:		// 19. 調整対象固定資産を課税業務用に転用した調整額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2222B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2222B, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 13 );
			break;

		case ID2222C:		// 19. 調整対象固定資産を課税業務用に転用した調整額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2222C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2222C, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 13 );
			break;

		case ID2223A:		// 20. 差引控除対象仕入税額
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2223A, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2223A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2223A, CkMy) ){
				// 金額変更
				F2223c |= 0x01;
				memmove( F2223A, CkMy, MONY_BUF_SIZE );
//-- '15.12.28 --
//				memmove( (*m_pSnFhyo22Data)->Sn_2F21A, F2223A, MONY_BUF_SIZE );
//---------------
				memmove( (*m_pSnFhyo22Data)->Sn_2F22A, F2223A, MONY_BUF_SIZE );
//---------------
				if( m_pArith->l_test(F2223A) < 0 ){
					...
					memset( F2223A, '\0', MONY_BUF_SIZE );	// 控除対象仕入税額０円クリア
				}
				else{
					// 控除対象仕入税額
					memset( F2224A, '\0', MONY_BUF_SIZE );	// 控除過大調整額０円クリア
				}
			}
			// 各入力の計算表示
			f22calq_all( 13 );
		...
		case ID2223B:		// 20. 差引控除対象仕入税額
			// 各入力の計算表示
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2223B, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2223B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2223B, CkMy) ){
				F2223c |= 0x10;
				memmove( F2223B, CkMy, MONY_BUF_SIZE );
//-- '15.12.28 --
//				memmove( (*m_pSnFhyo22Data)->Sn_2F21B, F2223B, MONY_BUF_SIZE );
//---------------
				memmove( (*m_pSnFhyo22Data)->Sn_2F22B, F2223B, MONY_BUF_SIZE );
//---------------
				if( m_pArith->l_test(F2223B) < 0 ){
					...
					memset( F2223B, '\0', MONY_BUF_SIZE );	// 控除対象仕入税額０円クリア
				}
				else{
					// 控除対象仕入税額
					memset( F2224B, '\0', MONY_BUF_SIZE );	// 控除過大調整額０円クリア
				}
			}
			// 各入力の計算表示
			f22calq_all( 13 );
		...
		case ID2223C:		// 20. 差引控除対象仕入税額
			// 各入力の計算表示
			memset( CkMy, '\0', MONY_BUF_SIZE );
			memmove( CkMy, F2223C, MONY_BUF_SIZE );
			diag_getdata( IDC_ICSDIAGCTRL2, ID2223C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( CkMy, DiagData.data_val, MONY_BUF_SIZE ); 
			if( m_pArith->l_cmp(F2223C, CkMy) ){
				F2223c |= 0x100;
				memmove( F2223C, CkMy, MONY_BUF_SIZE );
//-- '15.12.28 --
//				memmove( (*m_pSnFhyo22Data)->Sn_2F21C, F2223C, MONY_BUF_SIZE );
//---------------
				memmove( (*m_pSnFhyo22Data)->Sn_2F22C, F2223C, MONY_BUF_SIZE );
//---------------
				if( m_pArith->l_test(F2223C) < 0 ){
					...
					memset( F2223C, '\0', MONY_BUF_SIZE );	// 控除対象仕入税額０円クリア
				}
				else{
					// 控除対象仕入税額
					memset( F2224C, '\0', MONY_BUF_SIZE );	// 控除過大調整額０円クリア
				}
			}
			// 各入力の計算表示
			f22calq_all( 13 );
		...
		case ID2225A:		//  22. 貸倒回収に係る消費税額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2225A, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2225A, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 14 );
			break;

		case ID2225B:		//  22. 貸倒回収に係る消費税額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2225B, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2225B, DiagData.data_val, MONY_BUF_SIZE ); 

			// 各入力の計算表示
			f22calq_all( 14 );
			break;

		case ID2225C:		//  22. 貸倒回収に係る消費税額
			diag_getdata( IDC_ICSDIAGCTRL2, ID2225C, (struct IUnknown *)&DiagData, CIcsdiagctrl );
			memmove( F2225C, DiagData.data_val, MONY_BUF_SIZE ); 
...
void CShinFhyo22EX::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
	...
	if( wTerm == VK_DELETE ){	
		switch( index ){
			case ID2223A:		// 21. 控除対象仕入税額
				F2223c &= ~0x001;
				// 各入力の計算表示
				f22calq_all( 13 );
				m_Fhyo22Diag02.SetPosition( ID2223A );
				break;
			case ID2223B:		// 21. 控除対象仕入税額
				F2223c &= ~0x010;
				// 各入力の計算表示
				f22calq_all( 13 );
				m_Fhyo22Diag02.SetPosition( ID2223B );
				break;
			case ID2223C:		// 21. 控除対象仕入税額
				F2223c &= ~0x100;
				// 各入力の計算表示
				f22calq_all( 13 );
				m_Fhyo22Diag02.SetPosition( ID2223C );
				break;
			...
			case ID2211C:		// 11. 特定課税仕入れに係る消費税額
				...
				// 各入力の計算表示
				f22calq_all( 16 );
				m_Fhyo22Diag02.SetPosition( ID2211C );
				break;
			...
			case ID2219B:		// 16. 個別対応方式控除仕入税額
				...
				// 各入力の計算表示
				f22calq_all( 10 );
				m_Fhyo22Diag02.SetPosition( ID2219B );
				break;
			...
			case ID2219C:		// 16. 個別対応方式控除仕入税額
				...
				// 各入力の計算表示
				f22calq_all( 10 );
				m_Fhyo22Diag02.SetPosition( ID2219C );
				break;
			...
			case ID2220A:		// 17. 比例配分方式控除仕入税額
				...
				// 各入力の計算表示
				f22calq_all( 11 );
				m_Fhyo22Diag02.SetPosition( ID2220A );
				break;
			...
			case ID2220B:		// 17. 比例配分方式控除仕入税額
				...
				// 各入力の計算表示
				f22calq_all( 11 );
				m_Fhyo22Diag02.SetPosition( ID2220B );
				break;
			...
			case ID2220C:		// 17. 比例配分方式控除仕入税額
				...
				// 各入力の計算表示
				f22calq_all( 11 );
				m_Fhyo22Diag02.SetPosition( ID2220C );
				break;
...
void CShinFhyo22EX::CheckButtonIcsdiagctrl2(short index, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	...
	if( index == ID227EX ){
		...
		if( (*m_pSnFhyo22Data)->Sn_2FUWS < (*m_pSnHeadData)->m_UriRatioBorder ){
			// 課税売上に係る仕入税額
			memmove( F2217A, F2217AEX, 6 );
			memmove( F2217B, F2217BEX, 6 );
			memmove( F2217C, F2217CEX, 6 );
			// 共通の売上に係る仕入税額
			memmove( F2218A, F2218AEX, 6 );
			memmove( F2218B, F2218BEX, 6 );
			memmove( F2218C, F2218CEX, 6 );
		}
		(*m_pSnHeadData)->Sn_UPERS &= 0xef;
		if( DiagData.data_check ){
			(*m_pSnHeadData)->Sn_UPERS |= 0x10;
...
void CShinFhyo22EX::OnBnClickedButton1()
{
		...
		if( st == IDOK ){
			...
			if( !((*m_pSnTanaData)->Sn_tansw & 0x01) || ((*m_pSnHeadData)->Sn_Sign4&0x80) ){//[Y]非連動の場合は常に更新させる
				...
				if( (SV_mtype&0x10) || ((*m_pSnHeadData)->IsUseSemiUriRatio()==TRUE) ){
					if( SV_mtype&0x01 ){
						m_pArith->l_add( F2217C, F2217C, SV_mnz63[0] );		// 15. 課税売上に係る仕入税額
						m_pArith->l_add( F2218C, F2218C, SV_mnz63[2] );		// 16. 共通の売上に係る仕入税額

						if( (*m_pSnHeadData)->SVmzsw == 1 ){
							m_pArith->l_add( F2217B, F2217B, SV_mnz40[0] );		// 15. 課税売上に係る仕入税額
							m_pArith->l_add( F2218B, F2218B, SV_mnz40[2] );		// 16. 共通の売上に係る仕入税額
							m_pArith->l_add( F2217A, F2217A, SV_menzz[0] );	// 15. 課税売上に係る仕入税額
							m_pArith->l_add( F2218A, F2218A, SV_menzz[2] );	// 16. 共通の売上に係る仕入税額
						}
					}
					else{
						m_pArith->l_sub( F2217C, F2217C, SV_mnz63[0] );		// 15. 課税売上に係る仕入税額
						m_pArith->l_sub( F2218C, F2218C, SV_mnz63[2] );		// 16. 共通の売上に係る仕入税額
						if( (*m_pSnHeadData)->SVmzsw == 1 ){
							m_pArith->l_sub( F2217B, F2217B, SV_mnz40[0] );		// 15. 課税売上に係る仕入税額
							m_pArith->l_sub( F2218B, F2218B, SV_mnz40[2] );		// 16. 共通の売上に係る仕入税額
							m_pArith->l_sub( F2217A, F2217A, SV_menzz[0] );	// 15. 課税売上に係る仕入税額
							m_pArith->l_sub( F2218A, F2218A, SV_menzz[2] );	// 16. 共通の売上に係る仕入税額
						}
					}
				}
				...
				if( (*m_pSnTanaData)->Sn_MENZsw & 0x10 ){
					memset( F2214C, '\0', MONY_BUF_SIZE );
					m_pArith->l_add( F2214C, F2214C, (*m_pSnTanaData)->Sn_mnz63[0] );
					m_pArith->l_add( F2214C, F2214C, (*m_pSnTanaData)->Sn_mnz63[1] );
					m_pArith->l_add( F2214C, F2214C, (*m_pSnTanaData)->Sn_mnz63[2] );
					if( (*m_pSnHeadData)->SVmzsw == 1 ){
						memset( F2214B, '\0', MONY_BUF_SIZE );
						m_pArith->l_add( F2214B, F2214B, (*m_pSnTanaData)->Sn_mnz40[0] );
						m_pArith->l_add( F2214B, F2214B, (*m_pSnTanaData)->Sn_mnz40[1] );
						m_pArith->l_add( F2214B, F2214B, (*m_pSnTanaData)->Sn_mnz40[2] );

						memset( F2214A, '\0', MONY_BUF_SIZE );
						m_pArith->l_add( F2214A, F2214A, (*m_pSnTanaData)->Sn_menzz[0] );	//				 〃	   消費税額   (〃      )　〃
						m_pArith->l_add( F2214A, F2214A, (*m_pSnTanaData)->Sn_menzz[1] );	//				 〃	   消費税額   (〃      )　〃
						m_pArith->l_add( F2214A, F2214A, (*m_pSnTanaData)->Sn_menzz[2] );	//				 〃	   消費税額   (〃      )　〃
					}
					// 旧格納場所へ格納
					memmove( (*m_pSnTanaData)->Sn_MNZ63, F2214C, MONY_BUF_SIZE );
					if( (*m_pSnHeadData)->SVmzsw == 1 ){
						memmove( (*m_pSnTanaData)->Sn_MNZ40, F2214B, MONY_BUF_SIZE );
						memmove( (*m_pSnTanaData)->Sn_MENZZ, F2214A, MONY_BUF_SIZE );
					}
					if( (*m_pSnTanaData)->Sn_MENZsw & 0x01 ){
						m_pArith->l_neg( F2214A );
						m_pArith->l_neg( F2214B );
					...
					if( (F22UWS<(*m_pSnHeadData)->m_UriRatioBorder) || ((*m_pSnHeadData)->IsUseSemiUriRatio()==TRUE) ){
						if( (*m_pSnTanaData)->Sn_MENZsw & 0x01 ){
							m_pArith->l_sub( F2217C, F2217C, (*m_pSnTanaData)->Sn_mnz63[0] );
							m_pArith->l_sub( F2218C, F2218C, (*m_pSnTanaData)->Sn_mnz63[2] );
							if( (*m_pSnHeadData)->SVmzsw == 1 ){
								m_pArith->l_sub( F2217B, F2217B, (*m_pSnTanaData)->Sn_mnz40[0] );
								m_pArith->l_sub( F2218B, F2218B, (*m_pSnTanaData)->Sn_mnz40[2] );
								m_pArith->l_sub( F2217A, F2217A, (*m_pSnTanaData)->Sn_menzz[0] );
								m_pArith->l_sub( F2218A, F2218A, (*m_pSnTanaData)->Sn_menzz[2] );
							}
						}
						else{
							m_pArith->l_add( F2217C, F2217C, (*m_pSnTanaData)->Sn_mnz63[0] );
							m_pArith->l_add( F2218C, F2218C, (*m_pSnTanaData)->Sn_mnz63[2] );
							if( (*m_pSnHeadData)->SVmzsw == 1 ){
								m_pArith->l_add( F2217B, F2217B, (*m_pSnTanaData)->Sn_mnz40[0] );
								m_pArith->l_add( F2218B, F2218B, (*m_pSnTanaData)->Sn_mnz40[2] );
								m_pArith->l_add( F2217A, F2217A, (*m_pSnTanaData)->Sn_menzz[0] );
								m_pArith->l_add( F2218A, F2218A, (*m_pSnTanaData)->Sn_menzz[2] );
							}
						}
//--> '17.03.13 INS START
						if( (*m_pSnHeadData)->SVmzsw == 1 ){
							memmove( F2217AEX, F2217A, MONY_BUF_SIZE );
							memmove( F2217BEX, F2217B, MONY_BUF_SIZE );
							memmove( F2218AEX, F2218A, MONY_BUF_SIZE );
							memmove( F2218BEX, F2218B, MONY_BUF_SIZE );
						}
						memmove( F2217CEX, F2217C, MONY_BUF_SIZE );
						memmove( F2218CEX, F2218C, MONY_BUF_SIZE );
//<-- '17.03.13 INS END
					}
				}
				else{
...
void CShinFhyo22EX::OnBnClickedButton2()
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
...
// 各入力データ抽出
//-----------------------------------------------------------------------------
void CShinFhyo22EX::GetAllDiagData()
{
...
//　戻り値　：　次のフォーカスインデックス
//-----------------------------------------------------------------------------
int CShinFhyo22EX::UpDownFocusMoveCheck(int Index , int Move)
{
	int idx = 0;
	int i = 0 , j = 0;
...
// 初期ポジションセット
//-----------------------------------------------------------------------------
void CShinFhyo22EX::SetItemPosition()
{
	int index;
...
// 他表からの計算 ('14.05.20)
//-----------------------------------------------------------------------------
void CShinFhyo22EX::calq_by_OtherChange()
{
	m_DpSw = -1;
...
//			-1	：	エラー
//-----------------------------------------------------------------------------
int CShinFhyo22EX::SetCalqSw( int sw )
{
	F2223c = sw;
...
//-----------------------------------------------------------------------------
int CShinFhyo22EX::ChkTkkzIdx( short idx )
{
	int rSt = 0;

	if( (ID2210C<=idx) && (idx<=ID2211D) ){
		rSt = 1;
	}
...
// 返送値	計算した課税売上割合
//-----------------------------------------------------------------------------
unsigned short CShinFhyo22EX::GetCalqedKzwari()
{
	unsigned short	rtKzwari = F22UWS;
...
//-----------------------------------------------------------------------------
void CShinFhyo22EX::ClearTkkzMony()
{
	m_pArith->l_clear( F2210A );
	m_pArith->l_clear( F2210B );
	m_pArith->l_clear( F2210C );

	m_pArith->l_clear( F2211A );
	m_pArith->l_clear( F2211B );
	m_pArith->l_clear( F2211C );
