// ShinKaniEX2.cpp : 実装ファイル
//

#include "stdafx.h"
#include "ShinKaniEX2.h"
#include "ShinKaniEX2Idx.h"
...
IMPLEMENT_DYNAMIC(CShinKaniEX2, CSyzBaseDlg)

extern	HINSTANCE g_hInstance;

CShinKaniEX2::CShinKaniEX2(CWnd* pParent /*=NULL*/)
	: CSyzBaseDlg(CShinKaniEX2::IDD, pParent)
	, m_pParent(pParent)
...
CShinKaniEX2::~CShinKaniEX2()
{
}

void CShinKaniEX2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CShinKaniEX2, CSyzBaseDlg)
	ON_BN_CLICKED(IDC_BUTTON1, &CShinKaniEX2::OnBnClickedButton1)
	ON_WM_SIZE()
	ON_WM_VSCROLL()
	ON_WM_SHOWWINDOW()
	ON_WM_HSCROLL()
...
// CShinKaniEX2 メッセージ ハンドラ

BOOL CShinKaniEX2::OnInitDialog()
{
...
BOOL CShinKaniEX2::PreTranslateMessage(MSG* pMsg)
{
...
//			-1			：	エラー
//-----------------------------------------------------------------------------
int CShinKaniEX2::InitInfo( CH28SnHonpyoData **pSnHonpyoData, CH27SnFhyo10Data **pSnFhyo40Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo )
...
//2015.12.08 UPDATE START
	...
	if( ((CH26HyoView*)m_pParent)->GetShRevType() >= ID_ICSSH_MYNUMBER_REVTYPE ){
		...
		if((*m_pSnHeadData)->IsSoleProprietor() == TRUE ){ //個人
			...
			diag_index[0][0] = KANIEX_NUM_KOJIN1;
			diag_index[0][1] = KANIEX_NUM_KOJIN2;
			diag_index[0][2] = KANIEX_NUM_KOJIN3;
			diag_index[0][3] = -1;
			...
			diag_index[0][0] = KANIEX_NUM_HOJIN1;		
			diag_index[0][1] = KANIEX_NUM_HOJIN2;
			diag_index[0][2] = KANIEX_NUM_HOJIN3;
			diag_index[0][3] = KANIEX_NUM_HOJIN4;
			diag_index[0][4] = -1;
		...
		//法人番号
		int idx_cnt = 0;
		for( idx_cnt = KANIEX_NUM_HOJIN1; idx_cnt <= KANIEX_NUM_HOJINHYPHEN3; idx_cnt++ ){
			m_KaniDiag02.ModifyItem( idx_cnt, nHojinDispModify );
		}
		//個人番号
		for( idx_cnt = KANIEX_NUM_KOJIN1; idx_cnt <= KANIEX_NUM_KOJINHYPHEN2; idx_cnt++ ){
			m_KaniDiag02.ModifyItem( idx_cnt, nKojinDispModify );
		}

		m_ctrl_mynum.Init( this, kubun , (*m_pSnHeadData)->m_apno, TRUE, edit, m_KaniDiag02.GetSafeHwnd(), diag_index, MN_NO_PICKUP_DIAGDATA );
...
void CShinKaniEX2::DispGengo()
{
	...
	if( isReiwaPrnt ){
		...
		m_KaniDiag02.SetData(ID2003, (struct IUnknown *)&DiagData);
		// 申告年月日
		sprintf_s(buf, sizeof(buf), _T("%s  年  月  日"), SYZ26_REIWA_G_STR);
		DiagData.data_disp = buf;
		m_KaniDiag02.SetData(ID2085, (struct IUnknown *)&DiagData);
		// 指導年月日
		sprintf_s(buf, sizeof(buf), _T("  %s"), SYZ26_REIWA_G_STR);
		DiagData.data_disp = buf;
		m_KaniDiag02.SetData(ID2086, (struct IUnknown *)&DiagData);
	}
	else{
		// 提出年月日
...
// 初期データ表示
//-----------------------------------------------------------------------------
void CShinKaniEX2::DispInitKani()
{
	DIAGRAM_DATA	DiagData;
...
//<-- '18.12.18
	...
	if( (yy&0xff) && (mm&0xff) && (dd&0xff) ){ 
		...
		DiagData.data_disp = buf;
		m_KaniDiag02.SetData(ID2103, (struct IUnknown *)&DiagData);
	...
	if( (yy&0xff) && (mm&0xff) && (dd&0xff) ){
		...
		DiagData.data_disp = buf;
		m_KaniDiag02.SetData(ID2113, (struct IUnknown *)&DiagData);
...
//2018.03.13 UPDATE END
	...
	if( !(((*m_pSnHeadData)->Sn_SKKBN&0xff) % 2) )	{
		...
		if( (yy&0xff) && (mm&0xff) && (dd&0xff) ) { 
			...
			DiagData.data_disp = buf;
			m_KaniDiag02.SetData(ID2133, (struct IUnknown *)&DiagData);
		...
		if( (yy&0xff) && (mm&0xff) && (dd&0xff) ) { 
			...
			DiagData.data_disp = buf;
			m_KaniDiag02.SetData(ID2143, (struct IUnknown *)&DiagData);
...
// 初期データ表示
//-----------------------------------------------------------------------------
void CShinKaniEX2::DispInitKaniUpper()
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	char			buf[512] = {0};
...
//		Mony	：	表示金額
//-----------------------------------------------------------------------------
void CShinKaniEX2::dsp_prs( unsigned short Id, short Index, char *Mony )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
	char			MNY[MONY_BUF_SIZE] = {0};
...
//		Index	：	ダイアグラム内インデックス
//-----------------------------------------------------------------------------
void CShinKaniEX2::dsp_cls( unsigned short Id, short Index )
{
	DIAGRAM_DATA	DiagData;
	DiagData.data_imgdata = NULL;
...
// 終了処理
//-----------------------------------------------------------------------------
void CShinKaniEX2::EndJob()
{
	// エディット中だと正しく終了しない為
}


BEGIN_EVENTSINK_MAP(CShinKaniEX2, CSyzBaseDlg)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL2, 1, CShinKaniEX2::EditONIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL2, 2, CShinKaniEX2::EditOFFIcsdiagctrl2, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL2, 3, CShinKaniEX2::TerminationIcsdiagctrl2, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL3, 1, CShinKaniEX2::EditONIcsdiagctrl3, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL3, 2, CShinKaniEX2::EditOFFIcsdiagctrl3, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL3, 3, CShinKaniEX2::TerminationIcsdiagctrl3, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL4, 1, CShinKaniEX2::EditONIcsdiagctrl4, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL4, 2, CShinKaniEX2::EditOFFIcsdiagctrl4, VTS_I2)
	ON_EVENT(CShinKaniEX2, IDC_ICSDIAGCTRL4, 3, CShinKaniEX2::TerminationIcsdiagctrl4, VTS_I2 VTS_I2 VTS_I2 VTS_UNKNOWN)
END_EVENTSINK_MAP()


void CShinKaniEX2::EditONIcsdiagctrl2(short index)
{
...
void CShinKaniEX2::EditOFFIcsdiagctrl2(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKaniEX2::TerminationIcsdiagctrl2(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
//	char	Str[128] = {0};
	char buff[64] = {0};
					...
					// 経過措置有り
						if( (*m_pSnHeadData)->IsRevisedTaxReturn() ){
							...
							if ( ( *m_pSnHeadData )->IsRevisedTaxReturn() ) {
								...
								m_KaniDiag03.SetPosition ( ID3XM15 );	// 基準期間の課税売上高
								nowdiag = 3;
							}
						}	
					}
...
void CShinKaniEX2::EditONIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
void CShinKaniEX2::EditOFFIcsdiagctrl3(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKaniEX2::TerminationIcsdiagctrl3(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	...
	if( (wTerm==VK_TAB) || (wTerm==VK_LEFT) || (wTerm==VK_F2) || (wTerm==VK_UP) ){
		if( (*m_pSnHeadData)->Sn_Sign4&0x80 ){
			if( (*m_pSnHeadData)->SVmzsw == 1 ){//[Y]経過措置がある場合
				if ( ( *m_pSnHeadData )->Sn_SKKBN == 2 ) {	// 中間
					if ( index == ID3XM15 ) {
						m_KaniDiag02.SetPosition( ID2083 );
						nowdiag = 2;
					}
					else {
						m_KaniDiag03.SetPrevPosition();
					}
				}
				else if ( ( *m_pSnHeadData )->Sn_SKKBN == 4 ) {		// 修正中間
					if ( index == ID3XM13 ) {
						m_KaniDiag02.SetPosition( ID2083 );
						nowdiag = 2;
					}
...
void CShinKaniEX2::EditONIcsdiagctrl4(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	//-->Terminationへ移動 harino ('14.03.07)
	//RECT	rectA, rectB;
...
void CShinKaniEX2::EditOFFIcsdiagctrl4(short index)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
...
void CShinKaniEX2::TerminationIcsdiagctrl4(short index, short nChar, short length, LPUNKNOWN data)
{
	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( nChar == 0 ){
		return;
...
void CShinKaniEX2::OnBnClickedButton1()
{
...
//-----------------------------------------------------------------------------
//Kanisyo_calq
void CShinKaniEX2::calq_by_OtherChange( int ksw )
{
	m_DpSw = -1;
	m_kanisw = ksw;
	ReadData();
...
// 引数	no	：	
//-----------------------------------------------------------------------------
void CShinKaniEX2::SetItemPosition(int no )
{
	int index;
...
//		sign	：	変更サイン
//-----------------------------------------------------------------------------
void CShinKaniEX2::ChangeColor( unsigned short id, short Index, int sign )
{
	DIAGRAM_ATTRIBUTE	DA;
...
//			0	：	D列はA+B+C
//-----------------------------------------------------------------------------
int CShinKaniEX2::judge_skj_calq()
{
	if( m_pParent ){
...
//-----------------------------------------------------------------------------
void CShinKaniEX2::SetTaxOfficeInput()
{
	if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
		return;
	}
...
// 入力位置を保存 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinKaniEX2::SaveInputPosition()
{
	m_curInPos = -1;
...
// 入力位置を復元 ('15.03.02)
//-----------------------------------------------------------------------------
void CShinKaniEX2::RestoreInputPosition()
{
	if( m_curInPos == -1 ){
		return;
	}
...
//-----------------------------------------------------------------------------
void CShinKaniEX2::MoveFocusInEnd()
{
	// データを確定させる為にフォーカス移動
	if( m_curwnd == IDC_ICSDIAGCTRL2 ){
...
void CShinKaniEX2::OnSize(UINT nType, int cx, int cy)
{
...
void CShinKaniEX2::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CShinKaniEX2::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CShinKaniEX2::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: ここにメッセージ ハンドラ コードを追加するか、既定の処理を呼び出します。
	MoveDiagMNEdit();
...
void CShinKaniEX2::MoveDiagMNEdit()
{
	if( m_pParent && m_pParent->GetSafeHwnd() ){
//2016.06.22 UPDATE START
//		if( ((CH26HyoView*)m_pParent)->GetShRevType() >= ID_ICSSH_MYNUMBER_REVTYPE ){
		if((((CH26HyoView*)m_pParent)->GetShRevType() >= ID_ICSSH_MYNUMBER_REVTYPE)||((*m_pSnHeadData)->Sn_KDAYE >= 20160401 )){
//2016.06.22 UPDATE END
			if((*m_pSnHeadData)->IsSoleProprietor() == TRUE ){ //個人
				m_ctrl_mynum.MoveDiagMNEdit(KANIEX_NUM_KOJIN1,FALSE);
			}else{
				m_ctrl_mynum.MoveDiagMNEdit(KANIEX_NUM_HOJIN1,FALSE);
			}
			if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
...
//-----------------------------------------------------------------------------
void CShinKaniEX2::SetKanpuKinyuInput()
{
	if( (*m_pSnHeadData)->Sn_Sign4&0x01 ){
		return;
	}
