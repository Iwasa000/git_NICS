...
class CShinFhyo10EX : public CSyzBaseDlg
{
	DECLARE_DYNAMIC(CShinFhyo10EX)

public:
	CShinFhyo10EX(CWnd* pParent = NULL);   // 標準コンストラクタ
	virtual ~CShinFhyo10EX();

// ダイアログ データ
	enum { IDD = IDD_DIALOG_FHY10_EX };

	// 初期情報のセット
	int InitInfo( CH28SnFhyo10Data **pSnFhyo10Data, CSnHeadData **pSnHeadData, CArithEx *pArith, SYC_SHININFO *pShinInfo , CH28SnHonpyoData **pSnHonpyoData );
	// 初期化
	int Init();
	// ダイアグラム初期属性セット
	int InitDiagAttr();
	...
	// 課税資産の譲渡等の対価の額
	void f1calq_3_1( int sw );
	// 特定課税仕入れに係る支払対価の額
	void f1calq_3_2( int sw );
	// 消費税額(4%税率分)
	...
	// 売上の返還等対価に係る税額
	void f1calq_9_1( int sw );
	// 特定課税仕入れの返還等対価に係る税額
	void f1calq_9_2( int sw );
	// 貸倒れに係る税額
...
private:
	...
	char				F11_1A[MONY_BUF_SIZE],	//  1_1.A 課税資産の譲渡等の対価の額(税率３％分)
						F11_1B[MONY_BUF_SIZE],	//  1_1.B     　　　　〃　　　　　　(税率４％分)
						F11_1C[MONY_BUF_SIZE],	//  1_1.C         　　〃　　　　　　(税率６.３％分)
						F11_1T[MONY_BUF_SIZE];	//  1_1.D         　　〃　　　　　　(合計)

	char				F11_2A[MONY_BUF_SIZE],	//  1_2.A 特定課税仕入れに係る支払対価の額(税率３％分)
						F11_2B[MONY_BUF_SIZE],	//  1_2.B     　　　　　〃    　　　　　　(税率４％分)
						F11_2C[MONY_BUF_SIZE],	//  1_2.C     　　　　　〃    　　　　　　(税率６.３％分)
						F11_2T[MONY_BUF_SIZE];	//  1_2.D     　　　　　〃    　　　　　　(合計)
	...
	char				F15_1A[MONY_BUF_SIZE],	//  5_1.A 売上の返還等対価に係る税額(税率３％分)
						F15_1B[MONY_BUF_SIZE],	//  5_1.B     　　　　〃            (税率４％分)
						F15_1C[MONY_BUF_SIZE],	//  5_1.C     　　　　〃            (税率６.３％分)
						F15_1T[MONY_BUF_SIZE];	//  5_1.D     　　　　〃            (合計)
													 
	char				F15_2A[MONY_BUF_SIZE],	//  5_2.A 特定課税仕入れの返還等対価に係る税額(税率３％分)
						F15_2B[MONY_BUF_SIZE],	//  5_2.B     　　　　　　　〃　　            (税率４％分)
						F15_2C[MONY_BUF_SIZE],	//  5_2.C     　　　　　　　〃     　         (税率６.３％分)
						F15_2T[MONY_BUF_SIZE];	//  5_2.D     　　　　　　　〃       　       (合計)
...
//	CH26SnFhyo10Data	**m_pSnFhyo10Data;
	CH28SnFhyo10Data	**m_pSnFhyo10Data;
//	CH26SnHonpyoData	**m_pSnHonpyoData;		//[Y] 本表
	CH28SnHonpyoData	**m_pSnHonpyoData;		//[Y] 本表
