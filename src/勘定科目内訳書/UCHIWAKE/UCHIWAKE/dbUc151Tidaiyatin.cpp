// dbUc151Tidaiyatin.cpp : インプリメンテーション ファイル
//
...
IMPLEMENT_DYNAMIC(CdbUc151Tidaiyatin, CdbUc000Common)

CdbUc151Tidaiyatin::CdbUc151Tidaiyatin(CDatabase* pdb)
	: CdbUc000Common(pdb)
{
	//{{AFX_FIELD_INIT(CdbUc151Tidaiyatin)
	m_Youto = _T("");
	m_AdSeq = 0;
	m_AdOrder = 0;
	m_AdName1 = _T("");
...
// midori 152137 add <--
	...
	m_lo_TableName = STR_TBL_NAME_151;			// テーブル名を記述
}


CString CdbUc151Tidaiyatin::GetDefaultSQL()
{
	return _T("[dbo].[uc_151_tidaiyatin]");
}

void CdbUc151Tidaiyatin::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CdbUc151Tidaiyatin)
...
/////////////////////////////////////////////////////////////////////////////
// CdbUc151Tidaiyatin 診断

#ifdef _DEBUG
void CdbUc151Tidaiyatin::AssertValid() const
{
	CRecordset::AssertValid();
}

void CdbUc151Tidaiyatin::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc151Tidaiyatin::CreateNewRecord()
{
...
//	m_Kubun = _T("");		// ("") 地代・家賃の区分
	m_KnSeq = 0;			// ("") 地代・家賃の区分：シーケンス番号
	m_KnOrder = 0;			// ("") 地代・家賃の区分：順序番号
	m_KnName = _T("");		// ("") 地代・家賃の区分：名称
	m_Youto = _T("");		// ("") 借地物件の用途
	m_Ad2Add1 = _T("");		// ("") 所在地(1行目)
	m_Ad2Add2 = _T("");		// ("") 所在地(1行目)
	m_AdSeq = 0;			// (0)  買主：シーケンス番号
	m_AdOrder = 0;			// (0)  買主：順序番号
	m_AdName1 = _T("");		// ("") 買主：名称(1行目)
	m_AdName2 = _T("");		// ("") 買主：名称(2行目)
	m_AdAdd1 = _T("");		// ("") 買主：所在地
	m_AdAdd2 = _T("");		// ("") 買主：所在地
	m_SpDate = 0;			// (0)  支払対象期間(開始値)
	m_SpDate2 = 0;			// (0)  支払対象期間(終了値)
	m_Val = _T("");			// ("") 支払貸借料
	m_Teki = _T("");		// ("") 摘要
	m_ShowKeiZero = 0;		// (0)  ０円計表示フラグ
// midori 152137 add -->
	m_KnKana = _T("");		// ("") 地代・家賃の区分(カナ)
// midori 152137 add <--
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc151Tidaiyatin::CreateNewRecord2(int pNumPage,int pNumRow)
// 157153 del <--
// 157153 add -->
// ------------------------------------------------------------------------------------------------
//	CreateNewRecord2()
...
//
// ------------------------------------------------------------------------------------------------
int CdbUc151Tidaiyatin::CreateNewRecord2(int pNumPage,int pNumRow,int pNullSw)
// 157153 add <--
{
	CRecordset	rs(m_lo_pdb);
	CString		SqlStr;
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc151Tidaiyatin::UpdateYokukiZeroClear()
//{
//	CString	sql = GetSQLUpdateYokukiZeroClear("Val");		// 支払貸借料
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc151Tidaiyatin::UpdateYokukiZeroClear( int pSw )
{
// midori 152388 add -->
	CString	cs=_T("");
// midori 152388 add <--
	CString	sql = GetSQLUpdateYokukiZeroClear("Val",pSw);		// 支払貸借料
...
// midori 152388 add -->
	if(pSw != 0)	{
		...
		cs += _T(" KnOrder=0 AND Youto IS NULL");
		cs += _T(" AND Ad2Add1 IS NULL AND Ad2Add2 IS NULL");
		cs += _T(" AND AdName1 IS NULL AND AdAdd1 IS NULL AND AdAdd2 IS NULL");
		cs += _T(" AND SpDate=0 AND SpDate2=0 AND Val IS NULL AND Teki IS NULL");
...
***********************************************************************/
// virtual
int CdbUc151Tidaiyatin::UpdateDataTableWork()
{
