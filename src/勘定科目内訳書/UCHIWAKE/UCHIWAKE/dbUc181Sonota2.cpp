// dbUc181Sonota2.cpp : インプリメンテーション ファイル
//
...
IMPLEMENT_DYNAMIC(CdbUc181Sonota2, CdbUc000Common)

//CdbUc181Sonota2::CdbUc181Sonota2(CDatabase* pdb)
CdbUc181Sonota2::CdbUc181Sonota2(CDatabase* pdb, int inFormSeq)
	: CdbUc000Common(pdb)
{
	//{{AFX_FIELD_INIT(CdbUc181Sonota2)
	m_Seq = 0;
	m_NumPage = 0;
	m_NumRow = 0;
	m_FgFunc = 0;
...
// midori 152137 add <--
	...
	m_lo_TableName = STR_TBL_NAME_181;			// テーブル名を記述
	m_lo_FormSeq = inFormSeq;					// フォームシーケンス番号を保持
												// (その他帳表の場合のみ FormSeq 必要)
}


CString CdbUc181Sonota2::GetDefaultSQL()
{
	return _T("[dbo].[uc_181_sonota2]");
}

void CdbUc181Sonota2::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CdbUc181Sonota2)
...
#ifdef _DEBUG
void CdbUc181Sonota2::AssertValid() const
{
	CRecordset::AssertValid();
}

void CdbUc181Sonota2::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc181Sonota2::CreateNewRecord()
{
...
//			retVal = val.m_iVal;
	...
	m_FormSeq = m_lo_FormSeq;	// (0)  その他②の様式番号
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc181Sonota2::CreateNewRecord2(int pNumPage,int pNumRow)
// 157153 del <--
// 157153 add -->
// ------------------------------------------------------------------------------------------------
//	CreateNewRecord2()
...
//
// ------------------------------------------------------------------------------------------------
int CdbUc181Sonota2::CreateNewRecord2(int pNumPage,int pNumRow,int pNullSw)
// 157153 add <--
{
	CRecordset	rs(m_lo_pdb);
	CString		SqlStr;
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc181Sonota2::UpdateYokukiZeroClear()
//{
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc181Sonota2::UpdateYokukiZeroClear( int pSw)
{
// midori 152388 add -->
	CString	cs=_T("");
// midori 152388 add <--
...
// midori 152388 add -->
	if(pSw != 0)	{
		...
		cs += _T(" KnOrder=0");
		cs += _T(" AND Item IS NULL AND Val IS NULL AND Teki IS NULL");
		sql += cs;
	}
// midori 152388 add <--
	return	ExecuteSQLWork(sql);
...
//@
***********************************************************************/
CString CdbUc181Sonota2::GetSQLKeiInsert(int nPage, CMyStringArray* arryColumn, CString strKei/*=0*/, int nFgFunc/*=0*/, CMyArray* arryTotalAll/*=NULL*/)
{
	int nRuikei = 0;
	CString strSQL;
	CString strTmp;
...
***********************************************************************/
// virtual
int CdbUc181Sonota2::GetSortIkkatuOrder()
{
	return m_KnOrder;
}