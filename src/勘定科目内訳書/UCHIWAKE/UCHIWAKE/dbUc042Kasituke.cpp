// dbUc042Kasituke.cpp : インプリメンテーション ファイル
//
...
IMPLEMENT_DYNAMIC(CdbUc042Kasituke, CdbUc000Common)

CdbUc042Kasituke::CdbUc042Kasituke(CDatabase* pdb)
	: CdbUc000Common(pdb)
{
	//{{AFX_FIELD_INIT(CdbUc042Kasituke)
	m_Seq = 0;
	m_NumPage = 0;
	m_NumRow = 0;
	m_FgFunc = 0;
...
// midori 152137 add <--
	...
	m_lo_TableName = STR_TBL_NAME_042;			// テーブル名を記述
}


CString CdbUc042Kasituke::GetDefaultSQL()
{
	return _T("[dbo].[uc_042_kasituke]");
}

void CdbUc042Kasituke::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CdbUc042Kasituke)
...
#ifdef _DEBUG
void CdbUc042Kasituke::AssertValid() const
{
	CRecordset::AssertValid();
}

void CdbUc042Kasituke::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc042Kasituke::CreateNewRecord()
{
...
//			retVal = val.m_iVal;
	...
	m_AdSeq = 0;			// (0)  貸付先：(取引)シーケンス番号
	m_AdOrder = 0;			// (0)  貸付先：(取引)順序番号
	m_AdName1 = _T("");		// ("") 貸付先：(取引)貸付先(1行目)
	m_AdName2 = _T("");		// ("") 貸付先：(取引)貸付先(2行目)
	m_AdAdd1 = _T("");		// ("") 貸付先：(取引)所在地(1行目)
	m_AdAdd2 = _T("");		// ("") 貸付先：(取引)所在地(2行目)
	m_Kankei = _T("");		// ("") 貸付先：法人・代表者との関係
	m_Val = _T("");			// ("") 期末現在高
	m_Risoku = _T("");		// ("") 期中の受取利息
	m_Rate = _T("");		// ("") 利率
	m_Riyuu = _T("");		// ("") 貸付理由
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc042Kasituke::CreateNewRecord2(int pNumPage,int pNumRow)
// 157153 del <--
// 157153 add -->
// ------------------------------------------------------------------------------------------------
//	CreateNewRecord2()
...
//
// ------------------------------------------------------------------------------------------------
int CdbUc042Kasituke::CreateNewRecord2(int pNumPage,int pNumRow,int pNullSw)
// 157153 add <--
{
	CRecordset	rs(m_lo_pdb);
	CString		SqlStr;
...
//								DB_ERR_OK (0) 以外：失敗
//***********************************************************************/
//int CdbUc042Kasituke::UpdateYokukiZeroClear()
//{
//	CString	sql = GetSQLUpdateYokukiZeroClear("Val");			// 期末現在高
//	sql += GetSQLUpdateYokukiZeroClear("Risoku");				// 期中の受取利息額
//	//CString		strCommand;
//	//CString		strTemp;
//
//	//// クリア対象項目（2項目）
...
//	//strCommand	+=	_T("Risoku = 0 ");						// 期中の受取利息額
//
...
/**********************************************************************
	...
	戻値
			int					DB_ERR_OK (0)     ：成功
								DB_ERR_OK (0) 以外：失敗
***********************************************************************/
int CdbUc042Kasituke::UpdateYokukiZeroClear( int pSw )
{
// midori 152388 add -->
	CString	cs=_T("");
// midori 152388 add <--
	CString	sql = GetSQLUpdateYokukiZeroClear("Val",pSw);			// 期末現在高
	sql += GetSQLUpdateYokukiZeroClear("Risoku",pSw);				// 期中の受取利息額
// midori 152388 add -->
	if(pSw != 0)	{
		cs.Format(_T("UPDATE %s SET FgFunc=%d WHERE FgFunc<=%d AND "),m_lo_TableName,ID_FGFUNC_NULL,ID_FGFUNC_DATA);
		cs += _T(" AdName1 IS NULL AND AdName2 IS NULL AND Kankei IS NULL");
		cs += _T(" AND AdAdd1 IS NULL AND AdAdd2 IS NULL AND Val IS NULL");
		cs += _T(" AND Risoku IS NULL AND Rate IS NULL AND Riyuu IS NULL AND Naiyou IS NULL");
...
***********************************************************************/
// virtual
int CdbUc042Kasituke::UpdateDataTableWork()
{
...
//@
***********************************************************************/
CString CdbUc042Kasituke::GetSQLSumColumn()
{
	CString strSQL;
...
//@
***********************************************************************/
int CdbUc042Kasituke::GetColumnNum()
{
	return 2;
}
...
//@
***********************************************************************/
CString CdbUc042Kasituke::GetSQLKeiInsert(int nPage, CMyStringArray* arryColumn, CString strKei/*=0*/, int nFgFunc/*=0*/, CMyArray* arryTotalAll/*=NULL*/)
{
	int nRuikei = 0;
	CString strSQL;
	CString strTmp;
...
//********************************************************************************
CString CdbUc042Kasituke::GetSQLZeroJudgment()
{
	CString strSQL;
	...
	if((m_OutZeroNull & BIT_D0) && (m_OutZeroNull & BIT_D1))
		...
		strSQL.Format("((%s IS NULL) AND (Risoku IS NULL))", GetZeroMoneyMoveField() );
...
//		【戻値】	CString SQLのWHERE句の一部
//********************************************************************************
CString CdbUc042Kasituke::GetSQLValIsNull()
{
	CString strSQL;
...
***********************************************************************/
long CdbUc042Kasituke::GetColumnSortAddressBank(BYTE ItemSeq)
{
...
***********************************************************************/
// virtual
int CdbUc042Kasituke::GetSortIkkatuOrder()
{
	return m_AdOrder;
}