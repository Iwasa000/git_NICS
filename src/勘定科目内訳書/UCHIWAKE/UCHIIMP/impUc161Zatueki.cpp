...
// コンストラクタ
CimpUc161Zatueki::CimpUc161Zatueki(void)
	: CimpUc000Common()
{
	m_pUcData				= NULL;
// midori 180404 add -->
...
//**********************************************************************
//virtual
int CimpUc161Zatueki::Init(CDatabase* pDB)
{
	//--------------------------------------------------
	// 全帳票共通のコード
	//--------------------------------------------------
...
// 登録番号（法人番号）_23/11/21 add <--
	...
	// 内訳書テーブルとの接続を開始
	m_pUcCmn = (CdbUc000Common*)(new CdbUc161Zatueki(pDB));

	// InfSubテーブルの初期化
	m_pUcInfSub->Init( m_nFormSeq );

	// inf_form_typeテーブルの初期化
	m_pUcInfFormType->Init( ID_FORMNO_16 );
	...
	//--------------------------------------------------
	// １頁内の合計行を含む全体の行数
	m_RowMax		= ID_ROWKOBETU_161;
	// １頁内のデータ行数（合計行は含まず）
	m_RowMaxData	= ID_ROWKOBETU_161 - 1;
	// 帳表固定の合計行数（1:合計あり，0:なし）
	m_RowKei		= ID_ROWKEI_161;

	// 出力設定が通常の場合
	if( m_pUcInfFormType->m_FormType == 0 ){
		m_RowMax		= ID_ROWNORMAL_161;
		m_RowMaxData	= ID_ROWNORMAL_161 - 1;
	}
...
//**********************************************************************
//virtual
int CimpUc161Zatueki::SetCsvOneLineData(CStringArray* pstrCsvDataAry)
{
// No.200903 add -->
	int			len=0;
// No.200903 add <--
...
// midori 180404 add <--
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++
	// 一時的にテーブルアクセス領域を拡張
	m_pUcData = (CdbUc161Zatueki*)m_pUcCmn;
...
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 01 科目
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(1), MAX_KETA_161_KAMOKU*2) != TRUE) {
// -------------------------------------
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_KAMOKU*2) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_01);
		m_uImpCsvInfo.nLength = MAX_KETA_161_KAMOKU;
	...
	// 02 取引の内容１
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(2), MAX_KETA_161_NAIYOU) != TRUE) {
// -------------------------------------
	ofset++;
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_NAIYOU) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_02);
		m_uImpCsvInfo.nLength = MAX_KETA_161_NAIYOU/2;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
		nRet = IMP_REC_NG;
	...
	// 03 取引の内容２
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(3), MAX_KETA_161_NAIYOU) != TRUE) {
// -------------------------------------
	ofset++;
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_NAIYOU) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_03);
		m_uImpCsvInfo.nLength = MAX_KETA_161_NAIYOU/2;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
...
// 登録番号（法人番号）_23/11/21 cor -->
	//if(pstrCsvDataAry->GetAt(4).IsEmpty() == TRUE && pstrCsvDataAry->GetAt(5) != TRUE)	{
	//	pstrCsvDataAry->SetAt(4,pstrCsvDataAry->GetAt(5));
	//	pstrCsvDataAry->SetAt(5,_T(""));
	//}
// -------------------------------------
	ofset++;
	if(m_NewVer2 == TRUE)	{
...
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 04 相手先１
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(4), MAX_KETA_161_ADNAME1*2) != TRUE) {
// -------------------------------------
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_ADNAME1*2) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		if(bG_Kanso == TRUE)	strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_04_NEW);
		else					strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_04);
		m_uImpCsvInfo.nLength = MAX_KETA_161_ADNAME1;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
		nRet = IMP_REC_NG;
	...
	// 05 相手先２
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(5), MAX_KETA_161_ADNAME2*2) != TRUE) {
// -------------------------------------
	ofset++;
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_ADNAME2*2) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		if(bG_Kanso == TRUE)	strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_05_NEW);
		else					strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_05);
		m_uImpCsvInfo.nLength = MAX_KETA_161_ADNAME2;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
		nRet = IMP_REC_NG;
	...
	// 06 所在地（住所）１
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(6), MAX_KETA_161_ADADD1*2) != TRUE) {
// -------------------------------------
	ofset++;
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_ADADD1*2) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		if(bG_Kanso == TRUE)	strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_06_NEW);
		else					strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_06);
		m_uImpCsvInfo.nLength = MAX_KETA_161_ADADD1;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
		nRet = IMP_REC_NG;
	...
	// 07 所在地（住所）２
// 登録番号（法人番号）_23/11/21 cor -->
	//if (CheckImportStr(pstrCsvDataAry->GetAt(7), MAX_KETA_161_ADADD2*2) != TRUE) {
// -------------------------------------
	ofset++;
	if (CheckImportStr(pstrCsvDataAry->GetAt(ofset), MAX_KETA_161_ADADD2*2) != TRUE) {
// 登録番号（法人番号）_23/11/21 cor <--
		if(bG_Kanso == TRUE)	strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_07_NEW);
		else					strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_07);
		m_uImpCsvInfo.nLength = MAX_KETA_161_ADADD2;
		m_uImpCsvInfo.nKind = 0;	// 0:文字
		m_ErrMap.SetAt((int)m_ErrMap.GetCount(),m_uImpCsvInfo);
		CsvInfoClr(&m_uImpCsvInfo);
		nRet = IMP_REC_NG;
...
// 登録番号（法人番号）_23/11/21 cor <--
		strcpy_s(m_uImpCsvInfo.szColumn, sizeof(m_uImpCsvInfo.szColumn), CSV_161_ITEM_08);
...
//	// 06 所在地（住所）１
//	// 07 所在地（住所）２
//	if (CheckEntryAddress(	pstrCsvDataAry->GetAt(4), pstrCsvDataAry->GetAt(5),
//							pstrCsvDataAry->GetAt(6), pstrCsvDataAry->GetAt(7),
...
//		m_pUcData->m_AdName1	= pstrCsvDataAry->GetAt(4);
//		m_pUcData->m_AdName2	= pstrCsvDataAry->GetAt(5);
//		m_pUcData->m_AdAdd1		= pstrCsvDataAry->GetAt(6);
//		m_pUcData->m_AdAdd2		= pstrCsvDataAry->GetAt(7); 
//// midori 152137 add -->
//		m_pUcData->m_AdKana		= fc.Convert(pstrCsvDataAry->GetAt(4));
//// midori 152137 add <--
//
//		// 08 金額
...
//**********************************************************************
//virtual
int CimpUc161Zatueki::ImpCsvOneLineData(CStringArray* pstrCsvDataAry)
{
	//--------------------------------------------------
	// 前処理（全帳票共通のコード）
	//--------------------------------------------------
...
// midori 190504 add <--
	...
	// 01 科目
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(1),&strTemp,MAX_KETA_161_KAMOKU*2);
	//pstrCsvDataAry->SetAt(1,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_KAMOKU*2);
...
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 02 取引の内容１
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(2),&strTemp,MAX_KETA_161_NAIYOU);
	//pstrCsvDataAry->SetAt(2,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_NAIYOU);
	pstrCsvDataAry->SetAt(ofset,strTemp);
	ofset++;
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 03 取引の内容２
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(3),&strTemp,MAX_KETA_161_NAIYOU);
...
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 04 相手先１
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(4),&strTemp,MAX_KETA_161_ADNAME1*2);
	//strTemp = ufc.DeleteRightSpace(strTemp);
	//pstrCsvDataAry->SetAt(4,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_ADNAME1*2);
	strTemp = ufc.DeleteRightSpace(strTemp);
	pstrCsvDataAry->SetAt(ofset,strTemp);
	ofset++;
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 05 相手先２
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(5),&strTemp,MAX_KETA_161_ADNAME2*2);
	//strTemp = ufc.DeleteRightSpace(strTemp);
	//pstrCsvDataAry->SetAt(5,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_ADNAME2*2);
	strTemp = ufc.DeleteRightSpace(strTemp);
	pstrCsvDataAry->SetAt(ofset,strTemp);
	ofset++;
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 06 所在地（住所）１
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(6),&strTemp,MAX_KETA_161_ADADD1*2);
	//strTemp = ufc.DeleteRightSpace(strTemp);
	//pstrCsvDataAry->SetAt(6,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_ADADD1*2);
	strTemp = ufc.DeleteRightSpace(strTemp);
	pstrCsvDataAry->SetAt(ofset,strTemp);
	ofset++;
// 登録番号（法人番号）_23/11/21 cor <--
	...
	// 07 所在地（住所）２
// 登録番号（法人番号）_23/11/21 cor -->
	//GetSpritString(pstrCsvDataAry->GetAt(7),&strTemp,MAX_KETA_161_ADADD2*2);
	//strTemp = ufc.DeleteRightSpace(strTemp);
	//pstrCsvDataAry->SetAt(7,strTemp);
// -------------------------------------
	GetSpritString(pstrCsvDataAry->GetAt(ofset),&strTemp,MAX_KETA_161_ADADD2*2);
	strTemp = ufc.DeleteRightSpace(strTemp);
	pstrCsvDataAry->SetAt(ofset,strTemp);
	ofset++;
// 登録番号（法人番号）_23/11/21 cor <--
...
//**********************************************************************
//virtual
int CimpUc161Zatueki::GetCsvOneLineData(CStringArray* pstrCsvDataAry)
{
	//--------------------------------------------------
	// 前処理（全帳票共通のコード）
	//--------------------------------------------------
...
// 登録番号（法人番号）_23/11/21 cor -->
		...
		//// 04 相手先１
		//pstrCsvDataAry->SetAt(4, m_pUcData->m_AdName1);

		//// 05 相手先２
		//pstrCsvDataAry->SetAt(5, m_pUcData->m_AdName2);
		//
		//// 06 所在地（住所）１
		//pstrCsvDataAry->SetAt(6, m_pUcData->m_AdAdd1);

		//// 07 所在地（住所）２
		//pstrCsvDataAry->SetAt(7, m_pUcData->m_AdAdd2);
...
//		RET		なし
//********************************************************************************
void	CimpUc161Zatueki::virTableEditIkkatuLine( int nType, CdbUc000Common* rsData,
									CString strName, CALCKEI_INFO uCalcKei, SORTKAMOKU_INFO uKamoku )
{
	...
	case 0:
		...
		m_pUcData->m_Naiyou = strName;					// 名称：金融機関名
