...
{
	m_Brush.CreateSolidBrush( SELECT_BACK_COL );
}
...
#define NMAP_STYLE(src, dest) if(!(dwStyle & (src))) dwText |= (dest)
	...
	CPen penFrame;
	penFrame.CreatePen(PS_SOLID, 2, SELECT_BACK_COL);
	CPen* pOldPen = dc.SelectObject(&penFrame);

//	dc.Rectangle( client_rect );
//	dc.FillSolidRect( client_rect, SELECT_BACK_COL );
	dc.FrameRect( client_rect, &m_Brush );
