...
//MyTrace( "taskrc %d,%d, %d,%d\n", taskrc.left, taskrc.top, taskrc.right, taskrc.bottom );
	...
	int max = p_DnpView->get_MaxLineEX();
...
TRACE("\nnPos = %d, curPos = %d\n", nPos, curPos );
		...
		case SB_LINEUP:
			...
			break;
		case SB_PAGEDOWN:
			p_DnpView->Vs_Linemove( DP_SCROLL_NL_F );	TRACE( "SB_PAGEDOWN\n" );
			break;
		case SB_PAGEUP:
			p_DnpView->Vs_Linemove( -DP_SCROLL_NL_F );	TRACE( "SB_PAGEUP\n");
			break;
		case SB_THUMBTRACK:
