...
typedef	struct	tag_SheetTable {
	...
	int			karisgn[DP_SCROLL_MAX];	//借方位置データ有無サイン (1=データ有, 99=伝票修正時に追加して他プログラム移動のタイミングで更新されたデータ)
	int			kasisgn[DP_SCROLL_MAX];	//貸方位置データ有無サイン (1=データ有, 99= 　同上 )
...
public:
	...
	void	DenpScanInit( CDipZmSub* pZm );
...
protected:
	int		set_sheet( DWORD icno, int vect );
