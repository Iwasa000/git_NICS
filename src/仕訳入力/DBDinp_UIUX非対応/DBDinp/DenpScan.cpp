...
// イニシャライズ
void	CDenpScan::DenpScanInit( CDipZmSub* pZm )
{
	m_pZm = pZm;
}
...
int		CDenpScan::SetLastSheet()
{
	...
	if( set_sheet( m_daICNO[m_daINDEX], 0 ) == 0 ) {
		...
		if( ! m_Sheet.dp_end ) {
			while(1) {
				//最終ページまで検索
				if( set_sheet( m_daICNO[m_daINDEX], 1 ) != 0 )
					return -1;
...
//	return -1 ===> データなし
//
int CDenpScan::set_sheet( DWORD icno, int vect )
{
	CString filter;
...
BOOL CDenpScan::IsFukugoSiwake( int ln )
{
	if( m_Sheet.dtype == TYPE_FURIKAE ) {
		...
		if( m_Sheet.karisgn[idx] == 99 || m_Sheet.kasisgn[idx] == 99 ) {
			//伝票修正時の追加は、諸口詰めないモードでも複合仕訳がありうる。
		}
		else {
			//諸口を詰めない場合は、複合仕訳としない。
