...
/////////////////////////////////////////////////////////////////////////////
// CHusenDlg ダイアログ
#define HS_SEL_PN		HSN_HSEL_PN		
#define HS_CMT_PN		HSN_COMMENT_PN	
...
CHusenDlg::CHusenDlg(CWnd* pParent /*=NULL*/, int type )
	: ICSDialog(CHusenDlg::IDD, pParent)
{
...
// 初期化処理
void CHusenDlg::HusenInp_Init( HSEN_DATA *crec, HSEN_DATA *srec )
{
	m_pCREC = crec;
	m_pSREC = srec;
	...
	if( m_pCREC->hsen_inpsg ) {
		if( m_pCREC->hsen_sel < 0 )
			m_Hcbo.SetCurSel(-1);
		else
			m_Hcbo.SetCurSel( m_pCREC->hsen_sel );
		strcpy_s( buf, sizeof buf, m_pCREC->hsen_cmt );
		var.pbVal = (BYTE*)buf;
		DBdata_set( this, IDC_ICSDBEDTCTRL1, &var, ICSDBEDT_TYPE_STRING, 0 );
	}
	else {
		int sgn = (m_pSREC->hsen_sel);
		...
		strcpy_s( buf, sizeof buf, srec->hsen_cmt );
		var.pbVal = (BYTE*)buf;
		DBdata_set( this, IDC_ICSDBEDTCTRL1, &var, ICSDBEDT_TYPE_STRING, 0 );
	}
...
BEGIN_MESSAGE_MAP(CHusenDlg, ICSDialog)
...
//	ControlInit( 82, 36, -1 );
	...
	if( m_Type ) {
		CRect rect;
		m_DBedt.GetWindowRect(&rect);

		ScreenToClient( &rect );
		double xRate = (double)169/(double)53;

		int wd = rect.Width();
		int new_wd = wd * xRate;
		GetWindowRect( &rect );	//ダイアログサイズ変更
		MoveWindow( rect.left, rect.top, rect.Width() + (new_wd-wd), rect.Height() );

		m_DBedt.SetMaxLen( 40 );
	}

	m_DBedt.EnableDelete(0);

//	m_DBedt.MoveWindow( rect.left, rect.top, new_wd, rect.Height() );
...
void CHusenDlg::OnKillfocusHsCombo() 
{
	...
	if( sel != CB_ERR ) {
		m_pCREC->hsen_sel = sel;
		m_pParent->SendMessage( WM_HSINP_INEND, MAKELONG(1, 0), (LPARAM)m_pCREC );
	}	
}

BEGIN_EVENTSINK_MAP(CHusenDlg, ICSDialog)
...
void CHusenDlg::OnKillFocusIcsdbedtctrl1(long inplen) 
{
	...
	TerminationHusenComment( 0, inplen, 0 );
}
...
//	if( inplen ) {
		...
		if( m_MODEFY_OK ) {
			...
			if( var.pbVal != NULL )	strncpy_s( buf, sizeof buf, (char*)var.pbVal, keta );
			strcpy_s( m_pCREC->hsen_cmt, sizeof m_pCREC->hsen_cmt, buf );
...
long CHusenDlg::TermWork( int pn, long nChar )
{
	...
	if( nChar == VK_RIGHT || nChar == VK_LEFT || nChar == VK_F2 ) {
		to_parent( nChar );
		ret = 0;
	}
...
{
	m_pParent->SendMessage( WM_HSINP_INEND, MAKELONG(0, nChar), (LPARAM)m_pCREC );
}

//消費税選択後の移動
void CHusenDlg::set_nextpos()
{
	switch( get_nowpn() ) {
		//追加消費税区分
		case HS_SEL_PN:
		...
		//税率
		case HS_CMT_PN:
			TerminationHusenComment( VK_RETURN, 0, 0 );
			break;
	}
}
	...
	if( pMsg->message == WM_KEYDOWN ) {
		if( pMsg->wParam == VK_F2 ) {
			if( pMsg->hwnd == m_Hcbo.m_hWnd ) {
				if( ! TermWork( HS_SEL_PN, VK_F2 ) )
					return TRUE;
			}
			else if( pMsg->hwnd == m_DBedt.m_hWnd ) {
				if( ! TermWork( HS_CMT_PN, VK_UP ) )
					return TRUE;
			}
		}
	}
...
void CHusenDlg::OnSize(UINT nType, int cx, int cy)
{
	ICSDialog::OnSize(nType, cx, cy);

	// TODO: ここにメッセージ ハンドラ コードを追加します。
	if( IsWindow( m_DBedt.m_hWnd ) ) {
		DBedtMove();
	}
}
...
void CHusenDlg::DBedtMove()
{
	CRect rect, edtRect, stRC;

	GetWindowRect( &rect );

	m_DBedt.GetWindowRect( &edtRect );

	GetDlgItem(IDC_STATIC2)->GetWindowRect( &stRC );
	ScreenToClient( &stRC );

	edtRect.right = rect.right - stRC.left;
	ScreenToClient( &edtRect );

	m_DBedt.MoveWindow( edtRect.left, edtRect.top, edtRect.Width(), edtRect.Height() );
}