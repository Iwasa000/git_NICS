...
/*-------------*/
	...
	int		_SelectJob2(SelectParam* par, int nChar = -1);
	void	ParamCopy();
/*-------------*/
	int		_SelectUpdate();
	...
	int   BankAITKkmkCount(int* bankinfo, int infcnt, CString tkstr, int dcsw);
	int	  _AITKkmkSelect(int nChar, BOOL bNowPage = FALSE);
	void  _AITKkmkSelectSub(int page);

	int	  _InvNoSelect(int nChar, BOOL bNowPage = FALSE, int prev_pn =-1);
	void  _InvNoSelectSub(CArray<INVOICE_INFO>* pAry, int page);
	int	  _GetInvNoSelect(SelResult* result, int sel_no);
	void  _RenewInvno();
...
protected:
	...
	void	_init_kengyokmk_old();	// 05.26 /10
	void	_init_kengyokmk();		// 10.06 /17
	...
	int		km_dsp( int *knp  );
	int		kmk_50search( char* kana, int kmhani = 0 );
	void	kn50dsp( KMKANA_DTA *knp, int syusgn = 0 );
	int		get_hyojikncod( int n );
	char	*get_hyojiknam( int n, int syusgn = 0, DBKNREC* knrec =NULL);
...
//	int			KCD50C[AC_MAX*PG_MAX];
	...
	CArray<DBTKREC, DBTKREC&>	m_TKYkana;	//科目・仕訳対応摘要のカナ順
...
//	int		m_KjKanaPG;
	...
	int		m_bHjiyu;	// 0 = 変動事由なし。 1 = 変動事由あり, D4:ON = 医療H29/04/02 以降
...
//	CArray<INDKMK_DTA, INDKMK_DTA&>	m_INDkmk;
	...
	AnalyzeAJ *m_pAnaAJ;
	std::deque<CString> m_AIdcod;	//AI摘要科目コード

	int		m_KanaSearch;	//カナ検索タイプ[0=1文字, 1=複数文字]

	// インボイス関連
	bool m_bInvMaster, m_bInvUse, m_bInvDisp;
	CArray<INVOICE_INFO> m_InvAry;
	CArray<INVOICE_INFO> m_DispInvAry;
	char svInvKana[10];
	int	 m_InvPG, m_InvMax;
...
CSelectSub::CSelectSub()
{
	...
	m_bInvMaster = m_bInvUse = m_bInvDisp = false;
	::ZeroMemory(svInvKana, sizeof svInvKana);
	m_InvMax = 0;
	m_InvPG = 0;
}
...
int CSelectSub::InitSub( CDBZmSub* pZm, CWnd* pwnd, SELECTDATAPROC fix, SELECTDATAPROC sel32 )
{
	...
	m_KanaSearch = GetKanaSearchType();
...
void CSelectSub::_init_kamoku( int update/*= 0*/)
{
	...
	if( m_DRsgn == SL_IND_ALL ) {
		if( bKengyo && !(m_pZm->zvol->apno == 0x10 && (m_pZm->zvol->ind_table&0x01)) ) {
			_init_kengyokmk_old();	//No.16-212 は未出荷にしてほしい (2017/11/08)
			return;
		}
	}
	...
	// 不使用科目があった場合は、空欄となっているので詰める。
	if( kmkhyoji > 0 ) {
		const int d_max = 40;

		for( int pg = 0; pg < PG_MAX; pg++ ) {
			char	zerobuf[(d_max+1)*4] = {0};

			int maxval = d_max;

			for( int n = 0; n < maxval; n++ ) { 
				if( KDPAG[pg][n] == 0 ) {
					int chk_size = (d_max - (n+1))*4;

					if( chk_size > 0 ) {
						if( memcmp( &KDPAG[pg][n+1], zerobuf, chk_size ) == 0 )
							break;
						else {
							memcpy( &KDPAG[pg][n], &KDPAG[pg][n+1], chk_size );
							KDPAG[pg][d_max-1] = 0;
							//不使用が連続している場合のため
							maxval--;
							n--;
			...
			m_bHjiyu = 0;
			{
				...
				if( ret == ID_ZMG_H29REV_IRYO_TYPE ) {
					...
					do {
						...
						if( (ksh.hn_cod >= 101 && ksh.hn_cod <= 150) ) {
							if( (ksh.hn_pflg & 0x01)  ) {
								m_KabNam.Add( ksh );
							}
						}
						else if( (ksh.hn_cod >= 161 && ksh.hn_cod <= 180) ) {
							if( (ksh.hn_pflg & 0x01) ) {
								m_IgaiNam.Add( ksh );
							}
						}
						// ksh.hn_cod --> 186 ～ 190　(過去の誤謬による累積的影響額) 仕訳では設定させない。
...
/*-------------*/
...
#endif
	case SL_AITKKMK_PN:
	case SL_BKAITKKMK_PN:
		sel_ok = 1;

		// コード表示時は 2桁 入力でも コード入力として扱う
		if( m_KmkMode == SL_KMK_CODE ) {
			sel_ok = 0;
		}
		else {
			if( strlen(selstr) > 2 )
		...
		if( !sel_ok ) {//番号選択
			...
			for( auto dcod : m_AIdcod ) {
				...
				if( i > ((page - 1) * m_PageMax) ) {
					...
					if( cnt == no ) {
						DBKNREC* pKn = DB_PjisToKnrec(dcod, m_pZm);
						if( pKn && check_kamokurestrict(pKn) ) {
						result->brn_bmn = -1;
						result->km_code = dcod;
						result->brn_no = -1;
						ret = TRUE;
			...
			if( _InputItemCheck(ans, chk, 0, FALSE) ) {
				result->brn_bmn = -1;
				result->km_code = ans.kd_code;
				result->brn_no = -1;
				ret = TRUE;
			}
		}
	...
	case SL_INVKANA_PN:
		...
		if( sel_no > 0 ) {
			if( _GetInvNoSelect(result, sel_no) )
				ret = TRUE;
		}
	}
...
int CSelectSub::_SelectJob( SelPar* par, int nChar/*=-1*/, SelParEx* parex/* = NULL*/ )
{
	...
	//AI摘要科目がない場合は、科目表示する
	if( par->sel_pn == SL_AITKKMK_PN && nChar == -1 ) {
		if( AITKkmkCount(par->tkybrn, par->sel_flg) == 0 ) {
	...
	case SL_KAMOKU_PN:
		...
		break;
	case SL_KMKANA_PN:
	case SL_KMHANIKANA_PN:
		if( nChar == -1 ) {
			if( kmk_50search( par->kmk_kana, par->sel_pn == SL_KMHANIKANA_PN ? 1 : 0 ) ) {
				::ZeroMemory( svKana, sizeof svKana );
				memcpy( svKana, par->kmk_kana, sizeof par->kmk_kana );
				m_spp.sp_prevpn = prev_pn;
				m_spp.sp_nowpn = m_spp.sp_nowpn;
				...
				if( Order_flg ) {
					m_spp.sp_prevpn = prev_pn;
					if( par->sel_pn == SL_KMHANIKANA_PN ) {
						m_spp.sp_nowpn = SL_KMHANI_PN;
					}
					else {
						m_spp.sp_nowpn = SL_KAMOKU_PN;
					}
	...
	case SL_BRNTKY_PN:
		if( nChar == -1 ) {
			...
			if( sg == FALSE ) {
				...
				if( m_spp.sp_prevpn == SL_AITKKMK_PN || m_spp.sp_prevpn == SL_BKAITKKMK_PN ) {
	...
	case SL_AITKKMK_PN:
	case SL_BKAITKKMK_PN:
		_AITKkmkSelect(nChar);
		break;
	case SL_INVNO_PN:
	case SL_INVKANA_PN:
		rval = _InvNoSelect(nChar, FALSE, prev_pn);
		break;
...
// SelectParam -> SelectPar にコピー
void CSelectSub::ParamCopy()
{
	m_NowPar.sel_pn = m_NowParam.sel_pn;
	m_NowPar.sel_flg = m_NowParam.sel_flg;

	m_NowPar.dbt = m_NowParam.dbt;
	m_NowPar.cre = m_NowParam.cre;
	memcpy(m_NowPar.tky_kana, m_NowParam.tky_kana, sizeof m_NowParam.tky_kana);

	// 枝番摘要
	m_NowPar.tkybrn = m_NowParam.tkybrn;		//AI摘要科目時は、摘要文字列をセットする。
	m_NowPar.brn_bmn = m_NowParam.brn_bmn;
	m_NowPar.brn_cd = m_NowParam.brn_cd;

	memcpy(m_NowPar.brn_kana, m_NowParam.brn_kana, sizeof m_NowParam.brn_kana);
}

//--------------------------------------------------------------
//	選択処理（通帳AI摘要科目)を行う
//	SelectParam* par ----- 選択表示パラメータ
//	int nChar　		 ----- キー
//
//	返送値 科目カナ検索で、該当なしの場合は、-1
//　　　　 科目入力検索で、該当科目が一科目のみの場合は、1
//　　　　 科目入力検索で、該当なしの場合は、-2
//--------------------------------------------------------------
int CSelectSub::_SelectJob2(SelectParam* param, int nChar/*=-1*/)
{
	int prev_pn, rval;
	prev_pn = m_spp.sp_nowpn;
	m_spp.sp_prevpn = prev_pn;
	m_spp.sp_nowpn = param->sel_pn;
	BOOL ret, sg;
	...
	//AI摘要科目がない場合は、科目表示する
	if( param->sel_pn == SL_BKAITKKMK_PN && nChar == -1 ) {
		int bkcnt = 0;
		int bkinfo[10] = {0};

		bkcnt = param->bkcnt;
		for( int i = 0; i < sizeof(param->bkinfo) / sizeof(param->bkinfo[0]); i++ ) {
			bkinfo[i] = param->bkinfo[i];
		}

		if( BankAITKkmkCount(bkinfo, bkcnt, param->tkybrn, param->sel_flg) == 0 ) {
			param->sel_pn = SL_KAMOKU_PN;
		}
	}
	switch( param->sel_pn ) {
	case SL_AITKKMK_PN:
...
void CSelectSub::_BmonSelectSub( int page )
{
	...
	do {
		...
		if( i > ((page-1) * m_PageMax) ) {
			...
			sprintf_s( tmp, sizeof tmp, "%s%s", bf, (LPCTSTR)bn->bnbnm );
	...
	do {
		...
		if( i > ((page-1) * m_PageMax) ) {
			...
			if( kj->KKI_KYMD > 0 )	//完成工事
				sprintf_s( tmp, sizeof tmp, "%s*%s", bf, (LPCTSTR)kj->KKI_RYNM );
			else
				sprintf_s( tmp, sizeof tmp, "%s %s", bf, (LPCTSTR)kj->KKI_RYNM );
	...
	//	resetpag( m_CDPG );
		...
		if( nChar == VK_HOME )
		{
			if( !m_BrnTkySel )
			{
				...
				if( m_spp.sp_nowpn == SL_KMINPCD_PN || m_spp.sp_nowpn == SL_NPSKMINPCD_PN ) {
					m_spp.sp_prevpn = m_spp.sp_nowpn;
					m_spp.sp_nowpn = (m_spp.sp_prevpn == SL_KMINPCD_PN) ? SL_KAMOKU_PN : SL_NPSYUS_PN;
				}
				if( m_NowPar.sel_pn == SL_KMINPCD_PN ) {
					m_NowPar.sel_pn = SL_KAMOKU_PN;
				}
				if( m_NowPar.sel_pn == SL_NPSKMINPCD_PN ) {
					m_NowPar.sel_pn = SL_NPSYUS_PN;
				}
...
void CSelectSub::SelDspRst()
{
	...
	if( m_spp.sp_nowpn != SL_TKY_PN )
	{
		...
		else if( m_spp.sp_nowpn == SL_INVNO_PN || m_spp.sp_nowpn == SL_INVKANA_PN ) {
			// インボイス登録番号
			_InvNoSelect(-2, TRUE, -1);
		}
		else {
			if( m_ACsel )
			{
				j = m_ACsel - 1;
				if( kmk_50search( m_NowPar.kmk_kana, m_spp.sp_nowpn == SL_KMHANIKANA_PN ? 1 : 0 ) == FALSE )
					m_ACsel = 0;
				else {
...
// 摘要カナソート
static
int __cdecl DBTKREC_KanaCompare(/*void *context,*/ const void *ele1, const void *ele2 )
{
	int ret;
	...
	ret = memcmp( dw1->TKana, dw2->TKana, sizeof dw1->TKana );
	//カナが同じ場合は、コード順
	if( ret == 0 ) {
		if( dw1->TKcod > dw2->TKcod )
			ret = 1;
		else if( dw1->TKcod < dw2->TKcod )
			ret = -1;
	}
...
int CSelectSub::kmk_50search( char* kana, int kmhani/*=0*/ )
{
	int i, j;
	CString sKana;
	sKana.Format( "%s", kana );
	...
	for( i = j = 0; i < m_pZm->knm_info->reccount; i++ ) {
		...
		//科目範囲
		if( kmhani ) {
			DWORD dwCode = pKn->kncod;
			hl_rev(&dwCode, 4);

			if( m_dwStart != 0 ) {
				if( dwCode < m_dwStart ) continue;
			}
			if( m_dwEnd != 0 ) {
				if( dwCode > m_dwEnd ) continue;
			}
		}
...
// 選択表示用科目名称作成（消費税属性付）
//										04.17 /98 追加
char* CSelectSub::get_hyojiknam( int n, int syusgn/*=0*/, DBKNREC* knrec/*=NULL*/)
{
	static CString pbuf;
	CString fmt;
	unsigned char	ch;
...
//MyTrace( "_brntky_sch kana = %s, bDisp = %d\n", kana, bDISP );
//TRACE( "CSelectSub sKana.IsEmpty() %d, length %d\n", sKana.IsEmpty(), sKana.GetLength() );
	...
	{
//MyTrace( "_brntky_sch kana = %s, *結果なし* m_KanaSearch(%d)\n", kana, m_KanaSearch );

		//複数文字カナ検索で、以前のカナ検索が有効な場合
		if( m_KanaSearch > 0 ) {
			if( kana != NULL || kana[0] != '\0') {
				//今回検索結果がなしの場合
				if( tkbrn->bt_reccnt > 0 && ptbl->rec_n == 0 ) {
					//もとの検索に戻す
#ifdef CLOSE
					int len = strlen(kana);
					// 濁点を消してみる
					if( (BYTE)kana[len - 1] == (BYTE)'ﾞ' || (BYTE)kana[len - 1] == (BYTE)'ﾟ' ) {
						CString ktmp;
						LPSTR ptr = ktmp.GetBuffer(64);
						memcpy(ptr, kana, len - 1);
						ptr[len - 1] = '\0';
						ktmp.ReleaseBuffer();

						ret = m_BrnTkyo.get_brtektb(&ptbl, bmn, code, ktmp, AC_MAX, 0, FALSE, m_BrnCurMode);
						if( ret ) {
							tkbrn->bt_bmn = ptbl->bmn;
							tkbrn->bt_code = ptbl->code;
							tkbrn->bt_kana = ptbl->brn_kn50;
...
#endif
					if( !ret ) {
						ret = m_BrnTkyo.get_brtektb(&ptbl, bmn, code, tkbrn->bt_kana, AC_MAX, 0, FALSE, m_BrnCurMode);
					}
//	MyTrace("_brntky_sch *前カナ = %s\n", tkbrn->bt_kana);
	...
	if( m_dwOPT&0x04 ) {
		...
		do {
			...
			if( m_RTKblk[dc_sw] == m_pZm->rtrec->rtpgn ) {
				...
				sprintf_s( temp, sizeof temp, "%s", (LPCTSTR)m_pZm->tkrec->tkana );
				memcpy( rec.TKana, temp, sizeof rec.TKana );

				rec.TKcsg	= m_pZm->tkrec->tksgn[0];
				rec.TKszk	= m_pZm->tkrec->tksgn[1];

				sprintf_s( temp, sizeof temp, "%s",  (LPCTSTR)m_pZm->tkrec->tkname );
		...
		if( max > 0 ) {
			qsort( &m_TKYkana[0], max, sizeof DBTKREC, DBTKREC_KanaCompare );
			...
			do {
				...
				memcpy( temp, m_TKYkana[i-1].TKnam, sizeof m_TKYkana[i-1].TKnam );

				l = get_strcnt( (char*)(LPCTSTR)temp, SEL_LEN );
				temp[l] = '\0';

				TKSEL_PG[selpnt] = m_TKYkana[i-1].TKcod;
				SelVal32[selpnt].number = selpnt+ 1;
...
BOOL CSelectSub::tky_select50_dsp( char *data, BOOL bDISP/*=TRUE*/ )
{
	...
	if( i )
	{
		...
		//空白データ表示
		if( allsw ) {
			for( int n = 0; n < AC_MAX; n++ ) {
				SelVal32[n].number = n + 1;
				SelVal32[n].Data = "";
			}
		...
		if( m_pZm->tkrec->Requery( "", 0 ) == 0 ) {
			...
			if( m_pZm->tkrec->st != -1 ) {
				do {
					...
					sprintf_s( buf, sizeof buf, "%s",  (LPCTSTR)m_pZm->tkrec->tkana );
					memcpy( rec.TKana, buf, sizeof rec.TKana );

					rec.TKcsg	= m_pZm->tkrec->tksgn[0];
					rec.TKszk	= m_pZm->tkrec->tksgn[1];

					sprintf_s( buf, sizeof buf, "%s",  (LPCTSTR)m_pZm->tkrec->tkname );
...
void CSelectSub::_HjiyuSelectSub( int page, int typ )
{
	...
	if( typ == 0 ) {
		if( m_bHjiyu&0x10 ) {
			strcpy_s( m_spp.seljob, sizeof m_spp.seljob, "《出資金等の変動事由》" );
		}
		else {
		...
		for( i = 0; i < m_KabNam.GetCount(); i++ ) {
			if( (i+1) > ((page-1) * m_PageMax) ) {
				...
				sprintf_s( tmp, sizeof tmp, "%s", (LPCTSTR)m_KabNam[i].hn_name );
				sprintf_s( fmt, sizeof fmt, "%%.%ds", get_strcnt( tmp, SEL_LEN ) );
		...
		if( m_bHjiyu&0x10 ) {
			strcpy_s( m_spp.seljob, sizeof m_spp.seljob, "《出資金等以外の変動事由》" );
		}
		else {
		...
		for( i = 0; i < m_IgaiNam.GetCount(); i++ ) {
			if( (i+1) > ((page-1) * m_PageMax) ) {
				...
				sprintf_s( tmp, sizeof tmp, "%s", (LPCTSTR)m_IgaiNam[i].hn_name );
				sprintf_s( fmt, sizeof fmt, "%%.%ds", get_strcnt( tmp, SEL_LEN ) );
...
BOOL CSelectSub::GetJiyuMeisyo( BYTE jycode, char* pJynam )
{
	int i, len;
	BOOL bRet = FALSE;
	BOOL bKabCheck = FALSE;
	BOOL bIgaiCheck = FALSE;
	
	if( (m_bHjiyu & 0x10)) {
		if( jycode >= 101 && jycode <= 150 ) {
			bKabCheck = TRUE;
		}
		if( jycode >= 161 && jycode <= 180 ) {
			bIgaiCheck = TRUE;
		}
	}
	else {
...
int  CSelectSub::ScanKHKbnMake()
{
	...
	if( ! m_bHjiyu ) {
...
#endif
			sprintf_s( ksh.ks_name, sizeof ksh.ks_name, "%2d.%.40s", m_KabNam[i].hn_cod, (LPCTSTR)m_KabNam[i].hn_name );
			m_KshScan.Add( ksh );
		}
		//株主資本以外の変動事由
		max = m_IgaiNam.GetCount();
		for( i = 0; i < max; i++ ) { 
			...
			sprintf_s( ksh.ks_name, sizeof ksh.ks_name, "%2d.%.40s", m_IgaiNam[i].hn_cod, (LPCTSTR)m_IgaiNam[i].hn_name );
			m_KshScan.Add( ksh );
		}
...
void CSelectSub::_KamokuRestrict( DWORD dwFlag )
{
	...
	///	_init_kamoku(1);	// 固定科目等を再セット
		...
		if(Order_flg) {//-ver 1.0.3.14  //100106
			...
			{
				j = m_ACsel - 1;
				if( kmk_50search( m_NowPar.kmk_kana, m_spp.sp_nowpn == SL_KMHANIKANA_PN ? 1 : 0) == FALSE ) no_data = m_ACsel;
		///			m_ACsel = 0;
				else {
					fixcreat( (int *)&FIXTBL[0] );
					m_fixproc( SelFix8, &m_spp, 8, m_pWnd );
...
//MyTrace( "_check_kamokurestrict type=%02x, (%08x),[%.14s] ind_ctg = %02x\n", type, dwCode, pKn->knnam, pKn->ind_ctg );
				...
				if( pKn->ind_ctg == 0x00 || pKn->ind_ctg == 0x07 || (pKn->ind_ctg&0x0f) == m_pZm->zvol->ind_sgn2 ){
					...
					if(Order_flg){
						if(pKn->ind_ctg == chk)					return TRUE;
							...
							if (m_DRsgn & SL_IND_ALL && (pKn->ind_ctg & chk) )
								return TRUE;
...
// 引数	pImgMng	：	セットするイメージ管理クラス
//-----------------------------------------------------------------------------
void CSelectData::SetAITkmkClass(AnalyzeAJ *pAnaAJ)
{
	m_pSub->SetAITkmkClass(pAnaAJ);
}
...
//	dcsw ：	0=借方, 1=貸方
//-----------------------------------------------------------------------------
int	CSelectData::AITKkmkCount(CString tkstr, int dcsw)
{
	return	m_pSub->AITKkmkCount(tkstr, dcsw);
}
...
//	dcsw ：	0=借方, 1=貸方
//-----------------------------------------------------------------------------
int	CSelectData::BankAITKkmkCount(int* bankseq, int bkcnt, CString tkstr, int dcsw)
{
	return	m_pSub->BankAITKkmkCount(bankseq, bkcnt, tkstr, dcsw);
}


// 引数の構造体拡張 (06.03/22)
int	CSelectData::SelectJob(SelectParam* param, int nChar)
{
	return	m_pSub->_SelectJob2(param, nChar);
}

// 登録番号 更新
void CSelectData::RenewInvno()
{
	m_pSub->_RenewInvno();
}
...
//
// 兼業時で、全業種指定でタブって登録されている分 10.06 /17
//
...
//
void CSelectSub::_init_kengyokmk_old()
{
...
int CSelectSub::slip_tky_dsp( int page, BOOL bSameImg/*=FALSE*/ )
{
	...
	for( i = 0; i < m_SlipTkyCnt; i++ ) {
		if( (i+1) > ((page-1) * m_PageMax) ) {
			...
			sprintf_s( tmp, sizeof tmp, "%s", (LPCTSTR)m_MatchStrAry[i] );
			sprintf_s( fmt, sizeof fmt, "%%.%ds", get_strcnt( tmp, SEL_LEN ) );
...
// 引数	pImgMng	：	セットするイメージ管理クラス
//-----------------------------------------------------------------------------
void CSelectSub::SetAITkmkClass(AnalyzeAJ *pAnaAJ)
{
	m_pAnaAJ = pAnaAJ;
}
...
// AI摘要での科目数を検索
int CSelectSub::AITKkmkCount(CString tkstr, int dcsw)
{
	m_AIkmkMax = 0;
	...
	DBKNREC* pKn;
	std::deque<CString> dcod;
//#ifdef _DEBUG
//	int add = 0;
//	for( int n = 0; n < m_pZm->knm_info->reccount; n++ ) {
//		pKn = m_pZm->pKnrec + n;
//		DWORD dwCode = pKn->kncod;
//		hl_rev(&dwCode, 4);
//		if( dwCode > 0x07000000 ) {
//			CString tmp;
//			tmp.Format("%08x", dwCode);
//			dcod.push_back(tmp);
//			add++;
//		}
//		if( add > 72 )	break;
//	}
//#else
	m_pAnaAJ->GetTekiyoList(tkstr, dcsw, dcod);
	//CString test;
	//m_pAnaAJ->GetTekiyo(tkstr, dcsw, test);
//#endif
	m_AIdcod.clear();

	//不使用チェック
	for( auto cod : dcod ) {
		pKn = DB_PjisToKnrec(cod, m_pZm);
		if( pKn ) {
			m_AIdcod.push_back(cod);
		}
	}
	m_AIkmkMax = m_AIdcod.size();
...
// AI摘要での科目数を検索
int CSelectSub::BankAITKkmkCount(int* bankinfo, int infcnt, CString tkstr, int dcsw)
{
	m_AIkmkMax = 0;

//	MyTrace("BankAITKkmkCount TOP infcnt = %d, tkstr = %s, dcsw(%d)\n", infcnt, tkstr, dcsw);
	...
	//	MyTrace("BankAITKkmkCount bkinfo[%d] = %d\n", i, bankinfo[i]);
	//}

	if( infcnt >= 4 ) {
		m_pAnaAJ->GetTekiyoList2(bankinfo, tkstr, dcsw, dcod);
	}
	else {
		m_pAnaAJ->GetTekiyoList(bankinfo, tkstr, dcsw, dcod);
	}
	...
	for( auto cod : dcod ) {
		...
		if( pKn ) {
			m_AIdcod.push_back(cod);
//			MyTrace("BankAITKkmkCount cod = %s\n", cod);
		}
	}
	m_AIkmkMax = m_AIdcod.size();
...
//	BOOL bNowPage ... TRUE で現在ページを表示する
//----------------------------------------------------------
int	CSelectSub::_AITKkmkSelect(int nChar, BOOL bNowPage/*=FALSE*/)
{
	int pg, pgcnt;
	...
	if( nChar == -1 ) {
		if( !bNowPage )
			m_AIkmkPG = 1;
		_AITKkmkSelectSub(m_AIkmkPG);
	}
	else if( nChar == VK_NEXT ) {
		pg = m_AIkmkPG;
		pg++;
		pgcnt = (m_AIkmkMax / m_PageMax);
		if( (m_AIkmkMax % m_PageMax) )	pgcnt++;
	...
	else if( nChar == VK_HOME ) {	// 科目選択を表示する
		int prev_pn;
		m_AIkmkPG = 1;
		prev_pn = m_spp.sp_nowpn;
		m_spp.sp_prevpn = prev_pn;
...
//	int page ....... 表示ページ
//----------------------------------------------------------
void CSelectSub::_AITKkmkSelectSub(int page)
{
	CBMNAME* bn;
	int i, cnt;
	i = cnt = 0;
	...
	strcpy_s(m_spp.seljob, sizeof m_spp.seljob, "《摘要科目履歴》");
	...
	for( auto dcod : m_AIdcod ) {
		...
		if( i > ((page - 1) * m_PageMax) ) {
			...
			if( pKn ) {
				if( !check_kamokurestrict(pKn) ) {
					p = "";
					(pSelVal + idx)->number = bKcod ? -1 : idx + 1;
				}
				else {
					p = get_hyojiknam(0, 0, pKn);
					(pSelVal + idx)->number = idx + 1;
				}
			}
			else {
				p = "";
...
// nChar == -1 初回, nChar==-2 (リセット呼び出し)
int CSelectSub::_InvNoSelect(int nChar, BOOL bNowPage/*=FALSE*/, int prev_pn/*=-1*/)
{
	int pg, pgcnt;
	int rval = 0;
	...
	if( nChar <= -1 ) {
		...
		char prevInvKana[10] = {0};

//MyTrace("_InvNoSelect nChar(%d), m_NowPar.tky_kana = %s\n", nChar, m_NowPar.tky_kana );

		if( nChar == -1 ) {
			if( m_NowPar.tky_kana[0] ) {
				memcpy(prevInvKana, svInvKana, sizeof prevInvKana);
				::ZeroMemory(svInvKana, sizeof svInvKana);
				memcpy(svInvKana, m_NowPar.tky_kana, sizeof m_NowPar.tky_kana);
				kanLen = strlen(svInvKana);
				bKana = TRUE;
			}
			else {
				::ZeroMemory(svInvKana, sizeof svInvKana);
		...
		if( bKana ) {
			...
			tmpAry.Copy(m_DispInvAry);

			m_DispInvAry.RemoveAll();
			int cnt = m_InvAry.GetCount();

			//登録番号の摘要をカナで検索
			int dispcnt = 0;
			for( int n = 0; n < cnt; n++ ) {
				if( memcmp(m_InvAry[n].inv_tkKana, svInvKana, kanLen) == 0 ) {
					dispcnt++;
					m_DispInvAry.Add(m_InvAry[n]);
				}
			}
	//MyTrace("_InvNoSelect dispcnt = (%d)\n", dispcnt);
	//MyTrace("_InvNoSelect nChar(%d), svInvKana = %s, prevInvKana = %s\n", nChar, svInvKana, prevInvKana);
	
			if( dispcnt == 0 ) {
				// 検索結果なしの場合は、直前データにもどす(デフォルト)
				m_spp.sp_prevpn = prev_pn;
				m_spp.sp_nowpn = SL_INVNO_PN;
				rval = -1;

				//直前カナデータ
				if( prevInvKana[0] ) {
					kanLen = strlen(prevInvKana);
					for( int n = 0; n < cnt; n++ ) {
						if( memcmp(m_InvAry[n].inv_tkKana, prevInvKana, kanLen) == 0 ) {
							dispcnt++;
							m_DispInvAry.Add(m_InvAry[n]);
						}
					}
					memcpy( svInvKana, prevInvKana, sizeof svInvKana);
				}
				else {
					::ZeroMemory(svInvKana, sizeof svInvKana);
					//すべての登録番号
					if( dispcnt == 0 ) {
						m_DispInvAry.Copy(m_InvAry);
					}
				}

				m_InvMax = m_DispInvAry.GetCount();
	//MyTrace("_InvNoSelect m_InvMax = (%d)\n", m_InvMax);

				_InvNoSelectSub(&m_DispInvAry, m_InvPG);
			}
			else {
				m_InvMax = m_DispInvAry.GetCount();
				_InvNoSelectSub(&m_DispInvAry, m_InvPG);
			...
			m_InvMax = m_InvAry.GetCount();
			_InvNoSelectSub(&m_InvAry, m_InvPG);
		}
	}
	else if( nChar == VK_NEXT ) {
		pg = m_InvPG;
		pg++;
		pgcnt = (m_InvMax / m_PageMax);
		if( (m_InvMax % m_PageMax) )	pgcnt++;

		if( pgcnt >= pg ) {
			m_InvPG = pg;
			_InvNoSelectSub(svInvKana[0] ? &m_DispInvAry : &m_InvAry, m_InvPG);
		}
	}
	else if( nChar == VK_PRIOR ) {
...
//	int page ....... 表示ページ
//----------------------------------------------------------
void CSelectSub::_InvNoSelectSub(CArray<INVOICE_INFO>* pAry, int page)
{
	CBMNAME* bn;
	int i, cnt, sel_idx;
	i = cnt = 0;
	...
	strcpy_s(m_spp.seljob, sizeof m_spp.seljob, "《登録番号》");

	int aryMax = pAry->GetCount();
	m_InvPG = page;
	TRACE("_InvNoSelectSub page = %d\n", page);

	for( i = sel_idx = 0; i < aryMax; i++ ) {
		...
		if( (i+1) > ((page - 1) * m_PageMax) ) {
			...
			if( i < aryMax ) {
				p = pAry->GetAt(i).inv_tkNam;
			}
			else {
				p = "";
			}
			(pSelVal + sel_idx)->Data = p;
			(pSelVal + sel_idx)->number = sel_idx + 1;
			sel_idx++;
		}
	}
...
int CSelectSub::_GetInvNoSelect(SelResult* result, int sel_no)
{
	CArray<INVOICE_INFO>* pAry;

	MyTrace("_GetInvNoSelect svInvKana(%s)\n", svInvKana );

	if( svInvKana[0] ) {
		pAry = &m_DispInvAry;

	MyTrace("_GetInvNoSelect(1) cnt(%d)\n", pAry->GetCount());
...
MyTrace("_GetInvNoSelect(2) aryMax(%d)\n", aryMax, page);

//	TRACE("_InvNoSelectSub page = %d\n", page);

	cnt = 0;
	result->sel_pn = SL_INVNO_PN;
	...
	for( i = 0; i < aryMax; i++ ) {
		...
		if( (i + 1) > ((page - 1) * m_PageMax) ) {
			...
			if( cnt == sel_no ) {
				result->selname = pAry->GetAt(i).inv_invNo;
				result->selname.Replace(_T("T"), _T(""));
				result->tky_code = pAry->GetAt(i).inv_tkNo;
				result->tky = pAry->GetAt(i).inv_tkNam;
				ret = 1;
				break;
			}
		}
